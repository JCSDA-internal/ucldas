...
use LND_domains, only : pass_var, CORNER
...
use LND_open_boundary, only : ocean_OBC_type, OBC_NONE, OBC_DIRECTION_E
use LND_open_boundary, only : OBC_DIRECTION_W, OBC_DIRECTION_N, OBC_DIRECTION_S
use LND_open_boundary, only : OBC_segment_type
...
!> Control structure for LND_set_visc
...
subroutine set_viscous_BBL(u, v, h, tv, visc, G, GV, US, CS, symmetrize)
  ...
  if (.not.associated(CS)) call LND_error(FATAL,"LND_set_viscosity(BBL): "//&
...
subroutine set_viscous_ML(u, v, h, tv, forces, visc, dt, G, GV, US, CS, symmetrize)
  ...
  if (.not.associated(CS)) call LND_error(FATAL,"LND_set_viscosity(visc_ML): "//&
         "Module must be initialized before it is used.")
  ...
  if (associated(forces%frac_shelf_u) .neqv. associated(forces%frac_shelf_v)) &
    call LND_error(FATAL, "set_viscous_ML: one of forces%frac_shelf_u and "//&
...
subroutine set_visc_register_restarts(HI, GV, param_file, visc, restart_CS)
  ...
  character(len=40)  :: mdl = "LND_set_visc"  ! This module's name.
  ...
  if (useKPP) then
    ! LND_bkgnd_mixing uses Kv_slow when KPP is defined.
...
!> Initializes the LND_set_visc control structure
...
# include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "set_visc_init called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
  ...
  if (CS%bottomdraglaw) then
    ...
    if (use_regridding .and. (.not. CS%BBL_use_EOS)) &
      call LND_error(FATAL,"When using UCLAND in ALE mode it is required to "//&
  ...
  if (CS%bottomdraglaw) then
    ...
    if (CS%BBL_use_tidal_bg) then
      ...
      call LND_read_data(filename, 'tideamp', CS%tideamp, G%domain, timelevel=1, scale=US%m_to_Z*US%T_to_s)
