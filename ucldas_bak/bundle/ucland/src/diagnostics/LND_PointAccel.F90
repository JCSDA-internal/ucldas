...
!! in the u- or v-lndentum balance at a given point.  Usually
...
use LND_io, only : open_file
use LND_io, only : APPEND_FILE, ASCII_FILE, MULTIPLE, SINGLE_FILE
use LND_time_manager, only : time_type, get_time, get_date, set_date, operator(-)
...
!> The control structure for the LND_PointAccel module
...
! names as the variables they point to in LND.F90
...
!  if (.not.associated(CS)) return
  ...
  ! Open up the file for output if this is the first call.
    if (CS%u_file < 0) then
      ...
      if (CS%u_file < 0) then
        call LND_error(NOTE, 'Unable to open file '//trim(CS%u_trunc_file)//'.')
        return
      endif
    endif
    file = CS%u_file
  ...
  ! Open up the file for output if this is the first call.
    if (CS%v_file < 0) then
      ...
      call open_file(CS%v_file, trim(CS%v_trunc_file), action=APPEND_FILE, &
                     form=ASCII_FILE, threading=MULTIPLE, fileset=SINGLE_FILE)
      if (CS%v_file < 0) then
        call LND_error(NOTE, 'Unable to open file '//trim(CS%v_trunc_file)//'.')
        return
      endif
    endif
    file = CS%v_file
...
subroutine PointAccel_init(MIS, Time, G, param_file, diag, dirs, CS)
  type(ocean_internal_state), &
                        target, intent(in)    :: MIS  !< For "LND Internal State" a set of pointers
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_PointAccel" ! This module's name.
