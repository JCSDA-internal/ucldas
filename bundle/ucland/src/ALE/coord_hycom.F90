...
subroutine init_coord_hycom(CS, nk, coordinateResolution, target_density, interp_CS)
  ...
  if (associated(CS)) call LND_error(FATAL, "init_coord_hycom: CS already associated!")
  allocate(CS)
...
subroutine set_hycom_params(CS, max_interface_depths, max_layer_thickness, interp_CS)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "set_hycom_params: CS not associated")

  if (present(max_interface_depths)) then
    if (size(max_interface_depths) /= CS%nk+1) &
      call LND_error(FATAL, "set_hycom_params: max_interface_depths inconsistent size")
  ...
  if (present(max_layer_thickness)) then
    if (size(max_layer_thickness) /= CS%nk) &
      call LND_error(FATAL, "set_hycom_params: max_layer_thickness inconsistent size")
