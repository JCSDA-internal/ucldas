...
use LND_io, only : file_exists, LND_read_data, slasher, vardesc, var_desc, query_vardesc
...
type, public :: OCMIP2_CFC_CS ; private
  ...
  type(tracer_registry_type), pointer :: tr_Reg => NULL() !< A pointer to the UCLAND tracer registry
  ...
  type(LND_restart_CS), pointer :: restart_CSp => NULL()  !< Model restart control structure
...
!> Register the OCMIP2 CFC tracers to be used with LND and read the parameters
...
! to be used with LND.
  ...
  ! Local variables
  character(len=40)  :: mdl = "LND_OCMIP2_CFC" ! This module's name.
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "register_OCMIP2_CFC called with an "// &
  ...
  if ((CS%ind_cfc_11_flux < 0) .or. (CS%ind_cfc_11_flux < 0)) then
    ...
    call LND_ERROR(WARNING, "CFCs are currently only set up to be run in " // &
...
subroutine init_tracer_CFC(h, tr, name, land_val, IC_val, G, US, CS)
  ...
  if (len_trim(CS%IC_file) > 0) then
    ...
    if (.not.file_exists(CS%IC_file, G%Domain)) &
      call LND_error(FATAL, "initialize_OCMIP2_CFC: Unable to open "//CS%IC_file)
    if (CS%Z_IC_file) then
      OK = tracer_Z_init(tr, h, CS%IC_file, name, G, US)
      if (.not.OK) then
        OK = tracer_Z_init(tr, h, CS%IC_file, trim(name), G, US)
        if (.not.OK) call LND_error(FATAL,"initialize_OCMIP2_CFC: "//&
                "Unable to read "//trim(name)//" from "//&
                trim(CS%IC_file)//".")
      endif
    else
      call LND_read_data(CS%IC_file, trim(name), tr, G%Domain)
    endif
  else
