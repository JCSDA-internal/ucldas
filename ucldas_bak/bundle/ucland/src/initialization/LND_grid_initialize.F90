...
module LND_grid_initialize

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_checksums, only : hchksum, Bchksum
use LND_checksums, only : uvchksum, hchksum_pair, Bchksum_pair
use LND_domains, only : pass_var, pass_vector, pe_here, root_PE, broadcast
use LND_domains, only : AGRID, BGRID_NE, CGRID_NE, To_All, Scalar_Pair
use LND_domains, only : To_North, To_South, To_East, To_West
use LND_domains, only : LND_define_domain, LND_define_IO_domain
use LND_domains, only : LND_domain_type
use LND_dyn_horgrid, only : dyn_horgrid_type, set_derived_dyn_horgrid
use LND_error_handler, only : LND_error, LND_mesg, FATAL, is_root_pe
...
use LND_io, only : LND_read_data, read_data, slasher, file_exists
use LND_io, only : CORNER, NORTH_FACE, EAST_FACE
...
type, public :: GPS ; private
  ...
  logical :: isotropic !< If true, an isotropic grid on a sphere (also known as a Mercator grid)
                   ...
                   !! direction are _not_ independent. In LND the meridional extent is determined
...
#include "version_variable.h"
  ...
  call callTree_enter("set_grid_metrics(), LND_grid_initialize.F90")
  call log_version(param_file, "LND_grid_init", version, "")
  call get_param(param_file, "LND_grid_init", "GRID_CONFIG", config, &
  ...
  call get_param(param_file, "LND_grid_init", "DEBUG", debug, &
...
subroutine set_grid_metrics_from_mosaic(G, param_file, US)
  ...
  character(len=64)  :: mdl = "LND_grid_init set_grid_metrics_from_mosaic"
  integer :: err=0, ni, nj, global_indices(4)
  type(LND_domain_type) :: SGdom ! Supergrid domain
  ...
  call callTree_enter("set_grid_metrics_from_mosaic(), LND_grid_initialize.F90")
  ...
  if (.not.file_exists(filename)) &
    call LND_error(FATAL," set_grid_metrics_from_mosaic: Unable to open "//&
                           trim(filename))
  ...
  if (associated(G%domain%maskmap)) then
     call LND_define_domain(global_indices, SGdom%layout, SGdom%mpp_domain, &
            xflags=G%domain%X_FLAGS, yflags=G%domain%Y_FLAGS, &
            xhalo=SGdom%nihalo, yhalo=SGdom%njhalo, &
            xextent=exni,yextent=exnj, &
            symmetry=.true., name="LND_MOSAIC", maskmap=G%domain%maskmap)
  else
     call LND_define_domain(global_indices, SGdom%layout, SGdom%mpp_domain, &
            xflags=G%domain%X_FLAGS, yflags=G%domain%Y_FLAGS, &
            xhalo=SGdom%nihalo, yhalo=SGdom%njhalo, &
            xextent=exni,yextent=exnj, &
            symmetry=.true., name="LND_MOSAIC")
  endif

  call LND_define_IO_domain(SGdom%mpp_domain, SGdom%io_layout)
  deallocate(exni)
  deallocate(exnj)

  ! Read X from the supergrid
  tmpZ(:,:) = 999.
  call LND_read_data(filename, 'x', tmpZ, SGdom, position=CORNER)
  ...
  tmpZ(:,:) = 999.
  call LND_read_data(filename, 'y', tmpZ, SGdom, position=CORNER)
  ...
  tmpU(:,:) = 0. ; tmpV(:,:) = 0.
  call LND_read_data(filename,'dx',tmpV,SGdom,position=NORTH_FACE)
  call LND_read_data(filename,'dy',tmpU,SGdom,position=EAST_FACE)
  ...
  tmpT(:,:) = 0.
  call LND_read_data(filename, 'area', tmpT, SGdom)
...
subroutine set_grid_metrics_cartesian(G, param_file, US)
  ...
  character(len=48) :: mdl  = "LND_grid_init set_grid_metrics_cartesian"
  ...
  call callTree_enter("set_grid_metrics_cartesian(), LND_grid_initialize.F90")
...
subroutine set_grid_metrics_spherical(G, param_file, US)
  ...
  real :: m_to_L  ! A unit conversion factor [L m-1 ~> nondim]
  character(len=48)  :: mdl  = "LND_grid_init set_grid_metrics_spherical"
  ...
  call callTree_enter("set_grid_metrics_spherical(), LND_grid_initialize.F90")
  m_to_L = 1.0 ; if (present(US)) m_to_L = US%m_to_L
...
subroutine set_grid_metrics_mercator(G, param_file, US)
  ...
  character(len=48)  :: mdl = "LND_grid_init set_grid_metrics_mercator"
  ...
  call callTree_enter("set_grid_metrics_mercator(), LND_grid_initialize.F90")
  ...
  call get_param(param_file, mdl, "ISOTROPIC", GP%isotropic, &
                 ...
                 "direction are _not_ independent. In LND the meridional "//&
...
subroutine initialize_masks(G, PF, US)
  ...
  character(len=40)  :: mdl = "LND_grid_init initialize_masks"
  integer :: i, j

  call callTree_enter("initialize_masks(), LND_grid_initialize.F90")
...
end module LND_grid_initialize
