...
subroutine init_coord_sigma(CS, nk, coordinateResolution)
  ...
  if (associated(CS)) call LND_error(FATAL, "init_coord_sigma: CS already associated!")
  allocate(CS)
...
subroutine set_sigma_params(CS, min_thickness)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "set_sigma_params: CS not associated")
