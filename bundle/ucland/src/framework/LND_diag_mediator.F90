...
use LND_checksums,        only : chksum0, zchksum
use LND_checksums,        only : hchksum, uchksum, vchksum, Bchksum
use LND_coms,             only : PE_here
use LND_cpu_clock,        only : cpu_clock_id, cpu_clock_begin, cpu_clock_end
use LND_cpu_clock,        only : CLOCK_MODULE, CLOCK_ROUTINE
use LND_error_handler,    only : LND_error, FATAL, WARNING, is_root_pe, assert
use LND_file_parser,      only : get_param, log_version, param_file_type
use LND_grid,             only : ocean_grid_type
use LND_io,               only : slasher, vardesc, query_vardesc, lnd_read_data
use LND_io,               only : get_filename_appendix
use LND_safe_alloc,       only : safe_alloc_ptr, safe_alloc_alloc
use LND_string_functions, only : lowercase
...
use LND_diag_remap,       only : diag_remap_ctrl
use LND_diag_remap,       only : diag_remap_update
use LND_diag_remap,       only : diag_remap_calc_hmask
use LND_diag_remap,       only : diag_remap_init, diag_remap_end, diag_remap_do_remap
use LND_diag_remap,       only : vertically_reintegrate_diag_field, vertically_interpolate_diag_field
use LND_diag_remap,       only : diag_remap_configure_axes, diag_remap_axes_configured
use LND_diag_remap,       only : diag_remap_get_axes_info, diag_remap_set_active
use LND_diag_remap,       only : diag_remap_diag_registration_closed
use LND_diag_remap,       only : horizontally_average_diag_field
...
use LND_diag_manager_wrapper, only : register_diag_field_fms
...
!integer :: PPP=111  ! x:point,y:point,z:point, this kind of diagnostic is not currently present in diag_table.UCLAND
!integer :: PPS=112  ! x:point,y:point,z:sum  , this kind of diagnostic is not currently present in diag_table.UCLAND
!integer :: PPM=113  ! x:point,y:point,z:mean , this kind of diagnostic is not currently present in diag_table.UCLAND
...
subroutine set_axes_info_dsamp(G, GV, param_file, diag_cs, id_zl_native, id_zi_native)
  ...
  do dl=2,MAX_DSAMP_LEV
     if(dl .ne. 2) call LND_error(FATAL, "set_axes_info_dsamp: Downsample level other than 2 is not supported yet!")
     if (G%symmetric) then
...
subroutine set_masks_for_axes_dsamp(G, diag_cs)
  ...
  do dl=2,MAX_DSAMP_LEV
     if(dl .ne. 2) call LND_error(FATAL, "set_masks_for_axes_dsamp: Downsample level other than 2 is not supported!")
     do c=1, diag_cs%num_diag_coords
        ! Level/layer h-points in diagnostic coordinate
        axes => diag_cs%remap_axesTL(c)
...
subroutine define_axes_group(diag_cs, handles, axes, nz, vertical_coordinate_number, &
  ...
  n = size(handles)
  if (n<1 .or. n>3) call LND_error(FATAL, "define_axes_group: wrong size for list of handles!")
  ...
  if (present(x_cell_method)) then
    if (axes%rank<2) call LND_error(FATAL, 'define_axes_group: ' // &
  ...
  if (present(v_cell_method)) then
    if (axes%rank/=1 .and. axes%rank/=3) call LND_error(FATAL, 'define_axes_group: ' // &
...
subroutine post_data_2d_low(diag, field, diag_cs, is_static, mask)
  ...
  elseif ( size(field,1) == cszi + 1 ) then
    ...
    call LND_error(FATAL,"post_data_2d_low: "//trim(diag%debug_str)//trim(mesg))
  endif
  ...
  if (diag_cs%diag_as_chksum) then
    if (diag%axes%is_h_point) then
      ...
      call LND_error(FATAL, "post_data_2d_low: unknown axis type.")
    endif
  else
    if (is_stat) then
      if (present(mask)) then
...
subroutine post_data_3d(diag_field_id, field, diag_cs, is_static, mask, alt_h)
  ...
  do while (associated(diag))
    ...
    if (diag%v_extensive .and. .not.diag%axes%is_native) then
      ...
      if (present(mask)) then
        call LND_error(FATAL,"post_data_3d: no mask for regridded field.")
      endif
...
subroutine post_data_3d_low(diag, field, diag_cs, is_static, mask)
  ...
  elseif ( size(field,1) == cszi + 1 ) then
    ...
    write (mesg,*) " peculiar size ",size(field,1)," in i-direction\n"//&
       "does not match one of ", cszi, cszi+1, dszi, dszi+1
    call LND_error(FATAL,"post_data_3d_low: "//trim(diag%debug_str)//trim(mesg))
  endif
  ...
  if ((diag%conversion_factor /= 0.) .and. (diag%conversion_factor /= 1.)) then
    ...
    if (diag%fms_xyave_diag_id>0) then
      ...
      if (isv_c < lbound(locfield,1)) call LND_error(FATAL, &
      ...
      if (jsv_c < lbound(locfield,2)) call LND_error(FATAL, &
        "It is an error to average a staggered diagnostic field that does not "//&
  ...
  if (diag%fms_diag_id>0) then
    if (diag_cs%diag_as_chksum) then
      if (diag%axes%is_h_point) then
        ...
        call LND_error(FATAL, "post_data_3d_low: unknown axis type.")
      endif
    else
      if (is_stat) then
        if (present(mask)) then
...
!! handled outside of the UCLAND diagnostics infrastructure.
function get_diag_time_end(diag_cs)
  ...
  ! outside of the UCLAND infrastructure, such as via the generic tracer code.
...
integer function register_diag_field(module_name, field_name, axes_in, init_time, &
  ...
  real,             optional, intent(in) :: range(2) !< Valid range of a variable (not used in LND?)
  logical,          optional, intent(in) :: mask_variant !< If true a logical mask must be provided with
                                                         !! post_data calls (not used in LND?)
  logical,          optional, intent(in) :: verbose !< If true, FMS is verbose (not used in LND?)
  logical,          optional, intent(in) :: do_not_log !< If true, do not log something (not used in LND?)
  character(len=*), optional, intent(out):: err_msg !< String into which an error message might be
                                                         !! placed (not used in LND?)
  character(len=*), optional, intent(in) :: interp_method !< If 'none' indicates the field should not
                                                         !! be interpolated as a scalar
  integer,          optional, intent(in) :: tile_count !< no clue (not used in LND?)
  ...
  axes => axes_in
  LND_missing_value = axes%diag_cs%missing_value
  if (present(missing_value)) LND_missing_value = missing_value
  ...
  active = register_diag_field_expand_cmor(dm_id, module_name, field_name, axes, &
             init_time, long_name=long_name, units=units, missing_value=LND_missing_value, &
  ...
  do i=1,diag_cs%num_diag_coords
    ...
    if (axes_in%rank == 3) then
      ...
      ! When the LND_diag_to_Z module has been obsoleted we can assume remap_axes will
  ...
  do dl=2,MAX_DSAMP_LEV
     ...
     if (axes_in%rank == 3 .or. axes_in%rank == 2 ) then
        ...
        elseif (axes_in%id == diag_cs%axesCv1%id) then
           ...
           call LND_error(WARNING,"register_diag_field: Could not find a proper axes for " &
...
logical function register_diag_field_expand_cmor(dm_id, module_name, field_name, axes, init_time, &
  ...
  logical,          optional, intent(in) :: mask_variant !< If true a logical mask must be provided
                                                         !! with post_data calls (not used in LND?)
  logical,          optional, intent(in) :: verbose !< If true, FMS is verbose (not used in LND?)
  logical,          optional, intent(in) :: do_not_log !< If true, do not log something (not used in LND?)
  character(len=*), optional, intent(out):: err_msg !< String into which an error message might be
                                                         !! placed (not used in LND?)
  ...
  fms_id = register_diag_field_expand_axes(module_name, field_name, axes, init_time, &
             long_name=long_name, units=units, missing_value=LND_missing_value, &
             range=range, mask_variant=mask_variant, standard_name=standard_name, &
             verbose=verbose, do_not_log=do_not_log, err_msg=err_msg, &
             interp_method=interp_method, tile_count=tile_count)
  ...
  if (present(cmor_field_name) .and. .not. diag_cs%diag_as_chksum) then
    ...
    ! If attributes are present for LND variable names, use them first for the register_diag_field
    ...
    fms_id = register_diag_field_expand_axes(module_name, cmor_field_name, axes, init_time,    &
               ...
               missing_value=LND_missing_value, range=range, mask_variant=mask_variant,               &
...
subroutine add_xyz_method(diag, axes, x_cell_method, y_cell_method, v_cell_method, v_extensive)
  ...
  if (present(v_extensive)) then
     if (present(v_cell_method)) call LND_error(FATAL, "attach_cell_methods: " // &
...
subroutine attach_cell_methods(id, axes, ostring, cell_methods, &
  ...
  if (present(cell_methods)) then
    if (present(x_cell_method) .or. present(y_cell_method) .or. present(v_cell_method) &
      ...
      call LND_error(FATAL, "attach_cell_methods: " // &
    ...
    if (present(v_cell_method)) then
      if (present(v_extensive)) call LND_error(FATAL, "attach_cell_methods: " // &
           'Vertical cell method was specified along with the vertically extensive flag.')
...
function register_scalar_field(module_name, field_name, init_time, diag_cs, &
  ...
  LND_missing_value = diag_cs%missing_value
  if (present(missing_value)) LND_missing_value = missing_value
  ...
  if (present(cmor_field_name)) then
    ...
    ! If attributes are present for LND variable names, use them first for the register_static_field
    ! call for CMOR verison of the variable
    if (present(units)) posted_cmor_units = units
    if (present(standard_name)) posted_cmor_standard_name = standard_name
    if (present(long_name)) posted_cmor_long_name = long_name
    ...
    fms_id = register_diag_field_fms(module_name, cmor_field_name, init_time, &
       long_name=trim(posted_cmor_long_name), units=trim(posted_cmor_units), &
       missing_value=LND_missing_value, range=range, &
    ...
    case ("L")
      select case (hor_grid)
        ...
        case ("z")
          axes => diag_cs%axeszL
        case default
          call LND_error(FATAL, "ocean_register_diag: " // &
...
!> diag_mediator_init initializes the LND diag_mediator and opens the available
...
# include "version_variable.h"
  character(len=40) :: mdl = "LND_diag_mediator" ! This module's name.
  ...
  if (is_root_pe() .and. (diag_CS%available_diag_doc_unit < 0)) then
    ...
    ! Find an unused unit number.
      ...
      if ((.not.opened) .or. (ios /= 0)) then
        call LND_error(FATAL, "Failed to open available diags file "//trim(doc_path)//".")
      endif
    endif
  endif
  ...
  if (is_root_pe() .and. (diag_CS%chksum_iounit < 0) .and. diag_CS%diag_as_chksum) then
    ...
    ! Find an unused unit number.
      ...
      if ((.not.opened) .or. (ios /= 0)) then
        call LND_error(FATAL, "Failed to open checksum diags file "//trim(doc_path)//".")
      endif
    endif
  endif
...
subroutine diag_update_remap_grids(diag_cs, alt_h, alt_T, alt_S, update_intensive, update_extensive )
  ...
  if (diag_cs%diag_grid_overridden) then
     call LND_error(FATAL, "diag_update_remap_grids was called, but current grids in "// &
...
function i2s(a,n_in)
  ...
  ! Perhaps this belongs elsewhere in the UCLAND code?
...
subroutine downsample_diag_indices_get(fo1, fo2, dl, diag_cs, isv, iev, jsv, jev)
  ...
  if(first_check) then
     if(mod(diag_cs%ie-diag_cs%is+1, dl) .ne. 0 .OR. mod(diag_cs%je-diag_cs%js+1, dl) .ne. 0) then
        ...
        call LND_error(FATAL,"downsample_diag_indices_get: "//trim(mesg))
     endif
     first_check = .false.
  endif
  ...
  !The rest is not taken with the full UCLAND diag_table
...
subroutine downsample_diag_field_3d(locfield, locfield_dsamp, dl, diag_cs, diag, isv, iev, jsv, jev, mask)
  ...
  elseif (associated(diag%axes%mask3d)) then
     ...
     call LND_error(FATAL, "downsample_diag_field_3d: Cannot downsample without a mask!!! ")
  endif
...
subroutine downsample_diag_field_2d(locfield, locfield_dsamp, dl, diag_cs, diag, isv, iev, jsv, jev, mask)
  ...
  elseif (associated(diag%axes%mask2d)) then
     ...
     call LND_error(FATAL, "downsample_diag_field_2d: Cannot downsample without a mask!!! ")
  endif
...
!        do ii=i0,i0+dl-1 ; do jj=j0,j0+dl-1
  ...
  elseif(method == MSK) then !The input field is a mask, subsample
     ...
     call LND_error(FATAL, "downsample_field_3d: "//trim(mesg)//" "//trim(diag%debug_str))
  endif
...
!        do ii=i0,i0+dl-1 ; do jj=j0,j0+dl-1
  ...
  elseif(method == MSK) then !The input field is a mask, subsample
     ...
     write (mesg,*) " unknown sampling method: ",method
     call LND_error(FATAL, "downsample_field_2d: "//trim(mesg)//" "//trim(diag%debug_str))
  endif
