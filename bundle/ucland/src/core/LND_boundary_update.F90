...
module LND_boundary_update

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_cpu_clock,             only : cpu_clock_id, cpu_clock_begin, cpu_clock_end, CLOCK_ROUTINE
use LND_diag_mediator,         only : time_type
use LND_error_handler,         only : LND_mesg, LND_error, FATAL, WARNING
...
use LND_open_boundary,         only : ocean_obc_type, update_OBC_segment_data
use LND_open_boundary,         only : OBC_registry_type, file_OBC_CS
use LND_open_boundary,         only : register_file_OBC, file_OBC_end
...
!> The control structure for the LND_boundary_update module
...
! character(len=40)  :: mdl = "LND_boundary_update" ! This module's name.
...
# include "version_variable.h"
  if (associated(CS)) then
    call LND_error(WARNING, "call_OBC_register called with an associated "// &
                            "control structure.")
    return
...
end module LND_boundary_update
