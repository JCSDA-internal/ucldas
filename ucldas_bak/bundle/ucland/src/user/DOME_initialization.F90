...
use LND_open_boundary, only : ocean_OBC_type, OBC_NONE, OBC_SIMPLE
use LND_open_boundary,   only : OBC_segment_type, register_segment_tracer
use LND_tracer_registry, only : tracer_registry_type, tracer_type
use LND_tracer_registry, only : tracer_name_lookup
...
# include "version_variable.h"
  ...
  call LND_mesg("  DOME_initialization.F90, DOME_initialize_topography: setting topography", 5)
...
subroutine DOME_initialize_thickness(h, G, GV, param_file, just_read_params)
  ...
  call LND_mesg("  DOME_initialization.F90, DOME_initialize_thickness: setting thickness", 5)
...
!  The remaining calls to set_up_sponge_field can be in any order. !
  if ( associated(tv%T) ) then
    call LND_error(FATAL,"DOME_initialize_sponges is not set up for use with"//&
...
! Local variables
  ...
  if (OBC%number_of_segments /= 1) then
    call LND_error(WARNING, 'Error in DOME OBC segment setup', .true.)
