...
# include "version_variable.h"
  character(len=16) :: mdl = 'LND_verticalGrid'

  if (associated(GV)) call LND_error(FATAL, &
  ...
  if (abs(H_power) > 300) call LND_error(FATAL, "verticalGridInit: "//&
  ...
  call get_param(param_file, mdl, "NK", nk, &
                 ...
                 static_value=NK_)
  if (nk /= NK_) call LND_error(FATAL, "verticalGridInit: " // &
       "Mismatched number of layers NK_ between LND_memory.h and param_file")
...
function get_tr_flux_units(GV, tr_units, tr_vol_conc_units, tr_mass_conc_units)
  ...
  if (cnt == 0) call LND_error(FATAL, "get_tr_flux_units: One of the three "//&
    "arguments tr_units, tr_vol_conc_units, or tr_mass_conc_units "//&
    "must be present.")
  if (cnt > 1) call LND_error(FATAL, "get_tr_flux_units: Only one of "//&
