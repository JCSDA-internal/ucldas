...
The UCLAND source code is divided into a tree of directories (folders) to group
...
`pkg/`
  ...
  symbolic-links to extract the parts the UCLAND uses.
...
`dynamic/`, `dynamic_symmetric/`
  ...
  be included at compile time. If neither is used then a `LND_memory.h` file
...
`coupled_driver/`
  ...
  compiling UCLAND along with at least a sea-ice model and possibly all other
