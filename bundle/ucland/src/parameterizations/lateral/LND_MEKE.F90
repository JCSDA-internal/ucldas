...
use LND_error_handler, only : LND_error, FATAL, WARNING, NOTE, LND_mesg
...
use LND_io,            only : vardesc, var_desc
use LND_restart,       only : LND_restart_CS, register_restart_field, query_initialized
use LND_unit_scaling,  only : unit_scale_type
use LND_variables,     only : vertvisc_type
...
type, public :: MEKE_CS ; private
  ...
  real :: viscosity_coeff_Ku !< The scaling coefficient in the expression for
                        !! viscosity used to parameterize lateral harmonic lndentum mixing
                        !! by unresolved eddies represented by MEKE.
  real :: viscosity_coeff_Au !< The scaling coefficient in the expression for
                        !! viscosity used to parameterize lateral biharmonic lndentum mixing
                        !! by unresolved eddies represented by MEKE.
  ...
  integer :: id_GM_src = -1, id_lnd_src = -1, id_GME_snk = -1, id_decay = -1
...
subroutine step_forward_MEKE(MEKE, h, SN_u, SN_v, visc, dt, G, GV, US, CS, hu, hv)
  ...
  if (.not.associated(CS)) call LND_error(FATAL, &
         "LND_MEKE: Module must be initialized before it is used.")
  if (.not.associated(MEKE)) call LND_error(FATAL, &
         "LND_MEKE: MEKE must be initialized before it is used.")
...
!   call LND_error(FATAL, "LND_MEKE: MEKE%MEKE is not associated!")
    return
  endif

    if (CS%debug) then
      if (associated(MEKE%lnd_src)) &
        call hchksum(MEKE%lnd_src, 'MEKE lnd_src', G%HI, scale=US%RZ3_T3_to_W_m2*US%L_to_Z**2)
    ...
    if (associated(MEKE%lnd_src)) then
      !$OMP parallel do default(shared)
      do j=js,je ; do i=is,ie
        src(i,j) = src(i,j) - CS%MEKE_FrCoeff*I_mass(i,j)*MEKE%lnd_src(i,j)
      enddo ; enddo
    endif
 ...
 !   enddo ; enddo
    ...
    if (CS%id_lnd_src>0) call post_data(CS%id_lnd_src, MEKE%lnd_src, CS%diag)
...
!> Initializes the LND_MEKE module and reads parameters.
...
logical function MEKE_init(Time, G, US, param_file, diag, CS, MEKE, restart_CS)
  ...
  type(LND_restart_CS),    pointer       :: restart_CS !< Restart control structure for LND_MEKE.
...
# include "version_variable.h"
  character(len=40)  :: mdl = "LND_MEKE" ! This module's name.
  ...
  if (.not. associated(MEKE)) then
    ...
    call LND_error(WARNING, "MEKE_init called with NO associated "// &
                            "MEKE-type structure.")
    return
  endif
  ...
  call LND_mesg("MEKE_init: reading parameters ", 5)
  ...
  call get_param(param_file, mdl, "MEKE_VISCOSITY_COEFF_KU", CS%viscosity_coeff_Ku, &
                 ...
                 "viscosity used to parameterize harmonic lateral lndentum mixing by"//&
  ...
  call get_param(param_file, mdl, "MEKE_VISCOSITY_COEFF_AU", CS%viscosity_coeff_Au, &
                 "If non-zero, is the scaling coefficient in the expression for"//&
                 "viscosity used to parameterize biharmonic lateral lndentum mixing by"//&
                 "unresolved eddies represented by MEKE. Can be negative to"//&
                 "represent backscatter from the unresolved eddies.", &
                 units="nondim", default=0.0)
  ...
  if (CS%viscosity_coeff_Ku/=0. .and. .not. laplacian) call LND_error(FATAL, &
                 "LAPLACIAN must be true if MEKE_VISCOSITY_COEFF_KU is true.")

  if (CS%viscosity_coeff_Au/=0. .and. .not. biharmonic) call LND_error(FATAL, &
...
! Register fields for output from this module.
  ...
  CS%id_lnd_src = register_diag_field('ocean_model', 'MEKE_lnd_src',diag%axesT1, Time, &
     'MEKE energy available from lndentum', &
     'W m-2', conversion=US%RZ3_T3_to_W_m2*US%L_to_Z**2)
  if (.not. associated(MEKE%lnd_src)) CS%id_lnd_src = -1
  ...
  if (CS%initialize) call LND_error(WARNING, &
...
!> Allocates memory and register restart fields for the LND_MEKE module.
...
! Allocate control structure
  if (associated(MEKE)) then
    call LND_error(WARNING, "MEKE_alloc_register_restart called with an associated "// &
                             "MEKE type.")
    return
...
! Allocate memory
  call LND_mesg("MEKE_alloc_register_restart: allocating and registering", 5)
  ...
  if (MEKE_FrCoeff>=0. .or. MEKE_GMECoeff>=0.)  then
    allocate(MEKE%lnd_src(isd:ied,jsd:jed)) ; MEKE%lnd_src(:,:) = 0.0
  endif
...
subroutine MEKE_end(MEKE, CS)
  type(MEKE_type), pointer :: MEKE !< A structure with MEKE-related fields.
  type(MEKE_CS),   pointer :: CS   !< The control structure for LND_MEKE.
  ...
  if (associated(MEKE%lnd_src)) deallocate(MEKE%lnd_src)
...
!! diffusion (module lnd_thickness_diffuse) and optionally in along
!! isopycnal mixing of tracers (module lnd_tracer_hor_diff).
