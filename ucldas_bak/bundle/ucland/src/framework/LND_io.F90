...
use LND_domains,          only : LND_domain_type, AGRID, BGRID_NE, CGRID_NE
use LND_domains,          only : get_simple_array_i_ind, get_simple_array_j_ind
use LND_file_parser,      only : log_version, param_file_type
...
public :: num_timelevels, LND_read_data, LND_read_vector, ensembler
public :: reopen_file, slasher, write_field, write_version_number, LND_io_init
...
interface LND_read_data
  module procedure LND_read_data_4d
  module procedure LND_read_data_3d
  module procedure LND_read_data_2d
  module procedure LND_read_data_1d
end interface

!> Read a pair of data fields representing the two components of a vector from a file
interface LND_read_vector
  module procedure LND_read_vector_3d
  module procedure LND_read_vector_2d
end interface
...
subroutine create_file(unit, filename, vars, novars, fields, threading, timeunit, G, dG, GV, checksums)
  ...
  type(LND_domain_type), pointer :: Domain => NULL()
...
! Define the coordinates.
  do k=1,novars
    select case (vars(k)%hor_grid)
      ...
      case ('1') ! Do nothing.
      case default
        call LND_error(WARNING, "LND_io create_file: "//trim(vars(k)%name)//&
    ...
    select case (vars(k)%z_grid)
      ...
      case ('1') ! Do nothing.
      case default
        call LND_error(FATAL, "LND_io create_file: "//trim(vars(k)%name)//&
    ...
    select case (t_grid(1:1))
      ...
      case ('p') ; use_periodic = .true.
        if (len_trim(t_grid(2:8)) <= 0) call LND_error(FATAL, &
          "LND_io create_file: No periodic axis length was specified in "//&
        ...
        if (var_periods == -9999999) call LND_error(FATAL, &
          "LND_io create_file: Failed to read the number of periods from "//&
          trim(vars(k)%t_grid) // " in the periodic axes of variable "//&
          trim(vars(k)%name)//" in file "//trim(filename))
        if (var_periods < 1) call LND_error(FATAL, "LND_io create_file: "//&
        ...
        if ((num_periods > 0) .and. (var_periods /= num_periods)) &
          call LND_error(FATAL, "LND_io create_file: "//&
  ...
  if ((use_lath .or. use_lonh .or. use_latq .or. use_lonq)) then
    if (.not.domain_set) call LND_error(FATAL, "create_file: "//&
  ...
  if ((use_layer .or. use_int) .and. .not.present(GV)) call LND_error(FATAL, &
...
! (and in case of GNU does) get confused and crash.
  ...
  if (use_periodic) then
    if (num_periods <= 1) call LND_error(FATAL, "LND_io create_file: "//&
...
subroutine reopen_file(unit, filename, vars, novars, fields, threading, timeunit, G, dG, GV)
  ...
  if (.not.exists) then
    ...
    if (nvar == -1) then
      ...
      call LND_error(WARNING,"LND_io: "//mesg)
    ...
    elseif (nvar /= novars) then
      ...
      call LND_error(FATAL,"LND_io: "//mesg)
    endif
...
!      !call LND_error(NOTE,"LND_io: "//mesg)
!    enddo
  endif
...
! -------------------------------------------------------------------
  ...
  if (.not.axis_found) call LND_error(FATAL, "LND_io read_axis_data: "//&
...
function num_timelevels(filename, varname, min_dims) result(n_time)
  ...
  if (status /= NF90_NOERR) then
    call LND_error(WARNING,"num_timelevels: "//&
  ...
  if (present(min_dims)) then
    if (ndims < min_dims-1) then
      ...
      call LND_error(WARNING, "num_timelevels: variable "//trim(varname)//&
  ...
  if (status /= NF90_NOERR) call LND_error(WARNING,"num_timelevels: "//&
...
subroutine safe_string_copy(str1, str2, fieldnm, caller)
  ...
  if (len(trim(str1)) > len(str2)) then
    if (present(fieldnm) .and. present(caller)) then
      call LND_error(FATAL, trim(caller)//" attempted to copy the overly long"//&
        " string "//trim(str1)//" into "//trim(fieldnm))
    else
      call LND_error(FATAL, "safe_string_copy: The string "//trim(str1)//&
...
subroutine LND_read_data_1d(filename, fieldname, data, timelevel, scale)
...
end subroutine LND_read_data_1d
...
subroutine LND_read_data_2d(filename, fieldname, data, LND_Domain, &
  ...
  call read_data(filename, fieldname, data, LND_Domain%mpp_domain, &
                 timelevel=timelevel, position=position)

  if (present(scale)) then ; if (scale /= 1.0) then
    call get_simple_array_i_ind(LND_Domain, size(data,1), is, ie)
    call get_simple_array_j_ind(LND_Domain, size(data,2), js, je)
    data(is:ie,js:je) = scale*data(is:ie,js:je)
  endif ; endif

end subroutine LND_read_data_2d

!> This function uses the fms_io function read_data to read a distributed
!! 3-D data field named "fieldname" from file "filename".  Valid values for
!! "position" include CORNER, CENTER, EAST_FACE and NORTH_FACE.
subroutine LND_read_data_3d(filename, fieldname, data, LND_Domain, &
                            timelevel, position, scale)
  character(len=*),       intent(in)    :: filename  !< The name of the file to read
  character(len=*),       intent(in)    :: fieldname !< The variable name of the data in the file
...
end subroutine LND_read_data_3d

!> This function uses the fms_io function read_data to read a distributed
!! 4-D data field named "fieldname" from file "filename".  Valid values for
!! "position" include CORNER, CENTER, EAST_FACE and NORTH_FACE.
subroutine LND_read_data_4d(filename, fieldname, data, LND_Domain, &
                            timelevel, position, scale)
  character(len=*),       intent(in)    :: filename  !< The name of the file to read
  character(len=*),       intent(in)    :: fieldname !< The variable name of the data in the file
...
end subroutine LND_read_data_4d
...
subroutine LND_read_vector_2d(filename, u_fieldname, v_fieldname, u_data, v_data, LND_Domain, &
  ...
  call read_data(filename, u_fieldname, u_data, LND_Domain%mpp_domain, &
                 timelevel=timelevel, position=u_pos)
  call read_data(filename, v_fieldname, v_data, LND_Domain%mpp_domain, &
                 timelevel=timelevel, position=v_pos)

  if (present(scale)) then ; if (scale /= 1.0) then
    call get_simple_array_i_ind(LND_Domain, size(u_data,1), is, ie)
    call get_simple_array_j_ind(LND_Domain, size(u_data,2), js, je)
    u_data(is:ie,js:je) = scale*u_data(is:ie,js:je)
    call get_simple_array_i_ind(LND_Domain, size(v_data,1), is, ie)
    call get_simple_array_j_ind(LND_Domain, size(v_data,2), js, je)
    v_data(is:ie,js:je) = scale*v_data(is:ie,js:je)
  endif ; endif

end subroutine LND_read_vector_2d


!> This function uses the fms_io function read_data to read a pair of distributed
!! 3-D data fields with names given by "[uv]_fieldname" from file "filename".  Valid values for
!! "stagger" include CGRID_NE, BGRID_NE, and AGRID.
subroutine LND_read_vector_3d(filename, u_fieldname, v_fieldname, u_data, v_data, LND_Domain, &
                              timelevel, stagger, scalar_pair, scale)
  character(len=*),       intent(in)    :: filename  !< The name of the file to read
  character(len=*),       intent(in)    :: u_fieldname !< The variable name of the u data in the file
  character(len=*),       intent(in)    :: v_fieldname !< The variable name of the v data in the file
...
end subroutine LND_read_vector_3d


!> Initialize the LND_io module
subroutine LND_io_init(param_file)
  type(param_file_type), intent(in) :: param_file  !< structure indicating the open file to
                                                   !! parse for model parameter values.

! This include declares and sets the variable "version".
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_io" ! This module's name.

  call log_version(param_file, mdl, version)

end subroutine LND_io_init
...
!!       LND and named by name_output_file and open it for reading.
!!
