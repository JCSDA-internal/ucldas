...
character(len=40)  :: mdl = "LND_CVMix_shear"  !< This module's name.
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "CVMix_shear_init called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
...
! Set default, read and log parameters
  ...
  if ((NumberTrue) > 1) then
     call LND_error(FATAL, 'LND_CVMix_shear_init: '// &
...
! Forego remainder of initialization if not using this scheme
  ...
  CS%id_N2 = register_diag_field('ocean_model', 'N2_shear', diag%axesTi, Time, &
      'Square of Brunt-Vaisala frequency used by LND_CVMix_shear module', '1/s2', conversion=US%s_to_T**2)
  if (CS%id_N2 > 0) then
    allocate( CS%N2( SZI_(G), SZJ_(G), SZK_(G)+1 ) ) ; CS%N2(:,:,:) = 0.
  endif

  CS%id_S2 = register_diag_field('ocean_model', 'S2_shear', diag%axesTi, Time, &
      'Square of vertical shear used by LND_CVMix_shear module','1/s2', conversion=US%s_to_T**2)
  if (CS%id_S2 > 0) then
  ...
  CS%id_ri_grad = register_diag_field('ocean_model', 'ri_grad_shear', diag%axesTi, Time, &
      'Gradient Richarson number used by LND_CVMix_shear module','nondim')
  ...
  CS%id_ri_grad_smooth = register_diag_field('ocean_model', 'ri_grad_shear_smooth', &
       diag%axesTi, Time, &
      'Smoothed gradient Richarson number used by LND_CVMix_shear module','nondim')
  ...
  CS%id_kd = register_diag_field('ocean_model', 'kd_shear_CVMix', diag%axesTi, Time, &
      'Vertical diffusivity added by LND_CVMix_shear module', 'm2/s', conversion=US%Z2_T_to_m2_s)
  CS%id_kv = register_diag_field('ocean_model', 'kv_shear_CVMix', diag%axesTi, Time, &
      'Vertical viscosity added by LND_CVMix_shear module', 'm2/s', conversion=US%Z2_T_to_m2_s)
