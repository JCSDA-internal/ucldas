...
use LND_EOS, only : query_compressible
...
!  real :: dalpha
  ...
  if (.not.associated(CS)) call LND_error(FATAL, &
      "LND_PressureForce_Mont: Module must be initialized before it is used.")
  if (use_EOS) then
    if (query_compressible(tv%eqn_of_state)) call LND_error(FATAL, &
...
!>\namespace lnd_pressureforce_mont
!!
...
end module LND_PressureForce_Mont
