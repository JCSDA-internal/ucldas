...
module LND_diag_remap

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_coms,             only : reproducing_sum_EFP, EFP_to_real
use LND_coms,             only : EFP_type, assignment(=), EFP_sum_across_PEs
use LND_error_handler,    only : LND_error, FATAL, assert, WARNING
use LND_diag_vkernels,    only : interpolate_column, reintegrate_column
use LND_file_parser,      only : get_param, log_param, param_file_type
use LND_io,               only : slasher, lnd_read_data
use LND_io,               only : file_exists, field_size
use LND_string_functions, only : lowercase, extractWord
...
use LND_regridding,       only : set_regrid_params, get_regrid_size
use LND_regridding,       only : getCoordinateInterfaces
use LND_regridding,       only : get_zlike_CS, get_sigma_CS, get_rho_CS
...
subroutine diag_remap_configure_axes(remap_cs, GV, US, param_file)
  ...
  character(len=40)  :: mod  = "LND_diag_remap" ! This module's name.
...
!                           GV%Z_to_H*G%bathyT(i,j), sum(h(i,j,:)), zInterfaces)
      call LND_error(FATAL,"diag_remap_update: SLIGHT coordinate not coded for diagnostics yet!")
    elseif (remap_cs%vertical_coord == coordinateMode('HYCOM1')) then
!     call build_hycom1_column(remap_cs%regrid_cs, nz, &
!                           GV%Z_to_H*G%bathyT(i,j), sum(h(i,j,:)), zInterfaces)
      call LND_error(FATAL,"diag_remap_update: HYCOM1 coordinate not coded for diagnostics yet!")
    endif
    do k = 1,nz
