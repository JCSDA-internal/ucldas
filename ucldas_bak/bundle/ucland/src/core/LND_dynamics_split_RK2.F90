!> Time step the adiabatic dynamic core of LND using RK2 method.
module LND_dynamics_split_RK2

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_variables,    only : vertvisc_type, thermo_var_ptrs
use LND_variables,    only : BT_cont_type, alloc_bt_cont_type, dealloc_bt_cont_type
use LND_variables,    only : accel_diag_ptrs, ocean_internal_state, cont_diag_ptrs
use LND_forcing_type, only : mech_forcing

use LND_checksum_packages, only : LND_thermo_chksum, LND_state_chksum, LND_accel_chksum
...
use LND_diag_mediator,     only : diag_mediator_init, enable_averages
use LND_diag_mediator,     only : disable_averaging, post_data, safe_alloc_ptr
use LND_diag_mediator,     only : register_diag_field, register_static_field
use LND_diag_mediator,     only : set_diag_mediator_grid, diag_ctrl, diag_update_remap_grids
use LND_domains,           only : LND_domains_init
use LND_domains,           only : To_South, To_West, To_All, CGRID_NE, SCALAR_PAIR
use LND_domains,           only : To_North, To_East, Omit_Corners
use LND_domains,           only : create_group_pass, do_group_pass, group_pass_type
use LND_domains,           only : start_group_pass, complete_group_pass, pass_var
...
use LND_time_manager,      only : time_type, time_type_to_real, operator(+)
use LND_time_manager,      only : operator(-), operator(>), operator(*), operator(/)

use LND_ALE,                   only : ALE_CS
use LND_barotropic,            only : barotropic_init, btstep, btcalc, bt_mass_source
use LND_barotropic,            only : register_barotropic_restarts, set_dtbt, barotropic_CS
use LND_boundary_update,       only : update_OBC_data, update_OBC_CS
use LND_continuity,            only : continuity, continuity_init, continuity_CS
use LND_continuity,            only : continuity_stencil
use LND_CoriolisAdv,           only : CorAdCalc, CoriolisAdv_init, CoriolisAdv_CS
use LND_debugging,             only : check_redundant
use LND_grid,                  only : ocean_grid_type
use LND_hor_index,             only : hor_index_type
use LND_hor_visc,              only : horizontal_viscosity, hor_visc_init, hor_visc_CS
use LND_interface_heights,     only : find_eta
use LND_lateral_mixing_coeffs, only : VarMix_CS
use LND_MEKE_types,            only : MEKE_type
use LND_open_boundary,         only : ocean_OBC_type, radiation_open_bdry_conds
use LND_open_boundary,         only : open_boundary_zero_normal_flow
use LND_open_boundary,         only : open_boundary_test_extern_h, update_OBC_ramp
use LND_PressureForce,         only : PressureForce, PressureForce_init, PressureForce_CS
use LND_set_visc,              only : set_viscous_ML, set_visc_CS
use LND_thickness_diffuse,     only : thickness_diffuse_CS
use LND_tidal_forcing,         only : tidal_forcing_init, tidal_forcing_CS
use LND_unit_scaling,          only : unit_scale_type
use LND_vert_friction,         only : vertvisc, vertvisc_coef, vertvisc_remnant
use LND_vert_friction,         only : vertvisc_init, vertvisc_CS
use LND_vert_friction,         only : updateCFLtruncationValue
use LND_verticalGrid,          only : verticalGrid_type, get_thickness_units
use LND_verticalGrid,          only : get_flux_units, get_tr_flux_units
...
!> LND_dynamics_split_RK2 module control structure
type, public :: LND_dyn_split_RK2_CS ; private
  ...
  real ALLOCABLE_, dimension(NIMEMB_PTR_,NJMEM_,NKMEM_) :: visc_rem_u
              !< Both the fraction of the zonal lndentum originally in a
  ...
  real ALLOCABLE_, dimension(NIMEM_,NJMEMB_PTR_,NKMEM_) :: visc_rem_v
              !< Both the fraction of the meridional lndentum originally in
  ...
  type(accel_diag_ptrs), pointer :: ADp  !< A structure pointing to the various
                                         !! accelerations in the lndentum equations,
...
end type LND_dyn_split_RK2_CS


public step_LND_dyn_split_RK2
...
integer :: id_clock_horvisc, id_clock_lnd_update
...
!> RK2 splitting for time stepping LND adiabatic dynamics
subroutine step_LND_dyn_split_RK2(u, v, h, tv, visc, Time_local, dt, forces, p_surf_begin, p_surf_end, &
  ...
  type(LND_dyn_split_RK2_CS),        pointer       :: CS           !< module control structure
  ...
  if (showCallTree) call callTree_enter("step_LND_dyn_split_RK2(), LND_dynamics_split_RK2.F90")
  ...
  if (CS%debug) then
    call LND_state_chksum("Start predictor ", u, v, h, uh, vh, G, GV, US, symmetric=sym)
...
! pbce = dM/deta
  ...
  if (showCallTree) call callTree_wayPoint("done with PressureForce (step_LND_dyn_split_RK2)")
...
! CAu = -(f+zeta_av)/h_av vh + d/dx KE_av
  ...
  if (showCallTree) call callTree_wayPoint("done with CorAdCalc (step_LND_dyn_split_RK2)")
...
! u_bc_accel = CAu + PFu + diffu(u[n-1])
  ...
  if (CS%debug) then
    call LND_accel_chksum("pre-btstep accel", CS%CAu, CS%CAv, CS%PFu, CS%PFv, &
  ...
  if (showCallTree) call callTree_wayPoint("done with vertvisc_coef (step_LND_dyn_split_RK2)")
...
! u_accel_bt = layer accelerations due to barotropic solver
  if (associated(CS%BT_cont) .or. CS%BT_use_layer_fluxes) then
    ...
    if (showCallTree) call callTree_wayPoint("done with continuity[BT_cont] (step_LND_dyn_split_RK2)")
  endif
  ...
  if (showCallTree) call callTree_enter("btstep(), LND_barotropic.F90")
...
! up = u + dt_pred*( u_bc_accel + u_accel_bt )
  dt_pred = dt * CS%be
  call cpu_clock_begin(id_clock_lnd_update)
  ...
  do k=1,nz
    ...
    enddo ; enddo
  enddo
  call cpu_clock_end(id_clock_lnd_update)
...
!   call LND_state_chksum("Predictor 1", up, vp, h, uh, vh, G, GV, US, haloshift=1)
    call LND_accel_chksum("Predictor accel", CS%CAu, CS%CAv, CS%PFu, CS%PFv, &
             CS%diffu, CS%diffv, G, GV, US, CS%pbce, CS%u_accel_bt, CS%v_accel_bt, symmetric=sym)
    call LND_state_chksum("Predictor 1 init", u, v, h, uh, vh, G, GV, US, haloshift=2, &
                          symmetric=sym)
...
! u_av  <- u_av  + dt_pred d/dz visc d/dz u_av
  ...
  if (showCallTree) call callTree_wayPoint("done with vertvisc (step_LND_dyn_split_RK2)")
  if (G%nonblocking_updates) then
    call cpu_clock_end(id_clock_vertvisc)
  ...
  call cpu_clock_end(id_clock_continuity)
  if (showCallTree) call callTree_wayPoint("done with continuity (step_LND_dyn_split_RK2)")
...
!   call do_group_pass(CS%pass_hp_uv, G%Domain, clock=id_clock_pass)
  ...
  if (CS%begw /= 0.0) then
    ...
    call PressureForce(hp, tv, CS%PFu, CS%PFv, G, GV, US, CS%PressureForce_CSp, &
                       CS%ALE_CSp, p_surf, CS%pbce, CS%eta_PF)
    call cpu_clock_end(id_clock_pres)
    if (showCallTree) call callTree_wayPoint("done with PressureForce[hp=(1-b).h+b.h] (step_LND_dyn_split_RK2)")
  endif
  ...
  if (BT_cont_BT_thick) then
    call btcalc(h, G, GV, CS%barotropic_CSp, CS%BT_cont%h_u, CS%BT_cont%h_v, &
                OBC=CS%OBC)
    if (showCallTree) call callTree_wayPoint("done with btcalc[BT_cont_BT_thick] (step_LND_dyn_split_RK2)")
  endif

  if (CS%debug) then
    call LND_state_chksum("Predictor ", up, vp, hp, uh, vh, G, GV, US, symmetric=sym)
  ...
  ! call LND_state_chksum("Predictor avg ", u_av, v_av, h_av, uh, vh, G, GV, US)
...
! diffu = horizontal viscosity terms (u_av)
  ...
  if (showCallTree) call callTree_wayPoint("done with horizontal_viscosity (step_LND_dyn_split_RK2)")
...
! Calculate the lndentum forcing terms for the barotropic equations.
...
! u_bc_accel = CAu + PFu + diffu(u[n-1])
  ...
  if (CS%debug) then
    call LND_accel_chksum("corr pre-btstep accel", CS%CAu, CS%CAv, CS%PFu, CS%PFv, &
                          CS%diffu, CS%diffv, G, GV, US, CS%pbce, u_bc_accel, v_bc_accel, &
                          symmetric=sym)
  ...
  if (CS%debug) then
    ...
    call uvchksum("Corrector 1 [uv]h", uh, vh, G%HI, haloshift=2, &
                  symmetric=sym, scale=GV%H_to_m*US%L_to_m**2*US%s_to_T)
  ! call LND_state_chksum("Corrector 1", u, v, h, uh, vh, G, GV, US, haloshift=1)
    call LND_accel_chksum("Corrector accel", CS%CAu, CS%CAv, CS%PFu, CS%PFv, &
...
! h_av = (h_in + h_out)/2 . Going in to this line, h_av = h_in.
  ...
  !  Here various terms used in to update the lndentum equations are
  ...
  if (CS%debug) then
    call LND_state_chksum("Corrector ", u, v, h, uh, vh, G, GV, US, symmetric=sym)
 ...
 !  call LND_state_chksum("Corrector avg ", u_av, v_av, h_av, uh, vh, G, GV, US)
  endif

  if (showCallTree) call callTree_leave("step_LND_dyn_split_RK2()")

end subroutine step_LND_dyn_split_RK2
...
subroutine register_restarts_dyn_split_RK2(HI, GV, param_file, CS, restart_CS, uh, vh)
  ...
  type(LND_dyn_split_RK2_CS),    pointer       :: CS         !< module control structure
  type(LND_restart_CS),          pointer       :: restart_CS !< restart control structure
  ...
  type(vardesc)      :: vd(2)
  character(len=40)  :: mdl = "LND_dynamics_split_RK2" ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "register_restarts_dyn_split_RK2 called with an associated "// &
                             "control structure.")
    return
  endif
  allocate(CS)
...
subroutine initialize_dyn_split_RK2(u, v, h, uh, vh, eta, Time, G, GV, US, param_file, &
  ...
  type(LND_restart_CS),             pointer       :: restart_CS !< restart control structure
  real,                             intent(in)    :: dt         !< time step [T ~> s]
  type(accel_diag_ptrs),    target, intent(inout) :: Accel_diag !< points to lndentum equation terms for
                                                                !! budget analysis
  type(cont_diag_ptrs),     target, intent(inout) :: Cont_diag  !< points to terms in continuity equation
  type(ocean_internal_state),       intent(inout) :: MIS        !< "UCLAND internal state" used to pass
                                                                !! diagnostic pointers
...
# include "version_variable.h"
  ...
  if (.not.associated(CS)) call LND_error(FATAL, &
      "initialize_dyn_split_RK2 called with an unassociated control structure.")
  if (CS%module_is_initialized) then
    call LND_error(WARNING, "initialize_dyn_split_RK2 called with a control "// &
  ...
  call get_param(param_file, mdl, "TIDES", use_tides, &
                 "If true, apply tidal lndentum forcing.", default=.false.)
...
!  Accel_diag%u_av => CS%u_av ; Accel_diag%v_av => CS%v_av
  ...
  if (.not.associated(setVisc_CSp)) call LND_error(FATAL, &
  ...
  id_clock_Cor        = cpu_clock_id('(Ocean Coriolis & lnd advection)', grain=CLOCK_MODULE)
  ...
  id_clock_lnd_update = cpu_clock_id('(Ocean lndentum increments)',      grain=CLOCK_MODULE)
...
!> \namespace lnd_dynamics_split_rk2
!!
...
!!  Runge-Kutta time stepping scheme for the baroclinic lndentum
!!  equation and a forward-backward coupling between the baroclinic
!!  lndentum and continuity equations.  This split time-stepping
...
!!  LND code (although several of these solutions are routinely
!!  verified by comparison with the slower unsplit schemes).
!!
!!  The subroutine step_LND_dyn_split_RK2 actually does the time
...
!!  control structure with LND.F90 and the other LND_dynamics_...
!!  modules.

end module LND_dynamics_split_RK2
