...
use LND_debugging, only : chksum
use LND_EOS, only : calculate_density, EOS_type
...
use LND_io, only : open_file, LND_read_data, read_axis_data, SINGLE_FILE, MULTIPLE
...
use LND_verticalGrid, only : verticalGrid_type, setVerticalGridAxes
...
character(len=40) :: mdl = "LND_coord_initialization" !< This module's name.

contains

!> LND_initialize_coord sets up time-invariant quantities related to UCLAND's
!!   vertical coordinate.
subroutine LND_initialize_coord(GV, US, PF, write_geom, output_dir, tv, max_depth)
...
#include "version_variable.h"
  ...
  call callTree_enter("LND_initialize_coord(), LND_coord_initialization.F90")
...
! Set-up the layer densities, GV%Rlay, and reduced gravities, GV%g_prime.
  ...
  select case ( trim(config) )
    ...
    case default ; call LND_error(FATAL,"LND_initialize_coord: "// &
  ...
  if (debug) call chksum(US%R_to_kg_m3*GV%Rlay(:), "LND_initialize_coord: Rlay ", 1, nz)
  if (debug) call chksum(US%m_to_Z*US%L_to_m**2*US%s_to_T**2*GV%g_prime(:), "LND_initialize_coord: g_prime ", 1, nz)
...
! Write out all of the grid data used by this run.
  ...
  call callTree_leave('LND_initialize_coord()')

end subroutine LND_initialize_coord
...
subroutine set_coord_from_gprime(Rlay, g_prime, GV, US, param_file)
  ...
  call callTree_enter(trim(mdl)//"(), LND_coord_initialization.F90")
...
subroutine set_coord_from_TS_range(Rlay, g_prime, GV, US, param_file, eqn_of_state, P_Ref)
  ...
  if ((GV%nk_rho_varies > 0) .and. (nz < GV%nk_rho_varies+2)) &
    call LND_error(FATAL, "set_coord_from_TS_range requires that NZ >= NKML+NKBL+2.")
...
subroutine set_coord_from_file(Rlay, g_prime, GV, US, param_file)
  ...
  do k=1,nz ; if (g_prime(k) <= 0.0) then
    call LND_error(FATAL, "LND_initialization set_coord_from_file: "//&
...
!! to the vertical grid used by the LND ocean model.
...
end module LND_coord_initialization
