...
use LND_domains,              only: LND_infra_init, num_pes, root_pe, pe_here
use LND_file_parser,          only: get_param, log_version, param_file_type, close_param_file
use LND_get_input,            only: Get_LND_Input, directories
use LND_domains,              only: pass_var
use LND_error_handler,        only: LND_error, FATAL, is_root_pe
use LND_ocean_model_nuopc,    only: ice_ocean_boundary_type
use LND_grid,                 only: ocean_grid_type, get_global_grid_size
use LND_ocean_model_nuopc,    only: ocean_model_restart, ocean_public_type, ocean_state_type
use LND_ocean_model_nuopc,    only: ocean_model_init_sfc
use LND_ocean_model_nuopc,    only: ocean_model_init, update_ocean_model, ocean_model_end
use LND_ocean_model_nuopc,    only: get_ocean_grid, get_eps_omesh
use LND_cap_time,             only: AlarmInit
use LND_cap_methods,          only: lnd_import, lnd_export, lnd_set_geomtype
#ifdef CESMCOUPLED
...
!> Internal state type with pointers to three types defined by LND.
...
subroutine SetServices(gcomp, rc)
  ...
  ! local variables
  character(len=*),parameter  :: subname='(LND_cap:SetServices)'
...
!! to use.
!!
!! For this LND cap, we are using IPDv01.
!!
!! @param gcomp an ESMF_GridComp object
...
subroutine InitializeP0(gcomp, importState, exportState, clock, rc)
  ...
  character(len=*),parameter  :: subname='(LND_cap:InitializeP0)'
  ...
  call ESMF_LogWrite('LND_cap:DumpFields = '//trim(logmsg), ESMF_LOGMSG_INFO, rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
       line=__LINE__, &
       file=__FILE__)) &
       return
  ...
  call ESMF_LogWrite('LND_cap:OverwriteSlice = '//trim(logmsg), ESMF_LOGMSG_INFO, rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
       line=__LINE__, &
       file=__FILE__)) &
       return
  ...
  call ESMF_LogWrite('LND_cap:ProfileMemory = '//trim(logmsg), ESMF_LOGMSG_INFO, rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
       line=__LINE__, &
       file=__FILE__)) &
       return
  ...
  call ESMF_LogWrite('LND_cap:GridAttachArea = '//trim(logmsg), ESMF_LOGMSG_INFO, rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
       line=__LINE__, &
       file=__FILE__)) &
       return
  ...
  if (isPresent .and. isSet) then
     ...
     call ESMF_LogWrite('LND_cap:ScalarFieldName = '//trim(scalar_field_name), ESMF_LOGMSG_INFO, rc=rc)
     if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return
  ...
  if (isPresent .and. isSet) then
     ...
     call ESMF_LogWrite('LND_cap:ScalarFieldCount = '//trim(logmsg), ESMF_LOGMSG_INFO, rc=rc)
     if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, &
         file=__FILE__)) &
         return
  ...
  if (isPresent .and. isSet) then
     ...
     call ESMF_LogWrite('LND_cap:ScalarFieldIdxGridNX = '//trim(logmsg), ESMF_LOGMSG_INFO, rc=rc)
     if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, &
         file=__FILE__)) &
         return
  ...
  if (isPresent .and. isSet) then
     ...
     call ESMF_LogWrite('LND_cap:ScalarFieldIdxGridNY = '//trim(logmsg), ESMF_LOGMSG_INFO, rc=rc)
     if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=__LINE__, &
          file=__FILE__)) &
          return
...
subroutine InitializeAdvertise(gcomp, importState, exportState, clock, rc)
  ...
  character(len=*), parameter            :: subname='(LND_cap:InitializeAdvertise)'
...
!--------------------------------
  ...
  call ESMF_VMGet(VM, mpiCommunicator=mpi_comm_lnd, rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
    line=__LINE__, &
    file=__FILE__)) &
    return  ! bail out
  ...
  if (cesm_coupled) then
     ...
     if (isPresent .and. isSet) then
        ...
        select case (trim(calendar))
           ...
           case default
              call ESMF_LogSetError(ESMF_RC_ARG_BAD, &
                 msg=subname//": Calendar not supported in UCLAND: "//trim(calendar), &
                 line=__LINE__, file=__FILE__, rcToReturn=rc)
           end select
     else
        call set_calendar_type (NOLEAP)
  ...
  if (isPresent .and. isSet) then
     ...
     call ESMF_LogWrite('LND_cap:start_type unset - using input.nml for restart option', &
  ...
  if (len_trim(runtype) > 0) then
     call ESMF_LogWrite('LND_cap:startup = '//trim(runtype), ESMF_LOGMSG_INFO, rc=rc)
     if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, &
         file=__FILE__)) &
         return
  ...
  if (runtype == "initial") then
     ...
     if (cesm_coupled) then
        call ESMF_LogWrite('LND_cap: restart requested, using rpointer.ocn', ESMF_LOGMSG_WARNING)
        ...
        if (localPet == 0) then
           ! this hard coded for rpointer.ocn right now
            ...
            do
              ...
              if (len(restartfiles)-len(trim(restartfiles)) < len(trim(restartfile))) then
                call LND_error(FATAL, "Restart file name(s) too long.")
              endif
         ...
         call ESMF_LogWrite('LND_cap: restart requested, use input.nml', ESMF_LOGMSG_WARNING)
       endif
  ...
  call fld_list_add(fldsToOcn_num, fldsToOcn, "mean_zonal_lndent_flx"      , "will provide")
  call fld_list_add(fldsToOcn_num, fldsToOcn, "mean_merid_lndent_flx"      , "will provide")
...
subroutine InitializeRealize(gcomp, importState, exportState, clock, rc)
  ...
  character(len=*), parameter                :: subname='(LND_cap:InitializeRealize)'
  integer                         :: spatialDim
  ...
  if (geomtype == ESMF_GEOMTYPE_MESH) then
     ...
     if (localPet == 0) then
        write(logunit,*)'mesh file for ucland domain is ',trim(cvalue)
     endif
     ...
     ! Check for consistency of lat, lon and mask between mesh and ucland grid
     ...
     do n = 1,numOwnedElements
       ...
       if (diff_lon > eps_omesh) then
         frmt = "('ERROR: Difference between ESMF Mesh and UCLAND domain coords is "//&
       ...
       if (diff_lat > eps_omesh) then
        ...
        if (abs(maskMesh(n) - mask(n)) > 0) then
          frmt = "('ERROR: ESMF mesh and UCLAND domain masks are inconsistent! - "//&
                 "LND n, maskMesh(n), mask(n) = ',3(i8,2x))"
     ...
     call LND_RealizeFields(importState, fldsToOcn_num, fldsToOcn, "Ocn import", mesh=Emesh, rc=rc)
     if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, &
         file=__FILE__)) &
         return

     call LND_RealizeFields(exportState, fldsFrOcn_num, fldsFrOcn, "Ocn export", mesh=Emesh, rc=rc)
     if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, &
         file=__FILE__)) &
         return
     ...
     ! area, mask, and centers should be same size in lnd and esmf grid
     ...
     call LND_RealizeFields(importState, fldsToOcn_num, fldsToOcn, "Ocn import", grid=gridIn, rc=rc)
     if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, &
         file=__FILE__)) &
         return

     call LND_RealizeFields(exportState, fldsFrOcn_num, fldsFrOcn, "Ocn export", grid=gridOut, rc=rc)
     if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, &
         file=__FILE__)) &
         return
  ...
  !---------------------------------
  ! Set module variable geomtype in LND_cap_methods
  !---------------------------------
  call lnd_set_geomtype(geomtype)
...
subroutine DataInitialize(gcomp, rc)
  ...
  character(len=*),parameter  :: subname='(LND_cap:DataInitialize)'
  !--------------------------------
  ...
  call lnd_export(ocean_public, ocean_grid, ocean_state, exportState, clock, rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
    line=__LINE__, &
    file=__FILE__)) &
    return  ! bail out
  ...
  if (NUOPC_IsUpdated(exportState)) then
    ...
    call ESMF_LogWrite("UCLAND - Initialize-Data-Dependency SATISFIED!!!", ESMF_LOGMSG_INFO, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
...
subroutine ModelAdvance(gcomp, rc)
  ...
  character(len=128)                     :: fldname
  character(len=*),parameter             :: subname='(LND_cap:ModelAdvance)'
  character(len=8)                       :: suffix
  integer                                :: num_rest_files

  rc = ESMF_SUCCESS
  if(profile_memory) call ESMF_VMLogMemInfo("Entering LND Model_ADVANCE: ")
  ...
  if (cesm_coupled) then
    if (trim(runtype) == "initial") then
      ...
      ! Do not call UCLAND timestepping routine if the first cpl tstep of a startup run
      if (currTime == startTime) then
        call ESMF_LogWrite("UCLAND - Skipping the first coupling timestep", ESMF_LOGMSG_INFO, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=__LINE__, &
          file=__FILE__)) &
          return  ! bail out
      ...
      if (do_advance) then
         ...
         if (currTime == startTime + timeStep) then
            call ESMF_LogWrite("UCLAND - Stepping back one coupling timestep", ESMF_LOGMSG_INFO, rc=rc)
            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
                 line=__LINE__, &
                 file=__FILE__)) &
                 return  ! bail out
            ...
            call ESMF_LogWrite("UCLAND - doubling the coupling timestep", ESMF_LOGMSG_INFO, rc=rc)
            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
                 line=__LINE__, &
                 file=__FILE__)) &
                 return  ! bail out
  ...
  if (do_advance) then
     ...
     call lnd_import(ocean_public, ocean_grid, importState, ice_ocean_boundary, rc=rc)
     if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=__LINE__, &
          file=__FILE__)) &
          return  ! bail out
     ...
     if(profile_memory) call ESMF_VMLogMemInfo("Entering LND update_ocean_model: ")
     call update_ocean_model(Ice_ocean_boundary, ocean_state, ocean_public, Time, Time_step_coupled)
     if(profile_memory) call ESMF_VMLogMemInfo("Leaving LND update_ocean_model: ")
  ...
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
    ...
    if (cesm_coupled) then
        ...
        write(restartname,'(A,".ucland.r.",I4.4,"-",I2.2,"-",I2.2,"-",I5.5)') &
             trim(casename), year, month, day, seconds

        call ESMF_LogWrite("LND_cap: Writing restart :  "//trim(restartname), ESMF_LOGMSG_INFO, rc=rc)
      ...
      if (ESMF_AlarmIsRinging(stop_alarm, rc=rc)) then
        write(restartname,'(A)')"LND.res"
      else
        write(restartname,'(A,I4.4,"-",I2.2,"-",I2.2,"-",I2.2,"-",I2.2,"-",I2.2)') &
             "LND.res.", year, month, day, hour, minute, seconds
      endif
  ...
  if(profile_memory) call ESMF_VMLogMemInfo("Leaving LND Model_ADVANCE: ")
...
subroutine ModelSetRunClock(gcomp, rc)
  ...
  character(len=*),parameter :: subname='LND_cap:(ModelSetRunClock) '
...
subroutine ocean_model_finalize(gcomp, rc)
  ...
  integer                                :: i
  character(len=*),parameter  :: subname='(LND_cap:ocean_model_finalize)'

  write(*,*) 'LND: --- finalize called ---'
  rc = ESMF_SUCCESS
...
subroutine State_SetScalar(value, scalar_id, State, mytask, scalar_name, scalar_count,  rc)
  ...
  character(len=*), parameter     :: subname='(LND_cap:State_SetScalar)'
  !--------------------------------------------------------
...
subroutine LND_RealizeFields(state, nfields, field_defs, tag, grid, mesh, rc)
  ...
  character(len=*),parameter  :: subname='(LND_cap:LND_RealizeFields)'
  !--------------------------------------------------------
...
contains  !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  subroutine SetScalarField(field, rc)
    ...
    type(ESMF_Distgrid) :: distgrid
    type(ESMF_Grid)     :: grid
    character(len=*), parameter :: subname='(LND_cap:SetScalarField)'
...
end subroutine LND_RealizeFields
...
subroutine fld_list_add(num, fldlist, stdname, transferOffer, shortname)
  ...
  ! local variables
  integer :: rc
  character(len=*), parameter :: subname='(LND_cap:fld_list_add)'
...
!!
!! **This LND cap has been tested with UCLAND.**
!!
!! This document describes the LND NUOPC "cap", which is a light weight software layer that is
!! required when the [LND ocean model](https://github.com/NOAA-GFDL/UCLAND/tree/dev/master)
...
!! [LND wiki](https://github.com/NOAA-GFDL/UCLAND-Examples/wiki) for more documentation.
!!
...
!! standard way.
!!
!! The LND cap package includes the cap code itself (LND_cap.F90, LND_cap_methods.F90
!! and LND_cap_time.F90), a set of time utilities (time_utils.F90) for converting between ESMF and FMS
!! time type and two modules LND_ocean_model_nuopc.F90 and LND_surface_forcing_nuopc.F90. LND_surface_forcing_nuopc.F90
!! converts the input ESMF data (import data) to a LND-specific data type (surface_forcing_CS).
!! LND_ocean_model_nuopc.F90 contains routines for initialization, update and finalization of the ocean model state.
!!
!! @subsection CapSubroutines Cap Subroutines
!!
!! The LND cap modules contains a set of subroutines that are required
...
!! LND cap.  The "Phase" column says whether the subroutine is called during the
!! initialization, run, or finalize part of the coupled system run.
!!
!!<table>
!!<tr><th> Phase  <th> LND Cap Subroutine  <th> Description
!!<tr>
!!  <td> Init
!!  <td> [InitializeP0] (@ref LND_cap_mod::initializep0)
!!  <td> Sets the Initialize Phase Definition (IPD) version to use
!!<tr>
!!  <td> Init
!!  <td> [InitializeAdvertise] (@ref LND_cap_mod::initializeadvertise)
!!  <td> Advertises standard names of import and export fields
!!<tr>
!!  <td> Init
!!  <td> [InitializeRealize] (@ref LND_cap_mod::initializerealize)
!!  <td> Creates an ESMF_Grid or ESMF_Mesh as well as ESMF_Fields for import and export fields
!!<tr>
!!  <td> Run
!!  <td> [ModelAdvance] (@ref LND_cap_mod::modeladvance)
!!  <td> Advances the model by a timestep
!!<tr>
!!  <td> Final
!!  <td> [Finalize] (@ref LND_cap_mod::ocean_model_finalize)
!!  <td> Cleans up
!!</table>
!!
!!
...
!!
!! The cap can accomodate a LND tripolar grid which is represented either as a 2D `ESMF_Grid` or
!! as a 1D `ESMF_Mesh`. Other LND grids (e.g. a bipolar grid) can be represented as a 1d `ESMF_Mesh` only.
...
!! a 2D LND specific surface boundary type and the distinction between the two is no longer there.
!! Calls related to creating the grid are located in the [InitializeRealize]
!! (@ref LND_cap_mod::initializerealize) subroutine, which is called by the NUOPC infrastructure
...
!!  - an `ESMF_DELayout` is created based on the pelist from LND
...
!!    by retrieving those fields from the LND datatype `ocean_grid` elements.
!!
...
!!
!! During the [InitializeAdvertise] (@ref LND_cap_mod::initializeadvertise) phase, calls are
!! made to LND's native initialization subroutines, including `fms_init()`, `constants_init()`,
!! `field_manager_init()`, `diag_manager_init()`, and `set_calendar_type()`.  The MPI communicator
!! is pulled in through the ESMF VM object for the LND component. The dt and start time are set
!! from parameters from the incoming ESMF clock with calls to `set_time()` and `set_date().`
!!
!!
!! @subsection Run Run
!!
!! The [ModelAdvance] (@ref LND_cap_mod::modeladvance) subroutine is called by the NUOPC
!! infrastructure when it's time for LND to advance in time. During this subroutine, there is a
!! call into the LND update routine:
!!
!!      call update_ocean_model(Ice_ocean_boundary, Ocean_state, Ocean_public, Time, Time_step_coupled)
!!
...
!! - lnd_import is called and translates to the ESMF input data to a LND specific data type
!!    - lndentum flux vectors are rotated to internal grid
!!
!! After the call to `update_ocean_model()`, the cap performs these steps:
!! - lnd_export is called
!!   - the `ocean_mask` export is set to match that of the internal LND mask
...
!!
!! Vector rotations are applied to incoming lndentum fluxes (from regular lat-lon to tripolar grid) and
!! outgoing ocean currents (from tripolar to regular lat-lon). The rotation angles are provided
!! from the native LND grid by a call to `get_ocean_grid(Ocean_grid)`.
!! The cosine and sine of the rotation angle are:
!!
!!     ocean_grid%cos_rot(i,j)
!!     ocean_grid%sin_rot(i,j)
!!
!! The rotation of lndentum flux from regular lat-lon to tripolar is:
!! \f[
!! \begin{bmatrix}
!! \tau_x' \\
!! \tau_y'
...
!!
!! NUOPC infrastructure calls [ocean_model_finalize] (@ref LND_cap_mod::ocean_model_finalize)
!! at the end of the run. This subroutine is a hook to call into LND's native shutdown
!! procedures:
!!
...
!!
!! The following tables list the import and export fields currently set up in the LND cap.
!!
...
!!
!! The LND cap has an internal state type with pointers to three
!! types defined by LND. There is also a small wrapper derived type
...
!! [InitializeAdvertise] (@ref LND_cap_mod::initializeadvertise) phase.  Also during that
!! phase, the `ice_ocean_boundary` type members are all allocated using bounds retrieved
!! from `mpp_get_compute_domain()`.
!!
!! During the [InitializeRealize] (@ref LND_cap_mod::initializerealize) phase,
...
!! (@ref LND_cap_mod::dumplndinternal) to write out model internal fields to files
...
!!
!! At runtime, the LND cap can be configured with several options provided
...
!!    (@ref LND_cap_mod::modeladvance) subroutine and before and after the call to
!!   `update_ocean_model()`.
