!> The UCLAND facility for reading and writing restart files, and querying what has been read.
module LND_restart

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_domains, only : pe_here, num_PEs
...
use LND_io, only : create_file, fieldtype, file_exists, open_file, close_file
use LND_io, only : LND_read_data, read_data, get_filename_appendix
use LND_io, only : get_file_info, get_file_atts, get_file_fields, get_file_times
use LND_io, only : vardesc, var_desc, query_vardesc, modify_vardesc
use LND_io, only : MULTIPLE, NETCDF_FILE, READONLY_FILE, SINGLE_FILE
use LND_io, only : CENTER, CORNER, NORTH_FACE, EAST_FACE
use LND_time_manager, only : time_type, time_type_to_real, real_to_time
use LND_time_manager, only : days_in_month, get_date, set_date
use LND_transform_FMS, only : mpp_chksum => rotated_mpp_chksum
use LND_transform_FMS, only : write_field => rotated_write_field
...
type, public :: LND_restart_CS ; private
  ...
  character(len=240) :: restartfile !< The name or name root for LND restart files.
...
subroutine register_restart_field_as_obsolete(field_name, replacement_name, CS)
  ...
  type(LND_restart_CS), pointer :: CS          !< A pointer to a LND_restart_CS object (intent in/out)
...
subroutine register_restart_field_ptr3d(f_ptr, var_desc, mandatory, CS)
  ...
  if (.not.associated(CS)) call LND_error(FATAL, "LND_restart " // &
...
subroutine register_restart_pair_ptr2d(a_ptr, b_ptr, a_desc, b_desc, &
  ...
  type(LND_restart_CS), pointer :: CS   !< LND restart control structure
...
function query_initialized_name(name, CS) result(query_initialized)
  ...
  type(LND_restart_CS), pointer    :: CS !< A pointer to a LND_restart_CS object (intent in)
...
! queried again.
  ...
  if ((n==CS%novars+1) .and. (is_root_pe())) &
    call LND_error(NOTE,"LND_restart: Unknown restart variable "//name// &
                        " queried for initialization.")

  if ((is_root_pe()) .and. query_initialized) &
    call LND_error(NOTE,"LND_restart: "//name// &
...
! queried again.
  ...
  if (n==CS%novars+1) then
    if (is_root_pe()) &
      call LND_error(NOTE,"LND_restart: Unable to find "//name//" queried by pointer, "//&
...
subroutine save_restart(directory, time, G, CS, time_stamped, filename, GV, num_rest_files)
  ...
  type(LND_restart_CS),    pointer       :: CS    !< The control structure returned by a previous
...
! Get NetCDF ids for all of the restart files.
  ...
  if (num_file == 0) then
    ...
    call LND_error(FATAL,"LND_restart: "//mesg)
  endif
...
! Get the time from the first file in the list that has one.
  ...
  if (n>num_file) call LND_error(WARNING,"LND_restart: " // &
      ...
      if (t1 /= t2) then
        write(mesg,'("WARNING: Restart file ",I2," has time ",F10.4,"whereas &
         &simulation is restarted at ",F10.4," (differing by ",F10.4,").")')&
               m,t1,t2,t1-t2
        call LND_error(WARNING, "LND_restart: "//mesg)
      endif
    enddo
  endif
    ...
    do m=1, nvar
      ...
      do i=1,CS%num_obsolete_vars
        if (adjustl(lowercase(trim(varname))) == adjustl(lowercase(trim(CS%restart_obsolete(i)%field_name)))) then
            call LND_error(FATAL, "LND_restart restore_state: Attempting to use obsolete restart field "//&
      ...
      do i=1, nvar
        ...
        if (lowercase(trim(varname)) == lowercase(trim(CS%restart_field(m)%var_name))) then
          ...
          elseif (associated(CS%var_ptr2d(m)%p)) then  ! Read a 2d array.
            if (pos /= 0) then
              call LND_read_data(unit_path(n), varname, CS%var_ptr2d(m)%p, &
          ...
          elseif (associated(CS%var_ptr3d(m)%p)) then  ! Read a 3d array.
            if (pos /= 0) then
              call LND_read_data(unit_path(n), varname, CS%var_ptr3d(m)%p, &
                                 G%Domain, timelevel=1, position=pos)
            else ! This array is not domain-decomposed.  This variant may be under-tested.
          ...
          elseif (associated(CS%var_ptr4d(m)%p)) then  ! Read a 4d array.
            if (pos /= 0) then
              call LND_read_data(unit_path(n), varname, CS%var_ptr4d(m)%p, &
                                 G%Domain, timelevel=1, position=pos)
            else ! This array is not domain-decomposed.  This variant may be under-tested.
            ...
            call LND_error(FATAL, "LND_restart restore_state: No pointers set for "//trim(varname))
          endif
          ...
          if (is_root_pe() .and. is_there_a_checksum .and. (checksum_file(1) /= checksum_data)) then
             ...
             call LND_error(FATAL, "LND_restart(restore_state): "//trim(mesg) )
          endif
  ...
  do m=1,CS%novars
    if (.not.(CS%restart_field(m)%initialized)) then
      ...
      if (CS%restart_field(m)%mand_var) then
        call LND_error(FATAL,"LND_restart: Unable to find mandatory variable " &
...
function is_new_run(CS)
  ...
  if (.not.CS%new_run_set) call LND_error(FATAL, "LND_restart " // &
  ...
  do while (start_char <= len_trim(filename) )
    ...
    if ((fname(1:1)=='r') .and. ( len_trim(fname) == 1)) then
       ...
       if (len_trim(filename_appendix) > 0) then
        ...
        if (fexists) then
          ...
          if (is_root_pe() .and. (present(units))) &
            call LND_error(NOTE, "LND_restart: LND run restarted using : "//trim(filepath))
        else
          err = 1 ; exit
        endif
      ...
      if (fexists) then
        ...
        if (present(units)) &
          call LND_error(WARNING,"LND_restart: Unable to find restart file : "//trim(filepath))
      endif
...
subroutine restart_init(param_file, CS, restart_root)
  ...
  type(LND_restart_CS),  pointer    :: CS !< A pointer to a LND_restart_CS object that is allocated here
  character(len=*), optional, &
                         intent(in) :: restart_root !< A filename root that overrides the value
                                          ...
                                          !! other components than LND.

  logical :: rotate_index

! This include declares and sets the variable "version".
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_restart"   ! This module's name.
  logical :: all_default   ! If true, all parameters are using their default values.

  if (associated(CS)) then
    call LND_error(WARNING, "restart_init called with an associated control structure.")
    return
  endif
  allocate(CS)
  ...
  if (.not.present(restart_root)) then
    call get_param(param_file, mdl, "RESTARTFILE", CS%restartfile, &
                   default="LND.res", do_not_log=.true.)
    all_default = (all_default .and. (trim(CS%restartfile) == trim("LND.res")))
  endif
  ...
  if (present(restart_root)) then
    ...
    call get_param(param_file, mdl, "RESTARTFILE", CS%restartfile, &
                 "The name-root of the restart file.", default="LND.res")
  endif
  call get_param(param_file, mdl, "LARGE_FILE_SUPPORT", CS%large_file_support, &
...
subroutine restart_init_end(CS)
  type(LND_restart_CS),  pointer    :: CS !< A pointer to a LND_restart_CS object
...
!> Deallocate memory associated with a LND_restart_CS variable.
...
subroutine restart_error(CS)
  ...
  if (CS%novars > CS%max_fields) then
    ...
    call LND_error(FATAL,"LND_restart: Too many fields registered for " // &
           "restart.  Set MAX_FIELDS to be at least " // &
           trim(adjustl(num)) // " in the LND input file.")
  else
    call LND_error(FATAL,"LND_restart: Unspecified fatal error.")
  endif
