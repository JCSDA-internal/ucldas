...
use LND_file_parser, only : read_param, log_version, param_file_type
...
#include "version_variable.h"
  ...
  if (test_logic .and. .not.split) call LND_ERROR(FATAL, &
...
subroutine obsolete_logical(param_file, varname, warning_val, hint)
  ...
  if (test_logic) then
    if (fatal_err) then
      call LND_ERROR(FATAL, "LND_obsolete_params: "//trim(varname)//   &
           " is an obsolete run-time flag, and should not be used. "// &
           trim(hint_msg))
    else
      call LND_ERROR(WARNING, "LND_obsolete_params: "//trim(varname)// &
  ...
  if (len_trim(test_string) > 0) then
    ...
    if (only_warn) then
      call LND_ERROR(WARNING, &
             "LND_obsolete_params: "//trim(varname)// &
             " is an obsolete run-time flag. "//trim(hint_msg))
    else
...
end module LND_obsolete_params
