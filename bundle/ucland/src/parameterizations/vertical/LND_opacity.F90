...
!> The control structure with paramters for the LND_opacity module
...
subroutine set_opacity(optics, sw_total, sw_vis_dir, sw_vis_dif, sw_nir_dir, sw_nir_dif, &
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "set_opacity: "// &
  ...
  if (present(chl_2d) .or. present(chl_3d)) then
    ...
    call opacity_from_chl(optics, sw_total, sw_vis_dir, sw_vis_dif, sw_nir_dir, sw_nir_dif, &
                          G, GV, US, CS, chl_2d, chl_3d)
  else ! Use sw e-folding scale set by LND_input
...
!   global model, Geophys. Res. Let., , L05,603, 2005.
  ...
  if (present(chl_3d)) then
    ...
    do k=1,nz; do j=js,je ; do i=is,ie
      if ((G%mask2dT(i,j) > 0.5) .and. (chl_3d(i,j,k) < 0.0)) then
        ...
        call LND_error(FATAL, "LND_opacity opacity_from_chl: "//trim(mesg))
      endif
    enddo ; enddo ; enddo
  elseif (present(chl_2d)) then
    ...
    call LND_error(FATAL, "Either chl_2d or chl_3d must be preesnt in a call to opacity_form_chl.")
  endif
  ...
  select case (CS%opacity_scheme)
    ...
    case (MOREL_88)
      ...
      enddo ; enddo
    case default
      call LND_error(FATAL, "opacity_from_chl: CS%opacity_scheme is not valid.")
  end select
...
subroutine opacity_init(Time, G, GV, US, param_file, diag, CS, optics)
  ...
  character(len=40)  :: mdl = "LND_opacity"
...
# include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "opacity_init called with an associated"// &
...
! parameters for CHL_A routines
  ...
  if (CS%var_pen_sw) then
    ...
    if (len_trim(tmpstr) > 0) then
      ...
      select case (tmpstr)
        ...
        case default
          call LND_error(FATAL, "opacity_init: #DEFINE OPACITY_SCHEME "//&
                                  trim(tmpstr) // "in input file is invalid.")
      end select
      call LND_mesg('opacity_init: opacity scheme set to "'//trim(tmpstr)//'".', 5)
    endif
    if (CS%opacity_scheme == NO_SCHEME) then
      call LND_error(WARNING, "opacity_init: No scheme has successfully "//&
  ...
  call get_param(param_file, mdl, "PEN_SW_NBANDS", optics%nbands, &
                 ...
                 default=1)
  if (CS%Opacity_scheme == DOUBLE_EXP ) then
    if (optics%nbands /= 2) call LND_error(FATAL, &
        "set_opacity: \Cannot use a double_exp opacity scheme with nbands!=2.")
  elseif (CS%Opacity_scheme == SINGLE_EXP ) then
    if (optics%nbands /= 1) call LND_error(FATAL, &
