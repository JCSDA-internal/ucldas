...
use LND_dyn_horgrid, only : dyn_horgrid_type, rescale_dyn_horgrid_bathymetry
...
use LND_open_boundary, only : open_boundary_config, open_boundary_query
use LND_open_boundary, only : open_boundary_impose_normal_slope
use LND_open_boundary, only : open_boundary_impose_land_mask
! use LND_shared_initialization, only : LND_shared_init_init
use LND_shared_initialization, only : LND_initialize_rotation, LND_calculate_grad_Coriolis
use LND_shared_initialization, only : initialize_topography_from_file, apply_topography_edits_from_file
use LND_shared_initialization, only : initialize_topography_named, limit_topography, diagnoseMaximumDepth
use LND_shared_initialization, only : set_rotation_planetary, set_rotation_beta_plane, initialize_grid_rotation_angle
use LND_shared_initialization, only : reset_face_lengths_named, reset_face_lengths_file, reset_face_lengths_list
use LND_shared_initialization, only : read_face_length_list, set_velocity_depth_max, set_velocity_depth_min
use LND_shared_initialization, only : compute_global_grid_integrals, write_ocean_geometry_file
...
public LND_initialize_fixed, LND_initialize_rotation, LND_initialize_topography

contains

! -----------------------------------------------------------------------------
!> LND_initialize_fixed sets up time-invariant quantities related to UCLAND's
!!   horizontal grid, bathymetry, and the Coriolis parameter.
subroutine LND_initialize_fixed(G, US, OBC, PF, write_geom, output_dir)
  ...
  character(len=40)  :: mdl = "LND_fixed_initialization" ! This module's name.
  logical :: debug
! This include declares and sets the variable "version".
#include "version_variable.h"

  call callTree_enter("LND_initialize_fixed(), LND_fixed_initialization.F90")
  ...
  call LND_initialize_topography(G%bathyT, G%max_depth, G, PF, US)
!  call rescale_dyn_horgrid_bathymetry(G, US%Z_to_m)
  ...
  if (debug) then
    call hchksum(G%bathyT, 'LND_initialize_fixed: depth ', G%HI, haloshift=1, scale=US%Z_to_m)
    call hchksum(G%mask2dT, 'LND_initialize_fixed: mask2dT ', G%HI)
    call uvchksum('LND_initialize_fixed: mask2dC[uv]', G%mask2dCu, &
                  G%mask2dCv, G%HI)
    call qchksum(G%mask2dBu, 'LND_initialize_fixed: mask2dBu ', G%HI)
  endif
  ...
  call get_param(PF, mdl, "CHANNEL_CONFIG", config, &
                 ...
                 " \t\t text file, like LND_channel_list in the LND_SIS \n"//&
                 " \t\t test case.\n"//&
  ...
  select case ( trim(config) )
    ...
    case default ; call LND_error(FATAL, "LND_initialize_fixed: "// &
...
!  of the q grid points [s-1].
  call LND_initialize_rotation(G%CoriolisBu, G, PF, US=US)
!   Calculate the components of grad f (beta)
  call LND_calculate_grad_Coriolis(G%dF_dx, G%dF_dy, G, US=US)
  if (debug) then
    call qchksum(G%CoriolisBu, "LND_initialize_fixed: f ", G%HI, scale=US%s_to_T)
    call hchksum(G%dF_dx, "LND_initialize_fixed: dF_dx ", G%HI, scale=US%m_to_L*US%s_to_T)
    call hchksum(G%dF_dy, "LND_initialize_fixed: dF_dy ", G%HI, scale=US%m_to_L*US%s_to_T)
  endif
...
! Write out all of the grid data used by this run.
  ...
  call callTree_leave('LND_initialize_fixed()')

end subroutine LND_initialize_fixed

!> LND_initialize_topography makes the appropriate call to set up the bathymetry.  At this
!! point the topography is in units of [m], but this can be changed later.
subroutine LND_initialize_topography(D, max_depth, G, PF, US)
  ...
  character(len=40)  :: mdl = "LND_initialize_topography" ! This subroutine's name.
  character(len=200) :: config
  ...
  select case ( trim(config) )
    ...
    case default ;      call LND_error(FATAL,"LND_initialize_topography: "// &
...
end subroutine LND_initialize_topography

end module LND_fixed_initialization
