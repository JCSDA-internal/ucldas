...
use LND_diag_mediator, only : diag_mediator_init, set_diag_mediator_grid
use LND_diag_mediator, only : diag_ctrl, time_type, enable_averages, disable_averaging
...
use LND_io, only : file_exists, slasher, LND_read_data
use LND_restart, only : register_restart_field, query_initialized
...
use LND_ice_shelf_state, only : ice_shelf_state
use LND_coms, only : reproducing_sum, sum_across_PEs, max_across_PEs, min_across_PEs
use LND_checksums, only : hchksum, qchksum
...
subroutine register_ice_shelf_dyn_restarts(G, param_file, CS, restart_CS)
  ...
  character(len=40)  :: mdl = "LND_ice_shelf_dyn"  ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(FATAL, "LND_ice_shelf_dyn.F90, register_ice_shelf_dyn_restarts: "// &
...
# include "version_variable.h"
  ...
  if (.not.associated(CS)) then
    call LND_error(FATAL, "LND_ice_shelf_dyn.F90, initialize_ice_shelf_dyn: "// &
                          "called with an associated control structure.")
    return
  endif
  if (CS%module_is_initialized) then
    call LND_error(WARNING, "LND_ice_shelf_dyn.F90, initialize_ice_shelf_dyn was "//&
  ...
  if (shelf_mass_is_dynamic) then
    ...
    if (CS%GL_regularize .and. (CS%n_sub_regularize == 0)) call LND_error (FATAL, &
  ...
  if (active_shelf_dynamics) then
    ...
    if (CS%calve_to_mask) then
      call LND_mesg("  LND_ice_shelf.F90, initialize_ice_shelf: reading calving_mask")
      ...
      call LND_read_data(filename,trim(var_name),CS%calve_mask,G%Domain)
      do j=G%jsc,G%jec ; do i=G%isc,G%iec
...
!    call init_boundary_values(CS, G, time, ISS%hmask, CS%input_flux, CS%input_thickness, new_sim)
    ...
    if (new_sim) then
      call LND_mesg("LND_ice_shelf.F90, initialize_ice_shelf: initialize ice velocity.")
...
!    into partial cells
  ...
  h_after_uflux(:,:) = 0.0
  h_after_vflux(:,:) = 0.0
  ! call LND_mesg("LND_ice_shelf.F90: ice_shelf_advect called")
  ...
  if (LB%jsh < jsd) call LND_error(FATAL, &
...
!
! OD -this is important and we do not yet know where (in LND) it will come from. It represents
...
!! vertices of the cell using a locally orthogoal UCLAND grid.  Values are calculated at
!! points of gaussian quadrature.
...
end module LND_ice_shelf_dynamics
