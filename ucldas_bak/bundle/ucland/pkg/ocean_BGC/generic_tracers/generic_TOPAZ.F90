...
! 
! B. Reproducing original LND TOPAZ results of ocean_topaz.F90
! Reproducing LND TOPAZ results is generally less accurate since the whole vertical diffusion scheme is being
...
!## jgj NOTE: k_sio4_Sm = 1.0e-6 in lnd4p1
    ...
    ! per JPD 2009/12/22 Values above were tuned for lnd4p0/ESM2.1.  Try using original values from literature
...
!         prog       = .true.                          )
  ...
  subroutine generic_TOPAZ_update_from_source(tracer_list,Temp,Salt,rho_dzt,dzt,hblt_depth,&
    ...
    do k = 1, nk  ; do j = jsc, jec ; do i = isc, iec  
       ...
       !-----------------------------------------------------------------------
       !
       ! phyto(LARGE)%k_sio4 was topaz(n)%k_sio4_Sm in LND_TOPAZ code.
...
!---------------------------------------------------------------------
  ...
  subroutine generic_TOPAZ_set_boundary_values(tracer_list,SST,SSS,rho,ilb,jlb,tau)
    ...
    do j=jsc,jec ; do i=isc,iec
       ...
       !      identical with topaz code in LND topaz%Rho_0 must be replaced with rho(i,j,1,tau)
...
!
!       co2_alpha(i,j) = co2_alpha(i,j)* sc_no_term * topaz%Rho_0 !nnz: LND has rho(i,j,1,tau)  
!       co2_csurf(i,j) = co2_csurf(i,j)* sc_no_term * topaz%Rho_0 !nnz: LND has rho(i,j,1,tau) 
!
! in 'ocmip2_new' atmos_ocean_fluxes.F90 coupler formulation, the schmidt number is carried in explicitly
!
       co2_alpha(i,j) = co2_alpha(i,j) * topaz%Rho_0 !nnz: LND has rho(i,j,1,tau)  
       co2_csurf(i,j) = co2_csurf(i,j) * topaz%Rho_0 !nnz: LND has rho(i,j,1,tau) 
       ...
       o2_csurf(i,j) = o2_field(i,j,1,tau) * topaz%Rho_0 !nnz: LND has rho(i,j,1,tau)
