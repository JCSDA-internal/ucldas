...
!*  LND_ice_shelf.F90.  The arguments of each subroutine are           *
...
!********+*********+*********+*********+*********+*********+*********+**
  ...
  use LND_debugging,       only : LND_debugging_init
  use LND_diag_mediator,   only : diag_mediator_init, diag_mediator_infrastructure_init
  ...
  use LND_domains,         only : LND_domains_init, clone_LND_domain, pass_var
  ...
  use LND_time_manager,    only : JULIAN, GREGORIAN, NOLEAP, THIRTY_DAY_MONTHS, NO_CALENDAR
  ...
  use LND_ice_shelf, only : ice_shelf_save_restart, solo_step_ice_shelf
...
# include "version_variable.h"
  ...
  call LND_mesg('======== Model being driven by ice_shelf_driver ========', 2)
  ...
  if (file_exists(trim(dirs%restart_input_dir)//'ice_solo.res')) then
    ...
    elseif (calendar(1:1) /= ' ') then
      call LND_error(FATAL,'Shelf_driver: Invalid namelist value '//trim(calendar)//' for calendar')
    else
      call LND_error(FATAL,'Shelf_driver: No namelist value for calendar')
    endif
  endif
  ...
  if (.not.use_ice_shelf) call LND_error(FATAL, "Shelf_driver: Run stops unless ICE_SHELF is true.")
  ...
  ! This is the start of the code that is the counterpart of LND_initialization.
  ...
  call LND_domains_init(ocn_grid%domain, param_file)
  call hor_index_init(ocn_grid%Domain, HI, param_file)
  call create_dyn_horgrid(dG, HI)
  call clone_LND_domain(ocn_grid%Domain, dG%Domain)

  ! Initialize the ocean grid and topography.
  call LND_initialize_fixed(dG, US, OBC, param_file, .true., dirs%output_directory)
  call LND_grid_init(ocn_grid, param_file, US, HI)
  call copy_dyngrid_to_LND_grid(dG, ocn_grid, US)
  ...
  ! This is the end of the code that is the counterpart of LND_initialization.
...
!   grid => ice_shelf_CSp%grid
  ...
  if (years+months+days+hours+minutes+seconds > 0) then
    ...
    call LND_mesg('Segment run length determined from ice_solo_nml.', 2)
