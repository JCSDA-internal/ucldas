...
!! covariance matrices of the UCLDAS analysis.

module ucldas_covariance_mod
...
private
public :: ucldas_cov, ucldas_cov_setup, ucldas_cov_delete, &
          ucldas_cov_C_mult, ucldas_cov_sqrt_C_mult

!> Fortran derived type to hold configuration data for the UCLDAS background/model covariance
type :: ucldas_cov
   type(bump_type),     pointer :: conv(:)        !< convolution op from bump
   type(ucldas_state),    pointer :: bkg            !< Background field (or first guess)
 ...
 contains
   procedure :: setup => ucldas_cov_setup
   procedure :: delete => ucldas_cov_delete
   procedure :: mult => ucldas_cov_C_mult
   procedure :: sqrt_C_mult => ucldas_cov_sqrt_C_mult
   procedure :: getConv => ucldas_cov_get_conv
end type ucldas_cov
...
!> Setup for the UCLDAS model's 3d error covariance matrices (B and Q_i)
...
subroutine ucldas_cov_setup(self, f_conf, geom, bkg, vars)
  class(ucldas_cov),        intent(inout) :: self   !< The covariance structure
  type(fckit_configuration), intent(in) :: f_conf !< The configuration
  type(ucldas_geom),           intent(in) :: geom   !< Geometry
  type(ucldas_state),  target, intent(in) :: bkg    !< Background
  ...
  do i=1,size(f_conf_list)
    ...
    call ucldas_bump_correlation(self, self%conv(i), geom, f_conf2, f_conf_list(i), domain)
  end do

end subroutine ucldas_cov_setup

! ------------------------------------------------------------------------------

!> Delete for the UCLDAS model's 3d error covariance matrices

subroutine ucldas_cov_delete(self)
  class(ucldas_cov), intent(inout) :: self       !< The covariance structure
...
end subroutine ucldas_cov_delete

! ------------------------------------------------------------------------------

subroutine ucldas_cov_get_conv(self, field, conv)
  class(ucldas_cov),        intent(inout) :: self
  type(ucldas_field), pointer, intent(in) :: field
...
subroutine ucldas_cov_C_mult(self, dx)
  class(ucldas_cov),      intent(inout) :: self !< The covariance structure
  type(ucldas_increment), intent(inout) :: dx   !< Input: Increment
                                          !< Output: C dx
  integer :: i, z
  ...
  do i = 1, self%vars%nvars()
    ...
    do z = 1, field%nz
      call ucldas_2d_convol(field%val(:,:,z), conv, dx%geom)
    end do
  end do
end subroutine ucldas_cov_C_mult

! ------------------------------------------------------------------------------

subroutine ucldas_cov_sqrt_C_mult(self, dx)
  class(ucldas_cov),      intent(inout) :: self !< The covariance structure
  type(ucldas_increment), intent(inout) :: dx   !< Input: Increment
                                          !< Output: C^1/2 dx
  integer :: i, z, j
  ...
  do i = 1, self%vars%nvars()
    ...
    ! apply convolution
    do z = 1,field%nz
      call ucldas_2d_sqrt_convol(field%val(:,:,z), conv, dx%geom, scale)
    end do

  end do
end subroutine ucldas_cov_sqrt_C_mult

! ------------------------------------------------------------------------------

subroutine ucldas_bump_correlation(self, horiz_convol, geom, f_conf_bump, f_conf_domain, domain)
  class(ucldas_cov),        intent(inout) :: self   !< The covariance structure
...
end subroutine ucldas_bump_correlation

! ------------------------------------------------------------------------------

subroutine ucldas_2d_convol(dx, horiz_convol, geom)
...
end subroutine ucldas_2d_convol

! ------------------------------------------------------------------------------

subroutine ucldas_2d_sqrt_convol(dx, horiz_convol, geom, pert_scale)
  real(kind=kind_real), intent(inout) :: dx(:,:)
  type(bump_type),      intent(inout) :: horiz_convol
  type(ucldas_geom),         intent(in) :: geom
...
end subroutine ucldas_2d_sqrt_convol

end module ucldas_covariance_mod
