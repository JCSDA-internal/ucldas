!> This module implements boundary forcing for UCLAND.
module LND_forcing_type

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_array_transform, only : rotate_array, rotate_vector, rotate_array_pair
use LND_debugging,     only : hchksum, uvchksum
use LND_cpu_clock,     only : cpu_clock_id, cpu_clock_begin, cpu_clock_end, CLOCK_ROUTINE
use LND_diag_mediator, only : post_data, register_diag_field, register_scalar_field
use LND_diag_mediator, only : time_type, diag_ctrl, safe_alloc_alloc, query_averaging_enabled
...
use LND_EOS,           only : calculate_density_derivs, EOS_domain
use LND_file_parser,   only : get_param, log_param, log_version, param_file_type
use LND_grid,          only : ocean_grid_type
use LND_opacity,       only : sumSWoverBands, optics_type, extract_optics_slice, optics_nbands
use LND_spatial_means, only : global_area_integral, global_area_mean
...
public LND_forcing_chksum, LND_mech_forcing_chksum
...
!! liquid ocean simulated by LND.
!!
!! Data in this type is allocated in the module LND_surface_forcing.F90, of which there
!! are three: solo, coupled, and ice-shelf. Alternatively, they are allocated in
!! MESO_surface_forcing.F90, which is a special case of solo_driver/LND_surface_forcing.F90.
type, public :: forcing
  ...
  type(coupler_2d_bc_type) :: tr_fluxes !< This structure contains arrays of
     ...
     !! This is not a convenient convention, but imposed on UCLAND by the coupler.
...
!! used to drive the liquid ocean simulated by LND.
!! Data in this type is allocated in the module LND_surface_forcing.F90,
...
type, public :: forcing_diags
  ...
  ! lndentum flux and forcing diagnostic handles
  integer :: id_taux  = -1
  integer :: id_tauy  = -1
  integer :: id_ustar = -1
...
subroutine extractFluxes1d(G, GV, US, fluxes, optics, nsw, j, dt, &
  ...
  if (nsw > 0) then ; if (nsw /= optics_nbands(optics)) call LND_error(WARNING, &
    "mismatch in the number of bands of shortwave radiation in LND_forcing_type extract_fluxes.")
  endif

  if (.not.associated(fluxes%sw)) call LND_error(FATAL, &
    "LND_forcing_type extractFluxes1d: fluxes%sw is not associated.")

  if (.not.associated(fluxes%lw)) call LND_error(FATAL, &
    "LND_forcing_type extractFluxes1d: fluxes%lw is not associated.")

  if (.not.associated(fluxes%latent)) call LND_error(FATAL, &
    "LND_forcing_type extractFluxes1d: fluxes%latent is not associated.")

  if (.not.associated(fluxes%sens)) call LND_error(FATAL, &
    "LND_forcing_type extractFluxes1d: fluxes%sens is not associated.")

  if (.not.associated(fluxes%evap)) call LND_error(FATAL, &
    "LND_forcing_type extractFluxes1d: No evaporation defined.")

  if (.not.associated(fluxes%vprec)) call LND_error(FATAL, &
    "LND_forcing_type extractFluxes1d: fluxes%vprec not defined.")

  if ((.not.associated(fluxes%lprec)) .or. &
      (.not.associated(fluxes%fprec))) call LND_error(FATAL, &
    "LND_forcing_type extractFluxes1d: No precipitation defined.")
  ...
  do i=is,ie
    ...
    ! which case heat_content_evap is computed in LND_diabatic_driver.F90
  ...
  !   if (associated(fluxes%heat_content_cond)) fluxes%heat_content_cond(i,j) = 0.0 !??? --AJA
    ...
    ! in which case heat_content_vprec is computed in LND_diabatic_driver.F90.
...
! smg: new code
    ...
    ! inside LND_diabatic_driver.F90 and fill in fluxes%heat_content_massout.
...
!    endif
    ...
    if (calculate_diags) then
      ...
      ! wait until LND_diabatic_driver.F90.
      if (associated(fluxes%heat_content_lprec)) then
      ...
      ! vprec < 0 means remove water from ocean; set heat_content_vprec in LND_diabatic_driver.F90
      ...
      ! compute fluxes%heat_content_massout at the relevant point inside LND_diabatic_driver.F90.
...
subroutine LND_forcing_chksum(mesg, fluxes, G, US, haloshift)
  ...
  if (associated(fluxes%heat_content_massout)) &
    call hchksum(fluxes%heat_content_massout, mesg//" fluxes%heat_content_massout", G%HI, &
                 haloshift=hshift, scale=US%QRZ_T_to_W_m2)
end subroutine LND_forcing_chksum

!> Write out chksums for the driving mechanical forces.
subroutine LND_mech_forcing_chksum(mesg, forces, G, US, haloshift)
  character(len=*),        intent(in) :: mesg      !< message
...
end subroutine LND_mech_forcing_chksum
...
subroutine mech_forcing_SinglePointPrint(forces, G, i, j, mesg)
  ...
  write(0,'(2a)') 'LND_forcing_type, forcing_SinglePointPrint: Called from ',mesg
  write(0,'(a,2es15.3)') 'LND_forcing_type, forcing_SinglePointPrint: lon,lat = ',G%geoLonT(i,j),G%geoLatT(i,j)
  ...
  subroutine locMsg(array,aname)
    ...
    if (associated(array)) then
      write(0,'(3a,es15.3)') 'LND_forcing_type, mech_forcing_SinglePointPrint: ',trim(aname),' = ',array(i,j)
    else
      write(0,'(4a)') 'LND_forcing_type, mech_forcing_SinglePointPrint: ',trim(aname),' is not associated.'
    endif
...
subroutine forcing_SinglePointPrint(fluxes, G, i, j, mesg)
  ...
  subroutine locMsg(array,aname)
    ...
    if (associated(array)) then
      write(0,'(3a,es15.3)') 'LND_forcing_type, forcing_SinglePointPrint: ',trim(aname),' = ',array(i,j)
    else
      write(0,'(4a)') 'LND_forcing_type, forcing_SinglePointPrint: ',trim(aname),' is not associated.'
    endif
  end subroutine locMsg
...
subroutine fluxes_accumulate(flux_tmp, fluxes, G, wt2, forces)
  ...
  if (fluxes%dt_buoy_accum < 0) call LND_error(FATAL, "fluxes_accumulate: "//&
...
subroutine rotate_forcing(fluxes_in, fluxes, turns)
  ...
  if (coupler_type_initialized(fluxes%tr_fluxes)) &
    call LND_error(FATAL, "Rotation of tracer BC fluxes not yet implemented.")
...
!! from lndentum, heat, salt, and mass.  Boundary fluxes from other
!! tracers are treated by coupling to biogeochemical models. We
!! here present elements of how UCLAND assumes boundary fluxes are
...
!!
!! \subsection subsection_lndentum_fluxes Surface boundary lndentum fluxes
!!
!! The ocean surface exchanges lndentum with the overlying atmosphere,
!! sea ice, and land ice. The lndentum is exchanged as a horizontal
...
!! UCLAND/src/parameterizations/vertical/LND_diabatic_aux.F90
!!

end module LND_forcing_type
