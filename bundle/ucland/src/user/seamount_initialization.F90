...
subroutine seamount_initialize_temperature_salinity ( T, S, h, G, GV, param_file, &
  ...
  select case ( coordinateMode(verticalCoordinate) )
    case ( REGRIDDING_LAYER ) ! Initial thicknesses for layer isopycnal coordinates
      ! These parameters are used in LND_fixed_initialization.F90 when CONFIG_COORD="ts_range"
      ...
      do j=js,je ; do i=is,ie
        ...
        do k = 1,nz
          ...
          select case ( trim(density_profile) )
            ...
            case default
              call LND_error(FATAL, 'Unknown value for "INITIAL_DENSITY_PROFILE"')
          end select
          xi0 = xi1
        enddo
      enddo ; enddo
