!> This is the main driver for UCLAND in CIME
module ocn_comp_mct
...
! UCLAND modules
use LND,                  only: extract_surface_state
use LND_variables,        only: surface
use LND_domains,          only: LND_infra_init
use LND_restart,          only: save_restart
use LND_ice_shelf,        only: ice_shelf_save_restart
use LND_domains,          only: num_pes, root_pe, pe_here
use LND_grid,             only: ocean_grid_type, get_global_grid_size
use LND_error_handler,    only: LND_error, FATAL, is_root_pe, WARNING
use LND_time_manager,     only: time_type, set_date, set_time, set_calendar_type, NOLEAP
use LND_time_manager,     only: operator(+), operator(-), operator(*), operator(/)
use LND_time_manager,     only: operator(==), operator(/=), operator(>), get_time
...
use LND_domains,          only: AGRID, BGRID_NE, CGRID_NE, pass_vector
use mpp_domains_mod,      only: mpp_get_compute_domain

! Previously inlined - now in separate modules
use LND_ocean_model_mct,     only: ocean_public_type, ocean_state_type
use LND_ocean_model_mct,     only: ocean_model_init , update_ocean_model, ocean_model_end
use LND_ocean_model_mct,     only: convert_state_to_ocean_type
use LND_surface_forcing_mct, only: surface_forcing_CS, forcing_save_restart, ice_ocean_boundary_type
use ocn_cap_methods,      only: ocn_import, ocn_export

! FMS modules
use time_interp_external_mod, only : time_interp_external

! MCT indices structure and import and export routines that access lnd data
...
end type MCT_LND_Data

type(MCT_LND_Data)            :: glb !< global structure
...
!> This subroutine initializes UCLAND.
subroutine ocn_init_mct( EClock, cdata_o, x2o_o, o2x_o, NLFilename )
  ...
  integer                   :: LND_MCT_ID
  type(mct_gsMap), pointer  :: LND_MCT_gsMap => NULL() !< 2d, points to cdata
  type(mct_gGrid), pointer  :: LND_MCT_dom => NULL()   !< 2d, points to cdata
  type(mct_gsMap)           :: LND_MCT_gsMap3d         !< for 3d streams, local
  type(mct_gGrid)           :: LND_MCT_dom3d           !< for 3d streams, local

  ! time management
  integer                   :: ocn_cpl_dt   !< one ocn coupling interval in seconds. (to be received from cesm)
  real (kind=8)             :: lnd_cpl_dt   !< one ocn coupling interval in seconds. (internal)
  ...
  ! TODO: Change the following vars with the corresponding UCLAND vars
  ...
  call seq_cdata_setptrs(cdata_o, id=LND_MCT_ID, mpicom=mpicom_ocn, &
                         gsMap=LND_MCT_gsMap, dom=LND_MCT_dom, infodata=glb%infodata)
  ...
  ! instance control
  inst_name   = seq_comm_name(LND_MCT_ID)
  inst_index  = seq_comm_inst(LND_MCT_ID)
  inst_suffix = seq_comm_suffix(LND_MCT_ID)

  call t_startf('LND_init')

  ! Initialize UCLAND comm
  call LND_infra_init(mpicom_ocn)
  ...
  call get_LND_Input(param_file, dirs_tmp, check_params=.false.)
  ...
  if (runtype == "initial") then
    ...
    do
      ...
      if (len(trim(restartfiles))>1 .and. iostat<0) then
        ...
        call LND_error(FATAL, 'Error reading rpointer.ocn')
      endif
  ...
  call t_stopf('LND_init')

  ! Initialize MCT attribute vectors and indices
  call t_startf('LND_mct_init')

  if (debug .and. root_pe().eq.pe_here()) print *, "calling ocn_SetGSMap_mct"

  ! Set mct global seg maps:

  call ocn_SetGSMap_mct(mpicom_ocn, LND_MCT_ID, LND_MCT_GSMap, LND_MCT_GSMap3d)
  lsize = mct_gsMap_lsize(LND_MCT_gsmap, mpicom_ocn)

  ! Initialize mct ocn domain (needs ocn initialization info)

  if (debug .and. root_pe().eq.pe_here()) print *, "calling ocn_domain_mct"
  call ocn_domain_mct(lsize, LND_MCT_gsmap, LND_MCT_dom)
  !call ocn_domain_mct(lsize*km, LND_MCT_gsmap3d, LND_MCT_dom3d) !TODO: this is not used
  ...
  ! \todo Need interface to get dt from UCLAND
  ncouple_per_day = seconds_in_day / ocn_cpl_dt
  lnd_cpl_dt = seconds_in_day / ncouple_per_day
  if (lnd_cpl_dt /= ocn_cpl_dt) then
    write(glb%stdout,*) 'ERROR lnd_cpl_dt and ocn_cpl_dt must be identical'
    call exit(0)
  end if
  ...
  call ocn_export(glb%ind, glb%ocn_public, glb%grid, o2x_o%rattr, lnd_cpl_dt, ncouple_per_day)

  call t_stopf('LND_mct_init')
...
subroutine ocn_run_mct( EClock, cdata_o, x2o_o, o2x_o)
  ...
  logical :: debug=.false.
  type(time_type) :: time_start        !< Start of coupled time interval to pass to UCLAND
  type(time_type) :: coupling_timestep !< Coupled time interval to pass to UCLAND
  character(len=128) :: err_msg        !< Error message
  ...
  ! With this change, UCLAND starts at the same date as the other components, and runs for the same
  ! duration as other components, unlike POP, which would have one missing interval due to ocean
  ! lag. UCLAND accounts for this lag by doubling the duration of the first coupling interval.
  if (firstCall) then
  ...
  if (write_restart_at_eod) then
    ...
    write(restartname,'(A,".ucland.r.",I4.4,"-",I2.2,"-",I2.2,"-",I5.5)') trim(runid), year, month, day, seconds
...
!> Sets mct global segment maps for the LND decomposition.
!!
!! \todo Find out if we should only provide indirect indexing for ocean points and not land.
subroutine ocn_SetGSMap_mct(mpicom_ocn, LND_MCT_ID, gsMap_ocn, gsMap3d_ocn)
  integer,         intent(in)    :: mpicom_ocn  !< MPI communicator
  integer,         intent(in)    :: LND_MCT_ID  !< MCT component ID
  ...
  if (.not. associated(grid)) call LND_error(FATAL, 'ocn_comp_mct.F90, ocn_SetGSMap_mct():' // &
  ...
  call mct_gsMap_init(gsMap_ocn, gindex, mpicom_ocn, LND_MCT_ID, lsize, ni * nj)
...
!> Sets MCT global segment maps for the UCLAND decomposition
...
!!
!! \section section_ocn_import Fluxes imported from the coupler (MCT) to UCLAND
!! The following summarizes the mismatches between MCT and UCLAND in terms
!! of ice ocean fluxes.
!!
!! Redundancies:
...
!! x2o_Foxx_rof
!!
!! Variables in UCLAND fluxes that are **NOT** filled by the coupler:
...
!!
!! Variables in the coupler that are **NOT** used in UCLAND (i.e., no corresponding field in fluxes):
...
!!
!! \section section_ocn_export Fluxes exported from UCLAND to the coupler (MCT)
!!
