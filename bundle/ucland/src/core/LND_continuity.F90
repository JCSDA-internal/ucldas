...
use LND_continuity_PPM, only : continuity_PPM, continuity_PPM_init
use LND_continuity_PPM, only : continuity_PPM_stencil
use LND_continuity_PPM, only : continuity_PPM_end, continuity_PPM_CS
...
!> Control structure for lnd_continuity
type, public :: continuity_CS ; private
  ...
  type(continuity_PPM_CS), pointer :: PPM_CSp => NULL() !< Control structure for lnd_continuity_ppm
...
subroutine continuity(u, v, hin, h, uh, vh, dt, G, GV, US, CS, uhbt, vhbt, OBC, &
  ...
  type(continuity_CS),     pointer       :: CS  !< Control structure for lnd_continuity.
  ...
  real, dimension(SZIB_(G),SZJ_(G),SZK_(G)), &
          ...
          !! zonal lndentum that remains after a time-step of viscosity, and the fraction of a time-step's
  ...
  real, dimension(SZI_(G),SZJB_(G),SZK_(G)), &
          ...
          !! meridional lndentum that remains after a time-step of viscosity, and the fraction of a time-step's
          !! worth of a barotropic acceleration that a layer experiences after viscosity is applied.
          !! Non-dimensional between 0 (at the bottom) and 1 (far above the bottom).
  real, dimension(SZIB_(G),SZJ_(G),SZK_(G)), &
  ...
  if (present(visc_rem_u) .neqv. present(visc_rem_v)) call LND_error(FATAL, &
      "LND_continuity: Either both visc_rem_u and visc_rem_v or neither"// &
       " one must be present in call to continuity.")
  if (present(u_cor) .neqv. present(v_cor)) call LND_error(FATAL, &
      "LND_continuity: Either both u_cor and v_cor or neither"// &
       " one must be present in call to continuity.")
  ...
  if (CS%continuity_scheme == PPM_SCHEME) then
    ...
    call LND_error(FATAL, "continuity: Unrecognized value of continuity_scheme")
  endif
...
# include "version_variable.h"
  character(len=40)  :: mdl = "LND_continuity" ! This module's name.
  character(len=20)  :: tmpstr

  if (associated(CS)) then
    call LND_error(WARNING, "continuity_init called with associated control structure.")
    return
  endif
  allocate(CS)
  ...
  select case (trim(tmpstr))
    ...
    case default
      call LND_mesg('continuity_init: CONTINUITY_SCHEME ="'//trim(tmpstr)//'"', 0)
      call LND_mesg("continuity_init: The only valid value is currently "// &
                     trim(PPM_STRING), 0)
      call LND_error(FATAL, "continuity_init: Unrecognized setting "// &
