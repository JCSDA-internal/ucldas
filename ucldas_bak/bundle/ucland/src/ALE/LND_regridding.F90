...
module LND_regridding

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_error_handler, only : LND_error, FATAL, WARNING
use LND_file_parser,   only : param_file_type, get_param, log_param
use LND_io,            only : file_exists, field_exists, field_size, LND_read_data
use LND_io,            only : slasher
use LND_unit_scaling,  only : unit_scale_type
use LND_variables,     only : ocean_grid_type, thermo_var_ptrs
use LND_verticalGrid,  only : verticalGrid_type
use LND_EOS,           only : EOS_type, calculate_density
use LND_string_functions, only : uppercase, extractWord, extract_integer, extract_real

use LND_remapping, only : remapping_CS
...
subroutine initialize_regridding(CS, GV, US, max_depth, param_file, mdl, coord_mode, param_prefix, param_suffix)
  ...
  if (main_parameters .and. len_trim(param_suffix)>0) call LND_error(FATAL,trim(mdl)//', initialize_regridding: '// &
  ...
  if (index(trim(string),'UNIFORM')==1) then
    ...
    elseif (index(trim(string),'UNIFORM:')==1 .and. len_trim(string)>8) then
      ...
      call LND_error(FATAL,trim(mdl)//', initialize_regridding: '// &
  ...
  elseif (index(trim(string),'FILE:')==1) then
    ...
    if (.not. file_exists(fileName)) call LND_error(FATAL,trim(mdl)//", initialize_regridding: "// &
    ...
    if (len_trim(varName)==0) then
      ...
      else ;  call LND_error(FATAL,trim(mdl)//", initialize_regridding: "// &
   ...
   !if (.not. field_exists(fileName,trim(varName))) call LND_error(FATAL,trim(mdl)//", initialize_regridding: "// &
   !             "Specified field not found: Looking for '"//trim(varName)//"' ("//trim(string)//")")
    ...
    if (index(trim(varName),'interfaces=')==1) then
      ...
      if (ierr) call LND_error(FATAL,trim(mdl)//", initialize_regridding: "//&
      ...
      ke = nzf(1)-1
      if (ke < 1) call LND_error(FATAL, trim(mdl)//" initialize_regridding via Var "//trim(varName)//&
      ...
      if (CS%regridding_scheme == REGRIDDING_RHO) then
        ...
        call LND_read_data(trim(fileName), trim(varName), rho_target)
      else
        allocate(dz(ke))
        allocate(z_max(ke+1))
        call LND_read_data(trim(fileName), trim(varName), z_max)
      ...
      ke = nzf(1)
      allocate(dz(ke))
      call LND_read_data(trim(fileName), trim(varName), dz)
    endif
  ...
  elseif (index(trim(string),'HYBRID:')==1) then
    ...
    if (.not. file_exists(fileName)) call LND_error(FATAL,trim(mdl)//", initialize_regridding: HYBRID "// &
      "Specified file not found: Looking for '"//trim(fileName)//"' ("//trim(string)//")")
    varName = trim( extractWord(trim(string(8:)), 2) )
    if (.not. field_exists(fileName,varName)) call LND_error(FATAL,trim(mdl)//", initialize_regridding: HYBRID "// &
      "Specified field not found: Looking for '"//trim(varName)//"' ("//trim(string)//")")
    call LND_read_data(trim(fileName), trim(varName), rho_target)
  ...
  elseif (index(trim(string),'WOA09')==1) then
    if (len_trim(string)==5) then
      ...
      enddo
    elseif (index(trim(string),'WOA09:')==1) then
      if (len_trim(string)==6) call LND_error(FATAL,trim(mdl)//', initialize_regridding: '// &
                 'Expected string of form "WOA09:N" but got "'//trim(string)//'".')
      ke = extract_integer(string(7:len_trim(string)),'',1)
    endif
    if (ke>40 .or. ke<1) call LND_error(FATAL,trim(mdl)//', initialize_regridding: '// &
  ...
  if (main_parameters .and. coord_is_state_dependent) then
    ...
    elseif (index(trim(string),'FILE:')==1) then
      ...
      do_sum = .false.
      varName = trim( extractWord(trim(string(6:)), 2) )
      if (.not. field_exists(fileName,varName)) call LND_error(FATAL,trim(mdl)//", initialize_regridding: "// &
        "Specified field not found: Looking for '"//trim(varName)//"' ("//trim(string)//")")
      if (len_trim(varName)==0) then
      ...
      if (do_sum) then
        call LND_read_data(trim(fileName), trim(varName), dz_max)
    ...
    elseif (index(trim(string),'FILE:')==1) then
      ...
      call LND_read_data(trim(fileName), trim(varName), h_max)
...
!   The UCLAND interface positions are always calculated from the bottom up by
...
!------------------------------------------------------------------------------
  ...
  select case ( CS%regridding_scheme )
    ...
    case default
      call LND_error(FATAL,'LND_regridding, regridding_main: '//&
...
subroutine check_grid_column( nk, depth, h, dzInterface, msg )
  ...
  ! Integrate upwards for the interfaces consistent with the rest of UCLAND
  z_old = - depth
  ...
  do k = nk,1,-1
    ...
    if (h_new<0.) then
      ...
      call LND_error( FATAL, 'LND_regridding, check_grid_column: '//&
  ...
  if (dzInterface(1) /= 0.) call LND_error( FATAL, &
    'LND_regridding, check_grid_column: Non-zero dzInterface at surface! '//trim(msg))
  if (dzInterface(nk+1) /= 0.) call LND_error( FATAL, &
    'LND_regridding, check_grid_column: Non-zero dzInterface at bottom! '//trim(msg))
...
!  real, dimension(nk+1) :: ddz_g_s, ddz_g_d
  ...
  if ((z_old(nk+1) - z_old(1)) * (z_new(CS%nk+1) - z_new(1)) < 0.0) then
    call LND_error(FATAL, "filtered_grid_motion: z_old and z_new use different sign conventions.")
  ...
  if (debug) then
    do k=2,CS%nk+1
      if (sgn*(z_new(k)-z_new(k-1)) < -5e-16*(abs(z_new(k))+abs(z_new(k-1))) ) &
        call LND_error(FATAL, "filtered_grid_motion: z_new is tangled.")
    enddo
    do k=2,nk+1
      if (sgn*(z_old(k)-z_old(k-1)) < -5e-16*(abs(z_old(k))+abs(z_old(k-1))) ) &
        call LND_error(FATAL, "filtered_grid_motion: z_old is tangled.")
    enddo
...
!           call LND_error(WARNING, "filtered_grid_motion: Expect z_output to be tangled (sc).")
!         if (abs(ddz_g_d(k) - ddz_g_s(k)) > 1e-12*(abs(dz_g(k)+ddz_g_d(k)) + abs(dz_g(k)+ddz_g_s(k)))) &
!           call LND_error(WARNING, "filtered_grid_motion: Expect z_output to be tangled.")
!       endif
      endif
  ...
  if (debug) then
    ...
    do k=2,CS%nk+1
      if (sgn*((z_act(k))-z_act(k-1)) < -1e-15*(abs(z_act(k))+abs(z_act(k-1))) ) &
        call LND_error(FATAL, "filtered_grid_motion: z_output is tangled.")
    enddo
  endif
    ...
    do i = G%isc-1,G%iec+1
      ...
      if (abs(zNew(1)-zOld(1))>(nz-1)*0.5*epsilon(dh)*dh) then
        ...
        call LND_error( FATAL, &
               'LND_regridding, build_zstar_grid(): top surface has moved!!!' )
      endif
#endif
  ...
  do i = G%isc-1,G%iec+1
    do j = G%jsc-1,G%jec+1
      ...
      if (abs(zNew(1)-zOld(1))>(CS%nk-1)*0.5*epsilon(dh)*dh) then
        ...
        enddo
        call LND_error( FATAL, &
               'LND_regridding, build_sigma_grid: top surface has moved!!!' )
      endif
      dzInterface(i,j,1) = 0.
...
!------------------------------------------------------------------------------
  ...
  if (.not.CS%target_density_set) call LND_error(FATAL, "build_rho_grid: "//&
    ...
    do i = G%isc-1,G%iec+1
      ...
      do k = 2,nz
        if (zNew(k) > zOld(1)) then
          write(0,*) 'zOld=',zOld
          write(0,*) 'zNew=',zNew
          call LND_error( FATAL, 'LND_regridding, build_rho_grid: '//&
      ...
      if (abs(zNew(1)-zOld(1))>(nz-1)*0.5*epsilon(dh)*dh) then
        ...
        enddo
        call LND_error( FATAL, &
               'LND_regridding, build_rho_grid: top surface has moved!!!' )
      endif
#endif
...
subroutine build_grid_HyCOM1( G, GV, US, h, tv, h_new, dzInterface, CS )
  ...
  if (.not.CS%target_density_set) call LND_error(FATAL, "build_grid_HyCOM1 : "//&
...
subroutine build_grid_SLight(G, GV, US, h, tv, dzInterface, CS)
  ...
  if (.not.CS%target_density_set) call LND_error(FATAL, "build_grid_SLight : "//&
...
subroutine adjust_interface_motion( CS, nk, h_old, dz_int )
  ...
  do k = 1, min(CS%nk,nk)
    ...
    if (h_new < -3.0*h_err) then
      ...
      call LND_error( FATAL, 'LND_regridding: adjust_interface_motion() - '//&
  ...
  enddo
 !if (dz_int(1)/=0.) stop 'LND_regridding: adjust_interface_motion() surface moved'
...
! LND_input file or defaulted to 1.0e-3). When layers are too thin, they
...
!------------------------------------------------------------------------------
  ...
  select case ( scheme )
    ...
    case default
      call LND_error(FATAL, "LND_regridding, uniformResolution: "//&
...
subroutine setCoordinateResolution( dz, CS, scale )
  ...
  if (size(dz)/=CS%nk) call LND_error( FATAL, &
...
subroutine set_target_densities( CS, rho_int )
  ...
  if (size(CS%target_density)/=size(rho_int)) then
    call LND_error(FATAL, "set_target_densities inconsistent args!")
  endif
...
subroutine set_regrid_max_depths( CS, max_depths, units_to_H )
  ...
  if (max_depths(CS%nk+1) < max_depths(1)) then
    do K=1,CS%nk ; if (max_depths(K+1) > max_depths(K)) &
      call LND_error(FATAL, "Unordered list of maximum depths sent to set_regrid_max_depths!")
    enddo
  else
    do K=1,CS%nk ; if (max_depths(K+1) < max_depths(K)) &
      call LND_error(FATAL, "Unordered list of maximum depths sent to set_regrid_max_depths.")
...
function getCoordinateInterfaces( CS, undo_scaling )
  ...
  if (CS%regridding_scheme == REGRIDDING_RHO) then
    if (.not. CS%target_density_set) &
      call LND_error(FATAL, 'LND_regridding, getCoordinateInterfaces: '//&
...
function getCoordinateUnits( CS )
  ...
  select case ( CS%regridding_scheme )
    ...
    case default
      call LND_error(FATAL,'LND_regridding, getCoordinateUnits: '//&
                     'Unknown regridding scheme selected!')
  end select ! type of grid
...
function getCoordinateShortName( CS )
  ...
  select case ( CS%regridding_scheme )
    ...
    case default
      call LND_error(FATAL,'LND_regridding, getCoordinateShortName: '//&
                     'Unknown regridding scheme selected!')
  end select ! type of grid

end function getCoordinateShortName

!> Can be used to set any of the parameters for LND_regridding.
subroutine set_regrid_params( CS, boundary_extrapolation, min_thickness, old_grid_weight, &
  ...
  if (present(old_grid_weight)) then
    if (old_grid_weight<0. .or. old_grid_weight>1.) &
      call LND_error(FATAL,'LND_regridding, set_regrid_params: Weight is out side the range 0..1!')
  ...
  if (present(depth_of_time_filter_shallow) .or. present(depth_of_time_filter_deep)) then
    if (CS%depth_of_time_filter_deep<CS%depth_of_time_filter_shallow) call LND_error(FATAL,'LND_regridding, '//&
...
function getStaticThickness( CS, SSH, depth )
  ...
  select case ( CS%regridding_scheme )
    ...
    case ( REGRIDDING_ARBITRARY )
      getStaticThickness(:) = 0.  ! Not applicable
    case default
      call LND_error(FATAL,'LND_regridding, getStaticThickness: '//&
                     'Unknown regridding scheme selected!')
  end select ! type of grid
...
subroutine dz_function1( string, dz )
  ...
  if (prec == -1024.) call LND_error(FATAL,"dz_function1: "// &
...
!! decreasing k. This is consistent with the rest of UCLAND that uses position,
