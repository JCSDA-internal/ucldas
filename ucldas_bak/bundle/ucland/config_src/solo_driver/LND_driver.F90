...
!*  Ocean Model (LND).  A separate ocean interface for use with        *
!*  coupled models is provided in ocean_model_LND.F90.   These two     *
...
!*  the LND initialization routines, to the subroutine that steps      *
!*  the model, and coordinates the output and saving restarts.  A      *
!*  description of all of the files that constitute LND is found in    *
!*  the comments at the beginning of LND.F90.  The arguments of each   *
...
!********+*********+*********+*********+*********+*********+*********+**
  ...
  use LND_diag_mediator,   only : enable_averaging, disable_averaging, diag_mediator_end
  use LND_diag_mediator,   only : diag_ctrl, diag_mediator_close_registration
  use LND,                 only : initialize_LND, step_LND, LND_control_struct, LND_end
  use LND,                 only : extract_surface_state, finish_LND_initialization
  use LND,                 only : get_LND_state_elements, LND_state_is_synchronized
  use LND,                 only : step_offline
  ...
  use LND_file_parser,     only : close_param_file
  use LND_forcing_type,    only : forcing, mech_forcing, forcing_diagnostics
  use LND_forcing_type,    only : mech_forcing_diags, LND_forcing_chksum, LND_mech_forcing_chksum
  use LND_get_input,       only : directories
  use LND_grid,            only : ocean_grid_type
  use LND_io,              only : file_exists, open_file, close_file
  ...
  use LND_surface_forcing, only : surface_forcing_init, surface_forcing_CS
  ...
  use LND_time_manager,    only : increment_date, set_calendar_type, month_name
  use LND_time_manager,    only : JULIAN, GREGORIAN, NOLEAP, THIRTY_DAY_MONTHS
  use LND_time_manager,    only : NO_CALENDAR
  ...
  use LND_write_cputime,   only : write_cputime, LND_write_cputime_init
  use LND_write_cputime,   only : write_cputime_start_clock, write_cputime_CS
  ...
  use LND_ice_shelf, only : initialize_ice_shelf, ice_shelf_end, ice_shelf_CS
  use LND_ice_shelf, only : shelf_calc_flux, add_shelf_forces, ice_shelf_save_restart
...
#include <LND_memory.h>
  ...
  ! A suite of time types for use by LND
  ...
  type(LND_control_struct),  pointer :: LND_CSp => NULL()
...
#include "version_variable.h"
  character(len=40)  :: mod_name = "LND_main (LND_driver)" ! This module's name.
  ...
  call LND_mesg('======== Model being driven by LND_driver ========', 2)
  call callTree_waypoint("Program LND_main, LND_driver.F90")
...
!$OMP END PARALLEL
  ...
  if (sum(date) >= 0) then
    ...
    call initialize_LND(Time, Start_time, param_file, dirs, LND_CSp, restart_CSp, &
                        segment_start_time, offline_tracer_mode=offline_tracer_mode, &
                        diag_ptr=diag, tracer_flow_CSp=tracer_flow_CSp)
  else
    ! In this case, the segment starts at a time read from the LND restart file
    ! or left as Start_time by LND_initialize.
    Time = Start_time
    call initialize_LND(Time, Start_time, param_file, dirs, LND_CSp, restart_CSp, &
  ...
  call get_LND_state_elements(LND_CSp, G=grid, GV=GV, US=US, C_p_scaled=fluxes%C_p)
  Master_Time = Time

  call callTree_waypoint("done initialize_LND")

  call extract_surface_state(LND_CSp, sfc_state)
  ...
  if (use_waves) then
    call LND_wave_interface_init(Time, grid, GV, US, param_file, Waves_CSp, diag)
  else
  ...
  if (elapsed_time_master) &
    call LND_mesg("Using real elapsed time for the master clock.", 2)
  ...
  if (years+months+days+hours+minutes+seconds > 0) then
    ...
    call LND_mesg('Segment run length determined from ocean_solo_nml.', 2)
  ...
  if (Time >= Time_end) call LND_error(FATAL, &
    "LND_driver: The run has been started at or after the end time of the run.")
  ...
  call get_param(param_file, mod_name, "WRITE_CPU_STEPS", cpu_steps, &
                 ...
                 default=1000)
  call get_param(param_file, "LND", "DEBUG", debug, &
  ...
  if (cpu_steps > 0) &
    call LND_write_cputime_init(param_file, dirs%output_directory, Start_time, &
                                write_CPU_CSp)
  ...
  do while ((ns < nmax) .and. (Time < Time_end))
    call callTree_enter("Main loop, LND_driver.F90",ns)
    ...
    if (debug) then
      call LND_mech_forcing_chksum("After set forcing", forces, grid, US, haloshift=0)
      call LND_forcing_chksum("After set forcing", fluxes, grid, US, haloshift=0)
    endif
    ...
    if (ns==1) then
      call finish_LND_initialization(Time, dirs, LND_CSp, restart_CSp)
    endif

    ! This call steps the model over a time dt_forcing.
    Time1 = Master_Time ; Time = Master_Time
    if (offline_tracer_mode) then
      call step_offline(forces, fluxes, sfc_state, Time1, dt_forcing, LND_CSp)
    elseif (single_step_call) then
      call step_LND(forces, fluxes, sfc_state, Time1, dt_forcing, LND_CSp, Waves=Waves_CSP)
    else
      ...
      do n=1,n_max
        if (diabatic_first) then
          if (modulo(n-1,nts)==0) then
            ...
            call step_LND(forces, fluxes, sfc_state, Time2, dtdia, LND_CSp, &
                          do_dynamics=.false., do_thermodynamics=.true., &
                          start_cycle=(n==1), end_cycle=.false., cycle_length=dt_forcing)
          endif

          call step_LND(forces, fluxes, sfc_state, Time2, dt_dyn, LND_CSp, &
...
!   This is here to enable fractional-second time steps.
    ...
    if (.not. offline_tracer_mode) then
      if (fluxes%fluxes_used) then
        ...
        call LND_error(FATAL, "The solo LND_driver is not yet set up to handle "//&
...
!  See if it is time to write out a restart file - timestamped or not.
  ...
  if (Restart_control>=0) then
    if (.not.LND_state_is_synchronized(LND_CSp)) &
    ...
    if (.not.fluxes%fluxes_used .and. .not.offline_tracer_mode) call LND_error(FATAL, &
         "End of LND_main reached with unused buoyancy fluxes. "//&
  ...
  call callTree_waypoint("End LND_main")
