...
use LND_EOS,           only : calculate_specific_vol_derivs, calculate_density_derivs
use LND_error_handler, only : LND_error, FATAL, WARNING, callTree_showQuery
...
use LND_forcing_type,  only : forcing, extractFluxes1d, forcing_SinglePointPrint
use LND_grid,          only : ocean_grid_type
use LND_io,            only : slasher
use LND_opacity,       only : set_opacity, opacity_CS, extract_optics_slice, extract_optics_fields
use LND_opacity,       only : optics_type, optics_nbands, absorbRemainingSW, sumSWoverBands
use LND_tracer_flow_control, only : get_chl_from_model, tracer_flow_control_CS
...
subroutine differential_diffuse_T_S(h, tv, visc, dt, G, GV)
  ...
  if (.not.associated(tv%T)) call LND_error(FATAL, &
      "differential_diffuse_T_S: Called with an unassociated tv%T")
  if (.not.associated(tv%S)) call LND_error(FATAL, &
      "differential_diffuse_T_S: Called with an unassociated tv%S")
  if (.not.associated(visc%Kd_extra_T)) call LND_error(FATAL, &
      "differential_diffuse_T_S: Called with an unassociated visc%Kd_extra_T")
  if (.not.associated(visc%Kd_extra_S)) call LND_error(FATAL, &
...
subroutine find_uv_at_h(u, v, h, u_h, v_h, G, GV, US, ea, eb)
  ...
  if (present(ea) .neqv. present(eb)) call LND_error(FATAL, &
    ...
    if (CS%chl_from_file) then
      ...
      do j=js,je ; do i=is,ie
        if ((G%mask2dT(i,j) > 0.5) .and. (chl_2d(i,j) < 0.0)) then
          ...
          call LND_error(FATAL, "LND_diabatic_aux set_pen_shortwave: "//trim(mesg))
        endif
      enddo ; enddo
      ...
      if (.not.associated(tracer_flow_CSp)) call LND_error(FATAL, &
...
!> This routine is appropriate in LND_diabatic_driver due to its position within the time stepping.
      ...
      if (G%mask2dT(i,j)>0.) then
        ...
        do k=1,nz
          ...
          elseif (h2d(i,k) < 0.0) then ! h2d==0 is a special limit that needs no extra handling
            ...
            call LND_error(FATAL, "LND_diabatic_driver.F90, applyBoundaryFluxesInOut(): "//&
...
! The following check will be ignored if ignore_fluxes_over_land = true
  if (numberOfGroundings>0 .and. .not. CS%ignore_fluxes_over_land) then
    do i = 1, min(numberOfGroundings, maxGroundings)
      ...
      call LND_error(WARNING, "LND_diabatic_driver.F90, applyBoundaryFluxesInOut(): "//&
...
#include "version_variable.h"
  character(len=40)  :: mdl  = "LND_diabatic_aux" ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "diabatic_aux_init called with an "// &
