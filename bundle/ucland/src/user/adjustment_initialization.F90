...
#include "version_variable.h"
  ...
  if (.not.just_read) &
    call LND_mesg("initialize_thickness_uniform: setting thickness")
  ...
  select case ( coordinateMode(verticalCoordinate) )
    ...
    case default
      call LND_error(FATAL,"adjustment_initialize_thickness: "// &
...
subroutine adjustment_initialize_temperature_salinity(T, S, h, G, GV, param_file, &
  ...
  select case ( coordinateMode(verticalCoordinate) )
   ...
   !    T(:,:,k) = x
    ...
    case default
      call LND_error(FATAL,"adjustment_initialize_temperature_salinity: "// &
      "Unrecognized i.c. setup - set ADJUSTMENT_IC")
