...
use LND_time_manager, only : get_date, set_date
use LND_time_manager, only : time_type_to_real, real_to_time
...
!> Control structure for LND_controlled_forcing
...
!
  ...
  if (CS%num_cycle > 0) then
    ...
    if ((mr_mid < mr_st) .or. (mr_mid > mr_prev + 1.)) call LND_error(FATAL, &
          "apply ctrl_forcing: m_mid interpolation out of bounds; fix the code.")
    if ((mr_end < mr_st) .or. (mr_end > mr_prev + 2.)) call LND_error(FATAL, &
          "apply ctrl_forcing: m_end interpolation out of bounds; fix the code.")
    if (mr_end > mr_next) call LND_error(FATAL, &
    ...
    if ((wt_st < 0.0) .or. (wt_end < 0.0) .or. (wt_mid < 0.0)) &
      call LND_error(FATAL, "apply_ctrl_forcing: Negative m weights")
    if ((wt_st > 1.0) .or. (wt_end > 1.0) .or. (wt_mid > 1.0)) &
      call LND_error(FATAL, "apply_ctrl_forcing: Excessive m weights")
...
subroutine register_ctrl_forcing_restarts(G, param_file, CS, restart_CS)
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "register_ctrl_forcing_restarts called "//&
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_controlled_forcing" ! This module's name.
...
!> \namespace lnd_controlled_forcing
!!                                                                     *
...
end module LND_controlled_forcing
