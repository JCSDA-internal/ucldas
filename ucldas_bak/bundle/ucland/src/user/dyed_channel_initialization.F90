...
use LND_open_boundary,   only : OBC_registry_type, register_OBC
use LND_time_manager,    only : time_type, time_type_to_real
use LND_tracer_registry, only : tracer_registry_type, tracer_name_lookup
use LND_tracer_registry, only : tracer_type
...
function register_dyed_channel_OBC(param_file, CS, OBC_Reg)
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "register_dyed_channel_OBC called with an "// &
...
subroutine dyed_channel_set_OBC_tracer_data(OBC, G, GV, param_file, tr_Reg)
  ...
  if (.not.associated(OBC)) call LND_error(FATAL, 'dyed_channel_initialization.F90: '// &
  ...
  if (OBC%number_of_segments < ntr) then
    call LND_error(WARNING, "Error in dyed_obc segment setup")
