...
! LND_wave_speed by Hallberg, 2008.

use LND_debugging,     only : isnan => is_NaN
...
use LND_EOS,           only : calculate_density_derivs
use LND_error_handler, only : LND_error, FATAL, WARNING
use LND_file_parser,   only : log_version, param_file_type, get_param
...
!> The control structure for the LND_wave_structure module
...
subroutine wave_structure(h, tv, G, GV, US, cn, ModeNum, freq, CS, En, full_halos)
  ...
  !if (present(CS)) then
    if (.not. associated(CS)) call LND_error(FATAL, "LND_wave_structure: "// &
        ...
        if (drxh_sum >= 0.0) then
          ...
          if (kc >= max(3, ModeNum + 1)) then
            ...
            if (abs(z_int(kc+1)-htot(i,j)) > 1.e-14*htot(i,j)) then
              call LND_error(FATAL, "wave_structure: mismatch in total depths")
            endif
            ...
            if (present(En) .and. (freq**2*Kmag2 > 0.0)) then
              ...
              if (En(i,j) >= 0.0) then
                ...
                call LND_error(WARNING, "wave_structure: En < 0.0; setting to W0 to 0.0")
  ...
  elseif (method == 'TDMA_H') then
    ...
    do k=1,nrow-1
      if (abs(a(k+1)-c(k)) > 1.e-10*(abs(a(k+1))+abs(c(k)))) then
        call LND_error(WARNING, "tridiag_solver: matrix not symmetric; need symmetry when invoking TDMA_H")
      endif
    enddo
    alpha = -c
    ...
    if ((h(nrow)+alpha(nrow-1)*Q_prime+alpha(nrow)) == 0.0)then
      call LND_error(FATAL, "Tridiag_solver: this system is not stable.") ! ; overriding beta(nrow)
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_wave_structure"  ! This module's name.
  integer :: isd, ied, jsd, jed, nz

  isd = G%isd ; ied = G%ied ; jsd = G%jsd ; jed = G%jed ; nz = G%ke

  if (associated(CS)) then
    call LND_error(WARNING, "wave_structure_init called with an "// &
