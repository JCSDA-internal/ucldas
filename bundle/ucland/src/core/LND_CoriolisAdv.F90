!> Accelerations due to the Coriolis force and lndentum advection
...
!> Control structure for lnd_coriolisadv
...
!> Calculates the Coriolis and lndentum advection contributions to the acceleration.
subroutine CorAdCalc(u, v, h, uh, vh, CAu, CAv, OBC, AD, G, GV, US, CS)
  ...
  real, dimension(SZIB_(G),SZJ_(G),SZK_(G)), intent(out)   :: CAu !< Zonal acceleration due to Coriolis
                                                                  !! and lndentum advection [L T-2 ~> m s-2].
  ...
  type(CoriolisAdv_CS),                      pointer       :: CS  !< Control structure for LND_CoriolisAdv
...
!   uh(is-1,ie,js:je+1) and vh(is:ie+1,js-1:je).
  ...
  if (.not.associated(CS)) call LND_error(FATAL, &
         "LND_CoriolisAdv: Module must be initialized before it is used.")
  ...
  do k=1,nz
    ...
    ! force and lndentum advection.  On a Cartesian grid, this is
    !     CAu =  q * vh - d(KE)/dx.
...
subroutine CoriolisAdv_init(Time, G, GV, US, param_file, diag, AD, CS)
  ...
  type(CoriolisAdv_CS),    pointer       :: CS !< Control structure fro LND_CoriolisAdv
  ! Local variables
! This include declares and sets the variable "version".
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_CoriolisAdv" ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "CoriolisAdv_init called with associated control structure.")
    return
  endif
  allocate(CS)
  ...
  select case (tmpstr)
    ...
    case default
      call LND_mesg('CoriolisAdv_init: Coriolis_Scheme ="'//trim(tmpstr)//'"', 0)
      call LND_error(FATAL, "CoriolisAdv_init: Unrecognized setting "// &
  ...
  if (CS%Coriolis_Scheme == AL_BLEND) then
    ...
    if (CS%F_eff_max_blend < 2.0) call LND_error(WARNING, "CoriolisAdv_init: "//&
  ...
  select case (tmpstr)
    ...
    case default
      call LND_mesg('CoriolisAdv_init: KE_Scheme ="'//trim(tmpstr)//'"', 0)
      call LND_error(FATAL, "CoriolisAdv_init: "// &
  ...
  select case (uppercase(tmpstr))
    ...
    case default
      call LND_mesg('CoriolisAdv_init: PV_Adv_Scheme ="'//trim(tmpstr)//'"', 0)
      call LND_error(FATAL, "CoriolisAdv_init: "// &
