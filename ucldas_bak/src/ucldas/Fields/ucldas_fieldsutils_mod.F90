...
private
public :: fldinfo, ucldas_genfilename
...
function ucldas_genfilename (f_conf,length,vdate,domain_type)
  type(fckit_configuration),  intent(in) :: f_conf
  integer,                    intent(in) :: length
  type(datetime),             intent(in) :: vdate
  character(len=3), optional, intent(in) :: domain_type

  character(len=length)                  :: ucldas_genfilename
  ...
  if (typ=="fc" .or. typ=="ens") then
     ...
     ucldas_genfilename = TRIM(prefix) // "." // TRIM(referencedate) // "." // TRIM(sstep)
  endif

  if (typ=="an" .or. typ=="incr") then
     call datetime_to_string(vdate, validitydate)
     lenfn = lenfn + 1 + LEN_TRIM(validitydate)
     ucldas_genfilename = TRIM(prefix) // "." // TRIM(validitydate)
  endif
...
end function ucldas_genfilename

end module ucldas_fieldsutils_mod
