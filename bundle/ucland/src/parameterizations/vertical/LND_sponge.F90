...
use LND_error_handler, only : LND_error, FATAL, NOTE, WARNING, is_root_pe
use LND_file_parser, only : get_param, log_param, log_version, param_file_type
use LND_grid, only : ocean_grid_type
use LND_spatial_means, only : global_i_mean
...
!> This control structure holds memory and parameters for the LND_sponge module
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_sponge"  ! This module's name.
  logical :: use_sponge
  integer :: i, j, k, col, total_sponge_cols

  if (associated(CS)) then
    call LND_error(WARNING, "initialize_sponge called with an associated "// &
                            "control structure.")
    return
  endif
...
! Set default, read and log parameters
  ...
  call get_param(param_file, mdl, "SPONGE", use_sponge, &
                 ...
                 "specified from LND_initialization.F90.", default=.false.)

  if (.not.use_sponge) return
  allocate(CS)

  if (present(Iresttime_i_mean) .neqv. present(int_height_i_mean)) &
    call LND_error(FATAL, "initialize_sponge:  The optional arguments \n"//&
...
subroutine set_up_sponge_field(sp_val, f_ptr, G, nlay, CS, sp_val_i_mean)
  ...
  if (CS%fldno > MAX_FIELDS_) then
    write(mesg,'("Increase MAX_FIELDS_ to at least ",I3," in LND_memory.h or decrease &
           &the number of fields to be damped in the call to &
           &initialize_sponge." )') CS%fldno
    call LND_error(FATAL,"set_up_sponge_field: "//mesg)
  endif
  ...
  if (nlay/=CS%nz) then
    write(mesg,'("Danger: Sponge reference fields require nz (",I3,") layers.&
        & A field with ",I3," layers was passed to set_up_sponge_field.")') &
          CS%nz, nlay
    if (is_root_pe()) call LND_error(WARNING, "set_up_sponge_field: "//mesg)
  endif

  if (CS%do_i_mean_sponge) then
    if (.not.present(sp_val_i_mean)) call LND_error(FATAL, &
...
!                 set by a previous call to initialize_sponge.
  ...
  if (associated(CS%Rcv_ml_ref)) then
    call LND_error(FATAL, "set_up_sponge_ML_density appears to have been "//&
                           "called twice.")
  endif
...
! there is damping.
  ...
  if (CS%bulkmixedlayer .and. (.not.present(Rcv_ml))) &
    call LND_error(FATAL, "Rml must be provided to apply_sponge when using "//&
                           "a bulk mixed layer.")
  ...
  if (CS%do_i_mean_sponge) then
    ...
    if (CS%bulkmixedlayer) call LND_error(FATAL, "apply_sponge is not yet set up to "//&
...
!! effect on lndentum of the sponge may be accounted for later using
!! the movement of water recorded in these later arrays.

end module LND_sponge
