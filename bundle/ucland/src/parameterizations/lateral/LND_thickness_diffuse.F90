...
use LND_domains,               only : pass_var, CORNER, pass_vector
...
use LND_EOS,                   only : calculate_density_second_derivs
...
use LND_variables,             only : thermo_var_ptrs, cont_diag_ptrs
...
type, public :: thickness_diffuse_CS ; private
  ...
  logical :: use_GME_thickness_diffuse !< If true, passes GM coefficients to LND_hor_visc for use
                                 !! with GME closure.
...
subroutine thickness_diffuse(h, uhtr, vhtr, tv, dt, G, GV, US, MEKE, VarMix, CDp, CS)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "LND_thickness_diffuse: "//&
...
subroutine thickness_diffuse_full(h, e, Kh_u, Kh_v, tv, uhD, vhD, cg1, dt, G, GV, US, MEKE, &
  ...
  if (CS%use_FGNV_streamfn .and. .not. associated(cg1)) call LND_error(FATAL, &
...
!                   call LND_error(WARNING, "Corrective u-transport is up the thickness gradient.", .true.)
!                 endif
!                 if (((h(i,j,k) - 4.0*dt*G%IareaT(i,j)*uh_here(k)) - &
!                      (h(i+1,j,k) + 4.0*dt*G%IareaT(i+1,j)*uh_here(k))) * &
!                     (h(i,j,k) - h(i+1,j,k)) < 0.0) then
!                   call LND_error(WARNING, "Corrective u-transport is too large.", .true.)
!                 endif
!               endif
!             endif
!           else ! v-point
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_thickness_diffuse" ! This module's name.
...
!> \namespace lnd_thickness_diffuse
!!
...
!! \f$\kappa_{M}\f$ is the diffusivity calculated by the MEKE parameterization (lnd_meke module) and
!! \f$ r(\Delta x,L_d) \f$ is a function of the local resolution (ratio of grid-spacing, \f$\Delta x\f$,
!! to deformation radius, \f$L_d\f$). The length \f$L_s\f$ is provided by the lnd_lateral_mixing_coeffs module
...
!! \f$f(c_g,z)\f$ is calculated in module lnd_lateral_mixing_coeffs.
...
!! \f}
!!
!! \subsection section_khth_module_parameters Module lnd_thickness_diffuse parameters
!!
...
!! | \f$ \alpha_{M} \f$    | <code>MEKE_KHTH_FAC</code> (from lnd_meke module) |
!! | -                     | <code>KHTH_USE_EBT_STRUCT</code> (from lnd_lateral_mixing_coeffs module) |
...
end module LND_thickness_diffuse
