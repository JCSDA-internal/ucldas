...
character(len=40)  :: mdl = "LND_remapping" !< This module's name.
...
function isPosSumErrSignificant(n1, sum1, n2, sum2)
  ...
  if (sum1<0.) call LND_error(FATAL,'isPosSumErrSignificant: sum1<0 is not allowed!')
  if (sum2<0.) call LND_error(FATAL,'isPosSumErrSignificant: sum2<0 is not allowed!')
...
subroutine remapping_core_h(CS, n0, h0, u0, n1, h1, u1, h_neglect, h_neglect_edge)
  ...
  if (CS%check_remapping) then
    ...
    if ( (abs(u1tot-u0tot)>(u0err+u1err)+uh_err .and. abs(h1tot-h0tot)<h0err+h1err) &
      ...
      enddo
      call LND_error( FATAL, 'LND_remapping, remapping_core_h: '//&
  ...
  if (CS%check_remapping) then
    ...
    if ( (abs(u1tot-u0tot)>(u0err+u1err)+uh_err .and. abs(h1tot-h0tot)<h0err+h1err) &
      ...
      do k = 1, n0
        write(0,'(i3,1p6e24.16)') k,u0(k),ppoly_r_coefs(k,:)
      enddo
      call LND_error( FATAL, 'LND_remapping, remapping_core_w: '//&
             'Remapping result is inconsistent!' )
    endif
    endif ! method<5
  endif
...
subroutine build_reconstructions_1d( CS, n0, h0, u0, ppoly_r_coefs, &
  ...
  select case ( local_remapping_scheme )
    ...
    case ( REMAPPING_PQM_IH6IH5 )
      ...
      iMethod = INTEGRATION_PQM
    case default
      call LND_error( FATAL, 'LND_remapping, build_reconstructions_1d: '//&
...
subroutine check_reconstructions_1d(n0, h0, u0, deg, boundary_extrapolation, &
  ...
  do i0 = 1, n0
    ...
    if (problem_detected) then
      ...
      enddo
      call LND_error(FATAL, 'LND_remapping, check_reconstructions_1d: '// &
  ...
  do i_sub = 2, n0+n1
    ...
    if (debug_bounds) then
      if (method<5 .and.(u_sub(i_sub)<u0_min(i0) .or. u_sub(i_sub)>u0_max(i0))) then
        ...
        call LND_error( FATAL, 'LND_remapping, remap_via_sub_cells: '//&
...
real function average_value_ppoly( n0, u0, ppoly0_E, ppoly0_coefs, method, i0, xa, xb)
  ...
  if (xb > xa) then
    select case ( method )
      ...
      case default
        call LND_error( FATAL,'The selected integration method is invalid' )
    end select
  else ! dh == 0.
    select case ( method )
      case ( INTEGRATION_PCM )
...
! if (xL>x0jLr) x0jLr = xL
  ...
  if ( jL == -1 ) call LND_error(FATAL, &
          'LND_remapping, integrateReconOnInterval: '//&
...
subroutine setReconstructionType(string,CS)
  ...
  select case ( uppercase(trim(string)) )
    ...
    case ("PQM_IH6IH5")
      ...
      degree = 4
    case default
      call LND_error(FATAL, "setReconstructionType: "//&
...
logical function remapping_unit_tests(verbose)
  ...
  write(*,*) '==== LND_remapping: remapping_unit_tests ================='
  ...
  write(*,*) '===== LND_remapping: new remapping_unit_tests =================='
