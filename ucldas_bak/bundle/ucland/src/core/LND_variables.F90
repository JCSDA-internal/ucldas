!> Provides transparent structures with groups of UCLAND variables and supporting routines
module LND_variables

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_array_transform, only : rotate_array, rotate_vector
use LND_domains, only : LND_domain_type, get_domain_extent, group_pass_type
use LND_debugging, only : hchksum
use LND_error_handler, only : LND_error, FATAL
...
public allocate_surface_state, deallocate_surface_state, LND_thermovar_chksum
...
!> Pointers to various fields which may be used describe the surface state of LND, and which
...
type, public :: surface
  real, allocatable, dimension(:,:) :: &
    ...
    frazil, &      !< The energy needed to heat the ocean column to the freezing point during
                   !! the call to step_LND [Q R Z ~> J m-2].
    ...
    TempxPmE, &    !< The net inflow of water into the ocean times the temperature at which this
                   !! inflow occurs during the call to step_LND [degC R Z ~> degC kg m-2].
    salt_deficit, & !< The salt needed to maintain the ocean column above a minimum
                   !! salinity over the call to step_LND [kgSalt kg-1 R Z ~> kgSalt m-2].
    internal_heat  !< Any internal or geothermal heat sources that are applied to the ocean
                   !! integrated over the call to step_LND [degC R Z ~> degC kg m-2].
  ...
  type(coupler_2d_bc_type) :: tr_fields !< A structure that may contain an
       ...
       !###       HALOS!  THIS IS DONE TO CONFORM TO THE TREATMENT IN LND4, BUT I DON'T LIKE IT! -RWH
...
!> Pointers to all of the prognostic variables allocated in LND_variables.F90 and LND.F90.
!!
...
type, public :: accel_diag_ptrs
  ...
  real, pointer, dimension(:,:,:) :: &
    ...
    CAu => NULL(), &       !< Zonal Coriolis and lndentum advection accelerations [L T-2 ~> m s-2]
    CAv => NULL(), &       !< Meridional Coriolis and lndentum advection accelerations [L T-2 ~> m s-2]
...
subroutine rotate_surface_state(sfc_state_in, G_in, sfc_state, G, turns)
  ...
  if (coupler_type_initialized(sfc_state_in%tr_fields)) &
    call LND_error(FATAL, "Rotation of surface state tracers is not yet " &
        // "implemented.")
...
subroutine alloc_BT_cont_type(BT_cont, G, alloc_faces)
  ...
  if (associated(BT_cont)) call LND_error(FATAL, &
...
subroutine LND_thermovar_chksum(mesg, tv, G)
...
end subroutine LND_thermovar_chksum

end module LND_variables
