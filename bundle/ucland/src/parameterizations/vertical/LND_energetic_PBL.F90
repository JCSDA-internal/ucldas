...
use LND_coms, only : EFP_type, real_to_EFP, EFP_to_real, operator(+), assignment(=), EFP_sum_across_PEs
use LND_diag_mediator, only : post_data, register_diag_field, safe_alloc_alloc
...
use LND_wave_interface, only: wave_parameters_CS, Get_Langmuir_Number
...
!> This control structure holds parameters for the LND_energetic_PBL module
...
! The following are only used for diagnostics.
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "energetic_PBL: "//&
         "Module must be initialized before it is used.")

  if (.not. associated(tv%eqn_of_state)) call LND_error(FATAL, &
      "energetic_PBL: Temperature, salinity and an equation of state "//&
      "must now be used.")
  if (.NOT. associated(fluxes%ustar)) call LND_error(FATAL, &
...
! of the robust tridiagonal solvers used for tracers by UCLAND.  The results are
...
# include "version_variable.h"
  character(len=40)  :: mdl = "LND_energetic_PBL"  ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "mixedlayer_init called with an associated"//&
...
!/2. Options related to setting MSTAR
  ...
  if (mstar_mode == 0) then
    ...
    call LND_error(WARNING, "Use EPBL_MSTAR_SCHEME = CONSTANT instead of the archaic MSTAR_MODE = 0.")
  elseif (mstar_mode == 1) then
    call LND_error(FATAL, "You are using a legacy mstar mode in ePBL that has been phased out. "//&
  ...
  elseif (mstar_mode == 2) then
    tmpstr = OM4_STRING
    call LND_error(WARNING, "Use EPBL_MSTAR_SCHEME = OM4 instead of the archaic MSTAR_MODE = 2.")
  elseif (mstar_mode == 3) then
    tmpstr = RH18_STRING
    call LND_error(WARNING, "Use EPBL_MSTAR_SCHEME = REICHL_H18 instead of the archaic MSTAR_MODE = 3.")
  elseif (mstar_mode > 3) then
    call LND_error(FATAL, "An unrecognized value of the obsolete parameter MSTAR_MODE was specified.")
  endif
  ...
  select case (tmpstr)
    ...
    case default
      call LND_mesg('energetic_PBL_init: EPBL_MSTAR_SCHEME ="'//trim(tmpstr)//'"', 0)
      call LND_error(FATAL, "energetic_PBL_init: Unrecognized setting "// &
...
!/ Mixing Length Options
  ...
  if ( CS%Use_MLD_iteration .and. abs(CS%transLay_scale-0.5) >= 0.5) then
    call LND_error(FATAL, "If flag USE_MLD_ITERATION is true, then "//&
...
!/ Turbulent velocity scale in mixing coefficient
  ...
  if (wT_mode == 0) then
    ...
    call LND_error(WARNING, "Use EPBL_VEL_SCALE_SCHEME = CUBE_ROOT_TKE instead of the archaic EPBL_VEL_SCALE_MODE = 0.")
  elseif (wT_mode == 1) then
    tmpstr = RH18_STRING
    call LND_error(WARNING, "Use EPBL_VEL_SCALE_SCHEME = REICHL_H18 instead of the archaic EPBL_VEL_SCALE_MODE = 1.")
  elseif (wT_mode >= 2) then
    call LND_error(FATAL, "An unrecognized value of the obsolete parameter EPBL_VEL_SCALE_MODE was specified.")
  endif
  ...
  select case (tmpstr)
    ...
    case default
      call LND_mesg('energetic_PBL_init: EPBL_VEL_SCALE_SCHEME ="'//trim(tmpstr)//'"', 0)
      call LND_error(FATAL, "energetic_PBL_init: Unrecognized setting "// &
  ...
  if (CS%USE_LT) then
    ...
    if (LT_ENHANCE == 0) then
      tmpstr = NONE_STRING
      call LND_error(WARNING, "Use EPBL_LANGMUIR_SCHEME = NONE instead of the archaic LT_ENHANCE = 0.")
    elseif (LT_ENHANCE == 1) then
      call LND_error(FATAL, "You are using a legacy LT_ENHANCE mode in ePBL that has been phased out. "//&
                            "If you need to use this setting please report this error.  Also use "//&
                            "EPBL_LANGMUIR_SCHEME to specify the scheme for mstar.")
    elseif (LT_ENHANCE == 2) then
      tmpstr = RESCALED_STRING
      call LND_error(WARNING, "Use EPBL_LANGMUIR_SCHEME = RESCALE instead of the archaic LT_ENHANCE = 2.")
    elseif (LT_ENHANCE == 3) then
      tmpstr = ADDITIVE_STRING
      call LND_error(WARNING, "Use EPBL_LANGMUIR_SCHEME = ADDITIVE instead of the archaic LT_ENHANCE = 3.")
    elseif (LT_ENHANCE > 3) then
      call LND_error(FATAL, "An unrecognized value of the obsolete parameter LT_ENHANCE was specified.")
    endif
    ...
    select case (tmpstr)
      ...
      case default
        call LND_mesg('energetic_PBL_init: EPBL_LANGMUIR_SCHEME ="'//trim(tmpstr)//'"', 0)
        call LND_error(FATAL, "energetic_PBL_init: Unrecognized setting "// &
...
!! process, and it is dealt with elsewhere in the UCLAND code within
!! the module LND_kappa_shear.F90.  It is assumed that the heat,
