...
#include "ucldas/Increment/IncrementFortran.h"
...
namespace ucldas {
  ...
  {
    ucldas_increment_create_f90(keyFlds_, geom_->toFortran(), vars_);
    ucldas_increment_zero_f90(toFortran());
  ...
  {
    ucldas_increment_create_f90(keyFlds_, geom_->toFortran(), vars_);
    ucldas_increment_change_resol_f90(toFortran(), other.keyFlds_);
    ...
    : time_(other.time_), vars_(other.vars_), geom_(new Geometry(*other.geom_))
  {
    ucldas_increment_create_f90(keyFlds_, geom_->toFortran(), vars_);
    if (copy) {
      ucldas_increment_copy_f90(toFortran(), other.toFortran());
    } else {
      ucldas_increment_zero_f90(toFortran());
    }
  ...
  // -----------------------------------------------------------------------------
  Increment::~Increment() {
    ucldas_increment_delete_f90(toFortran());
  ...
  void Increment::diff(const State & x1, const State & x2) {
    ...
    ucldas_increment_diff_incr_f90(toFortran(), x1_at_geomres.toFortran(),
                                              x2_at_geomres.toFortran());
  }
  // -----------------------------------------------------------------------------
  Increment & Increment::operator=(const Increment & rhs) {
    time_ = rhs.time_;
    ucldas_increment_copy_f90(toFortran(), rhs.toFortran());
    return *this;
  }
  // -----------------------------------------------------------------------------
  Increment & Increment::operator+=(const Increment & dx) {
    ASSERT(this->validTime() == dx.validTime());
    ucldas_increment_self_add_f90(toFortran(), dx.toFortran());
    return *this;
  }
  // -----------------------------------------------------------------------------
  Increment & Increment::operator-=(const Increment & dx) {
    ASSERT(this->validTime() == dx.validTime());
    ucldas_increment_self_sub_f90(toFortran(), dx.toFortran());
    return *this;
  }
  // -----------------------------------------------------------------------------
  Increment & Increment::operator*=(const double & zz) {
    ucldas_increment_self_mul_f90(toFortran(), zz);
    return *this;
  }
  // -----------------------------------------------------------------------------
  void Increment::ones() {
    ucldas_increment_ones_f90(toFortran());
  }
  // -----------------------------------------------------------------------------
  void Increment::zero() {
    ucldas_increment_zero_f90(toFortran());
  ...
  void Increment::dirac(const eckit::Configuration & config) {
    ucldas_increment_dirac_f90(toFortran(), &config);
  ...
  void Increment::axpy(const double & zz, const Increment & dx,
    ...
    ucldas_increment_axpy_f90(toFortran(), zz, dx.toFortran());
  }
  // -----------------------------------------------------------------------------
  void Increment::accumul(const double & zz, const State & xx) {
    ucldas_increment_accumul_f90(toFortran(), zz, xx.toFortran());
  }
  // -----------------------------------------------------------------------------
  void Increment::schur_product_with(const Increment & dx) {
    ucldas_increment_self_schur_f90(toFortran(), dx.toFortran());
  }
  // -----------------------------------------------------------------------------
  double Increment::dot_product_with(const Increment & other) const {
    double zz;
    ucldas_increment_dot_prod_f90(toFortran(), other.toFortran(), zz);
    return zz;
  }
  // -----------------------------------------------------------------------------
  void Increment::random() {
    ucldas_increment_random_f90(toFortran());
  }
  ...
  oops::LocalIncrement Increment::getLocal(
    ...
    int nx, ny, nzo, nf;
    ucldas_increment_sizes_f90(toFortran(), nx, ny, nzo, nf);
    ...
    ucldas_increment_getpoint_f90(keyFlds_, iter.toFortran(), values[0],
                            values.size());
  ...
  void Increment::setLocal(const oops::LocalIncrement & values,
    ...
    ucldas_increment_setpoint_f90(toFortran(), iter.toFortran(), vals[0],
                            vals.size());
  }
  // -----------------------------------------------------------------------------
  /// ATLAS
  ...
  void Increment::setAtlas(atlas::FieldSet * afieldset) const {
    ucldas_increment_set_atlas_f90(toFortran(), geom_->toFortran(), vars_,
                                 afieldset->get());
  }
  // -----------------------------------------------------------------------------
  void Increment::toAtlas(atlas::FieldSet * afieldset) const {
    ucldas_increment_to_atlas_f90(toFortran(), geom_->toFortran(), vars_,
                                afieldset->get());
  }
  // -----------------------------------------------------------------------------
  void Increment::fromAtlas(atlas::FieldSet * afieldset) {
    ucldas_increment_from_atlas_f90(toFortran(), geom_->toFortran(), vars_,
                                  afieldset->get());
  }
  // -----------------------------------------------------------------------------
  /// I/O and diagnostics
  ...
  void Increment::read(const eckit::Configuration & files) {
    ...
    ucldas_increment_read_file_f90(toFortran(), &files, &dtp);
  }
  // -----------------------------------------------------------------------------
  void Increment::write(const eckit::Configuration & files) const {
    const util::DateTime * dtp = &time_;
    ucldas_increment_write_file_f90(toFortran(), &files, &dtp);
  }
  // -----------------------------------------------------------------------------
  void Increment::print(std::ostream & os) const {
    os << std::endl << "  Valid time: " << validTime();
    int n0, nf;
    ucldas_increment_sizes_f90(keyFlds_, n0, n0, n0, nf);
    std::vector<double> zstat(3*nf);
    ucldas_increment_gpnorm_f90(keyFlds_, nf, zstat[0]);
  ...
  double Increment::norm() const {
    double zz = 0.0;
    ucldas_increment_rms_f90(toFortran(), zz);
    return zz;
  }
  ...
  size_t Increment::serialSize() const {
    // Field
    size_t nn;
    ucldas_increment_serial_size_f90(toFortran(), geom_->toFortran(), nn);
  ...
  void Increment::serialize(std::vector<double> & vect) const {
    ...
    ucldas_increment_serialize_f90(toFortran(), geom_->toFortran(), nn,
                                 vect_field.data());
  ...
  void Increment::deserialize(const std::vector<double> & vect,
    ...
    ucldas_increment_deserialize_f90(toFortran(), geom_->toFortran(), vect.size(),
                                   vect.data(), index);
