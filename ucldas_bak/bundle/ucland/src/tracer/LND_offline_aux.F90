...
!> the LND_offline_main module
module LND_offline_aux

! This file is part of UCLAND. See LICENSE.md for the license.

use data_override_mod,    only : data_override_init, data_override
use LND_time_manager,     only : time_type, operator(-)
use LND_debugging,        only : check_column_integrals
use LND_domains,          only : pass_var, pass_vector, To_All
use LND_diag_vkernels,    only : reintegrate_column
use LND_error_handler,    only : callTree_enter, callTree_leave, LND_error, FATAL, WARNING, is_root_pe
...
use LND_opacity,          only : optics_type
  ...
  do j=js,je
    ...
    do i=is-1,ie
      ...
      if ( abs(sum(uh2d(I,:))-uh2d_sum(I)) > uh_neglect) &
        call LND_error(WARNING,"Column integral of uh does not match after "//&
  ...
  do i=is,ie
    ...
    do j=js-1,je
      ...
      if ( abs(sum(vh2d(J,:))-vh2d_sum(J)) > vh_neglect) then
          call LND_error(WARNING,"Column integral of vh does not match after "//&
          "barotropic redistribution")
      endif
      ...
      do k=1,nz
        ...
        if (abs(uh_remain)>0.0) then
          if (k<nz) then
            ...
            call LND_error(WARNING,"Water column cannot accommodate UH redistribution. Tracer may not be conserved")
          endif
        endif
      enddo ! k-loop
      ...
      do k=1,nz
        ...
        if (abs(vh_remain)>0.0) then
         if (k<nz) then
            ...
            call LND_error(WARNING,"Water column cannot accommodate VH redistribution. Tracer will not be conserved")
          endif
        endif ! k-loop
      enddo
...
subroutine update_offline_from_files(G, GV, nk_input, mean_file, sum_file, snap_file, surf_file, h_end, &
  ...
  if (read_ts_uvh) then
    ...
    vhtr(:,:,:) = 0.0
    ! Time-summed fields
    call LND_read_vector(sum_file, 'uhtr_sum', 'vhtr_sum', uhtr(:,:,1:nk_input), &
                         vhtr(:,:,1:nk_input), G%Domain, timelevel=ridx_sum)
    call LND_read_data(snap_file, 'h_end', h_end(:,:,1:nk_input), G%Domain, &
                       timelevel=ridx_snap,position=CENTER)
    call LND_read_data(mean_file, 'temp', temp_mean(:,:,1:nk_input), G%Domain, &
                       timelevel=ridx_sum,position=CENTER)
    call LND_read_data(mean_file, 'salt', salt_mean(:,:,1:nk_input), G%Domain, &
                       timelevel=ridx_sum,position=CENTER)
  endif
  ...
  call LND_read_data( mean_file, 'Kd_interface', Kd(:,:,1:nk_input+1), G%Domain, &
                  timelevel=ridx_sum,position=CENTER)
  ...
  if (do_ale) then
    ...
    fluxes%netMassOut(:,:) = 0.0
    fluxes%netMassIn(:,:) = 0.0
    call LND_read_data(surf_file,'massout_flux_sum',fluxes%netMassOut, G%Domain, &
        timelevel=ridx_sum)
    call LND_read_data(surf_file,'massin_flux_sum', fluxes%netMassIn,  G%Domain, &
        timelevel=ridx_sum)
  ...
  if (read_mld) then
    call LND_read_data(surf_file, 'ePBL_h_ML', mld, G%Domain, timelevel=ridx_sum)
  endif
  ...
  if (read_sw) then
    ...
    call LND_read_data(mean_file,'sw_vis', fluxes%sw_vis_dir, G%Domain, &
                       timelevel=ridx_sum, scale=G%US%W_m2_to_QRZ_T)
    call LND_read_data(mean_file,'sw_nir', fluxes%sw_nir_dir, G%Domain, &
                       timelevel=ridx_sum, scale=G%US%W_m2_to_QRZ_T)
...
subroutine update_offline_from_arrays(G, GV, nk_input, ridx_sum, mean_file, sum_file, snap_file, uhtr, vhtr, &
  ...
  if (.not. allocated(uhtr_all)) &
      call LND_error(FATAL, "uhtr_all not allocated before call to update_transport_from_arrays")
  if (.not. allocated(vhtr_all)) &
      call LND_error(FATAL, "vhtr_all not allocated before call to update_transport_from_arrays")
  if (.not. allocated(hend_all)) &
      call LND_error(FATAL, "hend_all not allocated before call to update_transport_from_arrays")
  if (.not. allocated(temp_all)) &
      call LND_error(FATAL, "temp_all not allocated before call to update_transport_from_arrays")
  if (.not. allocated(salt_all)) &
      call LND_error(FATAL, "salt_all not allocated before call to update_transport_from_arrays")
