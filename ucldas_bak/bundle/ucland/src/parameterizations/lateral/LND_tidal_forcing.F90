...
use LND_io,            only : field_exists, file_exists, LND_read_data
use LND_time_manager,  only : time_type, time_type_to_real
...
!> The control structure for the LND_tidal_forcing module
...
!! ALLOC is a macro defined in LND_memory.h for allocate or nothing with
!! static memory.
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_tidal_forcing" ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "tidal_forcing_init called with an associated "// &
                            "control structure.")
    return
  endif
  ...
  call get_param(param_file, mdl, "TIDE_M2", use_M2, &
                 "If true, apply tidal lndentum forcing at the M2 "//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call get_param(param_file, mdl, "TIDE_S2", use_S2, &
                 "If true, apply tidal lndentum forcing at the S2 "//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call get_param(param_file, mdl, "TIDE_N2", use_N2, &
                 "If true, apply tidal lndentum forcing at the N2 "//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call get_param(param_file, mdl, "TIDE_K2", use_K2, &
                 "If true, apply tidal lndentum forcing at the K2 "//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call get_param(param_file, mdl, "TIDE_K1", use_K1, &
                 "If true, apply tidal lndentum forcing at the K1 "//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call get_param(param_file, mdl, "TIDE_O1", use_O1, &
                 "If true, apply tidal lndentum forcing at the O1 "//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call get_param(param_file, mdl, "TIDE_P1", use_P1, &
                 "If true, apply tidal lndentum forcing at the P1 "//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call get_param(param_file, mdl, "TIDE_Q1", use_Q1, &
                 "If true, apply tidal lndentum forcing at the Q1 "//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call get_param(param_file, mdl, "TIDE_MF", use_MF, &
                 "If true, apply tidal lndentum forcing at the MF "//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call get_param(param_file, mdl, "TIDE_MM", use_MM, &
                 "If true, apply tidal lndentum forcing at the MM "//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  ...
  if (nc == 0) then
    call LND_error(FATAL, "tidal_forcing_init: "// &
  ...
  if (nc > MAX_CONSTITUENTS) then
    write(mesg,'("Increase MAX_CONSTITUENTS in LND_tidal_forcing.F90 to at least",I3, &
                &"to accommodate all the registered tidal constituents.")') nc
    call LND_error(FATAL, "LND_tidal_forcing"//mesg)
  endif
...
subroutine find_in_files(filenames, varname, array, G)
  ...
  do nf=1,size(filenames)
    ...
    if (field_exists(filenames(nf), varname, G%Domain%mpp_domain)) then
      call LND_read_data(filenames(nf), varname, array, G%Domain)
      return
    endif
  enddo
  ...
  do nf=size(filenames),1,-1
    if (file_exists(filenames(nf), G%Domain)) then
      call LND_error(FATAL, "LND_tidal_forcing.F90: Unable to find "// &
         trim(varname)//" in any of the tidal input files, last tried "// &
         trim(filenames(nf)))
    endif
  enddo

  call LND_error(FATAL, "LND_tidal_forcing.F90: Unable to find any of the "// &
