...
type, public :: ideal_age_tracer_CS ; private
  ...
  type(LND_restart_CS), pointer :: restart_CSp => NULL() !< A pointer to the restart controls structure
...
!> Register the ideal age tracer fields to be used with LND.
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "register_ideal_age_tracer called with an "// &
...
! and it sets up the tracer output.
  ...
  !  Read the tracer concentrations from a netcdf file.
        if (.not.file_exists(CS%IC_file, G%Domain)) &
          call LND_error(FATAL, "initialize_ideal_age_tracer: "// &
        ...
        if (CS%Z_IC_file) then
          ...
          if (.not.OK) then
            ...
            if (.not.OK) call LND_error(FATAL,"initialize_ideal_age_tracer: "//&
