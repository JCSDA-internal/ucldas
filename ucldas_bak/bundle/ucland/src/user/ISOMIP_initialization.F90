...
use LND_ALE_sponge, only : ALE_sponge_CS, set_up_ALE_sponge_field, initialize_ALE_sponge
use LND_sponge, only : sponge_CS, set_up_sponge_field, initialize_sponge
use LND_dyn_horgrid, only : dyn_horgrid_type
use LND_error_handler, only : LND_mesg, LND_error, FATAL, is_root_pe, WARNING
...
#include "version_variable.h"
  ...
  call LND_mesg("  ISOMIP_initialization.F90, ISOMIP_initialize_topography: setting topography", 5)
...
subroutine ISOMIP_initialize_thickness ( h, G, GV, US, param_file, tv, just_read_params)
  ...
  if (.not.just_read) &
    call LND_mesg("LND_initialization.F90, initialize_thickness_uniform: setting thickness")
  ...
  case default
      call LND_error(FATAL,"isomip_initialize: "// &
...
!   Set up sponges for ISOMIP configuration
  ...
  if (associated(CSp)) call LND_error(FATAL, &
        "ISOMIP_initialize_sponges called with an associated control structure.")
  if (associated(ACSp)) call LND_error(FATAL, &
  ...
  if (use_ALE) then
    ...
    select case ( coordinateMode(verticalCoordinate) )
     ...
     case ( REGRIDDING_ZSTAR, REGRIDDING_SIGMA_SHELF_ZSTAR )   ! Initial thicknesses for z coordinates
      ...
      case default
         call LND_error(FATAL,"ISOMIP_initialize_sponges: "// &
         "Unrecognized i.c. setup - set REGRIDDING_COORDINATE_MODE")
    ...
    call LND_read_data(filename, salt_var, S(:,:,:), G%Domain)
