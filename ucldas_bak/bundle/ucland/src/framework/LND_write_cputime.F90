...
module LND_write_cputime

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_coms, only : sum_across_PEs, num_pes
use LND_error_handler, only : LND_error, LND_mesg, FATAL, is_root_pe
use LND_io, only : open_file, close_file, APPEND_FILE, ASCII_FILE, WRITEONLY_FILE
use LND_file_parser, only : get_param, log_param, log_version, param_file_type
use LND_time_manager, only : time_type, get_time, operator(>)

implicit none ; private

public write_cputime, LND_write_cputime_init, LND_write_cputime_end, write_cputime_start_clock
...
!> Initialize the LND_write_cputime module.
subroutine LND_write_cputime_init(param_file, directory, Input_start_time, CS)
...
# include "version_variable.h"
  character(len=40)  :: mdl = 'LND_write_cputime'  ! This module's name.
  ...
  call get_param(param_file, mdl, "MAXCPU", CS%maxcpu, &
                 ...
                 "LND should run before saving a restart file and "//&
...
end subroutine LND_write_cputime_init

!> Close the LND_write_cputime module.
subroutine LND_write_cputime_end(CS)
  type(write_cputime_CS), pointer    :: CS    !< The control structure set up by a previous
                                              !! call to LND_write_cputime_init.
...
end subroutine LND_write_cputime_end
...
subroutine write_cputime(day, n, CS, nmax, call_end)
  ...
  logical,      optional, intent(in)    :: call_end !< If true, also call LND_write_cputime_end.
...
!     call LND_mesg(mesg)
  ...
  if (present(call_end)) then
    if (call_end) call LND_write_cputime_end(CS)
  endif

end subroutine write_cputime

!> \namespace lnd_write_cputime
!!
