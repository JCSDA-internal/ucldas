...
use LND_diag_mediator, only : time_type, diag_ctrl
...
use LND_get_input, only : Get_LND_input
...
use LND_sponge, only : sponge_CS
use LND_ALE_sponge, only : ALE_sponge_CS
...
use LND_OCMIP2_CFC, only : register_OCMIP2_CFC, initialize_OCMIP2_CFC, flux_init_OCMIP2_CFC
use LND_OCMIP2_CFC, only : OCMIP2_CFC_column_physics, OCMIP2_CFC_surface_state
use LND_OCMIP2_CFC, only : OCMIP2_CFC_stock, OCMIP2_CFC_end, OCMIP2_CFC_CS
...
use LND_generic_tracer, only : register_LND_generic_tracer, initialize_LND_generic_tracer
use LND_generic_tracer, only : LND_generic_tracer_column_physics, LND_generic_tracer_surface_state
use LND_generic_tracer, only : end_LND_generic_tracer, LND_generic_tracer_get, LND_generic_flux_init
use LND_generic_tracer, only : LND_generic_tracer_stock, LND_generic_tracer_min_max, LND_generic_tracer_CS
...
type, public :: tracer_flow_control_CS ; private
  ...
  logical :: use_LND_generic_tracer = .false.      !< If true, use the LND_generic_tracer packages
  ...
  type(LND_generic_tracer_CS), pointer :: LND_generic_tracer_CSp => NULL()
...
subroutine call_tracer_flux_init(verbosity)
  ...
  logical :: use_OCMIP_CFCs, use_LND_generic_tracer
  ...
  call get_LND_Input(param_file, check_params=.false.)

  call get_param(param_file, mdl, "USE_OCMIP2_CFC", use_OCMIP_CFCs, &
                 default=.false., do_not_log=.true.)
  call get_param(param_file, mdl, "USE_generic_tracer", use_LND_generic_tracer,&
                 default=.false., do_not_log=.true.)
  call close_param_file(param_file, quiet_close=.true.)

  if (use_OCMIP_CFCs) call flux_init_OCMIP2_CFC(verbosity=verbosity)
  if (use_LND_generic_tracer) then
    call LND_generic_flux_init(verbosity=verbosity)
  endif
...
# include "version_variable.h"
  character(len=40)  :: mdl = "LND_tracer_flow_control" ! This module's name.

  if (associated(CS)) then
    call LND_error(WARNING, "call_tracer_register called with an associated "// &
                            "control structure.")
    return
  ...
  call get_param(param_file, mdl, "USE_OCMIP2_CFC", CS%use_OCMIP2_CFC, &
                 "If true, use the LND_OCMIP2_CFC tracer package.", &
                 default=.false.)
  call get_param(param_file, mdl, "USE_generic_tracer", CS%use_LND_generic_tracer, &
                 "If true and _USE_GENERIC_TRACER is defined as a "//&
                 "preprocessor macro, use the LND_generic_tracer packages.", &
                 default=.false.)
...
!  for some reason.  This then overrides the run-time selection from above.
  ...
  if (CS%use_OCMIP2_CFC) CS%use_OCMIP2_CFC = &
    register_OCMIP2_CFC(HI, GV, param_file,  CS%OCMIP2_CFC_CSp, &
                        tr_Reg, restart_CS)
  if (CS%use_LND_generic_tracer) CS%use_LND_generic_tracer = &
    register_LND_generic_tracer(HI, GV, param_file,  CS%LND_generic_tracer_CSp, &
                                tr_Reg, restart_CS)
...
subroutine tracer_flow_control_init(restart, day, G, GV, US, h, param_file, diag, OBC, &
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "tracer_flow_control_init: "// &
...
!  Add other user-provided calls here.
  ...
  if (CS%use_OCMIP2_CFC) &
    call initialize_OCMIP2_CFC(restart, day, G, GV, US, h, diag, OBC, CS%OCMIP2_CFC_CSp, &
                                sponge_CSp)
  if (CS%use_LND_generic_tracer) &
    call initialize_LND_generic_tracer(restart, day, G, GV, US, h, param_file, diag, OBC, &
        CS%LND_generic_tracer_CSp, sponge_CSp, ALE_sponge_CSp)
...
subroutine get_chl_from_model(Chl_array, G, CS)
  ...
  if (CS%use_LND_generic_tracer) then
    call LND_generic_tracer_get('chl', 'field', Chl_array, CS%LND_generic_tracer_CSp)
  else
    call LND_error(FATAL, "get_chl_from_model was called in a configuration "// &
             "that is unable to provide a sensible model-based value.\n"// &
             "CS%use_LND_generic_tracer is false and no other viable options are on.")
  endif
...
subroutine call_tracer_set_forcing(sfc_state, fluxes, day_start, day_interval, G, CS)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "call_tracer_set_forcing"// &
         "Module must be initialized via call_tracer_register before it is used.")
!  if (CS%use_ideal_age) &
...
subroutine call_tracer_column_fns(h_old, h_new, ea, eb, fluxes, Hml, dt, G, GV, US, tv, optics, CS, &
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "call_tracer_column_fns: "// &
         "Module must be initialized via call_tracer_register before it is used.")
  ...
  if (present(evap_CFL_limit) .and. present(minimum_forcing_depth)) then
    ...
    if (CS%use_OCMIP2_CFC) &
      call OCMIP2_CFC_column_physics(h_old, h_new, ea, eb, fluxes, dt, &
                                     ...
                                     minimum_forcing_depth=minimum_forcing_depth)
    if (CS%use_LND_generic_tracer) then
      if (US%QRZ_T_to_W_m2 /= 1.0) call LND_error(FATAL, "LND_generic_tracer_column_physics "//&
            "has not been written to permit dimensionsal rescaling.  Set all 4 of the "//&
            "[QRZT]_RESCALE_POWER parameters to 0.")
      call LND_generic_tracer_column_physics(h_old, h_new, ea, eb, fluxes, Hml, US%T_to_s*dt, &
                                             G, GV, CS%LND_generic_tracer_CSp, tv, optics, &
                                             evap_CFL_limit=evap_CFL_limit, &
                                             minimum_forcing_depth=minimum_forcing_depth)
    endif
    if (CS%use_pseudo_salt_tracer) &
...
subroutine call_tracer_stocks(h, stock_values, G, GV, CS, stock_names, stock_units, &
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "call_tracer_stocks: "// &
       "Module must be initialized via call_tracer_register before it is used.")
...
! endif
  ...
  if (CS%use_OCMIP2_CFC) then
    ...
    call store_stocks("LND_OCMIP2_CFC", ns, names, units, values, index, stock_values, &
                       set_pkg_name, max_ns, ns_tot, stock_names, stock_units)
  endif
  ...
  if (CS%use_LND_generic_tracer) then
    ns = LND_generic_tracer_stock(h, values, G, GV, CS%LND_generic_tracer_CSp, &
                                   names, units, stock_index)
    call store_stocks("LND_generic_tracer", ns, names, units, values, index, stock_values, &
                       set_pkg_name, max_ns, ns_tot, stock_names, stock_units)
    nn=ns_tot-ns+1
    nn=LND_generic_tracer_min_max(nn, got_min_max, global_min,  global_max, &
                                  xgmin, ygmin, zgmin, xgmax, ygmax, zgmax ,&
                                  G, CS%LND_generic_tracer_CSp,names, units)
...
! This routine stores the stocks and does error handling for call_tracer_stocks.
  ...
  if ((index > 0) .and. (ns > 0)) then
    ...
    if (ns > 1) then
      call LND_error(FATAL,"Tracer package "//trim(pkg_name)//&
    ...
    elseif (ns+ns_tot > 1) then
      call LND_error(FATAL,"Tracer packages "//trim(pkg_name)//" and "//&
  ...
  if (ns_tot+ns > max_ns) then
    ...
    call LND_error(FATAL,"Attempted to return more tracer stock values (at least "//&
...
subroutine call_tracer_surface_state(sfc_state, h, G, CS)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "call_tracer_surface_state: "// &
         "Module must be initialized via call_tracer_register before it is used.")
...
!  Add other user-provided calls here.
  ...
  if (CS%use_LND_generic_tracer) &
    call LND_generic_tracer_surface_state(sfc_state, h, G, CS%LND_generic_tracer_CSp)
...
subroutine tracer_flow_control_end(CS)
  ...
  if (CS%use_LND_generic_tracer) call end_LND_generic_tracer(CS%LND_generic_tracer_CSp)
...
!> \namespace LND_tracer_flow_control
!!
...
!!
end module LND_tracer_flow_control
