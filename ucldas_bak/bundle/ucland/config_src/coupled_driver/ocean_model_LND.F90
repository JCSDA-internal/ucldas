!> Top-level module for the UCLAND ocean model in coupled mode.
module ocean_model_mod

! This file is part of UCLAND. See LICENSE.md for the license.

! This is the top level module for the UCLAND ocean model.  It contains routines
! for initialization, termination and update of ocean model state.  This
! particular version wraps all of the calls for UCLAND in the calls that had
! been used for LND4.
!
! This code is a stop-gap wrapper of the UCLAND code to enable it to be called
! in the same way as LND4.

use LND, only : initialize_LND, step_LND, LND_control_struct, LND_end
use LND, only : extract_surface_state, allocate_surface_state, finish_LND_initialization
use LND, only : get_LND_state_elements, LND_state_is_synchronized
use LND, only : get_ocean_stocks, step_offline
use LND_constants, only : CELSIUS_KELVIN_OFFSET, hlf
use LND_diag_mediator, only : diag_ctrl, enable_averaging, disable_averaging
use LND_diag_mediator, only : diag_mediator_close_registration, diag_mediator_end
use LND_domains, only : pass_var, pass_vector, AGRID, BGRID_NE, CGRID_NE
use LND_domains, only : TO_ALL, Omit_Corners
use LND_error_handler, only : LND_error, LND_mesg, FATAL, WARNING, is_root_pe
use LND_error_handler, only : callTree_enter, callTree_leave
use LND_file_parser, only : get_param, log_version, close_param_file, param_file_type
use LND_forcing_type, only : forcing, mech_forcing, allocate_forcing_type
use LND_forcing_type, only : fluxes_accumulate, get_net_mass_forcing
use LND_forcing_type, only : copy_back_forcing_fields
use LND_forcing_type, only : forcing_diagnostics, mech_forcing_diags
use LND_get_input, only : Get_LND_Input, directories
use LND_grid, only : ocean_grid_type
use LND_io, only : close_file, file_exists, read_data, write_version_number
use LND_marine_ice, only : iceberg_forces, iceberg_fluxes, marine_ice_init, marine_ice_CS
use LND_restart, only : LND_restart_CS, save_restart
use LND_string_functions, only : uppercase
use LND_surface_forcing_gfdl, only : surface_forcing_init, convert_IOB_to_fluxes
use LND_surface_forcing_gfdl, only : convert_IOB_to_forces, ice_ocn_bnd_type_chksum
use LND_surface_forcing_gfdl, only : ice_ocean_boundary_type, surface_forcing_CS
use LND_surface_forcing_gfdl, only : forcing_save_restart
use LND_time_manager, only : time_type, operator(>), operator(+), operator(-)
use LND_time_manager, only : operator(*), operator(/), operator(/=)
use LND_time_manager, only : operator(<=), operator(>=), operator(<)
use LND_time_manager, only : real_to_time, time_type_to_real
use LND_tracer_flow_control, only : call_tracer_register, tracer_flow_control_init
use LND_tracer_flow_control, only : call_tracer_flux_init
use LND_unit_scaling, only : unit_scale_type
use LND_variables, only : surface
use LND_verticalGrid, only : verticalGrid_type
use LND_ice_shelf, only : initialize_ice_shelf, shelf_calc_flux, ice_shelf_CS
use LND_ice_shelf, only : add_shelf_forces, ice_shelf_end, ice_shelf_save_restart
...
use LND_EOS, only : gsw_sp_from_sr, gsw_pt_from_ct
use LND_wave_interface, only: wave_parameters_CS, LND_wave_interface_init
use LND_wave_interface, only: LND_wave_interface_init_lite, Update_Surface_Waves

#include <LND_memory.h>

#ifdef _USE_GENERIC_TRACER
use LND_generic_tracer, only : LND_generic_tracer_fluxes_accumulate
#endif
...
type, public ::  ocean_public_type
  ...
  integer :: stagger = -999   !< The staggering relative to the tracer points
                    ...
                    !! Following LND5, stagger is BGRID_NE by default when the
...
type, public :: ocean_state_type ; private
  ...
  logical :: single_step_call !< If true, advance the state of LND with a single
  ...
  type(LND_control_struct), pointer :: &
    LND_CSp => NULL()         !< A pointer to the LND control structure
  type(ice_shelf_CS), pointer :: &
    Ice_shelf_CSp => NULL()   !< A pointer to the control structure for the
                              !! ice shelf model that couples with UCLAND.  This
  ...
  type(surface_forcing_CS), pointer :: &
    forcing_CSp => NULL()     !< A pointer to the LND forcing control structure
  type(LND_restart_CS), pointer :: &
    restart_CSp => NULL()     !< A pointer set to the restart control structure
                              !! that will be used for LND restart files.
...
#include "version_variable.h"
  ...
  call callTree_enter("ocean_model_init(), ocean_model_LND.F90")
  if (associated(OS)) then
    call LND_error(WARNING, "ocean_model_init called with an associated "// &
  ...
  call initialize_LND(OS%Time, Time_init, param_file, OS%dirs, OS%LND_CSp, &
                      OS%restart_CSp, Time_in, offline_tracer_mode=OS%offline_tracer_mode, &
                      diag_ptr=OS%diag, count_calls=.true.)
  call get_LND_state_elements(OS%LND_CSp, G=OS%grid, GV=OS%GV, US=OS%US, C_p=OS%C_p, &
  ...
  call get_param(param_file, mdl, "SINGLE_STEPPING_CALL", OS%single_step_call, &
                 "If true, advance the state of LND with a single step "//&
  ...
  call get_param(param_file, "LND", "THERMO_SPANS_COUPLING", OS%thermo_spans_coupling, &
  ...
  else ; call LND_error(FATAL,"ocean_model_init: OCEAN_SURFACE_STAGGER = "// &
  ...
  if (OS%use_waves) then
    call LND_wave_interface_init(OS%Time, OS%grid, OS%GV, OS%US, param_file, OS%Waves, OS%diag)
  else
    call LND_wave_interface_init_lite(param_file)
  endif
  ...
  if (present(gas_fields_ocn)) then
    ...
    call extract_surface_state(OS%LND_CSp, OS%sfc_state)
  ...
  call LND_mesg('==== Completed UCLAND Coupled Initialization ====', 2)
...
subroutine update_ocean_model(Ice_ocean_boundary, OS, Ocean_sfc, time_start_update, &
  ...
  type(time_type) :: Time_seg_start ! Stores the dynamic or thermodynamic ocean model time at the
                            ! start of this call to allow step_LND to temporarily change the time
                            ! as seen by internal modules.
  type(time_type) :: Time_thermo_start ! Stores the ocean model thermodynamics time at the start of
                            ! this call to allow step_LND to temporarily change the time as seen by
                            ! internal modules.
  type(time_type) :: Time1  ! The value of the ocean model's time at the start of a call to step_LND.
  ...
  integer :: n_max          ! The number of calls to step_LND dynamics in this call to update_ocean_model.
  ...
  call callTree_enter("update_ocean_model(), ocean_model_LND.F90")
  dt_coupling = time_type_to_real(Ocean_coupling_time_step)

  if (.not.associated(OS)) then
    call LND_error(FATAL, "update_ocean_model called with an unassociated "// &
  ...
  if (do_thermo .and. (time_start_update /= OS%Time)) &
    call LND_error(WARNING, "update_ocean_model: internal clock does not "//&
                            "agree with time_start_update argument.")
  if (do_dyn .and. (time_start_update /= OS%Time_dyn)) &
    call LND_error(WARNING, "update_ocean_model: internal dynamics clock does not "//&
                            "agree with time_start_update argument.")

  if (.not.(do_dyn .or. do_thermo)) call LND_error(FATAL, &
      "update_ocean_model called without updating either dynamics or thermodynamics.")
  if (do_dyn .and. do_thermo .and. (OS%Time /= OS%Time_dyn)) call LND_error(FATAL, &
  ...
  if (do_thermo) then
    if (OS%fluxes%fluxes_used) then
      ...
      call LND_generic_tracer_fluxes_accumulate(OS%fluxes, 1.0) ! Here weight=1, so just store the current fluxes
      ...
      call LND_generic_tracer_fluxes_accumulate(OS%flux_tmp, weight)
#endif
    endif
  endif

  ! The net mass forcing is not currently used in the UCLAND dynamics solvers, so this is may be unnecessary.
  ...
  if ((OS%nstep==0) .and. (OS%nstep_thermo==0)) then ! This is the first call to update_ocean_model.
    call finish_LND_initialization(OS%Time, OS%dirs, OS%LND_CSp, OS%restart_CSp)
  endif
  ...
  if (OS%offline_tracer_mode .and. do_thermo) then
    call step_offline(OS%forces, OS%fluxes, OS%sfc_state, Time1, dt_coupling, OS%LND_CSp)
  elseif ((.not.do_thermo) .or. (.not.do_dyn)) then
    ! The call sequence is being orchestrated from outside of update_ocean_model.
    call step_LND(OS%forces, OS%fluxes, OS%sfc_state, Time1, dt_coupling, OS%LND_CSp, &
  ...
  elseif (OS%single_step_call) then
    call step_LND(OS%forces, OS%fluxes, OS%sfc_state, Time1, dt_coupling, OS%LND_CSp, Waves=OS%Waves)
    ...
    do n=1,n_max
      if (OS%diabatic_first) then
        if (thermo_does_span_coupling) call LND_error(FATAL, &
            "LND is not yet set up to have restarts that work with "//&
            "THERMO_SPANS_COUPLING and DIABATIC_FIRST.")
        if (modulo(n-1,nts)==0) then
          dtdia = dt_dyn*min(nts,n_max-(n-1))
          call step_LND(OS%forces, OS%fluxes, OS%sfc_state, Time1, dtdia, OS%LND_CSp, &
                        Waves=OS%Waves, do_dynamics=.false., do_thermodynamics=.true., &
                        start_cycle=(n==1), end_cycle=.false., cycle_length=dt_coupling)
        endif

        call step_LND(OS%forces, OS%fluxes, OS%sfc_state, Time1, dt_dyn, OS%LND_CSp, &
        ...
        if (thermo_does_span_coupling) then
          dtdia = dt_therm
          step_thermo = LND_state_is_synchronized(OS%LND_CSp, adv_dyn=.true.)
...
subroutine ocean_model_restart(OS, timestamp)
  ...
  if (.not.LND_state_is_synchronized(OS%LND_CSp)) &
      call LND_error(WARNING, "End of LND_main reached with inconsistent "//&
         "dynamics and advective times.  Additional restart fields "//&
         "that have not been coded yet would be required for reproducibility.")
  if (.not.OS%fluxes%fluxes_used) call LND_error(FATAL, "ocean_model_restart "//&
...
subroutine ocean_model_end(Ocean_sfc, Ocean_state, Time)
  ...
  call LND_end(Ocean_state%LND_CSp)
...
!   restart behavior as now in FMS.
  ...
  if (.not.LND_state_is_synchronized(OS%LND_CSp)) &
    call LND_error(WARNING, "ocean_model_save_restart called with inconsistent "//&
         "dynamics and advective times.  Additional restart fields "//&
         "that have not been coded yet would be required for reproducibility.")
  if (.not.OS%fluxes%fluxes_used) call LND_error(FATAL, "ocean_model_save_restart "//&
       "was called with unused buoyancy fluxes.  For conservation, the ocean "//&
       "restart files can only be created after the buoyancy forcing is applied.")
...
!> This subroutine translates the coupler's ocean_data_type into LND's
!! surface state variable.  This may eventually be folded into the LND
...
subroutine convert_state_to_ocean_type(sfc_state, Ocean_sfc, G, US, patm, press_to_z)
  ...
  if (present(patm)) then
    ...
    if (.not.present(press_to_z)) call LND_error(FATAL, &
  ...
  elseif (Ocean_sfc%stagger == CGRID_NE) then
    ...
    call LND_error(FATAL, "convert_state_to_ocean_type: "//&
  ...
  select case (index)
    case (ISTOCK_WATER)  ! Return the mass of fresh water in the ocean in kg.
      if (OS%GV%Boussinesq) then
        call get_ocean_stocks(OS%LND_CSp, mass=value, on_PE_only=.true.)
      else  ! In non-Boussinesq mode, the mass of salt needs to be subtracted.
        call get_ocean_stocks(OS%LND_CSp, mass=value, salt=salt, on_PE_only=.true.)
        value = value - salt
      endif
    case (ISTOCK_HEAT)  ! Return the heat content of the ocean in J.
      call get_ocean_stocks(OS%LND_CSp, heat=value, on_PE_only=.true.)
    case (ISTOCK_SALT)  ! Return the mass of the salt in the ocean in kg.
       call get_ocean_stocks(OS%LND_CSp, salt=value, on_PE_only=.true.)
...
subroutine ocean_model_data2D_get(OS, Ocean, name, array2D, isc, jsc)
  use LND_constants, only : CELSIUS_KELVIN_OFFSET
  type(ocean_state_type),     pointer    :: OS    !< A pointer to the structure containing the
...
! The problem is %areaT is on LND domain but Ice_Ocean_Boundary%... is on mpp domain.
! We want to return the LND data on the mpp (compute) domain
! Get LND domain extents
...
!     enddo ; enddo
  ...
  case default
     call LND_error(FATAL,'get_ocean_grid_data2D: unknown argument name='//name)
  end select
...
subroutine ocean_model_data1D_get(OS, Ocean, name, value)
  ...
  case('c_p')
    value = OS%C_p
  case default
    call LND_error(FATAL,'get_ocean_grid_data1D: unknown argument name='//name)
  end select
...
subroutine ocean_model_get_UV_surf(OS, Ocean, name, array2D, isc, jsc)
  ...
  case('vb')
    ...
    enddo ; enddo
  case default
     call LND_error(FATAL,'ocean_model_get_UV_surf: unknown argument name='//name)
  end select
