...
function rotated_mpp_chksum_real_0d(field, pelist, mask_val, turns) &
  ...
  if (present(turns)) &
    call LND_error(FATAL, "Rotation not supported for 0d fields.")
...
function rotated_mpp_chksum_real_1d(field, pelist, mask_val, turns) &
  ...
  if (present(turns)) &
    call LND_error(FATAL, "Rotation not supported for 1d fields.")
...
! NOTE: In LND_io, write_field points to mpp_write, which supports a very broad
...
subroutine rotated_time_interp_external_2d(fms_id, time, data_in, interp, &
  ...
  if (present(mask_out)) &
    call LND_error(FATAL, "Rotation of masked output not yet support")
