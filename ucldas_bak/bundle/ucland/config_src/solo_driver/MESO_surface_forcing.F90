...
!  fluxes are in kg m-2 s-1 and positive for water moving into the ocean.
  ...
  if (CS%restorebuoy .and. first_call) then !#CTRL# .or. associated(CS%ctrl_forcing_CSp)) then
    ...
    call LND_read_data(trim(CS%inputdir)//trim(CS%SSTrestore_file), "SST", &
             CS%T_Restore(:,:), G%Domain)
    call LND_read_data(trim(CS%inputdir)//trim(CS%salinityrestore_file), "SAL", &
             CS%S_Restore(:,:), G%Domain)
    call LND_read_data(trim(CS%inputdir)//trim(CS%heating_file), "Heat", &
             CS%Heat(:,:), G%Domain, scale=US%W_m2_to_QRZ_T)
    call LND_read_data(trim(CS%inputdir)//trim(CS%PmE_file), "PmE", &
             CS%PmE(:,:), G%Domain, scale=US%m_to_Z*US%T_to_s)
    call LND_read_data(trim(CS%inputdir)//trim(CS%Solar_file), "NET_SOL", &
  ...
  if (CS%restorebuoy) then
    if (CS%use_temperature) then
      ...
      !   When modifying the code, comment out this error message.  It is here
      ! so that the original (unmodified) version is not accidentally used.
!      call LND_error(FATAL, "MESO_buoyancy_surface_forcing: " // &
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "MESO_surface_forcing_init called with an associated "// &
                             "control structure.")
    return
  endif
  allocate(CS)
