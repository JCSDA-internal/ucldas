...
module LND_ice_shelf_initialize
...
use LND_io, only: LND_read_data, file_exists, slasher
...
subroutine initialize_ice_thickness_from_file(h_shelf, area_shelf_h, hmask, G, US, PF)
  ...
  call LND_mesg("  LND_ice_shelf_init_profile.F90, initialize_thickness_from_file: reading thickness")
  ...
  call LND_read_data(filename, trim(thickness_varname), h_shelf, G%Domain, scale=US%m_to_Z)
  ...
  do j=jsc,jec
    do i=isc,iec
      ...
      elseif ((area_shelf_h(i,j) > 0) .and. (area_shelf_h(i,j) <= G%areaT(i,j))) then
        ...
        call LND_error(FATAL,mdl// " AREA IN CELL OUT OF RANGE")
      endif
    enddo
  enddo
...
subroutine initialize_ice_thickness_channel(h_shelf, area_shelf_h, hmask, G, US, PF)
  ...
  call LND_mesg(mdl//": setting thickness")
...
!         flux_bdry, PF)
!     case ("FILE");  call LND_error(FATAL,"LND_initialize: "// &
!       "Unrecognized topography setup "//trim(config))
!     case ("USER");  call LND_error(FATAL,"LND_initialize: "// &
!       "Unrecognized topography setup "//trim(config))
!     case default ;  call LND_error(FATAL,"LND_initialize: "// &
!       "Unrecognized topography setup "//trim(config))
!   end select
...
!   call LND_mesg(mdl//": setting boundary")
...
end module LND_ice_shelf_initialize
