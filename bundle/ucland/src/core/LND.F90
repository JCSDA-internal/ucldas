!> The central module of the UCLAND ocean model
module LND

! This file is part of UCLAND. See LICENSE.md for the license.

! Infrastructure modules
use LND_array_transform,      only : rotate_array, rotate_vector
use LND_debugging,            only : LND_debugging_init, hchksum, uvchksum
use LND_debugging,            only : check_redundant
use LND_checksum_packages,    only : LND_thermo_chksum, LND_state_chksum
use LND_checksum_packages,    only : LND_accel_chksum, LND_surface_chksum
use LND_coms,                 only : num_PEs
use LND_cpu_clock,            only : cpu_clock_id, cpu_clock_begin, cpu_clock_end
use LND_cpu_clock,            only : CLOCK_COMPONENT, CLOCK_SUBCOMPONENT
use LND_cpu_clock,            only : CLOCK_MODULE_DRIVER, CLOCK_MODULE, CLOCK_ROUTINE
use LND_diag_mediator,        only : diag_mediator_init, enable_averaging, enable_averages
use LND_diag_mediator,        only : diag_mediator_infrastructure_init
use LND_diag_mediator,        only : diag_set_state_ptrs, diag_update_remap_grids
use LND_diag_mediator,        only : disable_averaging, post_data, safe_alloc_ptr
use LND_diag_mediator,        only : register_diag_field, register_cell_measure
use LND_diag_mediator,        only : set_axes_info, diag_ctrl, diag_masks_set
use LND_diag_mediator,        only : set_masks_for_axes
use LND_diag_mediator,        only : diag_grid_storage, diag_grid_storage_init
use LND_diag_mediator,        only : diag_save_grids, diag_restore_grids
use LND_diag_mediator,        only : diag_copy_storage_to_diag, diag_copy_diag_to_storage
use LND_domains,              only : LND_domains_init, clone_LND_domain
use LND_domains,              only : sum_across_PEs, pass_var, pass_vector
use LND_domains,              only : To_North, To_East, To_South, To_West
use LND_domains,              only : To_All, Omit_corners, CGRID_NE, SCALAR_PAIR
use LND_domains,              only : create_group_pass, do_group_pass, group_pass_type
use LND_domains,              only : start_group_pass, complete_group_pass, Omit_Corners
use LND_error_handler,        only : LND_error, LND_mesg, FATAL, WARNING, is_root_pe
use LND_error_handler,        only : LND_set_verbosity, callTree_showQuery
use LND_error_handler,        only : callTree_enter, callTree_leave, callTree_waypoint
use LND_file_parser,          only : read_param, get_param, log_version, param_file_type
use LND_forcing_type,         only : forcing, mech_forcing
use LND_forcing_type,         only : LND_forcing_chksum, LND_mech_forcing_chksum
use LND_get_input,            only : Get_LND_Input, directories
use LND_io,                   only : LND_io_init, vardesc, var_desc
use LND_io,                   only : slasher, file_exists, LND_read_data
use LND_obsolete_params,      only : find_obsolete_params
use LND_restart,              only : register_restart_field, register_restart_pair
use LND_restart,              only : query_initialized, save_restart
use LND_restart,              only : restart_init, is_new_run, LND_restart_CS
use LND_spatial_means,        only : global_mass_integral
use LND_time_manager,         only : time_type, real_to_time, time_type_to_real, operator(+)
use LND_time_manager,         only : operator(-), operator(>), operator(*), operator(/)
use LND_time_manager,         only : operator(>=), operator(==), increment_date
use LND_unit_tests,           only : unit_tests
use coupler_types_mod,        only : coupler_type_send_data, coupler_1d_bc_type, coupler_type_spawn

! LND core modules
use LND_ALE,                   only : ALE_init, ALE_end, ALE_main, ALE_CS, adjustGridForIntegrity
use LND_ALE,                   only : ALE_getCoordinate, ALE_getCoordinateUnits, ALE_writeCoordinateFile
use LND_ALE,                   only : ALE_updateVerticalGridType, ALE_remap_init_conds, ALE_register_diags
use LND_ALE_sponge,            only : rotate_ALE_sponge, update_ALE_sponge_field
use LND_barotropic,            only : Barotropic_CS
use LND_boundary_update,       only : call_OBC_register, OBC_register_end, update_OBC_CS
use LND_coord_initialization,  only : LND_initialize_coord
use LND_diabatic_driver,       only : diabatic, diabatic_driver_init, diabatic_CS, extract_diabatic_member
use LND_diabatic_driver,       only : adiabatic, adiabatic_driver_init, diabatic_driver_end
use LND_diagnostics,           only : calculate_diagnostic_fields, LND_diagnostics_init
use LND_diagnostics,           only : register_transport_diags, post_transport_diagnostics
use LND_diagnostics,           only : register_surface_diags, write_static_fields
use LND_diagnostics,           only : post_surface_dyn_diags, post_surface_thermo_diags
use LND_diagnostics,           only : diagnostics_CS, surface_diag_IDs, transport_diag_IDs
use LND_dynamics_unsplit,      only : step_LND_dyn_unsplit, register_restarts_dyn_unsplit
use LND_dynamics_unsplit,      only : initialize_dyn_unsplit, end_dyn_unsplit
use LND_dynamics_unsplit,      only : LND_dyn_unsplit_CS
use LND_dynamics_split_RK2,    only : step_LND_dyn_split_RK2, register_restarts_dyn_split_RK2
use LND_dynamics_split_RK2,    only : initialize_dyn_split_RK2, end_dyn_split_RK2
use LND_dynamics_split_RK2,    only : LND_dyn_split_RK2_CS
use LND_dynamics_unsplit_RK2,  only : step_LND_dyn_unsplit_RK2, register_restarts_dyn_unsplit_RK2
use LND_dynamics_unsplit_RK2,  only : initialize_dyn_unsplit_RK2, end_dyn_unsplit_RK2
use LND_dynamics_unsplit_RK2,  only : LND_dyn_unsplit_RK2_CS
use LND_dyn_horgrid,           only : dyn_horgrid_type, create_dyn_horgrid, destroy_dyn_horgrid
use LND_EOS,                   only : EOS_init, calculate_density, calculate_TFreeze, EOS_domain
use LND_fixed_initialization,  only : LND_initialize_fixed
use LND_forcing_type,          only : allocate_forcing_type, allocate_mech_forcing
use LND_forcing_type,          only : deallocate_mech_forcing, deallocate_forcing_type
use LND_forcing_type,          only : rotate_forcing, rotate_mech_forcing
use LND_grid,                  only : ocean_grid_type, LND_grid_init, LND_grid_end
use LND_grid,                  only : set_first_direction, rescale_grid_bathymetry
use LND_hor_index,             only : hor_index_type, hor_index_init
use LND_hor_index,             only : rotate_hor_index
use LND_interface_heights,     only : find_eta
use LND_lateral_mixing_coeffs, only : calc_slope_functions, VarMix_init
use LND_lateral_mixing_coeffs, only : calc_resoln_function, calc_depth_function, VarMix_CS
use LND_MEKE,                  only : MEKE_init, MEKE_alloc_register_restart, step_forward_MEKE, MEKE_CS
use LND_MEKE_types,            only : MEKE_type
use LND_mixed_layer_restrat,   only : mixedlayer_restrat, mixedlayer_restrat_init, mixedlayer_restrat_CS
use LND_mixed_layer_restrat,   only : mixedlayer_restrat_register_restarts
use LND_obsolete_diagnostics,  only : register_obsolete_diagnostics
use LND_open_boundary,         only : ocean_OBC_type, OBC_registry_type
use LND_open_boundary,         only : register_temp_salt_segments
use LND_open_boundary,         only : open_boundary_register_restarts
use LND_open_boundary,         only : update_segment_tracer_reservoirs
use LND_open_boundary,         only : rotate_OBC_config, rotate_OBC_init
use LND_set_visc,              only : set_viscous_BBL, set_viscous_ML, set_visc_init
use LND_set_visc,              only : set_visc_register_restarts, set_visc_CS
use LND_sponge,                only : init_sponge_diags, sponge_CS
use LND_state_initialization,  only : LND_initialize_state
use LND_sum_output,            only : write_energy, accumulate_net_input
use LND_sum_output,            only : LND_sum_output_init, sum_output_CS
use LND_ALE_sponge,            only : init_ALE_sponge_diags, ALE_sponge_CS
use LND_thickness_diffuse,     only : thickness_diffuse, thickness_diffuse_init, thickness_diffuse_CS
use LND_tracer_advect,         only : advect_tracer, tracer_advect_init
use LND_tracer_advect,         only : tracer_advect_end, tracer_advect_CS
use LND_tracer_hor_diff,       only : tracer_hordiff, tracer_hor_diff_init
use LND_tracer_hor_diff,       only : tracer_hor_diff_end, tracer_hor_diff_CS
use LND_tracer_registry,       only : tracer_registry_type, register_tracer, tracer_registry_init
use LND_tracer_registry,       only : register_tracer_diagnostics, post_tracer_diagnostics_at_sync
use LND_tracer_registry,       only : post_tracer_transport_diagnostics
use LND_tracer_registry,       only : preALE_tracer_diagnostics, postALE_tracer_diagnostics
use LND_tracer_registry,       only : lock_tracer_registry, tracer_registry_end
use LND_tracer_flow_control,   only : call_tracer_register, tracer_flow_control_CS
use LND_tracer_flow_control,   only : tracer_flow_control_init, call_tracer_surface_state
use LND_tracer_flow_control,   only : tracer_flow_control_end
use LND_transcribe_grid,       only : copy_dyngrid_to_LND_grid, copy_LND_grid_to_dyngrid
use LND_transcribe_grid,       only : rotate_dyngrid
use LND_unit_scaling,          only : unit_scale_type, unit_scaling_init
use LND_unit_scaling,          only : unit_scaling_end, fix_restart_unit_scaling
use LND_variables,             only : surface, allocate_surface_state, deallocate_surface_state
use LND_variables,             only : thermo_var_ptrs, vertvisc_type
use LND_variables,             only : accel_diag_ptrs, cont_diag_ptrs, ocean_internal_state
use LND_variables,             only : rotate_surface_state
use LND_verticalGrid,          only : verticalGrid_type, verticalGridInit, verticalGridEnd
use LND_verticalGrid,          only : fix_restart_scaling
use LND_verticalGrid,          only : get_thickness_units, get_flux_units, get_tr_flux_units
use LND_wave_interface,        only : wave_parameters_CS, waves_end
use LND_wave_interface,        only : Update_Stokes_Drift

! ODA modules
use LND_oda_driver_mod,        only : ODA_CS, oda, init_oda, oda_end
use LND_oda_driver_mod,        only : set_prior_tracer, set_analysis_time, apply_oda_tracer_increments
! Offline modules
use LND_offline_main,          only : offline_transport_CS, offline_transport_init, update_offline_fields
use LND_offline_main,          only : insert_offline_main, extract_offline_main, post_offline_convergence_diags
use LND_offline_main,          only : register_diags_offline_transport, offline_advection_ale
use LND_offline_main,          only : offline_redistribute_residual, offline_diabatic_ale
use LND_offline_main,          only : offline_fw_fluxes_into_ocean, offline_fw_fluxes_out_ocean
use LND_offline_main,          only : offline_advection_layer, offline_transport_end
use LND_ALE,                   only : ale_offline_tracer_final, ALE_main_offline

implicit none ; private

#include <LND_memory.h>

! A note on unit descriptions in comments: UCLAND uses units that can be rescaled for dimensional
...
end type LND_diag_IDs

!> Control structure for the LND module, including the variables that describe
!! the state of the ocean.
type, public :: LND_control_struct
  ...
  logical :: use_ALE_algorithm  !< If true, use the ALE algorithm rather than layered
                    ...
                    !! function useRegridding() from the LND_regridding module.
  logical :: offline_tracer_mode = .false.
                    !< If true, step_offline() is called instead of step_LND().
                    !! This is intended for running UCLAND in offline tracer mode
  ...
  logical :: count_calls = .false.   !< If true, count the calls to step_LND, rather than the
  ...
  logical :: do_dynamics             !< If false, does not call step_LND_dyn_*. This is an
  ...
  real, dimension(:,:), pointer :: &
    p_surf_prev  => NULL(), &   !< surface pressure [R L2 T-2 ~> Pa] at end  previous call to step_LND
    p_surf_begin => NULL(), &   !< surface pressure [R L2 T-2 ~> Pa] at start of step_LND_dyn_...
    p_surf_end   => NULL()      !< surface pressure [R L2 T-2 ~> Pa] at end   of step_LND_dyn_...
  ...
  type(LND_diag_IDs)       :: IDs      !<  Handles used for diagnostics.
  ...
  type(LND_dyn_unsplit_CS),      pointer :: dyn_unsplit_CSp => NULL()
    !< Pointer to the control structure used for the unsplit dynamics
  type(LND_dyn_unsplit_RK2_CS),  pointer :: dyn_unsplit_RK2_CSp => NULL()
    !< Pointer to the control structure used for the unsplit RK2 dynamics
  type(LND_dyn_split_RK2_CS),    pointer :: dyn_split_RK2_CSp => NULL()
  ...
  type(tracer_registry_type),    pointer :: tracer_Reg => NULL()
    !< Pointer to the LND tracer registry
  type(tracer_advect_CS),        pointer :: tracer_adv_CSp => NULL()
    !< Pointer to the LND tracer advection control structure
  type(tracer_hor_diff_CS),      pointer :: tracer_diff_CSp => NULL()
    !< Pointer to the LND along-isopycnal tracer diffusion control structure
  ...
  type(ocean_OBC_type),          pointer :: OBC => NULL()
    !< Pointer to the LND open boundary condition type
  ...
  type(diagnostics_CS),          pointer :: diagnostics_CSp => NULL()
    !< Pointer to the LND diagnostics control structure
...
end type LND_control_struct

public initialize_LND, finish_LND_initialization, LND_end
public step_LND, step_offline
public extract_surface_state, get_ocean_stocks
public get_LND_state_elements, LND_state_is_synchronized
...
!> This subroutine orchestrates the time stepping of LND.  The adiabatic
!! dynamics are stepped by calls to one of the step_LND_dyn_...routines.
...
subroutine step_LND(forces_in, fluxes_in, sfc_state, Time_start, time_int_in, CS, &
  ...
  type(LND_control_struct), pointer :: CS            !< control structure from initialize_LND
  ...
  if (.not.(do_dyn .or. do_thermo)) call LND_error(FATAL,"Step_LND: "//&
  ...
  if (CS%debug) then
    call LND_state_chksum("Beginning of step_LND ", u, v, h, CS%uh, CS%vh, G, GV, US)
  endif

  showCallTree = callTree_showQuery()
  if (showCallTree) call callTree_enter("step_LND(), LND.F90")
  ...
  if (do_dyn) then
    ...
    if (CS%UseWaves) then
      ! Update wave information, which is presently kept static over each call to step_lnd
  ...
  if (CS%debug) then
    if (cycle_start) &
      call LND_state_chksum("Before steps ", u, v, h, CS%uh, CS%vh, G, GV, US)
    if (cycle_start) call check_redundant("Before steps ", u, v, G)
    if (do_dyn) call LND_mech_forcing_chksum("Before steps", forces, G, US, haloshift=0)
  ...
  do n=1,n_max
    ...
    if (showCallTree) call callTree_enter("DT cycles (step_LND) n=",n)
    ...
    if (CS%diabatic_first .and. (CS%t_dyn_rel_adv==0.0) .and. do_thermo) then ! do thermodynamics.
      ...
      elseif (thermo_does_span_coupling) then
        ...
        if ((fluxes%dt_buoy_accum > 0.0) .and. (dtdia > time_interval) .and. &
          ...
          call LND_error(FATAL, "step_LND: Mismatch between long thermodynamic "//&
            "timestep and time over which buoyancy fluxes have been accumulated.")
        endif
        call LND_error(FATAL, "LND is not yet set up to have restarts that work "//&
      ...
      if (dtdia > dt) then
        ...
        ! by the call to step_LND_thermo, noting that they begin at the same time.
      ...
      call step_LND_thermo(CS, G, GV, US, u, v, h, CS%tv, fluxes, dtdia, &
      ...
      if (showCallTree) call callTree_waypoint("finished diabatic_first (step_LND)")
    ...
    if (do_dyn) then
      ...
      call step_LND_dynamics(forces, CS%p_surf_begin, CS%p_surf_end, dt, &
      ...
      if (do_advection) then ! Do advective transport and lateral tracer mixing.
        call step_LND_tracer_dyn(CS, G, GV, US, h, Time_local)
        if (CS%diabatic_first .and. abs(CS%t_dyn_rel_thermo) > 1e-6*dt) call LND_error(FATAL, &
                "step_LND: Mismatch between the dynamics and diabatic times "//&
                "with DIABATIC_FIRST.")
      endif
    endif ! end of (do_dyn)
    ...
    if ((CS%t_dyn_rel_adv==0.0) .and. do_thermo .and. (.not.CS%diabatic_first)) then
      ...
      ! If the UCLAND dynamic and thermodynamic time stepping is being orchestrated
      ...
      if (CS%thermo_spans_coupling .and. (CS%dt_therm > 1.5*cycle_time) .and. &
        ...
        call LND_error(FATAL, "step_LND: Mismatch between dt_therm and dtdia "//&
                       "before call to diabatic.")
      endif

      ! If necessary, temporarily reset CS%Time to the center of the period covered
      ! by the call to step_LND_thermo, noting that they end at the same time.
    ...
    if (LND_state_is_synchronized(CS)) then
    !### Perhaps this should be if (CS%t_dyn_rel_thermo == 0.0)
      ...
      if (showCallTree) call callTree_waypoint("finished calculate_diagnostic_fields (step_LND)")
      call disable_averaging(CS%diag)
    ...
    if (showCallTree) call callTree_leave("DT cycles (step_LND)")
  ...
  if (showCallTree) call callTree_waypoint("calling extract_surface_state (step_LND)")
  ...
  if (LND_state_is_synchronized(CS)) &
  ...
  if (showCallTree) call callTree_leave("step_LND()")
  call cpu_clock_end(id_clock_ocean)

end subroutine step_LND

!> Time step the ocean dynamics, including the lndentum and continuity equations
subroutine step_LND_dynamics(forces, p_surf_begin, p_surf_end, dt, dt_thermo, &
  ...
  type(unit_scale_type),   pointer :: US => NULL() ! Pointer to a structure containing
                                                   ! various unit conversion factors
  type(LND_diag_IDs), pointer :: IDs => NULL() ! A structure with the diagnostic IDs.
  real, dimension(:,:,:), pointer :: &
    u => NULL(), & ! u : zonal velocity component [L T-1 ~> m s-1]
    v => NULL(), & ! v : meridional velocity component [L T-1 ~> m s-1]
    h => NULL()    ! h : layer thickness [H ~> m or kg m-2]
  ...
  if ((CS%t_dyn_rel_adv == 0.0) .and. CS%thickness_diffuse .and. CS%thickness_diffuse_first) then
    ...
    call disable_averaging(CS%diag)
    if (showCallTree) call callTree_waypoint("finished thickness_diffuse_first (step_LND)")
  ...
  if (bbl_time_int > 0.0) then
    ...
    if (showCallTree) call callTree_wayPoint("done with set_viscous_BBL (step_LND)")
    call disable_averaging(CS%diag)
  endif
  ...
  if (CS%do_dynamics .and. CS%split) then !--------------------------- start SPLIT
    ...
    call step_LND_dyn_split_RK2(u, v, h, CS%tv, CS%visc, Time_local, dt, forces, &
    ...
    if (showCallTree) call callTree_waypoint("finished step_LND_dyn_split (step_LND)")
  ...
  elseif (CS%do_dynamics) then ! ------------------------------------ not SPLIT
    ...
    if (CS%use_RK2) then
      call step_LND_dyn_unsplit_RK2(u, v, h, CS%tv, CS%visc, Time_local, dt, forces, &
               p_surf_begin, p_surf_end, CS%uh, CS%vh, CS%uhtr, CS%vhtr, &
               CS%eta_av_bc, G, GV, US, CS%dyn_unsplit_RK2_CSp, CS%VarMix, CS%MEKE)
    else
      call step_LND_dyn_unsplit(u, v, h, CS%tv, CS%visc, Time_local, dt, forces, &
               p_surf_begin, p_surf_end, CS%uh, CS%vh, CS%uhtr, CS%vhtr, &
               CS%eta_av_bc, G, GV, US, CS%dyn_unsplit_CSp, CS%VarMix, CS%MEKE, Waves=Waves)
    endif
    if (showCallTree) call callTree_waypoint("finished step_LND_dyn_unsplit (step_LND)")
  ...
  if (CS%thickness_diffuse .and. .not.CS%thickness_diffuse_first) then
    ...
    call cpu_clock_end(id_clock_thick_diff)
    call pass_var(h, G%Domain, clock=id_clock_pass, halo=max(2,CS%cont_stencil))
    if (showCallTree) call callTree_waypoint("finished thickness_diffuse (step_LND)")
  endif
...
end subroutine step_LND_dynamics

!> step_LND_tracer_dyn does tracer advection and lateral diffusion, bringing the
!! tracers up to date with the changes in state due to the dynamics.  Surface
!! sources and sinks and remapping are handled via step_LND_thermo.
subroutine step_LND_tracer_dyn(CS, G, GV, US, h, Time_local)
  type(LND_control_struct), intent(inout) :: CS     !< control structure
  ...
  ! call LND_thermo_chksum("Pre-advection ", CS%tv, G, US)
    call cpu_clock_end(id_clock_other)
  endif
  ...
  if (showCallTree) call callTree_waypoint("finished tracer advection/diffusion (step_LND)")
...
end subroutine step_LND_tracer_dyn

!> LND_step_thermo orchestrates the thermodynamic time stepping and vertical
!! remapping, via calls to diabatic (or adiabatic) and ALE_main.
subroutine step_LND_thermo(CS, G, GV, US, u, v, h, tv, fluxes, dtdia, &
                           Time_end_thermo, update_BBL, Waves)
  type(LND_control_struct), intent(inout) :: CS     !< Master LND control structure
  type(ocean_grid_type),    intent(inout) :: G      !< ocean grid structure
  ...
  is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec ; nz = G%ke
  showCallTree = callTree_showQuery()
  if (showCallTree) call callTree_enter("step_LND_thermo(), LND.F90")
  ...
  if (update_BBL) then
    ...
    call cpu_clock_end(id_clock_BBL_visc)
    if (showCallTree) call callTree_wayPoint("done with set_viscous_BBL (step_LND_thermo)")
  endif
  ...
  if (.not.CS%adiabatic) then
    if (CS%debug) then
      ...
      call uvchksum("Pre-diabatic [uv]h", CS%uhtr, CS%vhtr, G%HI, &
                    haloshift=0, scale=GV%H_to_m*US%L_to_m**2)
    ! call LND_state_chksum("Pre-diabatic ", u, v, h, CS%uhtr, CS%vhtr, G, GV, vel_scale=1.0)
      call LND_thermo_chksum("Pre-diabatic ", tv, G, US, haloshift=0)
      call check_redundant("Pre-diabatic ", u, v, G)
      call LND_forcing_chksum("Pre-diabatic", fluxes, G, US, haloshift=0)
    endif
    ...
    if (showCallTree) call callTree_waypoint("finished diabatic (step_LND_thermo)")

    ! Regridding/remapping is done here, at end of thermodynamics time step
    ! (that may comprise several dynamical time steps)
    ! The routine 'ALE_main' can be found in 'LND_ALE.F90'.
...
!         call pass_vector(u, v, G%Domain)
      ...
      if (CS%debug) then
        call LND_state_chksum("Pre-ALE ", u, v, h, CS%uh, CS%vh, G, GV, US)
      ...
      if (showCallTree) call callTree_waypoint("finished ALE_main (step_LND_thermo)")
    ...
    if (CS%debug .and. CS%use_ALE_algorithm) then
      call LND_state_chksum("Post-ALE ", u, v, h, CS%uh, CS%vh, G, GV, US)
    ...
    if (CS%debug) then
      ...
      call uvchksum("Post-diabatic [uv]h", CS%uhtr, CS%vhtr, G%HI, &
                    haloshift=0, scale=GV%H_to_m*US%L_to_m**2)
    ! call LND_state_chksum("Post-diabatic ", u, v, &
    ...
    ! call LND_thermo_chksum("Post-diabatic ", tv, G, US)
  ...
  if (showCallTree) call callTree_leave("step_LND_thermo(), LND.F90")

end subroutine step_LND_thermo


!> step_offline is the main driver for running tracers offline in UCLAND. This has been primarily
...
!! called here can be found in tracers/LND_offline_control.F90
subroutine step_offline(forces, fluxes, sfc_state, Time_start, time_interval, CS)
  ...
  if (CS%use_ALE_algorithm) then
    ...
    if (first_iter) then
      call LND_mesg("Reading in new offline fields")
    ...
    if (last_iter) then
      ...
      call LND_mesg("Last iteration of offline interval")
...
!> Initialize LND, including memory allocation, setting up parameters and diagnostics,
!! initializing the ocean state variables, and initializing subsidiary modules
subroutine initialize_LND(Time, Time_init, param_file, dirs, CS, restart_CSp, &
  ...
  type(LND_control_struct),  pointer       :: CS          !< pointer set in this routine to LND control structure
  type(LND_restart_CS),      pointer       :: restart_CSp !< pointer set in this routine to the
                                                          !! restart control structure that will
                                                          !! be used for LND.
  type(time_type), optional, intent(in)    :: Time_in     !< time passed to LND_initialize_state when
  ...
  logical,         optional, intent(in)    :: count_calls !< If true, nstep_tot counts the number of
                                                          !! calls to step_LND instead of the number of
                                                          !! dynamics timesteps.
  ! local variables
...
# include "version_variable.h"
  ...
  type(LND_restart_CS),  pointer      :: restart_CSp_tmp => NULL()
  type(group_pass_type) :: tmp_pass_uv_T_S_h, pass_uv_T_S_h

  real    :: default_val       ! default value for a parameter
  logical :: write_geom_files  ! If true, write out the grid geometry files.
  logical :: ensemble_ocean    ! If true, perform an ensemble gather at the end of step_LND
  logical :: new_sim
  ...
  type(ocean_internal_state)      :: LND_internal_state
  character(len=200) :: area_varname, ice_shelf_file, inputdir, filename

  if (associated(CS)) then
    call LND_error(WARNING, "initialize_LND called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
  ...
  call get_LND_input(param_file, dirs, default_input_filename=input_restart_file)

  verbosity = 2 ; call read_param(param_file, "VERBOSITY", verbosity)
  call LND_set_verbosity(verbosity)
  call callTree_enter("initialize_LND(), LND.F90")
  ...
  call log_version(param_file, "LND", version, "", log_to_all=.true., layout=.true., debugging=.true.)
  ...
  call get_param(param_file, "LND", "DO_UNIT_TESTS", do_unit_tests, &
  ...
  call get_param(param_file, "LND", "SPLIT", CS%split, &
                 "Use the split time stepping if true.", default=.true.)
  if (CS%split) then
    CS%use_RK2 = .false.
  else
    call get_param(param_file, "LND", "USE_RK2", CS%use_RK2, &
                 "If true, use RK2 instead of RK3 in the unsplit time stepping.", &
                 default=.false.)
  endif

  call get_param(param_file, "LND", "CALC_RHO_FOR_SEA_LEVEL", CS%calc_rho_for_sea_lev, &
  ...
  call get_param(param_file, "LND", "ENABLE_THERMODYNAMICS", use_temperature, &
                 "If true, Temperature and salinity are used as state "//&
                 "variables.", default=.true.)
  call get_param(param_file, "LND", "USE_EOS", use_EOS, &
  ...
  call get_param(param_file, "LND", "DIABATIC_FIRST", CS%diabatic_first, &
                 "If true, apply diabatic and thermodynamic processes, "//&
                 "including buoyancy forcing and mass gain or loss, "//&
                 "before stepping the dynamics forward.", default=.false.)
  call get_param(param_file, "LND", "USE_CONTEMP_ABSSAL", use_conT_absS, &
  ...
  call get_param(param_file, "LND", "ADIABATIC", CS%adiabatic, &
                 "There are no diapycnal mass fluxes if ADIABATIC is "//&
                 "true. This assumes that KD = KDML = 0.0 and that "//&
                 "there is no buoyancy forcing, but makes the model "//&
                 "faster by eliminating subroutine calls.", default=.false.)
  call get_param(param_file, "LND", "DO_DYNAMICS", CS%do_dynamics, &
  ...
  call get_param(param_file, "LND", "ADVECT_TS", advect_TS, &
  ...
  if (present(offline_tracer_mode)) then ! Only read this parameter in enabled modes
    call get_param(param_file, "LND", "OFFLINE_TRACER_MODE", CS%offline_tracer_mode, &
  ...
  call get_param(param_file, "LND", "USE_REGRIDDING", CS%use_ALE_algorithm, &
                 "If True, use the ALE algorithm (regridding/remapping). "//&
                 "If False, use the layered isopycnal algorithm.", default=.false. )
  call get_param(param_file, "LND", "BULKMIXEDLAYER", bulkmixedlayer, &
  ...
  call get_param(param_file, "LND", "THICKNESSDIFFUSE", CS%thickness_diffuse, &
                 "If true, interface heights are diffused with a "//&
                 "coefficient of KHTH.", default=.false.)
  call get_param(param_file, "LND",  "THICKNESSDIFFUSE_FIRST", &
  ...
  call get_param(param_file, "LND", "BATHYMETRY_AT_VEL", bathy_at_vel, &
                 "If true, there are separate values for the basin depths "//&
                 "at velocity points.  Otherwise the effects of topography "//&
                 "are entirely determined from thickness points.", &
                 default=.false.)
  call get_param(param_file, "LND", "USE_WAVES", CS%UseWaves, default=.false., &
                 do_not_log=.true.)

  call get_param(param_file, "LND", "DEBUG", CS%debug, &
                 "If true, write out verbose debugging data.", &
                 default=.false., debuggingParam=.true.)
  call get_param(param_file, "LND", "DEBUG_TRUNCATIONS", debug_truncations, &
                 "If true, calculate all diagnostics that are useful for "//&
                 "debugging truncations.", default=.false., debuggingParam=.true.)

  call get_param(param_file, "LND", "DT", CS%dt, &
                 "The (baroclinic) dynamics time step.  The time-step that "//&
                 "is actually used will be an integer fraction of the "//&
                 "forcing time-step (DT_FORCING in ocean-only mode or the "//&
                 "coupling timestep in coupled mode.)", units="s", scale=US%s_to_T, &
                 fail_if_missing=.true.)
  call get_param(param_file, "LND", "DT_THERM", CS%dt_therm, &
  ...
  call get_param(param_file, "LND", "THERMO_SPANS_COUPLING", CS%thermo_spans_coupling, &
  ...
  if (bulkmixedlayer) then
    ...
    call get_param(param_file, "LND", "HMIX_SFC_PROP", CS%Hmix, &
    ...
    call get_param(param_file, "LND", "HMIX_UV_SFC_PROP", CS%Hmix_UV, &
  ...
  call get_param(param_file, "LND", "HFREEZE", CS%HFrz, &
                 "If HFREEZE > 0, melt potential will be computed. The actual depth "//&
                 "over which melt potential is computed will be min(HFREEZE, OBLD), "//&
                 "where OBLD is the boundary layer depth. If HFREEZE <= 0 (default), "//&
                 "melt potential will not be computed.", units="m", default=-1.0, scale=US%m_to_Z)
  call get_param(param_file, "LND", "INTERPOLATE_P_SURF", CS%interp_p_surf, &
  ...
  if (CS%split) then
    call get_param(param_file, "LND", "DTBT", dtbt, default=-0.98)
    default_val = US%T_to_s*CS%dt_therm ; if (dtbt > 0.0) default_val = -1.0
    CS%dtbt_reset_period = -1.0
    call get_param(param_file, "LND", "DTBT_RESET_PERIOD", CS%dtbt_reset_period, &
  ...
  if (use_temperature) then
    call get_param(param_file, "LND", "FRAZIL", use_frazil, &
  ...
  if (CS%check_bad_sfc_vals) then
    ...
    call get_param(param_file, "LND", "BAD_VAL_SSS_MAX", CS%bad_val_sss_max, &
                 "The value of SSS above which a bad value message is "//&
                 "triggered, if CHECK_BAD_SURFACE_VALS is true.", units="PPT", &
                 default=45.0)
    call get_param(param_file, "LND", "BAD_VAL_SST_MAX", CS%bad_val_sst_max, &
                 "The value of SST above which a bad value message is "//&
                 "triggered, if CHECK_BAD_SURFACE_VALS is true.", &
                 units="deg C", default=45.0)
    call get_param(param_file, "LND", "BAD_VAL_SST_MIN", CS%bad_val_sst_min, &
                 "The value of SST below which a bad value message is "//&
                 "triggered, if CHECK_BAD_SURFACE_VALS is true.", &
                 units="deg C", default=-2.1)
    call get_param(param_file, "LND", "BAD_VAL_COLUMN_THICKNESS", CS%bad_val_col_thick, &
                 "The value of column thickness below which a bad value message is "//&
                 "triggered, if CHECK_BAD_SURFACE_VALS is true.", &
                 units="m", default=0.0, scale=US%m_to_Z)
  endif
  call get_param(param_file, "LND", "DEFAULT_2018_ANSWERS", default_2018_answers, &
                 "This sets the default value for the various _2018_ANSWERS parameters.", &
                 default=.false.)
  call get_param(param_file, "LND", "SURFACE_2018_ANSWERS", CS%answers_2018, &
  ...
  call get_param(param_file, "LND", "SAVE_INITIAL_CONDS", save_IC, &
                 "If true, write the initial conditions to a file given "//&
                 "by IC_OUTPUT_FILE.", default=.false.)
  call get_param(param_file, "LND", "IC_OUTPUT_FILE", CS%IC_file, &
                 "The file into which to write the initial conditions.", &
                 default="LND_IC")
  call get_param(param_file, "LND", "WRITE_GEOM", write_geom, &
  ...
  if (write_geom<0 .or. write_geom>2) call LND_error(FATAL,"LND: "//&
...
!                      ((write_geom==1) .and. is_new_run(restart_CSp)))
  ...
  if (CS%use_ALE_algorithm .and. bulkmixedlayer) call LND_error(FATAL, &
    "LND: BULKMIXEDLAYER can not currently be used with the ALE algorithm.")
  if (CS%use_ALE_algorithm .and. .not.use_temperature) call LND_error(FATAL, &
     "LND: At this time, USE_EOS should be True when using the ALE algorithm.")
  if (CS%adiabatic .and. use_temperature) call LND_error(WARNING, &
    "LND: ADIABATIC and ENABLE_THERMODYNAMICS both defined is usually unwise.")
  if (use_EOS .and. .not.use_temperature) call LND_error(FATAL, &
    "LND: ENABLE_THERMODYNAMICS must be defined to use USE_EOS.")
  if (CS%adiabatic .and. bulkmixedlayer) call LND_error(FATAL, &
    "LND: ADIABATIC and BULKMIXEDLAYER can not both be defined.")
  if (bulkmixedlayer .and. .not.use_EOS) call LND_error(FATAL, &
      "initialize_LND: A bulk mixed layer can only be used with T & S as "//&
      "state variables. Add USE_EOS = True to LND_input.")

  call get_param(param_file, 'LND', "ICE_SHELF", use_ice_shelf, default=.false., do_not_log=.true.)
  if (use_ice_shelf) then
     inputdir = "." ;  call get_param(param_file, 'LND', "INPUTDIR", inputdir)
     inputdir = slasher(inputdir)
     call get_param(param_file, 'LND', "ICE_THICKNESS_FILE", ice_shelf_file, &
                    "The file from which the ice bathymetry and area are read.", &
                    fail_if_missing=.true.)
     call get_param(param_file, 'LND', "ICE_AREA_VARNAME", area_varname, &
  ...
  call get_param(param_file, "LND", "ENSEMBLE_OCEAN", CS%ensemble_ocean, &
                 ...
                 "and at the end of step LND, we will perform a gather of the ensemble "//&
                 "members for statistical evaluation and/or data assimilation.", default=.false.)

  call callTree_waypoint("LND parameters read (initialize_LND)")

  ! Grid rotation test
  call get_param(param_file, "LND", "ROTATE_INDEX", CS%rotate_index, &
  ...
  if (CS%rotate_index) then
    ...
    if (num_PEs() /= 1) &
      call LND_error(FATAL, "Index rotation is only supported on one PE.")

    call get_param(param_file, "LND", "INDEX_TURNS", turns, &
  ...
  G_in => CS%G_in
#ifdef STATIC_MEMORY_
  call LND_domains_init(G_in%domain, param_file, symmetric=symmetric, &
  ...
  if (CS%rotate_index) then
    allocate(CS%G)
    call clone_LND_domain(G_in%Domain, CS%G%Domain, turns=turns)
  ...
  if (test_grid_copy .and. CS%rotate_index) &
    call LND_error(FATAL, "Grid cannot be copied during index rotation.")

  if (test_grid_copy) then ; allocate(G)
  else ; G => CS%G ; endif

  call callTree_waypoint("domains initialized (initialize_LND)")

  call LND_debugging_init(param_file)
  ...
  call clone_LND_domain(G_in%Domain, dG_in%Domain)

  ! Allocate initialize time-invariant LND variables.
  call LND_initialize_fixed(dG_in, US, OBC_in, param_file, write_geom_files, &
                            dirs%output_directory)

  call callTree_waypoint("returned from LND_initialize_fixed() (initialize_LND)")
  ...
  if (CS%debug .or. dG%symmetric) &
    call clone_LND_domain(dG%Domain, dG%Domain_aux, symmetric=.false.)

  call callTree_waypoint("grids initialized (initialize_LND)")

  call LND_timing_init(CS)
  ...
  ! Allocate and initialize space for the primary time-varying LND variables.
  ...
  if (use_temperature) then
    ...
    !   along segments.  Bit reproducibility requires that LND_initialize_state
  ...
  if (CS%use_ALE_algorithm) then
    call get_param(param_file, "LND", "NK_RHO_VARIES", GV%nk_rho_varies, default=0) ! Will default to nz later... -AJA
  endif
  ...
  if (debug_truncations) then
    ...
    LND_internal_state%u_prev => CS%u_prev
    LND_internal_state%v_prev => CS%v_prev
  ...
  LND_internal_state%u => CS%u ; LND_internal_state%v => CS%v
  LND_internal_state%h => CS%h
  LND_internal_state%uh => CS%uh ; LND_internal_state%vh => CS%vh
  if (use_temperature) then
    LND_internal_state%T => CS%T ; LND_internal_state%S => CS%S
  endif
  ...
  call callTree_waypoint("state variables allocated (initialize_LND)")
  ...
  ! Additional calls can be added to LND_tracer_flow_control.F90.
  ...
  call callTree_waypoint("restart registration complete (initialize_LND)")

  ! Initialize dynamically evolving fields, perhaps from restart files.
  call cpu_clock_begin(id_clock_LND_init)
  call LND_initialize_coord(GV, US, param_file, write_geom_files, &
                            dirs%output_directory, CS%tv, dG%max_depth)
  call callTree_waypoint("returned from LND_initialize_coord() (initialize_LND)")

  if (CS%use_ALE_algorithm) then
    call ALE_init(param_file, GV, US, dG%max_depth, CS%ALE_CSp)
    call callTree_waypoint("returned from ALE_init() (initialize_LND)")
  endif
  ...
  !     call clone_LND_domain(dG%Domain, G%Domain)

  ! NOTE: If indices are rotated, then G and G_in must both be initialized.
  !   If not rotated, then G_in and G are the same grid.
  if (CS%rotate_index) then
    call LND_grid_init(G, param_file, US, HI, bathymetry_at_vel=bathy_at_vel)
    call copy_dyngrid_to_LND_grid(dG, G, US)
    call destroy_dyn_horgrid(dG)
  endif
  call LND_grid_init(G_in, param_file, US, HI_in, bathymetry_at_vel=bathy_at_vel)
  call copy_dyngrid_to_LND_grid(dG_in, G_in, US)
  ...
  if (CS%debug .or. G%symmetric) then
    call clone_LND_domain(G%Domain, G%Domain_aux, symmetric=.false.)
  ...
  if (CS%rotate_index) then
    ...
    call LND_initialize_state(u_in, v_in, h_in, CS%tv, Time, G_in, GV, US, &
    ...
    if (associated(sponge_in_CSp)) then
      ...
      call LND_error(FATAL, "Index rotation of non-ALE sponge is not yet implemented.")
    endif
    ...
    call LND_initialize_state(CS%u, CS%v, CS%h, CS%tv, Time, G, GV, US, &
        param_file, dirs, restart_CSp, CS%ALE_CSp, CS%tracer_Reg, &
        CS%sponge_CSp, CS%ALE_sponge_CSp, CS%OBC, Time_in)
  endif

  call cpu_clock_end(id_clock_LND_init)
  call callTree_waypoint("returned from LND_initialize_state() (initialize_LND)")

! ! Need this after LND_initialize_state for DOME OBC stuff.
! if (associated(CS%OBC)) &
...
! call callTree_waypoint("restart registration complete (initialize_LND)")
  ...
  if (test_grid_copy) then
    ...
    call clone_LND_domain(G%Domain, CS%G%Domain)
    call LND_grid_init(CS%G, param_file, US)

    call copy_LND_grid_to_dyngrid(G, dg, US)
    call copy_dyngrid_to_LND_grid(dg, CS%G, US)

    call destroy_dyn_horgrid(dG)
    call LND_grid_end(G) ; deallocate(G)

    G => CS%G
    if (CS%debug .or. CS%G%symmetric) then
      call clone_LND_domain(CS%G%Domain, CS%G%Domain_aux, symmetric=.false.)
  ...
  if (ALE_remap_init_conds(CS%ALE_CSp) .and. .not. query_initialized(CS%h,"h",restart_CSp)) then
    ...
    call callTree_waypoint("Calling adjustGridForIntegrity() to remap initial conditions (initialize_LND)")
    call adjustGridForIntegrity(CS%ALE_CSp, G, GV, CS%h )
    call callTree_waypoint("Calling ALE_main() to remap initial conditions (initialize_LND)")
    if (use_ice_shelf) then
      filename = trim(inputdir)//trim(ice_shelf_file)
      if (.not.file_exists(filename, G%Domain)) call LND_error(FATAL, &
        "LND: Unable to open "//trim(filename))

      allocate(area_shelf_h(isd:ied,jsd:jed))
      allocate(frac_shelf_h(isd:ied,jsd:jed))
      call LND_read_data(filename, trim(area_varname), area_shelf_h, G%Domain, scale=US%m_to_L**2)
  ...
  ! This step has to be done after call to LND_initialize_state
  ! and before LND_diagnostics_init
  ...
  call callTree_waypoint("static fields written (initialize_LND)")
  ...
  call cpu_clock_end(id_clock_LND_init)
  call callTree_waypoint("ALE initialized (initialize_LND)")
  ...
  if (CS%split) then
    ...
    call initialize_dyn_split_RK2(CS%u, CS%v, CS%h, CS%uh, CS%vh, eta, Time, &
              ...
              CS%dt, CS%ADp, CS%CDp, LND_internal_state, CS%VarMix, CS%MEKE, &
  ...
  elseif (CS%use_RK2) then
    call initialize_dyn_unsplit_RK2(CS%u, CS%v, CS%h, Time, G, GV, US,     &
            ...
            CS%ADp, CS%CDp, LND_internal_state, CS%MEKE, CS%OBC,           &
  ...
  call callTree_waypoint("dynamics initialized (initialize_LND)")

  CS%mixedlayer_restrat = mixedlayer_restrat_init(Time, G, GV, US, param_file, diag, &
                                                  CS%mixedlayer_restrat_CSp, restart_CSp)
  if (CS%mixedlayer_restrat) then
    if (.not.(bulkmixedlayer .or. CS%use_ALE_algorithm)) &
      call LND_error(FATAL, "LND: MIXEDLAYER_RESTRAT true requires a boundary layer scheme.")
  ...
  call LND_diagnostics_init(LND_internal_state, CS%ADp, CS%CDp, Time, G, GV, US, &
  ...
  call callTree_waypoint("tracer registry now locked (initialize_LND)")
  ...
  !--- set up group pass for u,v,T,S and h. pass_uv_T_S_h also is used in step_LND
  call cpu_clock_begin(id_clock_pass_init)
  dynamics_stencil = min(3, G%Domain%nihalo, G%Domain%njhalo)
  ...
  call LND_sum_output_init(G_in, US, param_file, dirs%output_directory, &
                           CS%ntrunc, Time_init, CS%sum_output_CSp)

  ! Flag whether to save initial conditions in finish_LND_initialization() or not.
  ...
  !### This could perhaps go here instead of in finish_LND_initialization?
  ! call fix_restart_scaling(GV)
  ! call fix_restart_unit_scaling(US)

  call callTree_leave("initialize_LND()")
  call cpu_clock_end(id_clock_init)

end subroutine initialize_LND

!> Finishes initializing LND and writes out the initial conditions.
subroutine finish_LND_initialization(Time, dirs, CS, restart_CSp)
  ...
  type(LND_control_struct), pointer       :: CS          !< pointer to LND control structure
  type(LND_restart_CS),     pointer       :: restart_CSp !< pointer to the restart control
                                                         !! structure that will be used for LND.
  ! Local variables
  type(ocean_grid_type),   pointer :: G => NULL()  ! pointer to a structure containing
                                                   ! metrics and related information
  type(verticalGrid_type), pointer :: GV => NULL() ! Pointer to the vertical grid structure
  ...
  call cpu_clock_begin(id_clock_init)
  call callTree_enter("finish_LND_initialization()")

  ! Pointers for convenience
  G => CS%G ; GV => CS%GV ; US => CS%US

  !### Move to initialize_LND?
  call fix_restart_scaling(GV)
  call fix_restart_unit_scaling(US)
  ...
  call callTree_leave("finish_LND_initialization()")
  call cpu_clock_end(id_clock_init)

end subroutine finish_LND_initialization
...
subroutine register_diags(Time, G, GV, US, IDs, diag)
  ...
  type(LND_diag_IDs),      intent(inout) :: IDs   !< A structure with the diagnostic IDs.
...
subroutine LND_timing_init(CS)
  type(LND_control_struct), intent(in) :: CS  !< control structure set up by initialize_LND.
 ...
 id_clock_LND_init   = cpu_clock_id('(Ocean LND_initialize_state)', grain=CLOCK_MODULE)
...
end subroutine LND_timing_init
...
subroutine extract_surface_state(CS, sfc_state_in)
  type(LND_control_struct), pointer    :: CS !< Master LND control structure
  ...
  call callTree_enter("extract_surface_state(), LND.F90")
  G => CS%G ; G_in => CS%G_in ; GV => CS%GV ; US => CS%US
...
!   Determine the mean velocities in the uppermost depth_ml fluid.
  ...
  if (CS%check_bad_sfc_vals) then
      ...
      if (G%mask2dT(i,j)>0.) then
        ...
        if (localError) then
          ...
          if (numberOfErrors<9) then ! Only report details for the first few errors
            ...
            call LND_error(WARNING, trim(msg), all_print=.true.)
          elseif (numberOfErrors==9) then ! Indicate once that there are more errors
            call LND_error(WARNING, 'There were more unreported extreme events!', all_print=.true.)
    ...
    if (numberOfErrors>0) then
      ...
      call LND_error(FATAL, trim(msg))
    endif
  endif

  if (CS%debug) call LND_surface_chksum("Post extract_sfc", sfc_state, G, US, haloshift=0)
...
!> Return true if all phases of step_LND are at the same point in time.
function LND_state_is_synchronized(CS, adv_dyn) result(in_synch)
  type(LND_control_struct), pointer :: CS !< LND control structure
...
end function LND_state_is_synchronized

!> This subroutine offers access to values or pointers to other types from within
!! the LND_control_struct, allowing the LND_control_struct to be opaque.
subroutine get_LND_state_elements(CS, G, GV, US, C_p, C_p_scaled, use_temp)
  type(LND_control_struct),          pointer     :: CS !< LND control structure
...
end subroutine get_LND_state_elements
...
!> \namespace lnd
!!
!! Modular Ocean Model (LND) Version 6.0 (UCLAND)
!!
...
!!    * Niki Zadeh
!!
!!  LND ice-shelf code was developed by
!!  * Daniel Goldberg
!!  * Robert Hallberg
!!  * Chris Little
!!  * Olga Sergienko
!!
!!  \section section_overview Overview of LND
!!
!!  This program (LND) simulates the ocean by numerically solving
...
!!  of LND, in UCLAND these equations are horizontally discretized on an
!!  Arakawa C-grid.  (It remains to be seen whether a B-grid dynamic
!!  core will be revived in UCLAND at a later date; for now applications
!!  requiring a B-grid discretization should use LND5.1.)  UCLAND offers
...
!!  fixed density layers.  Version 6.0 of LND continues in the long
!!  tradition of a commitment to climate-quality ocean simulations
!!  embodied in previous versions of LND, even as it draws extensively
...
!!  developed at NOAA/GFDL.  LND has also benefited tremendously from
...
!!  numerics are second order accurate in space.  LND can run with an
...
!!  usually LND_input and LND_override, and the options choices are
!!  then documented for each run in LND_param_docs.
!!
!!    UCLAND integrates the equations forward in time in three distinct
...
!!  vertical viscosity and lndentum forcing, and interface height
...
!!  vertical coordinate.
!!
!!    The present file (LND.F90) orchestrates the main time stepping
...
!!  lndentum as well. As this is in addition to the vertical viscosity,
...
!!  ocean simulations.
!!
!!    LND has a number of noteworthy debugging capabilities.
!!  Excessively large velocities are truncated and LND will stop
...
!!  code the problem lies.  All of the test cases provided with LND
...
!!
!!  \section section_structure Structure of LND
!!
!!  About 115 other files of source code and 4 header files comprise
!!  the LND code, although there are several hundred more files that
!!  make up the FMS infrastructure upon which LND is built.  Each of
!!  the LND files contains comments documenting what it does, and
...
!!
!!    The typical LND directory tree is as follows:
!!
!! \verbatim
!!        ../LND
!!        |-- config_src
...
!!  will be described to give a broad overview of the LND code
!!  structure.
!!
...
!!    The files here are used to couple LND as a component in a larger
...
!!    conventions used by LND, and converts the LND surface fields
!!    back to the forms used by other FMS components.
!!
!!  * config_src/dynamic:
!!    The only file here is the version of LND_memory.h that is used
!!    for dynamic memory configurations of LND.
!!
!!  * config_src/solo_driver:
!!    The files here are include the _main driver that is used when
!!    LND is configured as an ocean-only model, as well as the files
!!    that specify the surface forcing in this configuration.
!!
!!    The directories under examples provide a large number of working
!!  configurations of LND, along with reference solutions for several
!!  different compilers on GFDL's latest large computer.  The versions
!!  of LND_memory.h in these directories need not be used if dynamic
!!  memory allocation is desired, and the answers should be unchanged.
!!
!!    The directories under src contain most of the LND files.  These
!!  files are used in every configuration using LND.
!!
!!  * src/core:
!!    The files here constitute the LND dynamic core.  This directory
!!    also includes files with the types that describe the model's
!!    lateral grid and have defined types that are shared across
!!    various LND modules to allow for more succinct and flexible
...
!!
!!  * src/framework:
!!    These files provide infrastructure utilities for LND.  Many are
...
!!    LND. When LND is adapted to use a modeling infrastructure
!!    distinct from FMS, most of the required changes are in this
!!    directory.
!!
!!  * src/initialization:
!!    These are the files that are used to initialize the LND grid
!!    or provide the initial physical state for LND.  These files are
...
!!  LND_input, and possibly one or two of the files in src/user.
!!  In addition, the diag_table (LND_diag_table) will commonly be
...
!!
!!
!!  There are 3 publicly visible subroutines in this file (LND.F90).
!!  * step_LND steps LND over a specified interval of time.
!!  * LND_initialize calls initialize and does other initialization
...
!!    exported structure.
!!
!!    The remaining subroutines in this file (src/core/LND.F90) are:
...
!!  * LND_timing_init initializes various CPU time clocks.
...
!!
!!  \section section_heat_budget Diagnosing LND heat budget
!!
!!  Here are some example heat budgets for the ALE version of UCLAND.
!!
!!  \subsection subsection_2d_heat_budget Depth integrated heat budget
!!
!!  Depth integrated heat budget diagnostic for LND.
!!
...
!!
!!  Here is an example 3d heat budget diagnostic for LND.
!!
...
!!
!! * OPOTTEMPTEND                   = net tendency of heat as diagnosed in LND.F90
...
!!   case UCLAND partitions tendencies into k > 1 layers.
!!
!! * FRAZIL_HEAT_TENDENCY generally has 3d structure, since UCLAND frazil calculation checks the
!!   full ocean column.
!!
...
!!
!!  Here is an example 2d heat budget (depth summed) diagnostic for LND.
!!
!! * OPOTTEMPTEND_2d = T_ADVECTION_XY_2d + OPOTTEMPPMDIFF_2d + HFDS
!!
!!
!!  Here is an example 3d salt budget diagnostic for LND.
!!
!! * OSALTTEND = S_ADVECTION_XY + SH_TENDENCY_VERT_REMAP + OSALTDIFF + OSALTPMDIFF
!!                + BOUNDARY_FORCING_SALT_TENDENCY
!!
!! * OSALTTEND                      = net tendency of salt as diagnosed in LND.F90
...
!!   the case when layers are tiny, in which case UCLAND partitions tendencies into k > 1 layers.
!!
!! * SFDSI = BOUNDARY_FORCING_SALT_TENDENCY[k=\@sum]
!!
!!  Here is an example 2d salt budget (depth summed) diagnostic for LND.
!!
