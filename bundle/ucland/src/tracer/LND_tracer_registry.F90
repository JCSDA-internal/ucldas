...
use LND_diag_mediator, only : diag_ctrl, register_diag_field, post_data, safe_alloc_ptr
use LND_diag_mediator, only : diag_grid_storage
use LND_diag_mediator, only : diag_copy_storage_to_diag, diag_save_grids, diag_restore_grids
...
use LND_io,            only : vardesc, query_vardesc, cmor_long_std
...
public LND_tracer_chksum, LND_tracer_chkinv
...
subroutine register_tracer(tr_ptr, Reg, param_file, HI, GV, name, longname, units, &
  ...
  type(LND_restart_CS), optional, pointer       :: restart_CS   !< A pointer to the restart control structure
  ...
  if (present(name)) then
    ...
    if (present(tr_desc)) call LND_error(WARNING, "LND register_tracer: "//&
  ...
  elseif (present(tr_desc)) then
    ...
    call LND_error(FATAL,"LND register_tracer: Either name or "//&
                   "tr_desc must be present when registering a tracer.")
  endif

  if (Reg%locked) call LND_error(FATAL, &
      "LND register_tracer was called for variable "//trim(Tr%name)//&
...
subroutine lock_tracer_registry(Reg)
  ...
  if (.not. associated(Reg)) call LND_error(WARNING, &
...
subroutine register_tracer_diagnostics(Reg, h, Time, diag, G, GV, US, use_ALE)
  ...
  if (.not. associated(Reg)) call LND_error(FATAL, "register_tracer_diagnostics: "//&
...
subroutine postALE_tracer_diagnostics(Reg, G, GV, diag, dt)
  ...
  do m=1,Reg%ntr ; if (Reg%Tr(m)%id_tr_vardec > 0) then
    m2 = Reg%Tr(m)%ind_tr_squared
    if (m2 < 1) call LND_error(FATAL, "Bad value of Tr%ind_tr_squared for "//trim(Reg%Tr(m)%name))
  ! Update squared quantities
    do k=1,nz ; do j=js,je ; do i=is,ie
...
!> Post tracer diganostics when that should only be posted when LND's state
...
subroutine LND_tracer_chksum(mesg, Tr, ntr, G)
...
end subroutine LND_tracer_chksum

!> Calculates and prints the global inventory of all tracers in the registry.
subroutine LND_tracer_chkinv(mesg, G, h, Tr, ntr)
  character(len=*),                         intent(in) :: mesg !< message that appears on the chksum lines
  type(ocean_grid_type),                    intent(in) :: G    !< ocean grid structure
...
end subroutine LND_tracer_chkinv
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_tracer_registry" ! This module's name.
  character(len=256) :: mesg    ! Message for error messages.
...
end module LND_tracer_registry
