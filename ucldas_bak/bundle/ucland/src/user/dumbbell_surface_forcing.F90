...
use LND_diag_mediator, only : register_diag_field, diag_ctrl
...
use LND_forcing_type, only : forcing, allocate_forcing_type
use LND_grid, only : ocean_grid_type
use LND_io, only : file_exists, read_data
use LND_safe_alloc, only : safe_alloc_ptr
use LND_time_manager, only : time_type, operator(+), operator(/), get_time
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "dumbbell_surface_forcing_init called with an associated "// &
                             "control structure.")
    return
  endif
  allocate(CS)
