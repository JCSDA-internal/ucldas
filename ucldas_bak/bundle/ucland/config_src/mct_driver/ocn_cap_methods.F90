module ocn_cap_methods
  ...
  use LND_surface_forcing_mct, only: ice_ocean_boundary_type
...
!> Maps incomping ocean data to UCLAND data structures
...
subroutine ocn_export(ind, ocn_public, grid, o2x, dt_int, ncouple_per_day)
  ...
  ! The mask comes from "grid" that uses the usual LND domain that has halos
  ...
  do j=grid%jsc, grid%jec
    ...
    do i=grid%isc,grid%iec
      ...
      ! Make a copy of ssh in order to do a halo update. We use the usual LND domain
