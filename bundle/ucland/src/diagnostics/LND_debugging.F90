...
!! debugging functions for UCLAND.  This routine is similar to it counterpart in
!! the SIS2 code, except for the use of the ocean_grid_type and by keeping them
!! separate we retain the ability to set up UCLAND and SIS2 debugging separately.
module LND_debugging

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_checksums, only : hchksum, Bchksum, qchksum, uvchksum, hchksum_pair
use LND_checksums, only : is_NaN, chksum, LND_checksums_init
use LND_coms, only : PE_here, root_PE, num_PEs
use LND_coms, only : min_across_PEs, max_across_PEs, reproducing_sum
use LND_domains, only : pass_vector, pass_var, pe_here
use LND_domains, only : BGRID_NE, AGRID, To_All, Scalar_Pair
...
public :: LND_debugging_init, totalStuff, totalTandS
public :: check_column_integral, check_column_integrals

! These interfaces come from LND_checksums.
...
!> LND_debugging_init initializes the LND_debugging module, and sets
!! the parameterts that control which checks are active for UCLAND.
subroutine LND_debugging_init(param_file)
  type(param_file_type),   intent(in)    :: param_file !< A structure to parse for run-time parameters
! This include declares and sets the variable "version".
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_debugging" ! This module's name.
  ...
  call LND_checksums_init(param_file)

end subroutine LND_debugging_init
...
subroutine check_redundant_vC2d(mesg, u_comp, v_comp, G, is, ie, js, je, &
  ...
  if (.not.associated(G%Domain_aux)) call LND_error(FATAL," check_redundant"//&
