...
module LND_generic_tracer

! This file is part of UCLAND. See LICENSE.md for the license.

#include <LND_memory.h>

! The following macro is usually defined in <fms_platform.h> but since UCLAND should not directly
...
#define _ALLOCATED allocated
  ...
  use LND_diag_mediator, only : diag_ctrl, get_diag_time_end
  use LND_error_handler, only : LND_error, FATAL, WARNING, NOTE, is_root_pe
  use LND_file_parser, only : get_param, log_param, log_version, param_file_type
  use LND_forcing_type, only : forcing, optics_type
  use LND_grid, only : ocean_grid_type
  use LND_hor_index, only : hor_index_type
  use LND_io, only : file_exists, LND_read_data, slasher
  use LND_restart, only : register_restart_field, query_initialized, LND_restart_CS
  use LND_spatial_means, only : global_area_mean
  use LND_sponge, only : set_up_sponge_field, sponge_CS
  use LND_ALE_sponge, only : set_up_ALE_sponge_field, ALE_sponge_CS
  use LND_time_manager, only : time_type, set_time
  use LND_tracer_diabatic, only : tracer_vertdiff, applyTracerBoundaryFluxesInOut
  use LND_tracer_registry, only : register_tracer, tracer_registry_type
  use LND_tracer_Z_init, only : tracer_Z_init
  use LND_tracer_initialization_from_Z, only : LND_initialize_tracer_from_Z
  use LND_unit_scaling, only : unit_scale_type
  use LND_variables, only : surface, thermo_var_ptrs
  ...
  !> An state hidden in module data that is very much not allowed in UCLAND
  ! ### This needs to be fixed
  logical :: g_registered = .false.

  public register_LND_generic_tracer, initialize_LND_generic_tracer
  public LND_generic_tracer_column_physics, LND_generic_tracer_surface_state
  public end_LND_generic_tracer, LND_generic_tracer_get
  public LND_generic_tracer_stock
  public LND_generic_flux_init
  public LND_generic_tracer_min_max
  public LND_generic_tracer_fluxes_accumulate

  !> Control structure for generic tracers
  type, public :: LND_generic_tracer_CS ; private
     ...
     type(LND_restart_CS), pointer :: restart_CSp => NULL() !< Restart control structure
  ...
  end type LND_generic_tracer_CS
...
contains
  ...
  !! Adds the tracers in the list of generic tracers to the set of LND tracers (i.e., LND-register them)
  !! Register these tracers for restart
  function register_LND_generic_tracer(HI, GV, param_file, CS, tr_Reg, restart_CS)
    type(hor_index_type),       intent(in)   :: HI         !< Horizontal index ranges
    type(verticalGrid_type),    intent(in)   :: GV         !< The ocean's vertical grid structure
    type(param_file_type),      intent(in)   :: param_file !< A structure to parse for run-time parameters
    type(LND_generic_tracer_CS), pointer     :: CS         !< Pointer to the control structure for this module
    type(tracer_registry_type), pointer      :: tr_Reg     !< Pointer to the control structure for the tracer
                                                           !! advection and diffusion module.
    type(LND_restart_CS),       pointer      :: restart_CS !< Pointer to the restart control structure.

! Local variables
    logical :: register_LND_generic_tracer

    character(len=128), parameter :: sub_name = 'register_LND_generic_tracer'
    ...
    register_LND_generic_tracer = .false.
    if (associated(CS)) then
       call LND_error(WARNING, "register_LND_generic_tracer called with an "// &
  ...
  ! Read all relevant parameters and write them to the model log.
    ...
    ntau=1 ! LND needs the fields at only one time step


    !   At this point G%mask2dT and CS%diag%axesTL are not allocated.
    ! postpone diag_registeration to initialize_LND_generic_tracer
    ...
    !
    ! LND-register the generic tracers
    !

    !Get the tracer list
    call generic_tracer_get_list(CS%g_tracer_list)
    if (.NOT. associated(CS%g_tracer_list)) call LND_error(FATAL, trim(sub_name)//&
    ...
    do
       ...
       !!nnz: LND field is 3D. Does this affect performance? Need it be override field?
       tr_ptr => tr_field(:,:,:,1)
    ...
    register_LND_generic_tracer = .true.
  end function register_LND_generic_tracer

  !>  Initialize phase II:  Initialize required variables for generic tracers
  !!  There are some steps of initialization that cannot be done in register_LND_generic_tracer
  ...
  subroutine initialize_LND_generic_tracer(restart, day, G, GV, US, h, param_file, diag, OBC, CS, &
    ...
    character(len=128), parameter :: sub_name = 'initialize_LND_generic_tracer'
    logical :: OK
    ...
    do
      if (INDEX(CS%IC_file, '_NULL_') /= 0) then
         call LND_error(WARNING,"The name of the IC_file "//trim(CS%IC_file)//&
                              " indicates no LND initialization was asked for the generic tracers."//&
                              "Bypassing the LND initialization of ALL generic tracers!")
         exit
      endif
      call g_tracer_get_alias(g_tracer,g_tracer_name)
      call g_tracer_get_pointer(g_tracer,g_tracer_name,'field',tr_field)
      ...
      if (.not.restart .or. (CS%tracers_may_reinit .and. &
       ...
       if (g_tracer%requires_src_info ) then
         call LND_error(NOTE,"initialize_LND_generic_tracer: "//&
                             "initializing generic tracer "//trim(g_tracer_name)//&
                             " using LND_initialize_tracer_from_Z ")

         call LND_initialize_tracer_from_Z(h, tr_ptr, G, GV, US, param_file,               &
       ...
       elseif(.not. g_tracer%requires_restart) then
        ...
        !  Read the tracer concentrations from a netcdf file.
          if (.not.file_exists(CS%IC_file)) call LND_error(FATAL, &
                  "initialize_LND_Generic_tracer: Unable to open "//CS%IC_file)
          if (CS%Z_IC_file) then
            OK = tracer_Z_init(tr_ptr, h, CS%IC_file, g_tracer_name, G, US)
            if (.not.OK) then
              OK = tracer_Z_init(tr_ptr, h, CS%IC_file, trim(g_tracer_name), G, US)
              if (.not.OK) call LND_error(FATAL,"initialize_LND_Generic_tracer: "//&
            ...
            call LND_read_data(CS%IC_file, trim(g_tracer_name), tr_ptr, G%Domain)
          endif
        else
          call LND_error(FATAL,"initialize_LND_generic_tracer: "//&
  ...
  end subroutine initialize_LND_generic_tracer
  ...
  !! flux as a source.
  subroutine LND_generic_tracer_column_physics(h_old, h_new, ea, eb, fluxes, Hml, dt, G, GV, CS, tv, optics, &
    ...
    ! Local variables
    character(len=128), parameter :: sub_name = 'LND_generic_tracer_column_physics'
    ...
    !Get the tracer list
    if (.NOT. associated(CS%g_tracer_list)) call LND_error(FATAL,&
    ...
    !Niki: This is moved out to ocean_model_LND.F90 because if dt_therm>dt_cpld we need to average
    !      the fluxes without coming into this subroutine.
    !      LND5 has to modified to conform.
    ...
    if (present(evap_CFL_limit) .and. present(minimum_forcing_depth)) then
      ! Last arg is tau which is always 1 for UCLAND
    ...
    ! Second arg is tau which is always 1 for UCLAND
  ...
  end subroutine LND_generic_tracer_column_physics
  ...
  function LND_generic_tracer_stock(h, stocks, G, GV, CS, names, units, stock_index)
    type(ocean_grid_type),              intent(in)    :: G    !< The ocean's grid structure
    type(verticalGrid_type),            intent(in)    :: GV   !< The ocean's vertical grid structure
    real, dimension(SZI_(G),SZJ_(G),SZK_(G)), intent(in) :: h !< Layer thicknesses [H ~> m or kg m-2]
    ...
    integer, optional,                  intent(in)    :: stock_index !< The coded index of a specific stock
                                                                     !! being sought.
    integer                                           :: LND_generic_tracer_stock !< Return value, the
...
! Local variables
    ...
    real, dimension(:,:,:,:), pointer   :: tr_field
    real, dimension(:,:,:), pointer     :: tr_ptr
    character(len=128), parameter :: sub_name = 'LND_generic_tracer_stock'

    integer :: i, j, k, is, ie, js, je, nz, m
    is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec ; nz = G%ke

    LND_generic_tracer_stock = 0
    ...
    LND_generic_tracer_stock = m

  end function LND_generic_tracer_stock
  ...
  function LND_generic_tracer_min_max(ind_start, got_minmax, gmin, gmax, xgmin, ygmin, zgmin, &
    ...
    character(len=*), dimension(:), intent(out)   :: names  !< The names of the stocks calculated.
    character(len=*), dimension(:), intent(out)   :: units  !< The units of the stocks calculated.
    integer                                       :: LND_generic_tracer_min_max !< Return value, the
...
! Local variables
    ...
    real, dimension(:,:,:,:), pointer   :: tr_field
    real, dimension(:,:,:), pointer     :: tr_ptr
    character(len=128), parameter :: sub_name = 'LND_generic_tracer_min_max'
    ...
    LND_generic_tracer_min_max = m

  end function LND_generic_tracer_min_max
  ...
  subroutine LND_generic_tracer_surface_state(sfc_state, h, G, CS)
    type(ocean_grid_type),                 intent(in)    :: G    !< The ocean's grid structure
...
! Local variables
    ...
    character(len=128), parameter :: sub_name = 'LND_generic_tracer_surface_state'
    ...
    call generic_tracer_coupler_set(sfc_state%tr_fields,&
         ST=sfc_state%SST,&
         SS=sfc_state%SSS,&
         rho=rho0,& !nnz: required for LND5 and previous versions.
         ilb=G%isd, jlb=G%jsd,&
...
!ALL PE subroutine on Ocean!  Due to otpm design the fluxes should be initialized like this on ALL PE's!
  subroutine LND_generic_flux_init(verbosity)
    ...
    character(len=128), parameter :: sub_name = 'LND_generic_flux_init'
    ...
    if (.NOT. associated(g_tracer_list)) then
       call LND_error(WARNING, trim(sub_name)// ": No generic tracer in the list.")
       return
    endif
  ...
  end subroutine LND_generic_flux_init

  subroutine LND_generic_tracer_fluxes_accumulate(flux_tmp, weight)
  ...
  end subroutine LND_generic_tracer_fluxes_accumulate

  !> Copy the requested tracer into an array.
  subroutine LND_generic_tracer_get(name,member,array, CS)
    ...
    character(len=128), parameter :: sub_name = 'LND_generic_tracer_get'

    call g_tracer_get_pointer(CS%g_tracer_list,name,member,array_ptr)
    array(:,:,:) = array_ptr(:,:,:)

  end subroutine LND_generic_tracer_get

  !> This subroutine deallocates the memory owned by this module.
  subroutine end_LND_generic_tracer(CS)
    type(LND_generic_tracer_CS), pointer :: CS   !< Pointer to the control structure for this module.
  ...
  end subroutine end_LND_generic_tracer
