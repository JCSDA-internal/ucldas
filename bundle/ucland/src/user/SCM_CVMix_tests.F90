...
use LND_time_manager,  only : time_type, operator(+), operator(/), time_type_to_real
use LND_unit_scaling,  only : unit_scale_type
...
# include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(FATAL, "SCM_CVMix_tests_surface_forcing_init called with an associated "// &
                          "control structure.")
    return
  endif
  allocate(CS)
