...
subroutine init_coord_adapt(CS, nk, coordinateResolution, m_to_H, kg_m3_to_R)
  ...
  if (associated(CS)) call LND_error(FATAL, "init_coord_adapt: CS already associated")
  allocate(CS)
...
subroutine set_adapt_params(CS, adaptTimeRatio, adaptAlpha, adaptZoom, adaptZoomCoeff, &
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "set_adapt_params: CS not associated")
