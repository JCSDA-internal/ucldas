...
use LND_open_boundary,   only : ocean_OBC_type, OBC_segment_tracer_type
...
!> Register tracer fields and subroutines to be used with LND.
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "DOME_register_tracer called with an "// &
  ...
  if (.not.restart) then
    if (len_trim(CS%tracer_IC_file) >= 1) then
      ...
      if (.not.file_exists(CS%tracer_IC_file, G%Domain)) &
        call LND_error(FATAL, "DOME_initialize_tracer: Unable to open "// &
                        CS%tracer_IC_file)
      do m=1,NTR
        call query_vardesc(CS%tr_desc(m), name, caller="initialize_DOME_tracer")
        call LND_read_data(CS%tracer_IC_file, trim(name), CS%tr(:,:,:,m), G%Domain)
      enddo
    else
      do m=1,NTR
...
! to set_up_sponge_field can simply be omitted.
    if (.not.associated(sponge_CSp)) &
      call LND_error(FATAL, "DOME_initialize_tracer: "// &
