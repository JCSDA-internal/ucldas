...
character(len=40)  :: mdl = "LND_bkgnd_mixing" !< This module's name.
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "bkgnd_mixing_init called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
  ...
  ! the need to include BULKMIXEDLAYER in LND_input
  ...
  if (CS%bulkmixedlayer) then
    ...
    if (CS%Kdml>0.) call LND_error(FATAL, &
...
!  call openParameterBlock(param_file,'LND_BACKGROUND_MIXING')
  ...
  if (CS%Bryan_Lewis_diffusivity .or. CS%horiz_varying_background) then
    ...
    if ( abs(CS%prandtl_bkgnd - prandtl_bkgnd_comp)>1.e-14) then
      call LND_error(FATAL,"set_diffusivity_init: The provided KD, KV,"//&
  ...
  if (CS%Kd>0.0 .and. (trim(CS%bkgnd_scheme_str)=="BRYAN_LEWIS_DIFFUSIVITY" .or.&
    ...
    call LND_error(WARNING, "set_diffusivity_init: a nonzero constant background "//&
  ...
  if (CS%Henyey_IGW_background .and. CS%Kd_tanh_lat_fn) call LND_error(FATAL, &
    "LND_bkgnd_mixing: KD_TANH_LAT_FN can not be used with HENYEY_IGW_BACKGROUND.")
...
!  call closeParameterBlock(param_file)
  ...
  CS%id_kd_bkgnd = register_diag_field('ocean_model', 'Kd_bkgnd', diag%axesTi, Time, &
      'Background diffusivity added by LND_bkgnd_mixing module', 'm2/s', conversion=US%Z2_T_to_m2_s)
  CS%id_kv_bkgnd = register_diag_field('ocean_model', 'Kv_bkgnd', diag%axesTi, Time, &
      'Background viscosity added by LND_bkgnd_mixing module', 'm2/s', conversion=US%Z2_T_to_m2_s)
...
subroutine check_bkgnd_scheme(CS,str)
  ...
  character(len=*), intent(in)   :: str !< Background scheme identifier deducted from LND_input
                                        !! parameters

  if (trim(CS%bkgnd_scheme_str)=="none") then
    CS%bkgnd_scheme_str = str
  else
     call LND_error(FATAL, "set_diffusivity_init: Cannot activate both "//trim(str)//" and "//&
