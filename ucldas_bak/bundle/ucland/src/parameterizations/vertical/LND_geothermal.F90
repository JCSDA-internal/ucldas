...
use LND_diag_mediator, only : register_static_field, time_type, diag_ctrl
...
subroutine geothermal(h, tv, dt, ea, eb, G, GV, US, CS, halo)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "LND_geothermal: "//&
  ...
  if (.not.associated(tv%T)) call LND_error(FATAL, "LND geothermal: "//&
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_geothermal"  ! module name
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "geothermal_init called with an associated"// &
  ...
  if (CS%dRcv_dT_inplace >= 0.0) call LND_error(FATAL, "geothermal_init: "//&
  ...
  if (len_trim(geo_file) >= 1) then
    ...
    call LND_read_data(filename, trim(geotherm_var), CS%geo_heat, G%Domain)
