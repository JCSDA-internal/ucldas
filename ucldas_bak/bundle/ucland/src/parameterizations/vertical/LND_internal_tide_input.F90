...
use LND_diag_mediator,    only : diag_ctrl, query_averaging_enabled
use LND_diag_mediator,    only : safe_alloc_ptr, post_data, register_diag_field
...
use LND_io,               only : slasher, vardesc, LND_read_data
use LND_isopycnal_slopes, only : vert_fill_TS
use LND_time_manager,     only : time_type, set_time, operator(+), operator(<=)
...
# include "version_variable.h"
  character(len=40)  :: mdl = "LND_int_tide_input"  ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "int_tide_input_init called with an associated "// &
                            "control structure.")
    return
  endif
  ...
  if (read_tideamp) then
    ...
    call LND_read_data(filename, 'tideamp', itide%tideamp, G%domain, timelevel=1, scale=US%m_s_to_L_T)
  endif
  ...
  call LND_read_data(filename, 'h2', itide%h2, G%domain, timelevel=1, scale=US%m_to_Z**2)
