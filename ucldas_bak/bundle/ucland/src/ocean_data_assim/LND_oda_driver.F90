!> Interfaces for UCLAND ensembles and data assimilation.
module LND_oda_driver_mod
...
use LND_diag_mediator, only : diag_ctrl, set_axes_info
use LND_error_handler, only : FATAL, WARNING, LND_error, LND_mesg, is_root_pe
...
use LND_fixed_initialization, only : LND_initialize_fixed, LND_initialize_topography
use LND_coord_initialization, only : LND_initialize_coord
use LND_file_parser, only : read_param, get_param, param_file_type
use LND_string_functions, only : lowercase
use LND_ALE, only : ALE_CS, ALE_initThicknessToCoord, ALE_init, ALE_updateVerticalGridType
use LND_domains, only : LND_domains_init, LND_domain_type, clone_LND_domain
use LND_remapping, only : remapping_CS, initialize_remapping, remapping_core_h
use LND_regridding, only : regridding_CS, initialize_regridding
use LND_regridding, only : regridding_main, set_regrid_params
use LND_unit_scaling, only : unit_scale_type, unit_scaling_init
use LND_variables, only : thermo_var_ptrs
use LND_verticalGrid, only : verticalGrid_type, verticalGridInit
...
type, public :: ODA_CS ; private
  ...
  type(ocean_grid_type), pointer :: Grid => NULL() !< UCLAND grid type and decomposition for the DA
...
! for the analysis
  call get_LND_input(PF,dirs,ensemble_num=0)

  call unit_scaling_init(PF, CS%US)

  call get_param(PF, "LND", "ASSIM_METHOD", assim_method,  &
       "String which determines the data assimilation method "//&
       "Valid methods are: \'EAKF\',\'OI\', and \'NO_ASSIM\'", default='NO_ASSIM')
  call get_param(PF, "LND", "ASSIM_FREQUENCY", CS%assim_frequency,  &
       "data assimilation frequency in hours")
  call get_param(PF, "LND", "USE_REGRIDDING", CS%use_ALE_algorithm , &
                "If True, use the ALE algorithm (regridding/remapping).\n"//&
                "If False, use the layered isopycnal algorithm.", default=.false. )
  call get_param(PF, "LND", "REENTRANT_X", CS%reentrant_x, &
       "If true, the domain is zonally reentrant.", default=.true.)
  call get_param(PF, "LND", "REENTRANT_Y", CS%reentrant_y, &
       "If true, the domain is meridionally reentrant.", &
       default=.false.)
  call get_param(PF,"LND", "TRIPOLAR_N", CS%tripolar_N, &
       "Use tripolar connectivity at the northern edge of the "//&
       "domain.  With TRIPOLAR_N, NIGLOBAL must be even.", &
       default=.false.)
  call get_param(PF,"LND", "NIGLOBAL", CS%ni, &
       "The total number of thickness grid points in the "//&
       "x-direction in the physical domain.")
  call get_param(PF,"LND", "NJGLOBAL", CS%nj, &
       "The total number of thickness grid points in the "//&
       "y-direction in the physical domain.")
  call get_param(PF, 'LND', "INPUTDIR", inputdir)
  ...
  call LND_domains_init(CS%Grid%Domain,PF,param_suffix='_ODA')
  allocate(HI)
  ...
  call clone_LND_domain(CS%Grid%Domain, dG%Domain,symmetric=.false.)
  call set_grid_metrics(dG,PF)
  call LND_initialize_topography(dg%bathyT,dG%max_depth,dG,PF)
  call LND_initialize_coord(CS%GV, CS%US, PF, .false., &
           dirs%output_directory, tv_dummy, dG%max_depth)
  call ALE_init(PF, CS%GV, CS%US, dG%max_depth, CS%ALE_CS)
  call LND_grid_init(CS%Grid, PF, global_indexing=.true.)
  call ALE_updateVerticalGridType(CS%ALE_CS, CS%GV)
  call copy_dyngrid_to_LND_grid(dG, CS%Grid, CS%US)
  ...
  call LND_read_data(basin_file,'basin',CS%oda_grid%basin_mask,CS%Grid%domain, timelevel=1)
...
subroutine set_prior_tracer(Time, G, GV, h, tv, CS)
  ...
  if (.not. associated(CS%Grid)) call LND_ERROR(FATAL,'ODA_CS ensemble horizontal grid not associated')
  if (.not. associated(CS%GV)) call LND_ERROR(FATAL,'ODA_CS ensemble vertical grid not associated')

  !! switch to global pelist
  call set_current_pelist(CS%filter_pelist)
  call LND_mesg('Setting prior')
...
subroutine get_posterior_tracer(Time, CS, h, tv, increment)
  ...
  !! switch to global pelist
  call set_current_pelist(CS%filter_pelist)
  call LND_mesg('Getting posterior')
...
!> \namespace LND_oda_driver_mod
!!
!! \section section_ODA The Ocean data assimilation (DA) and Ensemble Framework
!!
!! The DA framework implements ensemble capability in UCLAND.   Currently, this framework
...
!! interfacing UCLAND to other data assimilation packages as well.

end module LND_oda_driver_mod
