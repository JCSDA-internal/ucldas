...
# include "version_variable.h"
  character(len=16) :: mdl = "LND_unit_scaling"

  if (.not.present(US)) return

  if (associated(US)) call LND_error(FATAL, &
  ...
  if (abs(Z_power) > 300) call LND_error(FATAL, "unit_scaling_init: "//&
                 "Z_RESCALE_POWER is outside of the valid range of -300 to 300.")
  if (abs(L_power) > 300) call LND_error(FATAL, "unit_scaling_init: "//&
                 "L_RESCALE_POWER is outside of the valid range of -300 to 300.")
  if (abs(T_power) > 300) call LND_error(FATAL, "unit_scaling_init: "//&
                 "T_RESCALE_POWER is outside of the valid range of -300 to 300.")
  if (abs(R_power) > 300) call LND_error(FATAL, "unit_scaling_init: "//&
                 "R_RESCALE_POWER is outside of the valid range of -300 to 300.")
  if (abs(Q_power) > 300) call LND_error(FATAL, "unit_scaling_init: "//&
  ...
  US%RZ_to_kg_m2  = US%R_to_kg_m3 * US%Z_to_m
    ! It does not seem like US%kg_m2_to_RZ would be used enough in UCLAND to justify its existence.
  ...
  US%RL2_T2_to_Pa = US%R_to_kg_m3 * US%L_T_to_m_s**2
    ! It does not seem like US%Pa_to_RL2_T2 would be used enough in UCLAND to justify its existence.
