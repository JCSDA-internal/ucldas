...
use LND_domains,       only : pass_var, To_All, Omit_corners
use LND_diag_mediator, only : post_data, register_diag_field, safe_alloc_ptr
use LND_diag_mediator, only : diag_ctrl
use LND_debugging, only : uvchksum, hchksum
use LND_error_handler, only : LND_error, FATAL, WARNING, NOTE
...
use LND_open_boundary, only : ocean_OBC_type, OBC_SIMPLE, OBC_NONE, OBC_DIRECTION_E
use LND_open_boundary, only : OBC_DIRECTION_W, OBC_DIRECTION_N, OBC_DIRECTION_S
use LND_PointAccel, only : write_u_accel, write_v_accel, PointAccel_init
use LND_PointAccel, only : PointAccel_CS
...
use LND_variables, only : cont_diag_ptrs, accel_diag_ptrs
use LND_variables, only : ocean_internal_state
...
!> The control structure with parameters and memory for the LND_vert_friction module
type, public :: vertvisc_CS ; private
  ...
  real, pointer, dimension(:,:) :: a1_shelf_u => NULL() !< The u-lndentum coupling coefficient under
                           !! ice shelves [Z T-1 ~> m s-1]. Retained to determine stress under shelves.
  real, pointer, dimension(:,:) :: a1_shelf_v => NULL() !< The v-lndentum coupling coefficient under
                           !! ice shelves [Z T-1 ~> m s-1]. Retained to determine stress under shelves.
...
!! of lndentum.  Stress top and bottom boundary conditions are used.
!!
...
subroutine vertvisc(u, v, h, forces, visc, dt, OBC, ADp, CDp, G, GV, US, CS, &
  ...
  type(accel_diag_ptrs), intent(inout)   :: ADp    !< Accelerations in the lndentum
  ...
  if (.not.associated(CS)) call LND_error(FATAL,"LND_vert_friction(visc): "// &
  ...
  if (CS%StokesMixing) then
    ...
    if (.not. DoStokesMixing) &
      call LND_error(FATAL,"Stokes Mixing called without allocated"//&
...
!> Calculate the fraction of lndentum originally in a layer that remains
...
subroutine vertvisc_coef(u, v, h, forces, visc, dt, G, GV, US, CS, OBC)
  ...
  if (.not.associated(CS)) call LND_error(FATAL,"LND_vert_friction(coef): "// &
         "Module must be initialized before it is used.")
...
subroutine vertvisc_init(MIS, Time, G, GV, US, param_file, diag, ADp, dirs, &
  ...
  type(ocean_internal_state), &
                   target, intent(in)    :: MIS    !< The "LND Internal State", a set of pointers
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_vert_friction" ! This module's name.
  character(len=40)  :: thickness_units

  if (associated(CS)) then
    call LND_error(WARNING, "vertvisc_init called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
...
! Default, read and log parameters
  ...
  if (CS%direct_stress) then
    ...
    if (CS%Hmix_stress <= 0.0) call LND_error(FATAL, "vertvisc_init: " // &
  ...
  call get_param(param_file, mdl, "STOKES_MIXING_COMBINED", CS%StokesMixing, &
                 ...
                 Default=.false.)
  !BGR 04/04/2018{
  ! StokesMixing is required for UCLAND for some Langmuir mixing parameterization.
  ...
  !   LND_error to use, but do so at your own risk and with these points in mind.
  !}
  if (CS%StokesMixing) then
    call LND_error(FATAL, "Stokes mixing requires user intervention in the code.\n"//&
                          "  Model now exiting.  See LND_vert_friction.F90 for \n"//&
...
subroutine updateCFLtruncationValue(Time, CS, activate)
  ...
  call LND_error(NOTE, "LND_vert_friction: updateCFLtruncationValue set CFL"// &
                       " limit to "//trim(msg))
...
!> \namespace lnd_vert_friction
!! \author Robert Hallberg
!! \date April 1994 - October 2006
!!
!!  The vertical diffusion of lndentum is fully implicit.  This is
