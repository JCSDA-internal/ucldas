...
subroutine user_change_diff(h, tv, G, GV, US, CS, Kd_lay, Kd_int, T_f, S_f, Kd_int_add)
  ...
  if (.not.associated(CS)) call LND_error(FATAL,"user_set_diffusivity: "//&
  ...
  if (.not.range_OK(CS%lat_range)) then
    ...
    call LND_error(FATAL, "user_set_diffusivity: bad latitude range: \n  "//&
                    trim(mesg))
  endif
  if (.not.range_OK(CS%rho_range)) then
    write(mesg, '(4(1pe15.6))') CS%rho_range(1:4)
    call LND_error(FATAL, "user_set_diffusivity: bad density range: \n  "//&
                    trim(mesg))
  endif
