...
subroutine init_coord_zlike(CS, nk, coordinateResolution)
  ...
  if (associated(CS)) call LND_error(FATAL, "init_coord_zlike: CS already associated!")
  allocate(CS)
...
subroutine set_zlike_params(CS, min_thickness)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "set_zlike_params: CS not associated")
