module LND_surface_forcing_gfdl

! This file is part of UCLAND. See LICENSE.md for the license.

!#CTRL# use LND_controlled_forcing, only : apply_ctrl_forcing, register_ctrl_forcing_restarts
!#CTRL# use LND_controlled_forcing, only : controlled_forcing_init, controlled_forcing_end
!#CTRL# use LND_controlled_forcing, only : ctrl_forcing_CS
...
use LND_cpu_clock,        only : CLOCK_SUBCOMPONENT
use LND_diag_mediator,    only : diag_ctrl, safe_alloc_ptr, time_type
use LND_domains,          only : pass_vector, pass_var, fill_symmetric_edges
use LND_domains,          only : AGRID, BGRID_NE, CGRID_NE, To_All
use LND_domains,          only : To_North, To_East, Omit_Corners
use LND_error_handler,    only : LND_error, WARNING, FATAL, is_root_pe, LND_mesg
...
use LND_io,               only : slasher, write_version_number, LND_read_data
...
use LND_spatial_means,    only : adjust_area_mean_to_zero
...
!! which may be used to drive LND.  All fluxes are positive downward.
type, public :: surface_forcing_CS ; private
  integer :: wind_stagger       !< AGRID, BGRID_NE, or CGRID_NE (integer values
                                !! from LND_domains) to indicate the staggering of
  ...
  logical :: restore_salt       !< If true, the coupled LND driver adds a term to restore surface
                                !! salinity to a specified value.
  logical :: restore_temp       !< If true, the coupled LND driver adds a term to restore sea
...
!! and conventions that exactly conform to the use for UCLAND-based coupled models.
...
!> This subroutine translates the Ice_ocean_boundary_type into a LND
!! thermodynamic forcing type, including changes of units, sign conventions,
!! and putting the fields into arrays with LND-standard halos.
...
!! conventions, and putting the fields into arrays with LND-standard sized halos.
subroutine extract_IOB_stresses(IOB, index_bounds, Time, G, US, CS, taux, tauy, ustar, &
  ...
  integer :: wind_stagger  ! AGRID, BGRID_NE, or CGRID_NE (integers from LND_domains)
  ...
  if (associated(IOB%u_flux).neqv.associated(IOB%v_flux)) call LND_error(FATAL,"extract_IOB_stresses: "//&
            "associated(IOB%u_flux) /= associated(IOB%v_flux !!!")
  if (present(taux).neqv.present(tauy)) call LND_error(FATAL,"extract_IOB_stresses: "//&
            "present(taux) /= present(tauy) !!!")

  ! Set surface lndentum stress related fields as a function of staggering.
...
subroutine apply_force_adjustments(G, US, CS, Time, forces)
  ...
  if (overrode_x .or. overrode_y) then
    if (.not. (overrode_x .and. overrode_y)) call LND_error(FATAL,"apply_flux_adjustments: "//&
...
# include "version_variable.h"
  ...
  if (present(wind_stagger)) then
    ...
    else ; stagger = 'UNKNOWN' ; call LND_error(FATAL,"surface_forcing_init: WIND_STAGGER = "// &
    ...
    else ; call LND_error(FATAL,"surface_forcing_init: WIND_STAGGER = "// &
                          trim(stagger)//" is invalid.") ; endif
  endif
  ...
  if (CS%restore_salt) then
    ...
    if (CS%mask_srestore_marginal_seas) then
      call LND_read_data(basin_file,'basin',CS%basin_mask,G%domain, timelevel=1)
...
! work done against tides globally using OSU tidal amplitude.
  ...
  if (CS%read_TIDEAMP) then
    ...
    call LND_read_data(TideAmp_file,'tideamp',CS%TKE_tidal,G%domain,timelevel=1, scale=US%m_to_Z*US%T_to_s)
    do j=jsd, jed; do i=isd, ied
  ...
  if (CS%read_gust_2d) then
    ...
    call LND_read_data(gust_file, 'gustiness', CS%gust, G%domain, timelevel=1, &
...
! See whether sufficiently thick sea ice should be treated as rigid.
  ...
  call get_param(param_file, mdl, "ALLOW_ICEBERG_FLUX_DIAGNOSTICS", iceberg_flux_diags, &
                 ...
                 "as seen by UCLAND.", default=.false.)
  ...
  if (CS%restore_salt) then
    ...
    if (CS%mask_srestore) then ! read a 2-d file containing a mask for restoring fluxes
      ...
      call LND_read_data(flnam,'mask', CS%srestore_mask, G%domain, timelevel=1)
    endif
  endif
  ...
  if (CS%restore_temp) then
    ...
    if (CS%mask_trestore) then  ! read a 2-d file containing a mask for restoring fluxes
      ...
      call LND_read_data(flnam, 'mask', CS%trestore_mask, G%domain, timelevel=1)
    endif
  endif
...
subroutine check_mask_val_consistency(val, mask, i, j, varname, G)
  ...
  if ((mask == 0.) .and. (val /= 0.)) then
    ...
    error_message = "LND_surface_forcing: found non-zero value (="//trim(cval)//") over land "//&
                    "for variable "//trim(varname)//" at local point (i, j) = ("//trim(ci)//", "//trim(cj)//&
                    ", global point (iglo, jglo) = ("//trim(ciglo)//", "//trim(cjglo)//")"
    call LND_error(WARNING, error_message)
  endif

end subroutine

end module LND_surface_forcing_gfdl
