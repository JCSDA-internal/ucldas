...
!> The control structure holding parametes for the LND_entrain_diffusive module
...
! detail in Hallberg, Mon. Wea. Rev. 2000.
  ...
  if (.not. associated(CS)) call LND_error(FATAL, &
         "LND_entrain_diffusive: Module must be initialized before it is used.")

  if (.not.(present(Kd_Lay) .or. present(Kd_int))) call LND_error(FATAL, &
      "LND_entrain_diffusive: Either Kd_Lay or Kd_int must be present in call.")
  ...
  if ((.not.CS%bulkmixedlayer .and. .not.associated(fluxes%buoy)) .and. &
    ...
    if (is_root_pe()) call LND_error(NOTE, "Calculate_Entrainment: &
    ...
    if (is_root_pe()) call LND_error(FATAL, &
         "Either define BULKMIXEDLAYER in LND_input or use fluxes%buoy &
...
! also be returned.
  ...
  if (present(ddSlay_dE) .and. .not.present(dSlay)) call LND_error(FATAL, &
...
! constrained to be within the provided bounds.
  ...
  if (.not.CS%bulkmixedlayer) then
    call LND_error(FATAL, "determine_Ea_kb should not be called "//&
...
# include "version_variable.h"
  character(len=40)  :: mdl = "LND_entrain_diffusive" ! This module's name.

  if (associated(CS)) then
    call LND_error(WARNING, "entrain_diffusive_init called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
...
!> \namespace lnd_entrain_diffusive
!!
...
end module LND_entrain_diffusive
