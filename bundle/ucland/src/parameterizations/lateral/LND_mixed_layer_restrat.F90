...
use LND_diag_mediator, only : post_data, query_averaging_enabled, diag_ctrl
use LND_diag_mediator, only : register_diag_field, safe_alloc_ptr, time_type
use LND_diag_mediator, only : diag_update_remap_grids
use LND_domains,       only : pass_var, To_West, To_South, Omit_Corners
...
use LND_EOS,           only : calculate_density, EOS_domain
...
!> Control structure for lnd_mixed_layer_restrat
...
character(len=40)  :: mdl = "LND_mixed_layer_restrat" !< This module's name.
...
subroutine mixedlayer_restrat(h, uhtr, vhtr, tv, forces, dt, MLD, VarMix, G, GV, US, CS)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "LND_mixedlayer_restrat: "// &
...
subroutine mixedlayer_restrat_general(h, uhtr, vhtr, tv, forces, dt, MLD_in, VarMix, G, GV, US, CS)
  ...
  real :: lnd_mixrate     ! rate at which lndentum is homogenized within mixed layer [T-1 ~> s-1]
  ...
  if (.not.associated(tv%eqn_of_state)) call LND_error(FATAL, "LND_mixedlayer_restrat: "// &
         "An equation of state must be used with this module.")
  if (.not.associated(VarMix) .and. CS%front_length>0.) call LND_error(FATAL, "LND_mixedlayer_restrat: "// &
  ...
  elseif (CS%MLE_use_PBL_MLD) then
    if (.not. associated(MLD_in)) call LND_error(FATAL, "LND_mixedlayer_restrat: "// &
         "Argument MLD_in was not associated!")
    do j = js-1, je+1 ; do i = is-1, ie+1
      MLD_fast(i,j) = (CS%MLE_MLD_stretch * GV%Z_to_H) * MLD_in(i,j)
    enddo ; enddo
  else
    call LND_error(FATAL, "LND_mixedlayer_restrat: "// &
...
!$OMP                       private(rho_ml,h_vel,u_star,absf,lnd_mixrate,timescale,    &
...
subroutine mixedlayer_restrat_BML(h, uhtr, vhtr, tv, forces, dt, G, GV, US, CS)
  ...
  if (.not.use_EOS) call LND_error(FATAL, "LND_mixedlayer_restrat: "// &
         "An equation of state must be used with this module.")
...
!$OMP                       private(Rho0,h_vel,u_star,absf,lnd_mixrate,timescale,      &
...
# include "version_variable.h"
  ...
  if (.not.associated(CS)) then
    call LND_error(FATAL, "mixedlayer_restrat_init called without an associated control structure.")
  endif
...
subroutine mixedlayer_restrat_register_restarts(HI, param_file, CS, restart_CS)
  ...
  if (associated(CS)) call LND_error(FATAL, &
...
!> \namespace lnd_mixed_layer_restrat
!!
...
!! \f$ \tau \f$ is a time-scale for mixing lndentum across the mixed layer.
...
!! \todo Explain expression for lndentum mixing time-scale.
!!
...
end module LND_mixed_layer_restrat
