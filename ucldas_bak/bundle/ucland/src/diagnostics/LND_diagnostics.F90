...
module LND_diagnostics

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_coms,              only : reproducing_sum
use LND_density_integrals, only : int_density_dz
use LND_diag_mediator,     only : post_data, get_diag_time_end
use LND_diag_mediator,     only : register_diag_field, register_scalar_field
use LND_diag_mediator,     only : register_static_field, diag_register_area_ids
use LND_diag_mediator,     only : diag_ctrl, time_type, safe_alloc_ptr
use LND_diag_mediator,     only : diag_get_volume_cell_measure_dm_id
use LND_diag_mediator,     only : diag_grid_storage
use LND_diag_mediator,     only : diag_save_grids, diag_restore_grids, diag_copy_storage_to_diag
use LND_domains,           only : create_group_pass, do_group_pass, group_pass_type
use LND_domains,           only : To_North, To_East
...
use LND_spatial_means,     only : global_area_mean, global_layer_mean
use LND_spatial_means,     only : global_volume_mean, global_area_integral
use LND_tracer_registry,   only : tracer_registry_type, post_tracer_transport_diagnostics
use LND_unit_scaling,      only : unit_scale_type
use LND_variables,         only : thermo_var_ptrs, ocean_internal_state, p3d
use LND_variables,         only : accel_diag_ptrs, cont_diag_ptrs, surface
...
public LND_diagnostics_init, LND_diagnostics_end
...
!> The control structure for the LND_diagnostics module
...
subroutine calculate_diagnostic_fields(u, v, h, uh, vh, tv, ADp, CDp, p_surf, &
  ...
  type(accel_diag_ptrs),   intent(in)    :: ADp  !< structure with pointers to
                                                 !! accelerations in lndentum equation.
  ...
  if (loc(CS)==0) call LND_error(FATAL, &
...
subroutine calculate_energy_diagnostics(u, v, h, uh, vh, ADp, CDp, G, GV, US, CS)
  ...
  real, dimension(SZI_(G),SZJB_(G),SZK_(G)), &
                           intent(in)    :: vh   !< Transport through merid faces=v*h*dx,
                                                 !! [H L2 T-1 ~> m3 s-1 or kg s-1].
  type(accel_diag_ptrs),   intent(in)    :: ADp  !< Structure pointing to accelerations in lndentum equation.
...
! This subroutine calculates terms in the mechanical energy budget.
  ...
  if (associated(CS%KE)) then
    do k=1,nz ; do j=js,je ; do i=is,ie
      ...
      ! DELETE THE FOLLOWING...  Make this 0 to test the lndentum balance,
...
subroutine register_time_deriv(lb, f_ptr, deriv_ptr, CS)
  ...
  if (CS%num_time_deriv >= MAX_FIELDS_) then
    call LND_error(WARNING,"LND_diagnostics:  Attempted to register more than " // &
...
subroutine LND_diagnostics_init(MIS, ADp, CDp, Time, G, GV, US, param_file, diag, CS, tv)
  type(ocean_internal_state), intent(in)    :: MIS  !< For "LND Internal State" a set of pointers to
                                                    !! the fields and accelerations that make up the
                                                    !! ocean's internal physical state.
  type(accel_diag_ptrs),      intent(inout) :: ADp  !< Structure with pointers to lndentum equation
                                                    !! terms.
...
# include "version_variable.h"
  character(len=40)  :: mdl = "LND_diagnostics" ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "LND_diagnostics_init called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
...
end subroutine LND_diagnostics_init
...
subroutine LND_diagnostics_end(CS, ADp)
...
end subroutine LND_diagnostics_end

end module LND_diagnostics
