...
use LND_debugging, only : hchksum, qchksum, uvchksum
use LND_density_integrals, only : int_specific_vol_dp
use LND_density_integrals, only : find_depth_of_pressure_in_cell
use LND_coms, only : max_across_PEs, min_across_PEs, reproducing_sum
use LND_cpu_clock, only : cpu_clock_id, cpu_clock_begin, cpu_clock_end
use LND_cpu_clock, only :  CLOCK_ROUTINE, CLOCK_LOOP
use LND_domains, only : pass_var, pass_vector, sum_across_PEs, broadcast
use LND_domains, only : root_PE, To_All, SCALAR_PAIR, CGRID_NE, AGRID
...
use LND_file_parser, only : get_param, read_param, log_param, param_file_type
use LND_file_parser, only : log_version
use LND_get_input, only : directories
use LND_grid, only : ocean_grid_type, isPointInCell
use LND_interface_heights, only : find_eta
use LND_io, only : file_exists, field_size, LND_read_data, LND_read_vector, slasher
use LND_open_boundary, only : ocean_OBC_type, open_boundary_init, set_tracer_data
use LND_open_boundary, only : OBC_NONE, OBC_SIMPLE
use LND_open_boundary, only : open_boundary_query
use LND_open_boundary, only : set_tracer_data, initialize_segment_data
use LND_open_boundary, only : open_boundary_test_extern_h
use LND_open_boundary, only : fill_temp_salt_segments
use LND_open_boundary, only : update_OBC_segment_data
!use LND_open_boundary, only : set_3D_OBC_data
use LND_grid_initialize, only : initialize_masks, set_grid_metrics
use LND_restart, only : restore_state, determine_is_new_run, LND_restart_CS
use LND_sponge, only : set_up_sponge_field, set_up_sponge_ML_density
use LND_sponge, only : initialize_sponge, sponge_CS
use LND_ALE_sponge, only : set_up_ALE_sponge_field, initialize_ALE_sponge, ALE_sponge_CS
use LND_string_functions, only : uppercase, lowercase
use LND_time_manager, only : time_type
use LND_tracer_registry, only : tracer_registry_type
use LND_unit_scaling, only : unit_scale_type
use LND_variables, only : thermo_var_ptrs
use LND_verticalGrid, only : setVerticalGridAxes, verticalGrid_type
use LND_EOS, only : calculate_density, calculate_density_derivs, EOS_type, EOS_domain
use LND_EOS, only : convert_temp_salt_for_TEOS10
...
use LND_tracer_Z_init, only : tracer_Z_init_array, determine_temperature
use LND_ALE, only : ALE_initRegridding, ALE_CS, ALE_initThicknessToCoord
use LND_ALE, only : ALE_remap_scalar, ALE_build_grid, ALE_regrid_accelerated
use LND_ALE, only : TS_PLM_edge_values
use LND_regridding, only : regridding_CS, set_regrid_params, getCoordinateResolution
use LND_regridding, only : regridding_main
use LND_remapping, only : remapping_CS, initialize_remapping
use LND_remapping, only : remapping_core_h
use LND_horizontal_regridding, only : horiz_interp_and_extrap_tracer

implicit none ; private

#include <LND_memory.h>

public LND_initialize_state
...
character(len=40)  :: mdl = "LND_state_initialization" !< This module's name.

contains

!> Initialize temporally evolving fields, either as initial
!! conditions or by reading them from a restart (or saves) file.
subroutine LND_initialize_state(u, v, h, tv, Time, G, GV, US, PF, dirs, &
  ...
  type(directories),          intent(in)    :: dirs !< A structure containing several relevant
                                                    !! directory paths.
  type(LND_restart_CS),       pointer       :: restart_CS !< A pointer to the restart control
                                                    !! structure.
...
#include "version_variable.h"
  ...
  call callTree_enter("LND_initialize_state(), LND_state_initialization.F90")
  ...
  if (new_sim) then
    call LND_mesg("Run initialized internally.", 3)
  ...
  if (from_Z_file) then
    ...
    if (.NOT.use_temperature) call LND_error(FATAL,"LND_initialize_state : "//&
       "use_temperature must be true if INIT_LAYERS_FROM_Z_FILE is true")

    call LND_temp_salt_initialize_from_Z(h, tv, G, GV, US, PF, just_read_params=just_read)
    ...
    select case (trim(config))
       ...
       case ("coord")
         ...
         elseif (new_sim) then
           call LND_error(FATAL, "LND_initialize_state: USE_REGRIDDING must be True "//&
       ...
       case ("USER"); call user_initialize_thickness(h, G, GV, PF, &
                               just_read_params=just_read)
       case default ; call LND_error(FATAL,  "LND_initialize_state: "//&
...
!            " \t baroclinic_zone - an analytic baroclinic zone. \n"//&
  ...
  if (debug .and. new_sim) then
    call uvchksum("LND_initialize_state [uv]", u, v, G%HI, haloshift=1, scale=US%m_s_to_L_T)
  endif
  ...
  call get_param(PF, mdl, "TRIM_IC_FOR_P_SURF", trim_ic_for_p_surf, &
               ...
               do_not_log=just_read)
  if (depress_sfc .and. trim_ic_for_p_surf) call LND_error(FATAL, "LND_initialize_state: "//&
  ...
  if (debug) then
    call hchksum(h, "LND_initialize_state: h ", G%HI, haloshift=1, scale=GV%H_to_m)
    if ( use_temperature ) call hchksum(tv%T, "LND_initialize_state: T ", G%HI, haloshift=1)
    if ( use_temperature ) call hchksum(tv%S, "LND_initialize_state: S ", G%HI, haloshift=1)
    if ( use_temperature .and. debug_layers) then ; do k=1,nz
      write(mesg,'("LND_IS: T[",I2,"]")') k
      call hchksum(tv%T(:,:,k), mesg, G%HI, haloshift=1)
      write(mesg,'("LND_IS: S[",I2,"]")') k
...
!     call open_boundary_config(G, US, PF, OBC)
    ...
    elseif (.not. trim(config) == "none") then
      call LND_error(FATAL, "The open boundary conditions specified by "//&
...
! endif
  ...
  if (debug.and.associated(OBC)) then
    call hchksum(G%mask2dT, 'LND_initialize_state: mask2dT ', G%HI)
    call uvchksum('LND_initialize_state: mask2dC[uv]', G%mask2dCu,  &
                  G%mask2dCv, G%HI)
    call qchksum(G%mask2dBu, 'LND_initialize_state: mask2dBu ', G%HI)
  endif

  if (debug_OBC) call open_boundary_test_extern_h(G, GV, OBC, h)
  call callTree_leave('LND_initialize_state()')

end subroutine LND_initialize_state
...
subroutine initialize_thickness_from_file(h, G, GV, US, param_file, file_has_thickness, &
  ...
  if (.not.just_read) &
    call callTree_enter(trim(mdl)//"(), LND_state_initialization.F90")
  ...
  if ((.not.just_read) .and. (.not.file_exists(filename, G%Domain))) call LND_error(FATAL, &
  ...
  if (file_has_thickness) then
    ...
    call LND_read_data(filename, "h", h(:,:,:), G%Domain, scale=GV%m_to_H)
  else
    ...
    call LND_read_data(filename, "eta", eta(:,:,:), G%Domain, scale=US%m_to_Z)
...
subroutine adjustEtaToFitBathymetry(G, GV, US, eta, h)
  ...
  if ((contractions > 0) .and. (is_root_pe())) then
    ...
    call LND_error(WARNING, 'adjustEtaToFitBathymetry: '//mesg)
  endif
...
subroutine initialize_thickness_uniform(h, G, GV, param_file, just_read_params)
  ...
  if (G%max_depth<=0.) call LND_error(FATAL,"initialize_thickness_uniform: "// &
...
subroutine initialize_thickness_list(h, G, GV, US, param_file, just_read_params)
  ...
  e0(:) = 0.0
  call LND_read_data(filename, eta_var, e0(:), scale=US%m_to_Z)
...
subroutine initialize_thickness_search
  call LND_error(FATAL,"  LND_state_initialization.F90, initialize_thickness_search: NOT IMPLEMENTED")
...
subroutine convert_thickness(h, G, GV, US, tv)
  ...
  if (GV%Boussinesq) then
    call LND_error(FATAL,"Not yet converting thickness with Boussinesq approx.")
  else
...
subroutine depress_surface(h, G, GV, US, param_file, tv, just_read_params)
  ...
  call LND_read_data(filename, eta_srf_var, eta_sfc, G%Domain, scale=scale_factor)
...
!    endif
    if (eta_sfc(i,j) > eta(i,j,1)) then
      ...
      if (eta_sfc(i,j) - eta(i,j,nz+1) > 10.0*(eta(i,j,1) - eta(i,j,nz+1))) then
        dilate = 10.0
        call LND_error(WARNING, "Free surface height dilation attempted "//&
...
subroutine trim_for_ice(PF, G, GV, US, ALE_CSp, tv, h, just_read_params)
  ...
  call LND_read_data(filename, p_surf_var, p_surf, G%Domain, &
...
!    call LND_error(FATAL, "trim_for_ice: Does not work without ALE mode")
...
subroutine initialize_velocity_from_file(u, v, G, US, param_file, just_read_params)
  ...
  if (.not.just_read) call callTree_enter(trim(mdl)//"(), LND_state_initialization.F90")
  ...
  call LND_read_vector(filename, "u", "v", u(:,:,:), v(:,:,:), G%Domain, scale=US%m_s_to_L_T)
...
subroutine initialize_temp_salt_from_file(T, S, G, param_file, just_read_params)
  ...
  call LND_read_data(filename, temp_var, T(:,:,:), G%Domain)

  salt_filename = trim(inputdir)//trim(salt_file)
  if (.not.file_exists(salt_filename, G%Domain)) call LND_error(FATAL, &
     " initialize_temp_salt_from_file: Unable to open "//trim(salt_filename))

  call LND_read_data(salt_filename, salt_var, S(:,:,:), G%Domain)
...
subroutine initialize_temp_salt_from_profile(T, S, G, param_file, just_read_params)
  ...
  if (.not.file_exists(filename)) call LND_error(FATAL, &
     " initialize_temp_salt_from_profile: Unable to open "//trim(filename))

  ! Read the temperatures and salinities from a netcdf file.
  call LND_read_data(filename, "PTEMP", T0(:))
  call LND_read_data(filename, "SALT",  S0(:))
...
!  endif
  ...
  if (.not.file_exists(filename, G%Domain)) &
    call LND_error(FATAL, " initialize_sponges: Unable to open "//trim(filename))

  if (new_sponges .and. .not. use_ALE) &
    call LND_error(FATAL, " initialize_sponges: Newer sponges are currently unavailable in layered mode ")

  call LND_read_data(filename, "Idamp", Idamp(:,:), G%Domain, scale=US%T_to_s)

  ! Now register all of the fields which are damped in the sponge.
  ! By default, lndentum is advected vertically within the sponge, but
  ! lndentum is typically not damped within the sponge.
  ...
  if (.not. use_ALE) then
    ...
    call LND_read_data(filename, eta_var, eta(:,:,:), G%Domain, scale=US%m_to_Z)
  ...
  elseif (.not. new_sponges) then ! ALE mode
    ...
    if (siz(1) /= G%ieg-G%isg+1 .or. siz(2) /= G%jeg-G%jsg+1) &
      call LND_error(FATAL,"initialize_sponge_file: Array size mismatch for sponge data.")
...
!   if (siz(4) > 1) ALE_CSp%time_dependent_target = .true.
  ...
  ! sponge. By default, lndentum is advected vertically within the
  ! sponge, but lndentum is typically not damped within the sponge.
  ...
  if ( GV%nkml>0 .and. .not. new_sponges) then
    ...
    call LND_read_data(filename, potemp_var, tmp(:,:,:), G%Domain)
    call LND_read_data(filename, salin_var, tmp2(:,:,:), G%Domain)
  ...
  if ( use_temperature .and. .not. new_sponges) then
    ...
    call LND_read_data(filename, salin_var, tmp(:,:,:), G%Domain)
...
subroutine LND_temp_salt_initialize_from_Z(h, tv, G, GV, US, PF, just_read_params)
  type(ocean_grid_type),   intent(inout) :: G    !< The ocean's grid structure
  real, dimension(SZI_(G),SZJ_(G),SZK_(G)), &
  ...
  ! This include declares and sets the variable "version".
# include "version_variable.h"
  character(len=40)  :: mdl = "LND_initialize_layers_from_Z" ! This module's name.
  ...
  if (use_ice_shelf) then
    if (.not.file_exists(shelf_file, G%Domain)) call LND_error(FATAL, &
      "LND_temp_salt_initialize_from_Z: Unable to open shelf file "//trim(shelf_file))

    call LND_read_data(shelf_file, trim(area_varname), area_shelf_h, G%Domain, scale=US%m_to_L**2)
...
end subroutine LND_temp_salt_initialize_from_Z
...
subroutine LND_state_init_tests(G, GV, US, tv)
  type(ocean_grid_type),     intent(inout) :: G    !< The ocean's grid structure.
  type(verticalGrid_type),   intent(in)    :: GV   !< The ocean's vertical grid structure.
  type(unit_scale_type),     intent(in)    :: US   !< A dimensional unit scaling type
  type(thermo_var_ptrs),     intent(in)    :: tv   !< Thermodynamics structure.
...
end subroutine LND_state_init_tests

end module LND_state_initialization
