...
use fms_mod, only : fms_end, LND_infra_init => fms_init
...
public :: PE_here, root_PE, num_PEs, LND_infra_init, LND_infra_end
...
function reproducing_EFP_sum_2d(array, isr, ier, jsr, jer, overflow_check, err, only_on_PE) result(EFP_sum)
  ...
  if (num_PEs() > max_count_prec) call LND_error(FATAL, &
  ...
  if (present(isr)) then
    if (isr < is) call LND_error(FATAL, "Value of isr too small in reproducing_EFP_sum_2d.")
    is = isr
  endif
  if (present(ier)) then
    if (ier > ie) call LND_error(FATAL, "Value of ier too large in reproducing_EFP_sum_2d.")
    ie = ier
  endif
  if (present(jsr)) then
    if (jsr < js) call LND_error(FATAL, "Value of jsr too small in reproducing_EFP_sum_2d.")
    js = jsr
  endif
  if (present(jer)) then
    if (jer > je) call LND_error(FATAL, "Value of jer too large in reproducing_EFP_sum_2d.")
    je = jer
  endif
  ...
  if (present(err)) then
    ...
    if (NaN_error) then
      call LND_error(FATAL, "NaN in input field of reproducing_EFP_sum(_2d).")
    endif
    if (abs(max_mag_term) >= prec_error*pr(1)) then
      write(mesg, '(ES13.5)') max_mag_term
      call LND_error(FATAL,"Overflow in reproducing_EFP_sum(_2d) conversion of "//trim(mesg))
    endif
    if (overflow_error) then
      call LND_error(FATAL, "Overflow in reproducing_EFP_sum(_2d).")
    endif
  endif
...
function reproducing_sum_2d(array, isr, ier, jsr, jer, EFP_sum, reproducing, &
  ...
  is = 1 ; ie = size(array,1) ; js = 1 ; je = size(array,2 )
  if (present(isr)) then
    if (isr < is) call LND_error(FATAL, "Value of isr too small in reproducing_sum_2d.")
    is = isr
  endif
  if (present(ier)) then
    if (ier > ie) call LND_error(FATAL, "Value of ier too large in reproducing_sum_2d.")
    ie = ier
  endif
  if (present(jsr)) then
    if (jsr < js) call LND_error(FATAL, "Value of jsr too small in reproducing_sum_2d.")
    js = jsr
  endif
  if (present(jer)) then
    if (jer > je) call LND_error(FATAL, "Value of jer too large in reproducing_sum_2d.")
    je = jer
  endif
    ...
    if (debug .or. present(EFP_sum)) then
      ...
      if (overflow_error) then
        if (present(err)) then
          ...
          call LND_error(FATAL,"Repro_sum_2d: Overflow in real_to_ints conversion of "//trim(mesg))
        endif
      endif
    endif
...
function reproducing_sum_3d(array, isr, ier, jsr, jer, sums, EFP_sum, EFP_lay_sums, err, only_on_PE) &
  ...
  if (present(isr)) then
    if (isr < is) call LND_error(FATAL, "Value of isr too small in reproducing_sum(_3d).")
    is = isr
  endif
  if (present(ier)) then
    if (ier > ie) call LND_error(FATAL, "Value of ier too large in reproducing_sum(_3d).")
    ie = ier
  endif
  if (present(jsr)) then
    if (jsr < js) call LND_error(FATAL, "Value of jsr too small in reproducing_sum(_3d).")
    js = jsr
  endif
  if (present(jer)) then
    if (jer > je) call LND_error(FATAL, "Value of jer too large in reproducing_sum(_3d).")
    je = jer
  endif
  ...
  if (present(sums) .or. present(EFP_lay_sums)) then
    if (present(sums)) then ; if (size(sums) < ke) then
      call LND_error(FATAL, "Sums is smaller than the vertical extent of array in reproducing_sum(_3d).")
    endif ; endif
    ...
    if (present(err)) then
      ...
      if (NaN_error) call LND_error(FATAL, "NaN in input field of reproducing_sum(_3d).")
      if (abs(max_mag_term) >= prec_error*pr(1)) then
        write(mesg, '(ES13.5)') max_mag_term
        call LND_error(FATAL,"Overflow in reproducing_sum(_3d) conversion of "//trim(mesg))
      endif
      if (overflow_error) call LND_error(FATAL, "Overflow in reproducing_sum(_3d).")
    endif
...
function real_to_ints(r, prec_error, overflow) result(ints)
  ...
  elseif (.not.(rs < prec_err*pr(1))) then
    write(mesg, '(ES13.5)') r
    call LND_error(FATAL,"Overflow in real_to_ints conversion of "//trim(mesg))
  endif
...
function real_to_EFP(val, overflow)
  ...
  if (present(overflow)) then
    ...
    if (over) then
      ...
      call LND_error(FATAL,"Overflow in real_to_EFP conversion of "//trim(mesg))
    endif
  endif
...
subroutine EFP_list_sum_across_PEs(EFPs, nval, errors)
  ...
  if (error_found .and. .not.(present(errors))) then
    call LND_error(FATAL, "Overflow in EFP_list_sum_across_PEs.")
  endif
...
subroutine EFP_val_sum_across_PEs(EFP, error)
  ...
  if (error_found .and. .not.(present(error))) then
    call LND_error(FATAL, "Overflow in EFP_val_sum_across_PEs.")
  endif
