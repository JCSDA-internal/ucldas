...
module LND_dynamics_unsplit_RK2
...
!*                                                                     *
!*    The subroutine step_LND_dyn_unsplit_RK2 actually does the time   *
...
use LND_variables, only : ocean_internal_state, accel_diag_ptrs, cont_diag_ptrs
...
use LND_diag_mediator, only : set_diag_mediator_grid, diag_ctrl
...
!> LND_dynamics_unsplit_RK2 module control structure
type, public :: LND_dyn_unsplit_RK2_CS ; private
...
end type LND_dyn_unsplit_RK2_CS


public step_LND_dyn_unsplit_RK2, register_restarts_dyn_unsplit_RK2
public initialize_dyn_unsplit_RK2, end_dyn_unsplit_RK2

!>@{ CPU time clock IDs
integer :: id_clock_Cor, id_clock_pres, id_clock_vertvisc
integer :: id_clock_horvisc, id_clock_continuity, id_clock_lnd_update
...
!> Step the UCLAND dynamics using an unsplit quasi-2nd order Runge-Kutta scheme
subroutine step_LND_dyn_unsplit_RK2(u_in, v_in, h_in, tv, visc, Time_local, dt, forces, &
  ...
  type(LND_dyn_unsplit_RK2_CS),      pointer       :: CS      !< The control structure set up by
...
! all of the fields except h.  h is stepped separately.
  ...
  if (CS%debug) then
    call LND_state_chksum("Start Predictor ", u_in, v_in, h_in, uh, vh, G, GV, US)
  endif
...
! Accumulate mass flux for tracer transport
  ...
  if (CS%debug) then
    call LND_state_chksum("Corrector", u_in, v_in, h_in, uh, vh, G, GV, US)
...
end subroutine step_LND_dyn_unsplit_RK2
...
! This is where a control structure that is specific to this module would be allocated.
  if (associated(CS)) then
    call LND_error(WARNING, "register_restarts_dyn_unsplit_RK2 called with an associated "// &
                             "control structure.")
    return
  endif
  allocate(CS)
...
subroutine initialize_dyn_unsplit_RK2(u, v, h, Time, G, GV, US, param_file, diag, CS, &
  ...
  type(LND_dyn_unsplit_RK2_CS),              pointer       :: CS   !< The control structure set up
                                                                   !! by initialize_dyn_unsplit_RK2.
  type(LND_restart_CS),                      pointer       :: restart_CS !< A pointer to the restart
                                                                         !! control structure.
  type(accel_diag_ptrs),             target, intent(inout) :: Accel_diag !< A set of pointers to the
                                      !! various accelerations in the lndentum equations, which can
  ...
  ! Local variables
  character(len=40) :: mdl = "LND_dynamics_unsplit_RK2" ! This module's name.
  character(len=48) :: thickness_units, flux_units
...
# include "version_variable.h"
  ...
  if (CS%module_is_initialized) then
    call LND_error(WARNING, "initialize_dyn_unsplit_RK2 called with a control "// &
...
subroutine end_dyn_unsplit_RK2(CS)
  type(LND_dyn_unsplit_RK2_CS), pointer :: CS !< dyn_unsplit_RK2 control structure that
...
end module LND_dynamics_unsplit_RK2
