...
module LND_internal_tides

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_debugging,     only : is_NaN
use LND_diag_mediator, only : post_data, query_averaging_enabled, diag_axis_init
use LND_diag_mediator, only : register_diag_field, diag_ctrl, safe_alloc_ptr
use LND_diag_mediator, only : axes_grp, define_axes_group
use LND_domains, only       : AGRID, To_South, To_West, To_All
use LND_domains, only       : create_group_pass, do_group_pass, pass_var
use LND_domains, only       : group_pass_type, start_group_pass, complete_group_pass
...
use LND_io, only            : slasher, vardesc, LND_read_data, file_exists
use LND_restart, only       : register_restart_field, LND_restart_CS, restart_init, save_restart
use LND_spatial_means, only : global_area_mean
use LND_time_manager, only  : time_type, time_type_to_real, operator(+), operator(/), operator(-)
...
use LND_wave_structure, only: wave_structure_init, wave_structure, wave_structure_CS
...
!> This control structure has parameters for the LND_internal_tides module
...
subroutine propagate_int_tide(h, tv, cn, TKE_itidal_input, vel_btTide, Nb, dt, &
  ...
  elseif (CS%energized_angle <= CS%nAngle) then
    ...
    call LND_error(WARNING, "Internal tide energy is being put into a angular "//&
    ...
    do j=js,je ; do i=is,ie
      if (CS%En(i,j,a,fr,m)<0.0) then
        ...
        write(mesg,*) 'After first refraction: En<0.0 at ig=', id_g, ', jg=', jd_g, &
                      'En=',CS%En(i,j,a,fr,m)
        call LND_error(WARNING, "propagate_int_tide: "//trim(mesg))
        CS%En(i,j,a,fr,m) = 0.0
!        call LND_error(FATAL, "propagate_int_tide: stopped due to negative energy.")
      endif
    enddo ; enddo
  enddo ; enddo ; enddo
    ...
    do j=js,je ; do i=is,ie
      if (CS%En(i,j,a,fr,m)<0.0) then
        ...
        write(mesg,*) 'After leak loss: En<0.0 at ig=', id_g, ', jg=', jd_g, &
                      'En=',CS%En(i,j,a,fr,m)
        call LND_error(WARNING, "propagate_int_tide: "//trim(mesg), all_print=.true.)
        CS%En(i,j,a,fr,m) = 0.0
!       call LND_error(FATAL, "propagate_int_tide: stopped due to negative energy.")
      endif
    enddo ; enddo
          ...
          if (Fr2_max > 1.0) then
            ...
            do a=1,CS%nAngle
              ...
              if (abs(En_new - En_check) > 1e-10*US%kg_m3_to_R*US%m_to_Z**3*US%T_to_s**2) then
                call LND_error(WARNING, "LND_internal_tides: something is wrong with Fr-breaking.", &
                               all_print=.true.)
                write(mesg,*) "En_new=", En_new , "En_check=", En_check
                call LND_error(WARNING, "LND_internal_tides: "//trim(mesg), all_print=.true.)
              endif
            enddo
            ! Check (for debugging)
            ...
            if (abs(TKE_Froude_loss_check - TKE_Froude_loss_tot) > 1e-10) then
              call LND_error(WARNING, "LND_internal_tides: something is wrong with Fr energy update.", &
                             all_print=.true.)
...
subroutine sum_En(G, CS, En, label)
  ...
  !  call LND_error(FATAL, "Run stopped due to excessive internal tide energy change.")
  !endif
...
subroutine itidal_lowmode_loss(G, US, CS, Nb, Ub, En, TKE_loss_fixed, TKE_loss, dt, full_halos)
  ...
  do j=js,je ; do i=is,ie ; do m=1,CS%nMode ; do fr=1,CS%nFreq
    ...
    !    else
    !      call LND_error(WARNING, "itidal_lowmode_loss: energy loss greater than avalable, "// &
...
subroutine refract(En, cn, freq, dt, G, US, NAngle, use_PPMang)
  ...
  ! Do the refraction.
    ...
    do A=asd,aed ; do i=is,ie
      ...
      if (abs(CFL_ang(i,j,A)) > 1.0) then
        call LND_error(WARNING, "refract: CFL exceeds 1.", .true.)
  ...
  ! Update and copy back to En.
    do a=1,na ; do i=is,ie
      ...
      !  call LND_error(FATAL, "refract: OutFlux>Available")
      !endif
...
subroutine propagate_corner_spread(En, energized_wedge, NAngle, speed, dt, G, CS, LB)
  ...
  do j=jsh,jeh ; do i=ish,ieh
    do m=1,int(Nsubrays)
      ...
      if (CFL_max > 1.0) then
        call LND_error(WARNING, "propagate_corner_spread: CFL exceeds 1.", .true.)
      endif
...
subroutine propagate_x(En, speed_x, Cgx_av, dCgx, dt, G, US, Nangle, CS, LB)
  ...
  do a=1,Nangle ; do j=jsh,jeh ; do i=ish,ieh
    ...
    !    call LND_error(FATAL, "propagate_x: OutFlux>Available")
...
subroutine propagate_y(En, speed_y, Cgy_av, dCgy, dt, G, US, Nangle, CS, LB)
  ...
  do a=1,Nangle
    ...
    do j=jsh,jeh ; do i=ish,ieh
      ...
      !  call LND_error(WARNING, "propagate_y: OutFlux>Available prior to reflection", .true.)
  ...
  do a=1,Nangle ; do j=jsh,jeh ; do i=ish,ieh
    !  if ((En(i,j,a) + G%IareaT(i,j)*(Fdt_m(i,j,a) + Fdt_p(i,j,a))) < 0.0) & ! for debugging
    !    call LND_error(FATAL, "propagate_y: OutFlux>Available", .true.)
    En(i,j,a) = En(i,j,a) + G%IareaT(i,j)*(Fdt_m(i,j,a) + Fdt_p(i,j,a))
  enddo ; enddo ; enddo
...
subroutine reflect(En, NAngle, CS, G, LB)
  ...
  !     call LND_error(FATAL, "reflect: Energy detected out of bounds: "//trim(mesg), .true.)
  !   endif
  ! enddo ; enddo ; enddo
          ...
          if (En(i,j,a) > 0) then
            ...
            if (sin(angle_i(a) - angle_wall) >= 0.0) then
              ...
              if (.not. pref_cell(i+ios,j+jos)) then
                ...
                call LND_error(FATAL, "teleport: no receptive ocean cell at "//trim(mesg), .true.)
              endif
...
subroutine PPM_reconstruction_x(h_in, h_l, h_r, G, LB, simple_2nd)
  ...
  if ((isl-stencil < G%isd) .or. (iel+stencil > G%ied)) then
    write(mesg,'("In LND_internal_tides, PPM_reconstruction_x called with a ", &
...
subroutine PPM_reconstruction_y(h_in, h_l, h_r, G, LB, simple_2nd)
  ...
  if ((isl < G%isd) .or. (iel > G%ied)) then
    write(mesg,'("In LND_internal_tides, PPM_reconstruction_y called with a ", &
               & "x-halo that needs to be increased by ",i2,".")') &
...
!     call LND_error(WARNING, "register_int_tide_restarts called "//&
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_internal_tides" ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "internal_tides_init called "//&
                             "with an associated control structure.")
    return
  else
    allocate(CS)
  ...
  if (.not.use_temperature) call LND_error(FATAL, &
  ...
  if (use_int_tides) then
    if ((num_freq <= 0) .and. (num_mode <= 0) .and. (num_angle <= 0)) then
      call LND_error(WARNING, "Internal tides were enabled, but the number "//&
             "of requested frequencies, modes and angles were not all positive.")
      return
    endif
  else
    if ((num_freq > 0) .and. (num_mode > 0) .and. (num_angle > 0)) then
      call LND_error(WARNING, "Internal tides were not enabled, even though "//&
  ...
  if (CS%NFreq /= num_freq) call LND_error(FATAL, "Internal_tides_init: "//&
      "Inconsistent number of frequencies.")
  if (CS%NAngle /= num_angle) call LND_error(FATAL, "Internal_tides_init: "//&
      "Inconsistent number of angles.")
  if (CS%NMode /= num_mode) call LND_error(FATAL, "Internal_tides_init: "//&
      "Inconsistent number of modes.")
  if (4*(num_angle/4) /= num_angle) call LND_error(FATAL, &
  ...
  call LND_read_data(filename, 'h2', h2, G%domain, timelevel=1, scale=US%m_to_Z)
  ...
  if (file_exists(filename, G%domain)) then
    ...
    call LND_read_data(filename, 'refl_angle', CS%refl_angle, &
                       G%domain, timelevel=1)
  else
    if (trim(refl_angle_file) /= '' ) call LND_error(FATAL, &
  ...
  if (file_exists(filename, G%domain)) then
    ...
    call LND_read_data(filename, 'refl_pref', CS%refl_pref, G%domain, timelevel=1)
  else
    if (trim(refl_pref_file) /= '' ) call LND_error(FATAL, &
  ...
  if (file_exists(filename, G%domain)) then
    ...
    call LND_read_data(filename, 'refl_dbl', ridge_temp, G%domain, timelevel=1)
  else
    if (trim(refl_dbl_file) /= '' ) call LND_error(FATAL, &
  ...
  ! This should be done in LND_initialize_topography subroutine
  ! defined in LND_fixed_initialization.F90 (BDM)
  ...
  !call LND_read_data(filename, 'land_mask', G%mask2dCu, G%domain, timelevel=1)
  !call LND_read_data(filename, 'land_mask', G%mask2dCv, G%domain, timelevel=1)
  !call LND_read_data(filename, 'land_mask', G%mask2dT, G%domain, timelevel=1)
  ...
  !G%dy_Cu(:,:) = 0.0
  !call LND_read_data(filename, 'dy_Cu', G%dy_Cu, G%domain, timelevel=1, scale=US%m_to_L)
  ...
  !G%dx_Cv(:,:) = 0.0
  !call LND_read_data(filename, 'dx_Cv', G%dx_Cv, G%domain, timelevel=1, scale=US%m_to_L)
