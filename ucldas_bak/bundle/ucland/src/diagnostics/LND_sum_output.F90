...
use LND_coms, only : sum_across_PEs, PE_here, root_PE, num_PEs, max_across_PEs
use LND_coms, only : reproducing_sum, reproducing_sum_EFP, EFP_to_real, real_to_EFP
use LND_coms, only : EFP_type, operator(+), operator(-), assignment(=), EFP_sum_across_PEs
use LND_error_handler, only : LND_error, FATAL, WARNING, is_root_pe, LND_mesg
...
use LND_io, only : create_file, fieldtype, flush_file, open_file, reopen_file
use LND_io, only : file_exists, slasher, vardesc, var_desc, write_field, get_filename_appendix
use LND_io, only : APPEND_FILE, ASCII_FILE, SINGLE_FILE, WRITEONLY_FILE
use LND_open_boundary, only : ocean_OBC_type, OBC_segment_type
use LND_open_boundary, only : OBC_DIRECTION_E, OBC_DIRECTION_W, OBC_DIRECTION_N, OBC_DIRECTION_S
use LND_time_manager, only : time_type, get_time, get_date, set_time, operator(>)
use LND_time_manager, only : operator(+), operator(-), operator(*), operator(/)
use LND_time_manager, only : operator(/=), operator(<=), operator(>=), operator(<)
use LND_time_manager, only : get_calendar_type, time_type_to_real, NO_CALENDAR
use LND_tracer_flow_control, only : tracer_flow_control_CS, call_tracer_stocks
...
public write_energy, accumulate_net_input, LND_sum_output_init
...
!> The control structure for the LND_sum_output module
...
!> LND_sum_output_init initializes the parameters and settings for the LND_sum_output module.
subroutine LND_sum_output_init(G, US, param_file, directory, ntrnc, &
                               Input_start_time, CS)
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_sum_output" ! This module's name.
  character(len=200) :: energyfile  ! The name of the energy file.
  character(len=32) :: filename_appendix = '' !fms appendix to filename for ensemble runs

  if (associated(CS)) then
    call LND_error(WARNING, "LND_sum_output_init called with associated control structure.")
    return
  endif
  allocate(CS)
...
end subroutine LND_sum_output_init

!> LND_sum_output_end deallocates memory used by the LND_sum_output module.
subroutine LND_sum_output_end(CS)
  type(Sum_output_CS), pointer :: CS  !< The control structure returned by a
                                      !! previous call to LND_sum_output_init.
  if (associated(CS)) then
    if (CS%do_APE_calc) then
...
end subroutine LND_sum_output_end
...
! Calculate the maximum CFL numbers.
  ...
  if (date_stamped) then
    write(date_str,'("LND Date",i7,2("/",i2.2)," ",i2.2,2(":",i2.2))') &
  ...
  if (CS%ntrunc>CS%maxtrunc) then
    call LND_error(FATAL, "write_energy : Ocean velocity has been truncated too many times.")
  endif
  CS%ntrunc = 0
...
subroutine depth_list_setup(G, US, CS)
  ...
  if (CS%read_depth_list) then
    if (file_exists(CS%depth_list_file)) then
      ...
      if (is_root_pe()) call LND_error(WARNING, "depth_list_setup: "// &
...
subroutine create_depth_list(G, CS)
  type(ocean_grid_type), intent(in) :: G  !< The ocean's grid structure.
  type(Sum_output_CS),   pointer    :: CS !< The control structure set up in LND_sum_output_init,
...
subroutine write_depth_list(G, US, CS, filename, list_size)
  ...
  if (status /= NF90_NOERR) then
    call LND_error(WARNING, filename//trim(NF90_STRERROR(status)))
    return
  endif

  status = NF90_DEF_DIM(ncid, "list", list_size, dimid(1))
  if (status /= NF90_NOERR) call LND_error(WARNING, &
...
subroutine read_depth_list(G, US, CS, filename)
  ...
  mdl = "LND_sum_output read_depth_list:"

  status = NF90_OPEN(filename, NF90_NOWRITE, ncid)
  if (status /= NF90_NOERR) then
    call LND_error(FATAL,mdl//" Difficulties opening "//trim(filename)// &
  ...
  if (any([depth_attr_status, area_attr_status] == NF90_ENOTATT)) then
    ...
    if (CS%require_depth_list_chksum) then
      call LND_error(FATAL, trim(var_msg) // " aborting.")
    elseif (CS%update_depth_list_chksum) then
      call LND_error(WARNING, trim(var_msg) // " updating file.")
      call create_depth_list(G, CS)
      call write_depth_list(G, US, CS, CS%depth_list_file, CS%list_size+1)
      return
    else
  ...
  if (status /= NF90_NOERR) call LND_error(FATAL,mdl// &
        " Difficulties finding variable "//trim(var_msg)//&
        trim(NF90_STRERROR(status)))

  status = NF90_INQUIRE_VARIABLE(ncid, varid, ndims=ndim, dimids=var_dim_ids)
  if (status /= NF90_NOERR) then
    call LND_ERROR(FATAL,mdl//" cannot inquire about "//trim(var_msg)//&
        trim(NF90_STRERROR(status)))
  elseif (ndim > 1) then
    call LND_ERROR(FATAL,mdl//" "//trim(var_msg)//&
  ...
  status = NF90_CLOSE(ncid)
  if (status /= NF90_NOERR) call LND_error(WARNING, mdl// &
