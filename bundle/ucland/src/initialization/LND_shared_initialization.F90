...
module LND_shared_initialization

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_coms, only : max_across_PEs, reproducing_sum
...
use LND_file_parser, only : get_param, log_param, param_file_type, log_version
use LND_io, only : close_file, create_file, fieldtype, file_exists
use LND_io, only : LND_read_data, LND_read_vector, SINGLE_FILE, MULTIPLE
use LND_io, only : slasher, vardesc, write_field, var_desc
...
public LND_shared_init_init
public LND_initialize_rotation, LND_calculate_grad_Coriolis
...
! -----------------------------------------------------------------------------
!> LND_shared_init_init just writes the code version.
subroutine LND_shared_init_init(PF)
  type(param_file_type),   intent(in)    :: PF   !< A structure indicating the open file
                                                 !! to parse for model parameter values.

  character(len=40)  :: mdl = "LND_shared_initialization" ! This module's name.
...
end subroutine LND_shared_init_init
! -----------------------------------------------------------------------------

!> LND_initialize_rotation makes the appropriate call to set up the Coriolis parameter.
subroutine LND_initialize_rotation(f, G, PF, US)
...
! Set up the Coriolis parameter, f, either analytically or from file.
  character(len=40)  :: mdl = "LND_initialize_rotation" ! This subroutine's name.
  character(len=200) :: config

  call callTree_enter(trim(mdl)//"(), LND_shared_initialization.F90")
  ...
  select case (trim(config))
   ...
   !case ("nonrotating") ! Note from AJA: Missing case?
    case default ; call LND_error(FATAL,"LND_initialize: "// &
      "Unrecognized rotation setup "//trim(config))
  end select
  call callTree_leave(trim(mdl)//'()')
end subroutine LND_initialize_rotation

!> Calculates the components of grad f (Coriolis parameter)
subroutine LND_calculate_grad_Coriolis(dF_dx, dF_dy, G, US)
...
end subroutine LND_calculate_grad_Coriolis
...
subroutine initialize_topography_from_file(D, G, param_file, US)
  ...
  call LND_read_data(filename, trim(topo_varname), D, G%Domain, scale=m_to_Z)
...
subroutine apply_topography_edits_from_file(D, G, param_file, US)
  ...
  if (.not.file_exists(topo_edits_file, G%Domain)) call LND_error(FATAL, &
  ...
  if (ncstatus /= NF90_NOERR) call LND_error(FATAL, 'apply_topography_edits_from_file: '//&
  ...
  if (i /= G%ieg) call LND_error(FATAL, 'apply_topography_edits_from_file: '//&
  ...
  if (j /= G%jeg) call LND_error(FATAL, 'apply_topography_edits_from_file: '//&
  ...
  do n = 1, n_edits
    ...
    if (i>=G%isc .and. i<=G%iec .and. j>=G%jsc .and. j<=G%jec) then
      if (new_depth(n)/=0.) then
        ...
        call LND_error(FATAL, ' apply_topography_edits_from_file: '//&
...
! real :: south_lat, west_lon, len_lon, len_lat, Rad_earth
  ...
  call callTree_enter(trim(mdl)//"(), LND_shared_initialization.F90")
  call LND_mesg("  LND_shared_initialization.F90, initialize_topography_named: "//&
  ...
  if (max_depth<=0.) call LND_error(FATAL,"initialize_topography_named: "// &
...
!                  "The radius of the Earth.", units="m", default=6.378e6)
  ...
  elseif (trim(topog_config) == "halfpipe") then
    ...
    call LND_error(FATAL,"initialize_topography_named: "// &
...
! This subroutine sets up the Coriolis parameter for a beta-plane
  ...
  select case (axis_units(1:1))
    ...
    case ("m"); y_scl = 1.
    case ("c"); y_scl = 1.E-2
    case default ; call LND_error(FATAL, &
...
subroutine reset_face_lengths_named(G, param_file, name, US)
  ...
  select case ( trim(name) )
    ...
    case default ; call LND_error(FATAL, "reset_face_lengths_named: "//&
  ...
  do j=jsd,jed ; do I=IsdB,IedB
    if (L_to_m*G%dy_Cu(I,j) > L_to_m*G%dyCu(I,j)) then
      ...
      call LND_error(FATAL,"reset_face_lengths_named "//mesg)
    endif
...
subroutine reset_face_lengths_file(G, param_file, US)
  ...
  if (is_root_pe()) then ; if (.not.file_exists(filename)) &
    call LND_error(FATAL," reset_face_lengths_file: Unable to open "//&
                           trim(filename))
  endif

  call LND_read_vector(filename, "dyCuo", "dxCvo", G%dy_Cu, G%dx_Cv, G%Domain, scale=m_to_L)
  ...
  do j=jsd,jed ; do I=IsdB,IedB
    if (L_to_m*G%dy_Cu(I,j) > L_to_m*G%dyCu(I,j)) then
      write(mesg,'("dy_Cu of ",ES11.4," exceeds unrestricted width of ",ES11.4,&
                   ...
                   L_to_m*G%dy_Cu(I,j), L_to_m*G%dyCu(I,j), L_to_m*G%dy_Cu(I,j)-L_to_m*G%dyCu(I,j), &
                   G%geoLonCu(I,j), G%geoLatCu(I,j)
      call LND_error(FATAL,"reset_face_lengths_file "//mesg)
    endif
    G%areaCu(I,j) = G%dxCu(I,j) * G%dy_Cu(I,j)
    G%IareaCu(I,j) = 0.0
    if (G%areaCu(I,j) > 0.0) G%IareaCu(I,j) = G%mask2dCu(I,j) / (G%areaCu(I,j))
  ...
  if (num_lines > 0) then
    ...
    if (is_root_pe()) then
      ...
      if (nl_read /= num_lines) &
        call LND_error(FATAL, 'reset_face_lengths_list : Found different '// &
        ...
        if (is_root_PE()) then
          if (check_360) then
            if ((abs(u_lon(1,u_pt)) > len_lon) .or. (abs(u_lon(2,u_pt)) > len_lon)) &
              call LND_error(WARNING, "reset_face_lengths_list : Out-of-bounds "//&
          ...
          if (u_lat(1,u_pt) > u_lat(2,u_pt)) &
            call LND_error(WARNING, "reset_face_lengths_list : Out-of-order "//&
          ...
          if (u_lon(1,u_pt) > u_lon(2,u_pt)) &
            call LND_error(WARNING, "reset_face_lengths_list : Out-of-order "//&
               ...
               trim(filename))
          if (u_width(u_pt) < 0.0) &
            call LND_error(WARNING, "reset_face_lengths_list : Negative "//&
...
subroutine read_face_length_list(iounit, filename, num_lines, lines)
  ...
  do while(.true.)
    ...
    if (found_u .and. found_v) call LND_error(FATAL, &
...
9 call LND_error(FATAL, "read_face_length_list : "//&
...
subroutine compute_global_grid_integrals(G, US)
  ...
  if (G%areaT_global == 0.0) &
    call LND_error(FATAL, "compute_global_grid_integrals: "//&
...
!   vardesc is a structure defined in LND_io.F90.  The elements of
...
! (6) the variable's units
  ...
  ! had reduced dimensions, and that they are no longer needed in UCLAND. -RWH
...
end module LND_shared_initialization
