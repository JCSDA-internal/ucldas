!> This routine drives the diabatic/dianeutral physics for LND
module LND_diabatic_driver

! This file is part of UCLAND. See LICENSE.md for the license.


use LND_bulk_mixed_layer,    only : bulkmixedlayer, bulkmixedlayer_init, bulkmixedlayer_CS
use LND_debugging,           only : hchksum
use LND_checksum_packages,   only : LND_state_chksum, LND_state_stats
...
use LND_CVMix_shear,         only : CVMix_shear_is_used
use LND_CVMix_ddiff,         only : CVMix_ddiff_is_used
use LND_diabatic_aux,        only : diabatic_aux_init, diabatic_aux_end, diabatic_aux_CS
use LND_diabatic_aux,        only : make_frazil, adjust_salt, differential_diffuse_T_S, triDiagTS
use LND_diabatic_aux,        only : find_uv_at_h, diagnoseMLDbyDensityDifference, applyBoundaryFluxesInOut
use LND_diabatic_aux,        only : set_pen_shortwave
use LND_diag_mediator,       only : post_data, register_diag_field, safe_alloc_ptr
use LND_diag_mediator,       only : diag_ctrl, time_type, diag_update_remap_grids
use LND_diag_mediator,       only : diag_ctrl, query_averaging_enabled, enable_averages, disable_averaging
use LND_diag_mediator,       only : diag_grid_storage, diag_grid_storage_init, diag_grid_storage_end
use LND_diag_mediator,       only : diag_copy_diag_to_storage, diag_copy_storage_to_diag
use LND_diag_mediator,       only : diag_save_grids, diag_restore_grids
use LND_diapyc_energy_req,   only : diapyc_energy_req_init, diapyc_energy_req_end
use LND_diapyc_energy_req,   only : diapyc_energy_req_calc, diapyc_energy_req_test, diapyc_energy_req_CS
use LND_CVMix_conv,          only : CVMix_conv_init, CVMix_conv_cs
use LND_CVMix_conv,          only : CVMix_conv_end, calculate_CVMix_conv
use LND_domains,             only : pass_var, To_West, To_South, To_All, Omit_Corners
use LND_domains,             only : create_group_pass, do_group_pass, group_pass_type
use LND_tidal_mixing,        only : tidal_mixing_init, tidal_mixing_cs
use LND_tidal_mixing,        only : tidal_mixing_end
use LND_energetic_PBL,       only : energetic_PBL, energetic_PBL_init
use LND_energetic_PBL,       only : energetic_PBL_end, energetic_PBL_CS
use LND_energetic_PBL,       only : energetic_PBL_get_MLD
use LND_entrain_diffusive,   only : entrainment_diffusive, entrain_diffusive_init
use LND_entrain_diffusive,   only : entrain_diffusive_end, entrain_diffusive_CS
use LND_EOS,                 only : calculate_density, calculate_TFreeze, EOS_domain
use LND_error_handler,       only : LND_error, FATAL, WARNING, callTree_showQuery,LND_mesg
use LND_error_handler,       only : callTree_enter, callTree_leave, callTree_waypoint
use LND_file_parser,         only : get_param, log_version, param_file_type, read_param
use LND_forcing_type,        only : forcing, LND_forcing_chksum
use LND_forcing_type,        only : calculateBuoyancyFlux2d, forcing_SinglePointPrint
use LND_geothermal,          only : geothermal, geothermal_init, geothermal_end, geothermal_CS
use LND_grid,                only : ocean_grid_type
use LND_int_tide_input,      only : set_int_tide_input, int_tide_input_init
use LND_int_tide_input,      only : int_tide_input_end, int_tide_input_CS, int_tide_input_type
use LND_interface_heights,   only : find_eta
use LND_internal_tides,      only : propagate_int_tide
use LND_internal_tides,      only : internal_tides_init, internal_tides_end, int_tide_CS
use LND_kappa_shear,         only : kappa_shear_is_used
use LND_CVMix_KPP,           only : KPP_CS, KPP_init, KPP_compute_BLD, KPP_calculate
use LND_CVMix_KPP,           only : KPP_end, KPP_get_BLD
use LND_CVMix_KPP,           only : KPP_NonLocalTransport_temp, KPP_NonLocalTransport_saln
use LND_opacity,             only : opacity_init, opacity_end, opacity_CS
use LND_opacity,             only : absorbRemainingSW, optics_type, optics_nbands
use LND_open_boundary,       only : ocean_OBC_type
use LND_regularize_layers,   only : regularize_layers, regularize_layers_init, regularize_layers_CS
use LND_set_diffusivity,     only : set_diffusivity, set_BBL_TKE
use LND_set_diffusivity,     only : set_diffusivity_init, set_diffusivity_end
use LND_set_diffusivity,     only : set_diffusivity_CS
use LND_sponge,              only : apply_sponge, sponge_CS
use LND_ALE_sponge,          only : apply_ALE_sponge, ALE_sponge_CS
use LND_time_manager,        only : time_type, real_to_time, operator(-), operator(<=)
use LND_tracer_flow_control, only : call_tracer_column_fns, tracer_flow_control_CS
use LND_tracer_diabatic,     only : tracer_vertdiff
use LND_unit_scaling,        only : unit_scale_type
use LND_variables,           only : thermo_var_ptrs, vertvisc_type, accel_diag_ptrs
use LND_variables,           only : cont_diag_ptrs, LND_thermovar_chksum, p3d
use LND_verticalGrid,        only : verticalGrid_type, get_thickness_units
use LND_wave_speed,          only : wave_speeds
use LND_wave_interface,      only : wave_parameters_CS
...
!!  accompanying diapycnal advection of lndentum and tracers.
subroutine diabatic(u, v, h, tv, Hml, fluxes, visc, ADp, CDp, dt, Time_end, &
  ...
  type(accel_diag_ptrs),                     intent(inout) :: ADp       !< related points to accelerations in lndentum
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "LND_diabatic_driver: "// &
         "Module must be initialized before it is used.")
  if (dt == 0.0) call LND_error(FATAL, "LND_diabatic_driver: "// &
  ...
  if (CS%debug) then
    call LND_state_chksum("Start of diabatic ", u, v, h, G, GV, US, haloshift=0)
    call LND_forcing_chksum("Start of diabatic", fluxes, G, US, haloshift=0)
  endif
  if (CS%debugConservation) call LND_state_stats('Start of diabatic', u, v, h, tv%T, tv%S, G, GV, US)
  ...
  if (CS%debugConservation) call LND_state_stats('1st make_frazil', u, v, h, tv%T, tv%S, G, GV, US)
  ...
  if (associated(tv%T) .AND. associated(tv%frazil)) then
    ...
    if (CS%debugConservation) call LND_state_stats('2nd make_frazil', u, v, h, tv%T, tv%S, G, GV, US)
    call disable_averaging(CS%diag)
  ...
  if (CS%debugConservation) call LND_state_stats('leaving diabatic', u, v, h, tv%T, tv%S, G, GV, US)
...
subroutine diabatic_ALE_legacy(u, v, h, tv, Hml, fluxes, visc, ADp, CDp, dt, Time_end, &
  ...
  showCallTree = callTree_showQuery()
  if (showCallTree) call callTree_enter("diabatic_ALE_legacy(), LND_diabatic_driver.F90")
!   if (showCallTree) call callTree_enter("diabatic_ALE(), LND_diabatic_driver.F90")
  ...
  if (CS%use_geothermal) then
    ...
    if (CS%debugConservation) call LND_state_stats('geothermal', u, v, h, tv%T, tv%S, G, GV, US)
  endif
  ...
  if (CS%debug) call LND_state_chksum("before find_uv_at_h", u, v, h, G, GV, US, haloshift=0)
  ...
  if (CS%debug) &
    call LND_state_chksum("before set_diffusivity", u, v, h, G, GV, US, haloshift=CS%halo_TS_diff)
  ...
  if (CS%debug) then
    call LND_state_chksum("after set_diffusivity ", u, v, h, G, GV, US, haloshift=0)
    call LND_forcing_chksum("after set_diffusivity ", fluxes, G, US, haloshift=0)
    call LND_thermovar_chksum("after set_diffusivity ", tv, G)
  ...
  if (CS%useKPP) then
    ...
    if (CS%debug) then
      call LND_state_chksum("after KPP", u, v, h, G, GV, US, haloshift=0)
      call LND_forcing_chksum("after KPP", fluxes, G, US, haloshift=0)
      call LND_thermovar_chksum("after KPP", tv, G)
  ...
  if (CS%useKPP) then
    ...
    if (CS%debugConservation) call LND_state_stats('KPP_applyNonLocalTransport', u, v, h, tv%T, tv%S, G, GV, US)

    if (CS%debug) then
      call LND_state_chksum("after KPP_applyNLT ", u, v, h, G, GV, US, haloshift=0)
      call LND_forcing_chksum("after KPP_applyNLT ", fluxes, G, US, haloshift=0)
      call LND_thermovar_chksum("after KPP_applyNLT ", tv, G)
    endif
  endif ! endif for KPP
  ...
  if (associated(visc%Kd_extra_T) .and. associated(visc%Kd_extra_S) .and. associated(tv%T) .and. &
    ...
    if (CS%debugConservation) call LND_state_stats('differential_diffuse_T_S', u, v, h, tv%T, tv%S, G, GV, US)
  ...
  if (CS%debug) then
    call LND_forcing_chksum("after calc_entrain ", fluxes, G, US, haloshift=0)
    call LND_thermovar_chksum("after calc_entrain ", tv, G)
    call LND_state_chksum("after calc_entrain ", u, v, h, G, GV, US, haloshift=0)
  ...
  if (CS%debug) then
    call LND_forcing_chksum("after applyBoundaryFluxes ", fluxes, G, US, haloshift=0)
    call LND_thermovar_chksum("after applyBoundaryFluxes ", tv, G)
    call LND_state_chksum("after applyBoundaryFluxes ", u, v, h, G, GV, US, haloshift=0)
  endif
  if (showCallTree) call callTree_waypoint("done with applyBoundaryFluxes (diabatic)")
  if (CS%debugConservation)  call LND_state_stats('applyBoundaryFluxes', u, v, h, tv%T, tv%S, G, GV, US)
  ...
  if (CS%debug) then
    call LND_state_chksum("after negative check ", u, v, h, G, GV, US, haloshift=0)
    call LND_forcing_chksum("after negative check ", fluxes, G, US, haloshift=0)
    call LND_thermovar_chksum("after negative check ", tv, G)
  endif
  if (showCallTree) call callTree_waypoint("done with h=ea-eb (diabatic)")
  if (CS%debugConservation) call LND_state_stats('h=ea-eb', u, v, h, tv%T, tv%S, G, GV, US)
  ...
  if (associated(tv%T)) then
    ...
    !  BOUND_SALINITY=False in LND.F90.
  ...
  if (CS%debugConservation) call LND_state_stats('triDiagTS', u, v, h, tv%T, tv%S, G, GV, US)

  if (CS%debug) then
    call LND_state_chksum("after mixed layer ", u, v, h, G, GV, US, haloshift=0)
    call LND_thermovar_chksum("after mixed layer ", tv, G)
  endif
  ...
  if (CS%use_sponge) then
    ...
    if (CS%debug) then
      call LND_state_chksum("apply_sponge ", u, v, h, G, GV, US, haloshift=0)
      call LND_thermovar_chksum("apply_sponge ", tv, G)
    endif
  endif ! CS%use_sponge
...
!    hold,   &    ! layer thickness before diapycnal entrainment, and later
  ...
  if (.not. (CS%useALEalgorithm)) call LND_error(FATAL, "LND_diabatic_driver: "// &
         "The ALE algorithm must be enabled when using LND_diabatic_driver.")
...
!> Imposes the diapycnal mass fluxes and the accompanying diapycnal advection of lndentum and tracers
!! using the original UCLAND algorithms.
subroutine layered_diabatic(u, v, h, tv, Hml, fluxes, visc, ADp, CDp, dt, Time_end, &
  ...
  showCallTree = callTree_showQuery()
  if (showCallTree) call callTree_enter("layered_diabatic(), LND_diabatic_driver.F90")
  ...
  if (CS%bulkmixedlayer) then
    if (CS%debug) call LND_forcing_chksum("Before mixedlayer", fluxes, G, US, haloshift=0)
...
!    (5) Possibly splits buffer layer into two isopycnal layers (when using isopycnal coordinate)
      ...
      if (CS%debug) then
        call LND_state_chksum("After mixedlayer ", u, v, h, G, GV, US, haloshift=0)
        call LND_forcing_chksum("After mixedlayer", fluxes, G, US, haloshift=0)
      endif
      if (showCallTree) call callTree_waypoint("done with 1st bulkmixedlayer (diabatic)")
      if (CS%debugConservation) call LND_state_stats('1st bulkmixedlayer', u, v, h, tv%T, tv%S, G, GV, US)
    endif
  endif

  if (CS%debug) &
    call LND_state_chksum("before find_uv_at_h", u, v, h, G, GV, US, haloshift=0)
  if (CS%use_kappa_shear .or. CS%use_CVMix_shear) then
    if ((CS%ML_mix_first > 0.0) .or. CS%use_geothermal) then
  ...
  if (CS%bulkmixedlayer) then
    ...
    if (CS%debugConservation) call LND_state_stats('BML tridiag', u, v, h, tv%T, tv%S, G, GV, US)
    ...
    !    (5) Possibly splits the buffer layer into two isopycnal layers.
      ...
      if (CS%debug) call LND_state_chksum("find_uv_at_h1 ", u, v, h, G, GV, US, haloshift=0)
      ...
      if (CS%debugConservation) call LND_state_stats('2nd bulkmixedlayer', u, v, h, tv%T, tv%S, G, GV, US)
    endif
  ...
  if (CS%debugConservation) call LND_state_stats('regularize_layers', u, v, h, tv%T, tv%S, G, GV, US)
...
! For lndentum, it is only the net flux that homogenizes within
...
! Initialize halo regions of ea, eb, and hold to default values.
  ...
  if (CS%debug) then
    call LND_state_chksum("before u/v tridiag ", u, v, h, G, GV, US, haloshift=0)
  ...
  enddo
  if (CS%debug) then
    call LND_state_chksum("aft 1st loop tridiag ", u, v, h, G, GV, US, haloshift=0)
  endif
  !$OMP parallel do default(shared) private(hval,b1,d1,c1,eaval)
  do J=Jsq,Jeq
    do i=is,ie
  ...
  call cpu_clock_end(id_clock_tridiag)
  if (CS%debug) then
    call LND_state_chksum("after u/v tridiag ", u, v, h, G, GV, US, haloshift=0)
  endif
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_diabatic_driver" ! This module's name.

  if (associated(CS)) then
    call LND_error(WARNING, "adiabatic_driver_init called with an "// &
...
subroutine diabatic_driver_init(Time, G, GV, US, param_file, useALEalgorithm, diag, &
  ...
  type(accel_diag_ptrs),   intent(inout) :: ADp              !< pointers to accelerations in lndentum equations,
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "diabatic_driver_init called with an "// &
                            "associated control structure.")
    return
  else
    allocate(CS)
  ...
  if (CS%use_CVMix_ddiff .and. differentialDiffusion) then
    call LND_error(FATAL, 'diabatic_driver_init: '// &
  ...
  if (CS%use_int_tides) then
    ...
    do m=1,CS%nMode
      ...
      call LND_mesg("Registering "//trim(var_name)//", Described as: "//var_descript, 5)
    enddo
  endif
...
end module LND_diabatic_driver
