...
type, public :: oil_tracer_CS ; private
  ...
  type(tracer_registry_type), pointer :: tr_Reg => NULL() !< A pointer to the LND tracer registry
...
!> Register oil tracer fields and subroutines to be used with LND.
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "register_oil_tracer called with an "// &
...
subroutine initialize_oil_tracer(restart, day, G, GV, US, h, diag, OBC, CS, &
  ...
  !  Read the tracer concentrations from a netcdf file.
        if (.not.file_exists(CS%IC_file, G%Domain)) &
          call LND_error(FATAL, "initialize_oil_tracer: "// &
        ...
        if (CS%Z_IC_file) then
          ...
          if (.not.OK) then
            ...
            if (.not.OK) call LND_error(FATAL,"initialize_oil_tracer: "//&
                    "Unable to read "//trim(name)//" from "//&
                    trim(CS%IC_file)//".")
          endif
        else
          call LND_read_data(CS%IC_file, trim(name), CS%tr(:,:,:,m), G%Domain)
        endif
      else
