...
use LND_debugging, only : hchksum, Bchksum
...
# include "version_variable.h"
  character(len=40)  :: mdl = "LND_kappa_shear"  ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "kappa_shear_init called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
