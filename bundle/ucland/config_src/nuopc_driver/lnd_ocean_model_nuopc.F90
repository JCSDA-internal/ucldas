...
use LND_forcing_type,        only : allocate_forcing_type
use LND_forcing_type,        only : forcing, mech_forcing
use LND_forcing_type,        only : forcing_accumulate, copy_common_forcing_fields
use LND_forcing_type,        only : copy_back_forcing_fields, set_net_mass_forcing
use LND_forcing_type,        only : set_derived_forcing_fields
...
use LND_time_manager,        only : time_type, get_time, set_time, operator(>)
use LND_time_manager,        only : operator(+), operator(-), operator(*), operator(/)
use LND_time_manager,        only : operator(/=), operator(<=), operator(>=)
use LND_time_manager,        only : operator(<), real_to_time_type, time_type_to_real
...
use LND_surface_forcing_nuopc, only : surface_forcing_init, convert_IOB_to_fluxes
use LND_surface_forcing_nuopc, only : convert_IOB_to_forces, ice_ocn_bnd_type_chksum
use LND_surface_forcing_nuopc, only : ice_ocean_boundary_type, surface_forcing_CS
use LND_surface_forcing_nuopc, only : forcing_save_restart
...
type, public :: ocean_state_type ; private
  ...
  logical :: restore_salinity !< If true, the coupled LND driver adds a term to
                              !! restore salinity to a specified value.
  logical :: restore_temp     !< If true, the coupled LND driver adds a term to
  ...
  real :: eps_omesh           !< Max allowable difference between ESMF mesh and UCLAND
                              !! domain coordinates
...
#include "version_variable.h"
  ...
  call get_param(param_file, mdl, "EPS_OMESH",OS%eps_omesh, &
                 ...
                 "UCLAND domain coordinates in nuopc cap.", &
  ...
  if (is_root_pe()) &
    write(*,'(/12x,a/)') '======== COMPLETED LND INITIALIZATION ========'
...
subroutine update_ocean_model(Ice_ocean_boundary, OS, Ocean_sfc, &
  ...
  call callTree_enter("update_ocean_model(), LND_ocean_model_nuopc.F90")
  ...
  if (OS%fluxes%fluxes_used) then
    ...
    call LND_generic_tracer_fluxes_accumulate(OS%fluxes, weight) !here weight=1, just saving the current fluxes
#endif
  else
...
#ifdef _USE_GENERIC_TRACER
    call LND_generic_tracer_fluxes_accumulate(OS%flux_tmp, weight) !weight of the current flux in the running average
#endif
  endif
    ...
    do n=1,n_max
      if (OS%diabatic_first) then
        ...
        if (modulo(n-1,nts)==0) then
          ...
          call step_LND(OS%forces, OS%fluxes, OS%sfc_state, Time2, dtdia, OS%LND_CSp, &
                        Waves=OS%Waves, do_dynamics=.false., do_thermodynamics=.true., &
                        start_cycle=(n==1), end_cycle=.false., cycle_length=dt_coupling)
        endif

        call step_LND(OS%forces, OS%fluxes, OS%sfc_state, Time2, dt_dyn, OS%LND_CSp, &
...
end module LND_ocean_model_nuopc
