...
use LND_domains,       only : To_South, To_West, To_All
...
use LND_safe_alloc,    only : safe_alloc_ptr
...
public LND_wave_interface_init ! Public interface to fully initialize the wave routines.
public LND_wave_interface_init_lite ! Public interface to quick initialize this module.
...
public StokesMixing ! NOT READY - Public interface to add down-Stokes gradient
                    ! lndentum mixing (e.g. the approach of Harcourt 2013/2015)
...
type, public :: wave_parameters_CS ; private
  ...
  logical, public :: StokesMixing     !< This feature is in development and not ready.
                                      !! True if vertical mixing of lndentum
...
character(len=40)  :: mdl = "LND_wave_interface" !< This module's name.
...
!> Initializes parameters related to LND_wave_interface
subroutine LND_wave_interface_init(time, G, GV, US, param_file, CS, diag )
  ...
  if (associated(CS)) then
     call LND_error(FATAL, "wave_interface_init called with an associated"//&
                             "control structure.")
     return
  endif
  ...
  call get_param(param_file, mdl, "LAGRANGIAN_MIXING", CS%LagrangianMixing, &
       "Flag to use Lagrangian Mixing of lndentum", units="", &
       Default=.false.)
  if (CS%LagrangianMixing) then
    ! Force Code Intervention
    call LND_error(FATAL,"Should you be enabling Lagrangian Mixing? Code not ready.")
  endif
  call get_param(param_file, mdl, "STOKES_MIXING", CS%StokesMixing, &
       "Flag to use Stokes Mixing of lndentum", units="", &
       Default=.false.)
  if (CS%StokesMixing) then
    ! Force Code Intervention
    call LND_error(FATAL,"Should you be enabling Stokes Mixing? Code not ready.")
  endif
  ...
  if (CS%CoriolisStokes) then
    ! Force Code Intervention
    call LND_error(FATAL,"Should you be enabling Coriolis-Stokes? Code not ready.")
  endif
  ...
  case (NULL_STRING)! No Waves
    call LND_error(FATAL, "wave_interface_init called with no specified "//&
                           "WAVE_METHOD.")
  ...
  case (SURFBANDS_STRING)! Surface Stokes Drift Bands
    ...
    case (NULL_STRING)! Default
      call LND_error(FATAL, "wave_interface_init called with SURFACE_BANDS"//&
   ...
   case default! No method provided
     call LND_error(FATAL,'Check WAVE_METHOD.')
   end select

  case (DHH85_STRING)!Donelan et al., 1985 spectrum
    WaveMethod = DHH85
    call LND_error(WARNING,"DHH85 only ever set-up for uniform cases w/"//&
  ...
  return
end subroutine LND_wave_interface_init
...
subroutine LND_wave_interface_init_lite(param_file)
  type(param_file_type), intent(in) :: param_file !< Input parameter structure
  character*(5), parameter  :: NULL_STRING      = "EMPTY"
  character*(4), parameter  :: LF17_STRING      = "LF17"
  ...
  return
end subroutine LND_wave_interface_init_lite
...
subroutine Surface_Bands_by_data_override(day_center, G, GV, US, CS)
  ...
  if (.not.dataOverrideIsInitialized) then
    ...
    if (rcode_wn /= 0) then
      call LND_error(FATAL,"error opening file "//trim(SurfBandFileName)//&
            " in LND_wave_interface.")
    endif
    ...
    if (rcode_wn /= 0 .and. rcode_fr /= 0) then
      call LND_error(FATAL,"error finding variable "//trim(varread1)//&
         " or "//trim(varread2)//" in file "//trim(SurfBandFileName)//" in LND_wave_interface.")
    ...
    elseif (rcode_wn == 0) then
      ...
      if (rcode_wn /= 0) then
        call LND_error(FATAL, &
             'error inquiring dimensions LND_wave_interface.')
      endif
      rcode_wn = NF90_INQUIRE_DIMENSION(ncid, dims(1), dim_name(1), len=id)
      if (rcode_wn /= 0) then
        call LND_error(FATAL,"error reading dimension 1 data for "// &
             trim(varread1)//" in file "// trim(SurfBandFileName)//          &
             " in LND_wave_interface.")
      endif
      rcode_wn = NF90_INQ_VARID(ncid, dim_name(1), dim_id(1))
      if (rcode_wn /= 0) then
        call LND_error(FATAL,"error finding variable "//trim(dim_name(1))//&
          " in file "//trim(SurfBandFileName)//" in LND_wave_interace.")
      endif
    ...
    if (PartitionMode==0) then
      ...
      if (rcode_wn /= 0) then
        call LND_error(FATAL,&
             ...
             " in file "// trim(SurfBandFileName)//" in LND_wave_interface")
      endif
      NUMBANDS = ID
 ...
 ! Compute averaging depth for Stokes drift (negative)
  ...
  if (USE_MA) then
    if (.not.(present(H).and.present(U_H).and.present(V_H))) then
      call LND_error(Fatal,'Get_LA_waves requested to consider misalignment.')
    endif
    ContinueLoop = .true.
    bottom = 0.0
    do kk = 1,G%ke
  ...
  elseif (WaveMethod==Null_WaveMethod) then
    call LND_error(FATAL, "Get_Langmuir_number called without defining a WaveMethod. "//&
...
!!
!! Qing Li, 160606
!! - BGR port from CVMix to UCLAND Jan/25/2017
...
!! use for comparing UCLAND simulation to his LES
...
subroutine StokesMixing(G, GV, dt, h, u, v, Waves )
  ...
  type(verticalGrid_type), &
       intent(in)    :: GV    !< Ocean vertical grid
  real, intent(in)   :: dt    !< Time step of UCLAND [T ~> s] for explicit solver
  real, dimension(SZI_(G),SZJ_(G),SZK_(G)),&
  ...
  ! Local variables
  real :: dTauUp, dTauDn ! Vertical lndentum fluxes [Z T-1 m s-1]
...
subroutine CoriolisStokes(G, GV, DT, h, u, v, WAVES, US)
  ...
  type(verticalGrid_type), &
       intent(in)   :: GV     !< Ocean vertical grid
  real, intent(in)  :: Dt     !< Time step of UCLAND [s] CHECK IF PASSING RIGHT TIMESTEP
  real, dimension(SZI_(G),SZJ_(G),SZK_(G)),  &
       intent(in)    :: h     !< Layer thicknesses [H ~> m or kg m-2]
  real, dimension(SZIB_(G),SZJ_(G),SZK_(G)), &
       intent(inout) :: u     !< Velocity i-component [m s-1]
...
!! interpret surface wave data for UCLAND. In its original form, the
...
!! sea-state dependent lndentum fluxes, gas fluxes, and other wave
