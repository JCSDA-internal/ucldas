...
private
public :: write2pe, ucldas_str2int, ucldas_adjust, &
          ucldas_rho, ucldas_diff, ucldas_mld, nc_check, ucldas_remap_idw
...
elemental function ucldas_rho(sp, pt, p, lon, lat)
  real(kind=kind_real), intent(in)  :: pt, sp, p, lon, lat
  real(kind=kind_real) :: sa, ct, lon_rot, ucldas_rho
  ...
  ! Insitu density
  ucldas_rho = gsw_rho(sa,ct,p)

  return
end function ucldas_rho

! ------------------------------------------------------------------------------

function ucldas_mld(sp, pt, p, lon, lat)
  real(kind=kind_real), intent(in)  :: pt(:), sp(:), p(:), lon, lat

  real(kind=kind_real) :: lon_rot, ucldas_mld
  ...
  ! Mixed layer depth
  ucldas_mld = gsw_mlp(sa,ct,p)
  if (ucldas_mld>9999.9_kind_real) ucldas_mld = p(1)
  ucldas_mld = max(ucldas_mld, p(1))

  deallocate(sa, ct)

  return
end function ucldas_mld

! ------------------------------------------------------------------------------

subroutine ucldas_diff(dvdz,v,h)
...
! ------------------------------------------------------------------------------
!> Apply bounds
elemental function ucldas_adjust(std, minstd, maxstd)
  real(kind=kind_real), intent(in)  :: std, minstd, maxstd
  real(kind=kind_real) :: ucldas_adjust

  ucldas_adjust = min( max(std, minstd), maxstd)

end function ucldas_adjust

! ------------------------------------------------------------------------------
subroutine ucldas_str2int(str, int)
  character(len=*),intent(in) :: str
  integer,intent(out)         :: int

  read(str,*)  int
end subroutine ucldas_str2int

! ------------------------------------------------------------------------------
! inverse distance weighted remaping (modified Shepard's method)
subroutine ucldas_remap_idw(lon_src, lat_src, data_src, lon_dst, lat_dst, data_dst)
  ...
  ! done, cleanup
  call kd%final()
end subroutine ucldas_remap_idw

! ------------------------------------------------------------------------------
end module ucldas_utils
