...
module LND_lateral_boundary_diffusion
...
character(len=40) :: mdl = "LND_lateral_boundary_diffusion"       !< Name of this module
...
logical function lateral_boundary_diffusion_init(Time, G, param_file, diag, diabatic_CSp, CS)
  ...
  if (ASSOCIATED(CS)) then
    call LND_error(FATAL, "lateral_boundary_diffusion_init called with associated control structure.")
    return
  endif
  ...
  if ( .not. ASSOCIATED(CS%energetic_PBL_CSp) .and. .not. ASSOCIATED(CS%KPP_CSp) ) then
    call LND_error(FATAL,"Lateral boundary diffusion is true, but no valid boundary layer scheme was found")
  endif
...
real function bulk_average(boundary, nk, deg, h, hBLT, phi, ppoly0_E, ppoly0_coefs, method, k_top, zeta_top, k_bot, &
  ...
  elseif (boundary == BOTTOM) then
    ...
    call LND_error(FATAL, "bulk_average: a valid boundary type must be provided.")
  endif
...
subroutine boundary_k_range(boundary, nk, h, hbl, k_top, zeta_top, k_bot, zeta_bot)
  ...
  elseif ( boundary == BOTTOM ) then
    ...
    call LND_error(FATAL,"Houston, we've had a problem in boundary_k_range")
  endif
...
logical function test_layer_fluxes(verbose, nk, test_name, F_calc, F_ans)
  ...
  do k=1,nk
    if ( F_calc(k) /= F_ans(k) ) then
      ...
      write(stdunit,*) "LND_lateral_boundary_diffusion, UNIT TEST FAILED: ", test_name
...
!> \namespace lnd_lateral_boundary_diffusion
!!
...
!!
end module LND_lateral_boundary_diffusion
