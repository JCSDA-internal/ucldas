...
module LND_density_integrals

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_EOS,              only : EOS_type
use LND_EOS,              only : EOS_quadrature
use LND_EOS,              only : analytic_int_density_dz
use LND_EOS,              only : analytic_int_specific_vol_dp
use LND_EOS,              only : calculate_density
use LND_EOS,              only : calculate_spec_vol
use LND_EOS,              only : calculate_specific_vol_derivs
...
subroutine int_density_dz_generic_pcm(T, S, z_t, z_b, rho_ref, rho_0, G_e, HI, &
  ...
  if (present(useMassWghtInterp)) then ; if (useMassWghtInterp) then
    do_massWeight = .true.
    if (.not.present(bathyT)) call LND_error(FATAL, "int_density_dz_generic: "//&
        "bathyT must be present if useMassWghtInterp is present and true.")
    if (.not.present(dz_neglect)) call LND_error(FATAL, "int_density_dz_generic: "//&
...
! series for log(1-eps/1+eps) that assumes that |eps| < 0.34.
  ...
  if (present(useMassWghtInterp)) then ; if (useMassWghtInterp) then
    do_massWeight = .true.
    if (.not.present(bathyP)) call LND_error(FATAL, "int_spec_vol_dp_generic: "//&
        "bathyP must be present if useMassWghtInterp is present and true.")
    if (.not.present(dP_neglect)) call LND_error(FATAL, "int_spec_vol_dp_generic: "//&
...
subroutine find_depth_of_pressure_in_cell(T_t, T_b, S_t, S_b, z_t, z_b, P_t, P_tgt, &
  ...
  do while ( abs(Pa) > Pa_tol )
    ...
    if (Pa<Pa_left) then
      ...
      call LND_error(FATAL, 'find_depth_of_pressure_in_cell out of bounds negative: /n'//msg)
    elseif (Pa<0.) then
      Pa_left = Pa
      F_l = F_guess
    elseif (Pa>Pa_right) then
      write(msg,*) Pa_left,Pa,Pa_right,P_t-P_tgt,P_b-P_tgt
      call LND_error(FATAL, 'find_depth_of_pressure_in_cell out of bounds positive: /n'//msg)
    elseif (Pa>0.) then
      Pa_right = Pa
      F_r = F_guess
    else ! Pa == 0
...
end module LND_density_integrals

!> \namespace lnd_density_integrals
!!
