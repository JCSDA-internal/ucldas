...
use LND_io, only : close_file, fieldtype, file_exists
use LND_io, only : open_file, read_data, read_axis_data, SINGLE_FILE, MULTIPLE
use LND_io, only : slasher, vardesc, write_field
use LND_string_functions, only : uppercase
use LND_time_manager, only : time_type, get_external_field_size
use LND_time_manager, only : init_external_field
use LND_time_manager, only : get_external_field_axes, get_external_field_missing
use LND_transform_FMS, only : time_interp_external => rotated_time_interp_external
...
! character(len=40)  :: mdl = "LND_horizontal_regridding" ! This module's name.
...
subroutine fill_miss_2d(aout, good, fill, prev, G, smooth, num_pass, relc, crit, debug, answers_2018)
  ...
  do j=js,je ; do i=is,ie
    if (good_(i,j) == 0.0 .and. fill_pts(i,j) == 1.0) then
      ...
      call LND_error(WARNING, mesg, .true.)
      call LND_error(FATAL,"LND_initialize: "// &
...
subroutine horiz_interp_and_extrap_tracer_record(filename, varnam,  conversion, recnum, G, tr_z, &
  ...
  if (rcode /= 0) call LND_error(FATAL,"error opening file "//trim(filename)//&
                           " in hinterp_extrap")
  rcode = NF90_INQ_VARID(ncid, varnam, varid)
  if (rcode /= 0) call LND_error(FATAL,"error finding variable "//trim(varnam)//&
                                 " in file "//trim(filename)//" in hinterp_extrap")

  rcode = NF90_INQUIRE_VARIABLE(ncid, varid, ndims=ndims, dimids=dims)
  if (rcode /= 0) call LND_error(FATAL,'error inquiring dimensions hinterp_extrap')
  if (ndims < 3) call LND_error(FATAL,"Variable "//trim(varnam)//" in file "// &
              trim(filename)//" has too few dimensions.")

  rcode = NF90_INQUIRE_DIMENSION(ncid, dims(1), dim_name(1), len=id)
  if (rcode /= 0) call LND_error(FATAL,"error reading dimension 1 data for "// &
                trim(varnam)//" in file "// trim(filename)//" in hinterp_extrap")
  rcode = NF90_INQ_VARID(ncid, dim_name(1), dim_id(1))
  if (rcode /= 0) call LND_error(FATAL,"error finding variable "//trim(dim_name(1))//&
                                 " in file "//trim(filename)//" in hinterp_extrap")
  rcode = NF90_INQUIRE_DIMENSION(ncid, dims(2), dim_name(2), len=jd)
  if (rcode /= 0) call LND_error(FATAL,"error reading dimension 2 data for "// &
                trim(varnam)//" in file "// trim(filename)//" in hinterp_extrap")
  rcode = NF90_INQ_VARID(ncid, dim_name(2), dim_id(2))
  if (rcode /= 0) call LND_error(FATAL,"error finding variable "//trim(dim_name(2))//&
                                 " in file "//trim(filename)//" in hinterp_extrap")
  rcode = NF90_INQUIRE_DIMENSION(ncid, dims(3), dim_name(3), len=kd)
  if (rcode /= 0) call LND_error(FATAL,"error reading dimension 3 data for "// &
                trim(varnam)//" in file "// trim(filename)//" in hinterp_extrap")
  rcode = NF90_INQ_VARID(ncid, dim_name(3), dim_id(3))
  if (rcode /= 0) call LND_error(FATAL,"error finding variable "//trim(dim_name(3))//&
                                 " in file "//trim(filename)//" in hinterp_extrap")

  missing_value=0.0
  rcode = NF90_GET_ATT(ncid, varid, "_FillValue", missing_value)
  if (rcode /= 0) call LND_error(FATAL,"error finding missing value for "//&
       trim(varnam)//" in file "// trim(filename)//" in hinterp_extrap")
  ...
  if (rcode /= 0) call LND_error(FATAL,"error reading dimension 1 values for var_name "// &
  ...
  if (rcode /= 0) call LND_error(FATAL,"error reading dimension 2 values for var_name "// &
  ...
  if (rcode /= 0) call LND_error(FATAL,"error reading dimension 3 values for var_name "// &
  ...
  do k=1,kd
    ...
    if (is_ongrid) then
       ...
       if (rcode /= 0) call LND_error(FATAL,"hinterp_and_extract_from_Fie: "//&
...
end module LND_horizontal_regridding
