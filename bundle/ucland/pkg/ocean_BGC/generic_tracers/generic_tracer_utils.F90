...
#include <fms_platform.h>
  ...
  use fms_mod,           only: stdout
    ...
    use LND_diag_mediator, only : register_diag_field_LND=>register_diag_field
    use LND_diag_mediator, only : post_data_LND=>post_data, post_data_1d_k
    use LND_diag_mediator, only : g_diag_ctrl=>diag_ctrl
#else
...
!-----------------------------------------------------------------------
  ...
  !   ! LND keeps the field at 3 time levels, hence 4D.
  ...
  type g_tracer_type
     ...
     ! Tracer concentration field in space (and time)
     ! LND keeps the prognostic tracer fields at 3 time levels, hence 4D.
     real, pointer, dimension(:,:,:,:):: field  => NULL()
     !The following pointer is intended to point to prognostic tracer field in LND. Do not allocate!
     real, pointer,      dimension(:,:,:,:):: field4d_ptr => NULL()
     !The following pointer is intended to point to diagnostic tracer field in LND. Do not allocate!
...
contains
  ...
  subroutine g_tracer_add(node_ptr, package, name, longname, units,  prog, const_init_value,init_value,&
    ...
    !===================================================================
    !Note that const_init_value unlike init_value has special meaning in LND 
...
!            implementation    = 'ocmip2',                                                     &
  ...
  subroutine g_tracer_coupler_get(g_tracer_list,IOB_struc, weight, model_time)
    ...
    do  
       ...
       !runoff contributes to %stf in GOLD but not in LND, 
...
!     local parameters
  ...
  subroutine g_tracer_vertdiff_G(g_tracer, h_old, ea, eb, dt, kg_m2_to_H, m_to_H, tau, lnd)
    type(g_tracer_type),    pointer  :: g_tracer
    real, dimension(g_tracer_com%isd:,g_tracer_com%jsd:,:), intent(in) :: h_old, ea, eb
    real,                   intent(in) :: dt, kg_m2_to_H, m_to_H
    integer,                intent(in) :: tau
    logical,                                                intent(in), optional :: lnd
    ...
    !                     of h_old (H).
    !  (in,opt)  lnd - If true, then called from LND and don't do diagnostic,
    !                  if false or not present, then not from LND and do diagnostics.
  ...
  !  <DESCRIPTION>
  !   This subroutine solves a tridiagonal equation to find and set values of vertically diffused field for a tracer node.
  !   This is designed to calculate entrainments for LND tracers and then call the g_tracer_vertdiff_G() above.
  !  </DESCRIPTION>
  !  <TEMPLATE>
  !   call 
  !  </TEMPLATE>
  ...
  subroutine g_tracer_vertdiff_M(g_tracer,dh, dhw, diff_cbt, dt, rho0,tau)
    ...
    if(GOLDtridiag) then
       ...
       call g_tracer_vertdiff_G(g_tracer, dh, ea, eb, dt, 1.0, rho0, tau, lnd = .true.)
    ...
    elseif(IOWtridiag) then
       ...
       !
       !===== 2 ===================
       !Via LND's invtri routine with vertical movement added
       !===========================
       !
       !This is borrowed from LND invtri
       ...
       !
       !===== 2 ===================
       !Via LND's invtri routine. Works only for sink_rate=0
       !===========================
       !
       !This is borrowed from LND invtri
       !     call invtri (T_prog(n)%field(:,:,:,taup1), T_prog(n)%stf, &
...
!          endif
    ...
    if(errorstring .ne. '') then
       !The following cannot be FATAL for backward compatibility with LND5 and GOLD
  ...
  function g_register_diag_field(module_name, field_name, axes, init_time,         &
    ...
    logical,          optional, intent(in) :: mask_variant !< If true a logical mask must be provided with post_data calls (not used in LND?)
    ...
    character(len=*), optional, intent(out):: err_msg !< String into which an error message might be placed (not used in LND?)
    character(len=*), optional, intent(in) :: interp_method !< no clue (not used in LND?)
...
!            ": the diag_CD argument is not present and the model is compiled with _USE_UCLAND_DIAG for "//trim(field_name))
       !This is not thread-safe. It has to be fixed later.
       call g_tracer_get_diagCS(diag_CS_ptr)
    endif
    LND_missing_value = diag_CS_ptr%missing_value
    if(size(axes) .eq. 3) then
       g_register_diag_field = register_diag_field_LND(trim(module_name), field_name, diag_CS_ptr%axesTL, init_time,&
            long_name, units, LND_missing_value, range, mask_variant, standard_name,      &
            verbose, do_not_log, err_msg, interp_method, tile_count, cmor_field_name, &
            cmor_long_name, cmor_units, cmor_standard_name, cell_methods, &
            x_cell_method, y_cell_method, v_cell_method)
    elseif(size(axes) .eq. 2) then
       g_register_diag_field = register_diag_field_LND(trim(module_name), field_name, diag_CS_ptr%axesT1, init_time,&
            long_name, units, LND_missing_value, range, mask_variant, standard_name,      &
            verbose, do_not_log, err_msg, interp_method, tile_count, cmor_field_name, &
            cmor_long_name, cmor_units, cmor_standard_name, cell_methods, &
            x_cell_method, y_cell_method, v_cell_method)
    elseif(size(axes) .eq. 1) then
       g_register_diag_field = register_diag_field_LND(trim(module_name), field_name, diag_CS_ptr%axesTi, init_time,&
            long_name, units, LND_missing_value, range, mask_variant, standard_name,      &
            verbose, do_not_log, err_msg, interp_method, tile_count, cmor_field_name, &
            cmor_long_name, cmor_units, cmor_standard_name, cell_methods, &
            x_cell_method, y_cell_method, v_cell_method)
  ...
  LOGICAL FUNCTION g_send_data_0d(diag_field_id, field, time, err_msg, diag_CS)
    ...
    call post_data_LND(diag_field_id, field, diag_CS_ptr) 
    g_send_data_0d = .TRUE.
#else
  ...
  LOGICAL FUNCTION g_send_data_2d(diag_field_id, field, time, is_in, js_in, &
    ...
    call post_data_LND(diag_field_id, field, diag_CS_ptr)!, mask=rmask)         
    g_send_data_2d = .TRUE.
#else
