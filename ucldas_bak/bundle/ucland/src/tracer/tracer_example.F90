...
!> This subroutine is used to register tracer fields and subroutines to be used with LND.
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "USER_register_tracer_example called with an "// &
...
!  Read the tracer concentrations from a netcdf file.
      if (.not.file_exists(CS%tracer_IC_file, G%Domain)) &
        call LND_error(FATAL, "USER_initialize_tracer: Unable to open "// &
                        CS%tracer_IC_file)
      do m=1,NTR
...
! to set_up_sponge_field can simply be omitted.
    if (.not.associated(sponge_CSp)) &
      call LND_error(FATAL, "USER_initialize_tracer: "// &
