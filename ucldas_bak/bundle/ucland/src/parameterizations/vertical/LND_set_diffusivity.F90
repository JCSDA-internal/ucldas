...
use LND_diag_mediator,       only : post_data, register_diag_field
use LND_debugging,           only : hchksum, uvchksum, Bchksum, hchksum_pair
use LND_EOS,                 only : calculate_density, calculate_density_derivs, EOS_domain
...
use LND_full_convection,     only : full_convection
use LND_grid,                only : ocean_grid_type
use LND_internal_tides,      only : int_tide_CS, get_lowmode_loss
use LND_isopycnal_slopes,    only : vert_fill_TS
use LND_tidal_mixing,        only : tidal_mixing_CS, calculate_tidal_mixing
use LND_tidal_mixing,        only : setup_tidal_diagnostics, post_tidal_diagnostics
use LND_intrinsic_functions, only : invcosh
use LND_io,                  only : slasher, LND_read_data
use LND_kappa_shear,         only : calculate_kappa_shear, kappa_shear_init, Kappa_shear_CS
use LND_kappa_shear,         only : calc_kappa_shear_vertex, kappa_shear_at_vertex
use LND_CVMix_shear,         only : calculate_CVMix_shear, CVMix_shear_init, CVMix_shear_cs
use LND_CVMix_shear,         only : CVMix_shear_end
use LND_CVMix_ddiff,         only : CVMix_ddiff_init, CVMix_ddiff_end, CVMix_ddiff_cs
use LND_CVMix_ddiff,         only : compute_ddiff_coeffs
use LND_bkgnd_mixing,        only : calculate_bkgnd_mixing, bkgnd_mixing_init, bkgnd_mixing_cs
use LND_bkgnd_mixing,        only : bkgnd_mixing_end, sfc_bkgnd_mixing
use LND_open_boundary,       only : ocean_OBC_type, OBC_segment_type, OBC_NONE
use LND_open_boundary,       only : OBC_DIRECTION_E, OBC_DIRECTION_W, OBC_DIRECTION_N, OBC_DIRECTION_S
use LND_string_functions,    only : uppercase
use LND_unit_scaling,        only : unit_scale_type
use LND_variables,           only : thermo_var_ptrs, vertvisc_type, p3d
...
!> This control structure contains parameters for LND_set_diffusivity.
...
!! 4. Tidal mixing: many options available, see LND_tidal_mixing.F90;
...
subroutine set_diffusivity(u, v, h, u_h, v_h, tv, fluxes, optics, visc, dt, &
  ...
  if (showCallTree) call callTree_enter("set_diffusivity(), LND_set_diffusivity.F90")

  if (.not.associated(CS)) call LND_error(FATAL,"set_diffusivity: "//&
  ...
  if ((CS%use_CVMix_ddiff .or. CS%double_diffusion) .and. .not. &
     ...
     call LND_error(FATAL, "set_diffusivity: both visc%Kd_extra_T and "//&
  ...
  if (CS%debug) then
    ...
    if (CS%use_CVMix_ddiff) then
      call hchksum(visc%Kd_extra_T, "LND_set_diffusivity: Kd_extra_T", G%HI, haloshift=0, scale=US%Z2_T_to_m2_s)
      call hchksum(visc%Kd_extra_S, "LND_set_diffusivity: Kd_extra_S", G%HI, haloshift=0, scale=US%Z2_T_to_m2_s)
    endif
...
!! used in LND4.1, and taken from an NCAR technical note (REF?) that updates
...
! This routine adds diffusion sustained by flow energy extracted by bottom drag.
  ...
  ! The turbulence decay scale is 0.5*ustar/f from K&E & LND_vertvisc.F90
...
subroutine set_BBL_TKE(u, v, h, fluxes, visc, G, GV, US, CS, OBC)
  ...
  if (.not.associated(CS)) call LND_error(FATAL,"set_BBL_TKE: "//&
         "Module must be initialized before it is used.")
...
# include "version_variable.h"
  character(len=40)  :: mdl = "LND_set_diffusivity"  ! This module's name.
  real :: omega_frac_dflt
  integer :: i, j, is, ie, js, je
  integer :: isd, ied, jsd, jed

  if (associated(CS)) then
    call LND_error(WARNING, "diabatic_entrain_init called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
  ...
  if (CS%ML_radiation) then
    ...
    if (ML_use_omega) then
      call LND_error(WARNING, "ML_USE_OMEGA is depricated; use ML_OMEGA_FRAC=1.0 instead.")
      omega_frac_dflt = 1.0
    endif
  ...
  if (CS%simple_TKE_to_Kd .and. CS%Kd_max<=0.) call LND_error(FATAL, &
  ...
  if (CS%use_LOTW_BBL_diffusivity .and. CS%Kd_max<=0.) call LND_error(FATAL, &
  ...
  call get_param(param_file, mdl, "DOUBLE_DIFFUSION", CS%double_diffusion, &
                 ...
                 "based on double-diffusive parameterization from LND4/KPP.", &
                 default=.false.)
  ...
  if (CS%double_diffusion) then
    ...
    ! The default molecular viscosity follows the CCSM4.0 and LND4p1 defaults.
  ...
  if (CS%tm_csp%Int_tide_dissipation .and. CS%bkgnd_mixing_csp%Bryan_Lewis_diffusivity) &
    call LND_error(FATAL,"LND_Set_Diffusivity: "// &
...
end module LND_set_diffusivity
