...
use LND_constants,           only : hlv, hlf
use LND_cpu_clock,           only : cpu_clock_id, cpu_clock_begin, cpu_clock_end
use LND_cpu_clock,           only : CLOCK_MODULE
use LND_diag_mediator,       only : post_data, query_averaging_enabled
use LND_diag_mediator,       only : diag_ctrl, safe_alloc_ptr
use LND_domains,             only : pass_var, pass_vector, AGRID, To_South, To_West, To_All
use LND_domains,             only : fill_symmetric_edges, CGRID_NE
use LND_error_handler,       only : LND_error, FATAL, WARNING, LND_mesg, is_root_pe
...
use LND_forcing_type,        only : set_net_mass_forcing, copy_common_forcing_fields
use LND_forcing_type,        only : set_derived_forcing_fields
use LND_forcing_type,        only : forcing_diags, mech_forcing_diags, register_forcing_type_diags
use LND_forcing_type,        only : allocate_forcing_type, deallocate_forcing_type
use LND_forcing_type,        only : allocate_mech_forcing, deallocate_mech_forcing
use LND_grid,                only : ocean_grid_type
use LND_get_input,           only : Get_LND_Input, directories
use LND_io,                  only : file_exists, LND_read_data, LND_read_vector, slasher
use LND_io,                  only : EAST_FACE, NORTH_FACE, num_timelevels
use LND_restart,             only : register_restart_field, restart_init, LND_restart_CS
use LND_restart,             only : restart_init_end, save_restart, restore_state
use LND_time_manager,        only : time_type, operator(+), operator(/), get_time, time_type_to_real
...
!> Structure containing pointers to the forcing fields that may be used to drive LND.
...
!#CTRL#  type(ctrl_forcing_CS), pointer :: ctrl_forcing_CSp => NULL()
  type(LND_restart_CS), pointer :: restart_CSp => NULL() !< A pointer to the restart control structure
...
subroutine set_forcing(sfc_state, forces, fluxes, day_start, day_interval, G, US, CS)
  ...
  call callTree_enter("set_forcing, LND_surface_forcing.F90")
  ...
  if (CS%variable_winds .or. CS%first_call_set_forcing) then
    ...
    elseif (CS%variable_winds .and. .not.CS%first_call_set_forcing) then
      call LND_error(FATAL, &
       "LND_surface_forcing: Variable winds defined with no wind config")
    else
       call LND_error(FATAL, &
       "LND_surface_forcing:Unrecognized wind config "//trim(CS%wind_config))
    endif
  endif

  ! calls to various buoyancy forcing options
  if (CS%restorebuoy .and. .not.CS%variable_buoyforce) then
    call LND_error(FATAL, "With RESTOREBUOY = True, VARIABLE_BUOYFORCE = True should be used. "//&
  ...
  if ((CS%variable_buoyforce .or. CS%first_call_set_forcing) .and. &
    ...
    elseif (trim(CS%buoy_config) == "NONE") then
      call LND_mesg("LND_surface_forcing: buoyancy forcing has been set to omitted.")
    elseif (CS%variable_buoyforce .and. .not.CS%first_call_set_forcing) then
      call LND_error(FATAL, &
       "LND_surface_forcing: Variable buoy defined with no buoy config.")
    else
       call LND_error(FATAL, &
       "LND_surface_forcing: Unrecognized buoy config "//trim(CS%buoy_config))
    endif
  endif
...
subroutine wind_forcing_const(sfc_state, forces, tau_x0, tau_y0, day, G, US, CS)
  ...
  call callTree_enter("wind_forcing_const, LND_surface_forcing.F90")
...
subroutine wind_forcing_2gyre(sfc_state, forces, day, G, US, CS)
  ...
  call callTree_enter("wind_forcing_2gyre, LND_surface_forcing.F90")
  is   = G%isc  ; ie   = G%iec  ; js   = G%jsc  ; je   = G%jec
  Isq  = G%IscB ; Ieq  = G%IecB ; Jsq  = G%JscB ; Jeq  = G%JecB
...
subroutine wind_forcing_1gyre(sfc_state, forces, day, G, US, CS)
  ...
  call callTree_enter("wind_forcing_1gyre, LND_surface_forcing.F90")
  is   = G%isc  ; ie   = G%iec  ; js   = G%jsc  ; je   = G%jec
  Isq  = G%IscB ; Ieq  = G%IecB ; Jsq  = G%JscB ; Jeq  = G%JecB
...
subroutine wind_forcing_gyres(sfc_state, forces, day, G, US, CS)
  ...
  call callTree_enter("wind_forcing_gyres, LND_surface_forcing.F90")
  is   = G%isc  ; ie   = G%iec  ; js   = G%jsc  ; je   = G%jec
  Isq  = G%IscB ; Ieq  = G%IecB ; Jsq  = G%JscB ; Jeq  = G%JecB
...
subroutine wind_forcing_from_file(sfc_state, forces, day, G, US, CS)
  ...
  call callTree_enter("wind_forcing_from_file, LND_surface_forcing.F90")
  is   = G%isc  ; ie   = G%iec  ; js   = G%jsc  ; je   = G%jec
  Isq  = G%IscB ; Ieq  = G%IecB ; Jsq  = G%JscB ; Jeq  = G%JecB
  Pa_conversion = US%kg_m3_to_R*US%m_s_to_L_T**2*US%L_to_Z
...
!           time_lev-1,CS%wind_last_lev-1
    ...
    case ("A")
      ...
      call LND_read_vector(filename, CS%stress_x_var, CS%stress_y_var, &
    ...
    case ("C")
      if (G%symmetric) then
        if (.not.associated(G%Domain_aux)) call LND_error(FATAL, &
    ...
    case default
      call LND_error(FATAL, "wind_forcing_from_file: Unrecognized stagger "//&
                      trim(CS%wind_stagger)//" is not 'A' or 'C'.")
    end select

    if (read_Ustar) then
      call LND_read_data(filename, CS%Ustar_var, forces%ustar(:,:), &
...
subroutine wind_forcing_by_data_override(sfc_state, forces, day, G, US, CS)
  ...
  call callTree_enter("wind_forcing_by_data_override, LND_surface_forcing.F90")
...
subroutine buoyancy_forcing_from_files(sfc_state, fluxes, day, dt, G, US, CS)
  ...
  call callTree_enter("buoyancy_forcing_from_files, LND_surface_forcing.F90")
  ...
  if (time_lev_daily /= CS%buoy_last_lev_read) then
    ...
    call LND_read_data(CS%longwave_file, CS%LW_var, fluxes%lw(:,:), &
                       G%Domain, timelevel=time_lev, scale=US%W_m2_to_QRZ_T)
    if (CS%archaic_OMIP_file) then
      call LND_read_data(CS%longwaveup_file, "lwup_sfc", temp(:,:), G%Domain, &
    ...
    if (CS%archaic_OMIP_file) then
      call LND_read_data(CS%evaporation_file, CS%evap_var, fluxes%evap(:,:), &
    ...
    if (.not.CS%archaic_OMIP_file) then
      call LND_read_data(CS%latentheat_file, CS%latent_var, fluxes%latent(:,:), &
                     G%Domain, timelevel=time_lev, scale=US%W_m2_to_QRZ_T)
      do j=js,je ; do i=is,ie
        fluxes%latent_evap_diag(i,j) = fluxes%latent(i,j)
      enddo ; enddo
    ...
    if (CS%archaic_OMIP_file) then
      call LND_read_data(CS%sensibleheat_file, CS%sens_var, fluxes%sens(:,:), &
                     G%Domain, timelevel=time_lev, scale=-US%W_m2_to_QRZ_T)
    else
      call LND_read_data(CS%sensibleheat_file, CS%sens_var, fluxes%sens(:,:), &
                     G%Domain, timelevel=time_lev, scale=US%W_m2_to_QRZ_T)
    ...
    call LND_read_data(CS%shortwave_file, CS%SW_var, fluxes%sw(:,:), G%Domain, &
                       timelevel=time_lev, scale=US%W_m2_to_QRZ_T)
    if (CS%archaic_OMIP_file) then
      call LND_read_data(CS%shortwaveup_file, "swup_sfc", temp(:,:), G%Domain, &
                         timelevel=time_lev, scale=US%W_m2_to_QRZ_T)
      do j=js,je ; do i=is,ie
    ...
    call LND_read_data(CS%snow_file, CS%snow_var, &
             fluxes%fprec(:,:), G%Domain, timelevel=time_lev, scale=kg_m2_s_conversion)
    call LND_read_data(CS%rain_file, CS%rain_var, &
    ...
    if (CS%archaic_OMIP_file) then
      call LND_read_data(CS%runoff_file, CS%lrunoff_var, temp(:,:), &
                     G%Domain, timelevel=time_lev, scale=kg_m2_s_conversion)
      do j=js,je ; do i=is,ie
        fluxes%lrunoff(i,j) = temp(i,j)*US%m_to_L**2*G%IareaT(i,j)
      enddo ; enddo
      call LND_read_data(CS%runoff_file, CS%frunoff_var, temp(:,:), &
                     G%Domain, timelevel=time_lev, scale=kg_m2_s_conversion)
      do j=js,je ; do i=is,ie
        fluxes%frunoff(i,j) = temp(i,j)*US%m_to_L**2*G%IareaT(i,j)
      enddo ; enddo
    else
      call LND_read_data(CS%runoff_file, CS%lrunoff_var, fluxes%lrunoff(:,:), &
                     G%Domain, timelevel=time_lev, scale=kg_m2_s_conversion)
      call LND_read_data(CS%runoff_file, CS%frunoff_var, fluxes%frunoff(:,:), &
                     G%Domain, timelevel=time_lev, scale=kg_m2_s_conversion)
    endif
...
!     Read the SST and SSS fields for damping.
    if (CS%restorebuoy) then !#CTRL# .or. associated(CS%ctrl_forcing_CSp)) then
      ...
      call LND_read_data(CS%SSTrestore_file, CS%SST_restore_var, &
      ...
      call LND_read_data(CS%salinityrestore_file, CS%SSS_restore_var, &
    ...
    ! mass leaving the ocean has heat_content determined in LND_diabatic_driver.F90
    do j=js,je ; do i=is,ie
  ...
  if (CS%restorebuoy) then
    ...
    if (.not.CS%use_temperature) then
      call LND_error(FATAL, "buoyancy_forcing in LND_surface_forcing: "// &
...
subroutine buoyancy_forcing_from_data_override(sfc_state, fluxes, day, dt, G, US, CS)
  ...
  call callTree_enter("buoyancy_forcing_from_data_override, LND_surface_forcing.F90")
...
!     Read the SST and SSS fields for damping.
  ...
  ! assume liquid runoff enters ocean at SST
  ! assume solid runoff (calving) enters ocean at 0degC
  ! mass leaving ocean has heat_content determined in LND_diabatic_driver.F90
  do j=js,je ; do i=is,ie
    fluxes%evap(i,j)    = fluxes%evap(i,j)    * G%mask2dT(i,j)
    fluxes%lprec(i,j)   = fluxes%lprec(i,j)   * G%mask2dT(i,j)
    fluxes%fprec(i,j)   = fluxes%fprec(i,j)   * G%mask2dT(i,j)
...
subroutine buoyancy_forcing_zero(sfc_state, fluxes, day, dt, G, CS)
  ...
  call callTree_enter("buoyancy_forcing_zero, LND_surface_forcing.F90")
  is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec
...
subroutine buoyancy_forcing_const(sfc_state, fluxes, day, dt, G, US, CS)
  ...
  ! Local variables
  integer :: i, j, is, ie, js, je
  call callTree_enter("buoyancy_forcing_const, LND_surface_forcing.F90")
  is  = G%isc ; ie  = G%iec ; js  = G%jsc ; je  = G%jec
...
subroutine buoyancy_forcing_linear(sfc_state, fluxes, day, dt, G, US, CS)
  ...
  call callTree_enter("buoyancy_forcing_linear, LND_surface_forcing.F90")
  is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec
  ...
  if (CS%restorebuoy) then
    if (CS%use_temperature) then
      ...
      call LND_error(FATAL, "buoyancy_forcing_linear in LND_surface_forcing: "// &
...
# include "version_variable.h"
  ...
  character(len=40)  :: mdl = "LND_surface_forcing" ! This module's name.
  character(len=200) :: filename, gust_file ! The name of the gustiness input file.

  if (associated(CS)) then
    call LND_error(WARNING, "surface_forcing_init called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
  ...
  if (CS%read_gust_2d) then
    ...
    call LND_read_data(filename,'gustiness',CS%gust,G%domain, timelevel=1, &
...
!  All parameter settings are now known.
  ...
  call restart_init(param_file, CS%restart_CSp, "LND_forcing.res")
...
end module LND_surface_forcing
