...
use LND_coms,           only : max_across_PEs
use LND_debugging,      only : hchksum, is_NaN
use LND_diag_mediator,  only : time_type, diag_ctrl, safe_alloc_ptr, post_data
use LND_diag_mediator,  only : query_averaging_enabled, register_diag_field
...
type, public :: KPP_CS ; private
  ...
  integer :: NLT_shape                 !< UCLAND over-ride of CVMix NLT shape function
...
# include "version_variable.h"
  character(len=40) :: mdl = 'LND_CVMix_KPP' !< name of this module
  ...
  if (associated(CS)) call LND_error(FATAL, 'LND_CVMix_KPP, KPP_init: '// &
  ...
  call log_version(paramFile, mdl, version, 'This is the LND wrapper to CVMix:KPP\n' // &
  ...
  if (CS%n_smooth > G%domain%nihalo) then
    call LND_error(FATAL,'KPP smoothing number (N_SMOOTH) cannot be greater than NIHALO.')
  elseif (CS%n_smooth > G%domain%njhalo) then
    call LND_error(FATAL,'KPP smoothing number (N_SMOOTH) cannot be greater than NJHALO.')
  endif
...
! smg: for removal below
  ...
  if (CS%correctSurfLayerAvg) &
    call LND_error(FATAL,'Correct surface layer average disabled in code.  To recover \n'// &
...
! smg: for removal above
  ...
  call get_param(paramFile, mdl, 'NLT_SHAPE', string, &
                 'UCLAND method to set nonlocal transport profile. '//                          &
  ...
  select case ( trim(string) )
    ...
    case default ; call LND_error(FATAL,"KPP_init: "// &
  ...
  if (CS%MatchTechnique == 'MatchBoth' .and. (CS%interpType2 == 'cubic' .or. &
    ...
    call LND_error(FATAL,"If MATCH_TECHNIQUE=MatchBoth, INTERP_TYPE2 must be set to \n"//&
  ...
  call get_param(paramFile, mdl, 'CVMix_ZERO_H_WORK_AROUND', CS%min_thickness,                           &
                 ...
                 'of vanished layers. This is independent of MIN_THICKNESS used in other parts of LND.', &
                 units='m', default=0.)
...
! Local variables
  ...
  do j = G%jsc, G%jec
    do i = G%isc, G%iec
      ...
      ! If option "MatchBoth" is selected in CVMix, LND should be capable of matching.
      ...
      do k=1, G%ke+1
        if (Kviscosity(k) < 0. .or. Kdiffusivity(k,1) < 0.) then
          call LND_error(FATAL,"KPP_calculate, after CVMix_coeffs_kpp: "// &
      ...
      IF (CS%LT_K_ENHANCEMENT) then
        ...
        elseif (CS%LT_K_METHOD==LT_K_MODE_RW16) then
          ...
          LangEnhK = min(2.25, 1. + 1./CS%La_SL(i,j))
           ...
           !call LND_error(WARNING,"Unexpected behavior in LND_CVMix_KPP, see error in LT_K_ENHANCEMENT")
           LangEnhK = 1.0
        endif
        do k=1,G%ke
      ...
      ! The CVMix code has yet to update for thse options, so we compute in UCLAND.
      ...
      ! UCLAND recommended shape is the parabolic; it gives deeper boundary layer
...
subroutine KPP_compute_BLD(CS, G, GV, US, h, Temp, Salt, u, v, tv, uStar, buoyFlux, Waves)
  ...
  do j = G%jsc, G%jec
    do i = G%isc, G%iec
      ...
      ! on the OBLdepth calculation. It follows that used in LND5
      ! and POP.
      iFaceHeight(1) = 0.0 ! BBL is all relative to the surface
      ...
      do k=1,G%ke
        ...
        if (CS%Stokes_Mixing) then
          ! If lndentum is mixed down the Stokes drift gradient, then
      ...
      IF (CS%LT_VT2_ENHANCEMENT) then
        ...
        elseif (CS%LT_VT2_METHOD==LT_VT2_MODE_LF17) then
          ...
          enddo
           ...
           !This shouldn't be reached.
           !call LND_error(WARNING,"Unexpected behavior in LND_CVMix_KPP, see error in Vt2")
           LangEnhVT2(:) = 1.0
        endif
      else
        LangEnhVT2(:) = 1.0
...
!! used by NCAR. It has been found in idealized UCLAND tests to not be necessary.
!!
!! \sa
!! kpp_calculate(), kpp_applynonlocaltransport()
end module LND_CVMix_KPP
