...
use LND_domains,               only : pass_var, CORNER, pass_vector, AGRID, BGRID_NE
...
use LND_lateral_mixing_coeffs, only : VarMix_CS, calc_QG_Leith_viscosity
use LND_barotropic,            only : barotropic_CS, barotropic_get_tav
use LND_thickness_diffuse,     only : thickness_diffuse_CS, thickness_diffuse_get_KH
use LND_io,                    only : LND_read_data, slasher
use LND_MEKE_types,            only : MEKE_type
use LND_open_boundary,         only : ocean_OBC_type, OBC_DIRECTION_E, OBC_DIRECTION_W
use LND_open_boundary,         only : OBC_DIRECTION_N, OBC_DIRECTION_S, OBC_NONE
...
subroutine horizontal_viscosity(u, v, h, diffu, diffv, MEKE, VarMix, G, GV, US, &
  ...
  if (.not.associated(CS)) call LND_error(FATAL, &
         "LND_hor_visc: Module must be initialized before it is used.")
  ...
  if (associated(MEKE)) then
    if (associated(MEKE%lnd_src)) find_FrictWork = .true.
    backscat_subround = 0.0
    if (find_FrictWork .and. associated(MEKE%lnd_src) .and. (MEKE%backscatter_Ro_c > 0.0) .and. &
  ...
  if (associated(VarMix)) then
    ...
    (.not.associated(VarMix%Res_fn_h) .or. .not.associated(VarMix%Res_fn_q))) &
      call LND_error(FATAL, "LND_hor_visc: VarMix%Res_fn_h and " //&
  ...
  do k=1,nz
    ...
    if (find_FrictWork .and. associated(MEKE)) then ; if (associated(MEKE%lnd_src)) then
      if (k==1) then
        do j=js,je ; do i=is,ie
          MEKE%lnd_src(i,j) = 0.
        enddo ; enddo
      ...
      if (MEKE%backscatter_Ro_c /= 0.) then
        do j=js,je ; do i=is,ie
          ...
          MEKE%lnd_src(i,j) = MEKE%lnd_src(i,j) + GV%H_to_RZ * ( &
      ...
      do j=js,je ; do i=is,ie
        MEKE%lnd_src(i,j) = MEKE%lnd_src(i,j) + FrictWork(i,j,k)
      enddo ; enddo
    ...
    endif ; endif ! find_FrictWork and associated(lnd_src)
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_hor_visc"  ! module name
  is   = G%isc  ; ie   = G%iec  ; js   = G%jsc  ; je   = G%jec ; nz = G%ke
  Isq  = G%IscB ; Ieq  = G%IecB ; Jsq  = G%JscB ; Jeq  = G%JecB
  isd  = G%isd  ; ied  = G%ied  ; jsd  = G%jsd  ; jed  = G%jed
  IsdB = G%IsdB ; IedB = G%IedB ; JsdB = G%JsdB ; JedB = G%JedB
  if (associated(CS)) then
    call LND_error(WARNING, "hor_visc_init called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
  ...
  if (CS%Laplacian .or. get_all) then
    ...
    if (CS%Leith_Kh .or. get_all) then
      ...
      call get_param(param_file, mdl, "USE_QG_LEITH_VISC", CS%use_QG_Leith_visc, &
                 ...
                 default=.false.)
      if (CS%use_QG_Leith_visc .and. .not. CS%Leith_Kh) call LND_error(FATAL, &
  ...
  if (CS%use_GME) then
    call get_param(param_file, mdl, "SPLIT", split, &
                 "Use the split time stepping if true.", default=.true., &
                  do_not_log=.true.)
    if (.not. split) call LND_error(FATAL,"ERROR: Currently, USE_GME = True "// &
  ...
  if (CS%no_slip .and. CS%biharmonic) &
    call LND_error(FATAL,"ERROR: NOSLIP and BIHARMONIC cannot be defined "// &
                         "at the same time in LND.")
  if (.not.(CS%Laplacian .or. CS%biharmonic)) then
    ! Only issue inviscid warning if not in single column mode (usually 2x2 domain)
    if ( max(G%domain%niglobal, G%domain%njglobal)>2 ) call LND_error(WARNING, &
  ...
  if (CS%anisotropic) then
    ...
    select case (aniso_mode)
      ...
      case default
        call LND_error(FATAL, "LND_hor_visc: "//&
  ...
  if (CS%use_Kh_bg_2d) then
    ...
    call LND_read_data(trim(inputdir)//trim(filename), 'Kh', CS%Kh_bg_2d, &
  ...
  enddo ; enddo
  if (CS%Laplacian) then
   ! The 0.3 below was 0.4 in LND1.10.  The change in hq requires
  ...
  if (CS%biharmonic) then
    CS%id_Ah_h = register_diag_field('ocean_model', 'Ahh', diag%axesTL, Time,    &
        ...
        cmor_standard_name='ocean_lndentum_xy_biharmonic_diffusivity')
  ...
  if (CS%Laplacian) then
    CS%id_Kh_h = register_diag_field('ocean_model', 'Khh', diag%axesTL, Time,   &
        ...
        cmor_standard_name='ocean_lndentum_xy_laplacian_diffusivity')
...
!!
!! \section section_horizontal_viscosity Horizontal viscosity in LND
!!
