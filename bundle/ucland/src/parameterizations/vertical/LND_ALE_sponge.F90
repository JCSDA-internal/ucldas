...
use LND_time_manager, only : time_type, init_external_field, get_external_field_size, time_interp_external_init
use LND_remapping, only : remapping_cs, remapping_core_h, initialize_remapping
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "initialize_ALE_sponge_fixed called with an associated "// &
                            "control structure.")
    return
  endif
...
subroutine get_ALE_sponge_thicknesses(G, data_h, sponge_mask, CS)
  ...
  if (allocated(data_h)) call LND_error(FATAL, &
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "initialize_ALE_sponge_varying called with an associated "// &
                            "control structure.")
    return
  endif
! Set default, read and log parameters
...
subroutine set_up_ALE_sponge_field_fixed(sp_val, G, f_ptr, CS)
  ...
  if (CS%fldno > MAX_FIELDS_) then
    ...
    call LND_error(FATAL,"set_up_ALE_sponge_field: "//mesg)
  endif
...
subroutine apply_ALE_sponge(h, dt, G, GV, US, CS, Time)
  ...
  if (CS%time_varying_sponges) then
    if (.not. present(Time)) &
      call LND_error(FATAL,"apply_ALE_sponge: No time information provided")
    do m=1,CS%fldno
...
subroutine rotate_ALE_sponge(sponge_in, G_in, sponge, G, turns, param_file)
  ...
  if (associated(sponge_in%var_u%p) .or. associated(sponge_in%var_v%p)) &
    call LND_error(FATAL, "Rotation of ALE sponge velocities is not yet " &
      // "implemented.")
