...
use LND_EOS_linear, only : calculate_density_linear, calculate_spec_vol_linear
use LND_EOS_linear, only : calculate_density_derivs_linear
use LND_EOS_linear, only : calculate_specvol_derivs_linear, int_density_dz_linear
use LND_EOS_linear, only : calculate_density_second_derivs_linear
use LND_EOS_linear, only : calculate_compress_linear, int_spec_vol_dp_linear
use LND_EOS_Wright, only : calculate_density_wright, calculate_spec_vol_wright
use LND_EOS_Wright, only : calculate_density_derivs_wright
use LND_EOS_Wright, only : calculate_specvol_derivs_wright, int_density_dz_wright
use LND_EOS_Wright, only : calculate_compress_wright, int_spec_vol_dp_wright
use LND_EOS_Wright, only : calculate_density_second_derivs_wright
use LND_EOS_UNESCO, only : calculate_density_unesco, calculate_spec_vol_unesco
use LND_EOS_UNESCO, only : calculate_density_derivs_unesco, calculate_density_unesco
use LND_EOS_UNESCO, only : calculate_compress_unesco
use LND_EOS_NEMO,   only : calculate_density_nemo
use LND_EOS_NEMO,   only : calculate_density_derivs_nemo, calculate_density_nemo
use LND_EOS_NEMO,   only : calculate_compress_nemo
use LND_EOS_TEOS10, only : calculate_density_teos10, calculate_spec_vol_teos10
use LND_EOS_TEOS10, only : calculate_density_derivs_teos10
use LND_EOS_TEOS10, only : calculate_specvol_derivs_teos10
use LND_EOS_TEOS10, only : calculate_density_second_derivs_teos10
use LND_EOS_TEOS10, only : calculate_compress_teos10
use LND_EOS_TEOS10, only : gsw_sp_from_sr, gsw_pt_from_ct
use LND_TFreeze,    only : calculate_TFreeze_linear, calculate_TFreeze_Millero
use LND_TFreeze,    only : calculate_TFreeze_teos10
...
subroutine calculate_density_scalar(T, S, pressure, rho, EOS, rho_ref, scale)
  ...
  if (.not.associated(EOS)) call LND_error(FATAL, &
  ...
  select case (EOS%form_of_EOS)
    ...
    case default
      call LND_error(FATAL, "calculate_density_scalar: EOS is not valid.")
  end select
  ...
  select case (EOS%form_of_EOS)
    ...
    case (EOS_TEOS10)
      ...
      call calculate_density_second_derivs_teos10(T, S, pressure, d2RdSS, d2RdST, &
                                                  d2RdTT, d2RdSp, d2RdTP)
    case default
      call LND_error(FATAL, "calculate_stanley_density_scalar: EOS is not valid.")
  end select
...
subroutine calculate_density_array(T, S, pressure, rho, start, npts, EOS, rho_ref, scale)
  ...
  select case (EOS%form_of_EOS)
    ...
    case default
      call LND_error(FATAL, "calculate_density_array: EOS%form_of_EOS is not valid.")
  end select
  ...
  select case (EOS%form_of_EOS)
    ...
    case (EOS_TEOS10)
      ...
      call calculate_density_second_derivs_teos10(T, S, pressure, d2RdSS, d2RdST, &
                                                  d2RdTT, d2RdSp, d2RdTP, start, npts)
    case default
      call LND_error(FATAL, "calculate_stanley_density_array: EOS%form_of_EOS is not valid.")
  end select
...
subroutine calculate_spec_vol_array(T, S, pressure, specvol, start, npts, EOS, spv_ref, scale)
  ...
  select case (EOS%form_of_EOS)
    ...
    case default
      call LND_error(FATAL, "calculate_spec_vol_array: EOS%form_of_EOS is not valid.")
  end select
...
subroutine calculate_TFreeze_scalar(S, pressure, T_fr, EOS, pres_scale)
  ...
  select case (EOS%form_of_TFreeze)
    ...
    case default
      call LND_error(FATAL, "calculate_TFreeze_scalar: form_of_TFreeze is not valid.")
  end select
...
subroutine calculate_density_derivs_array(T, S, pressure, drho_dT, drho_dS, start, npts, EOS, scale)
  ...
  select case (EOS%form_of_EOS)
    ...
    case default
      call LND_error(FATAL, "calculate_density_derivs_array: EOS%form_of_EOS is not valid.")
  end select
...
subroutine calculate_density_derivs_scalar(T, S, pressure, drho_dT, drho_dS, EOS, scale)
  ...
  select case (EOS%form_of_EOS)
    ...
    case default
      call LND_error(FATAL, "calculate_density_derivs_scalar: EOS%form_of_EOS is not valid.")
  end select
    ...
    select case (EOS%form_of_EOS)
      ...
      case (EOS_TEOS10)
        call calculate_density_second_derivs_teos10(T, S, pressure, drho_dS_dS, drho_dS_dT, &
                                                    drho_dT_dT, drho_dS_dP, drho_dT_dP, start, npts)
      case default
        call LND_error(FATAL, "calculate_density_derivs: EOS%form_of_EOS is not valid.")
    end select
  else
    do j=start,start+npts-1 ; pres(j) = p_scale * pressure(j) ; enddo
    select case (EOS%form_of_EOS)
...
subroutine calculate_spec_vol_derivs_array(T, S, pressure, dSV_dT, dSV_dS, start, npts, EOS)
  ...
  select case (EOS%form_of_EOS)
    ...
    case (EOS_NEMO)
      ...
      enddo
    case default
      call LND_error(FATAL, "calculate_spec_vol_derivs_array: EOS%form_of_EOS is not valid.")
  end select
...
subroutine calculate_compress_array(T, S, press, rho, drho_dp, start, npts, EOS)
  ...
  select case (EOS%form_of_EOS)
    ...
    case default
      call LND_error(FATAL, "calculate_compress: EOS%form_of_EOS is not valid.")
  end select
...
subroutine analytic_int_specific_vol_dp(T, S, p_t, p_b, alpha_ref, HI, EOS, &
  ...
  if (EOS%EOS_quadrature) call LND_error(FATAL, "EOS_quadrature is set!")
  ...
  select case (EOS%form_of_EOS)
    ...
    case default
      call LND_error(FATAL, "No analytic integration option is available with this EOS!")
  end select
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_EOS" ! This module's name.
  ...
  select case (uppercase(tmpstr))
    ...
    case default
      call LND_error(FATAL, "interpret_eos_selection: EQN_OF_STATE "//&
                              trim(tmpstr) // "in input file is invalid.")
  end select
  call LND_mesg('interpret_eos_selection: equation of state set to "' // &
                trim(tmpstr)//'"', 5)
  ...
  select case (uppercase(tmpstr))
    ...
    case default
      call LND_error(FATAL, "interpret_eos_selection:  TFREEZE_FORM "//&
                              trim(tmpstr) // "in input file is invalid.")
  end select
  ...
  if ((EOS%form_of_EOS == EOS_TEOS10 .OR. EOS%form_of_EOS == EOS_NEMO) .AND. &
      ...
      call LND_error(FATAL, "interpret_eos_selection:  EOS_TEOS10 or EOS_NEMO \n" //&
...
subroutine EOS_use_linear(Rho_T0_S0, dRho_dT, dRho_dS, EOS, use_quadrature)
  ...
  if (.not.associated(EOS)) call LND_error(FATAL, &
    "LND_EOS.F90: EOS_use_linear() called with an unassociated EOS_type EOS.")
...
!> \namespace lnd_eos
!!
!! The LND_EOS module is a wrapper for various equations of state (e.g. Linear,
