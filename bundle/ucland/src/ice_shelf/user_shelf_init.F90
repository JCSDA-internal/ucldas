...
!! UCLAND ice shelf, using user-provided code.
...
use LND_time_manager, only : time_type, set_time, time_type_to_real
use LND_unit_scaling, only : unit_scale_type
! use LND_io, only : close_file, fieldtype, file_exists
! use LND_io, only : open_file, read_data, read_axis_data, SINGLE_FILE
! use LND_io, only : write_field, slasher
...
! This subroutine sets up the initial mass and area covered by the ice shelf.
  ...
  ! call LND_error(FATAL, "USER_shelf_init.F90, USER_set_shelf_mass: " // &
...
subroutine write_user_log(param_file)
  ...
  character(len=128) :: tagname = '$Name: LND_ogrp $'
