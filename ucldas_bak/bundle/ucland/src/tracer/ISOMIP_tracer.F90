...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "ISOMIP_register_tracer called with an "// &
  ...
  if (.not.restart) then
    if (len_trim(CS%tracer_IC_file) >= 1) then
      ...
      if (.not.file_exists(CS%tracer_IC_file, G%Domain)) &
        call LND_error(FATAL, "ISOMIP_initialize_tracer: Unable to open "// &
                        CS%tracer_IC_file)
      do m=1,NTR
...
!      call LND_error(FATAL, "ISOMIP_initialize_tracer: "// &
