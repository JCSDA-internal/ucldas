...
use LND_array_transform,      only : allocate_rotated_array
use LND_coms,                 only : sum_across_PEs
use LND_cpu_clock,            only : cpu_clock_id, cpu_clock_begin, cpu_clock_end, CLOCK_ROUTINE
use LND_diag_mediator,        only : diag_ctrl, time_type
use LND_domains,              only : pass_var, pass_vector
use LND_domains,              only : To_All, SCALAR_PAIR, CGRID_NE, CORNER
use LND_error_handler,        only : LND_mesg, LND_error, FATAL, WARNING, is_root_pe
use LND_error_handler,        only : NOTE
use LND_file_parser,          only : get_param, log_version, param_file_type, log_param
use LND_grid,                 only : ocean_grid_type, hor_index_type
use LND_dyn_horgrid,          only : dyn_horgrid_type
use LND_io,                   only : EAST_FACE, NORTH_FACE
use LND_io,                   only : slasher, read_data, field_size, SINGLE_FILE
use LND_io,                   only : vardesc, query_vardesc, var_desc
use LND_restart,              only : register_restart_field, register_restart_pair
use LND_restart,              only : query_initialized, LND_restart_CS
use LND_obsolete_params,      only : obsolete_logical, obsolete_int, obsolete_real, obsolete_char
use LND_string_functions,     only : extract_word, remove_spaces
use LND_time_manager,         only : time_type, time_type_to_real, operator(-)
use LND_tracer_registry,      only : tracer_type, tracer_registry_type, tracer_name_lookup
use time_interp_external_mod, only : init_external_field, time_interp_external
use time_interp_external_mod, only : time_interp_external_init
use LND_remapping,            only : remappingSchemesDoc, remappingDefaultScheme, remapping_CS
use LND_remapping,            only : initialize_remapping, remapping_core_h, end_remapping
use LND_regridding,           only : regridding_CS
...
character(len=40)  :: mdl = "LND_open_boundary" !< This module's name.
...
!> This routine is called from LND_initialize_fixed which
...
subroutine open_boundary_config(G, US, param_file, OBC)
  ...
  if (OBC%number_of_segments > 0) then
    ...
    if ((OBC%zero_vorticity .and. OBC%freeslip_vorticity) .or.  &
        ...
        (OBC%computed_vorticity .and. OBC%specified_vorticity))  &
         call LND_error(FATAL, "LND_open_boundary.F90, open_boundary_config:\n"//&
 ...
 !   call initialize_segment_data(G, OBC, param_file)
  ...
  if ((OBC%open_u_BCs_exist_globally .or. OBC%open_v_BCs_exist_globally) .and. &
       .not.G%symmetric ) call LND_error(FATAL, &
       "LND_open_boundary, open_boundary_config: "//&
...
subroutine initialize_segment_data(G, OBC, PF)
  ...
  ! param file so that I can use it later in step_LND in order to finish
  ...
  do n=1, OBC%number_of_segments
    ...
    ! LND_parameter_docs in circle_obcs on two processes.
!   call get_param(PF, mdl, segnam, segstr, 'xyz')
    ...
    if (num_fields == 0) then
      call LND_mesg('initialize_segment_data: num_fields = 0')
...
!       if (siz(4) == 1) segment%values_needed = .false.
        if (segment%on_pe) then
          if (OBC%brushcutter_mode .and. (modulo(siz(1),2) == 0 .or. modulo(siz(2),2) == 0)) then
            call LND_error(FATAL,'segment data are not on the supergrid')
          endif
          siz2(1)=1
...
subroutine setup_u_point_obc(OBC, G, US, segment_str, l_seg, PF, reentrant_y)
  ...
  do a_loop = 1,8 ! up to 8 options available
    ...
    elseif (trim(action_str(a_loop)) == 'SIMPLE_GRAD') then
      ...
      call LND_error(FATAL, "LND_open_boundary.F90, setup_u_point_obc: "//&
...
subroutine setup_v_point_obc(OBC, G, US, segment_str, l_seg, PF, reentrant_x)
  ...
  do a_loop = 1,8
    ...
    elseif (trim(action_str(a_loop)) == 'SIMPLE_GRAD') then
      ...
      call LND_error(FATAL, "LND_open_boundary.F90, setup_v_point_obc: "//&
                     "String '"//trim(action_str(a_loop))//"' not understood.")
    endif
    if (OBC%segment(l_seg)%nudged .or. OBC%segment(l_seg)%nudged_tan) then
      write(segment_param_str(1:43),"('OBC_SEGMENT_',i3.3,'_VELOCITY_NUDGING_TIMESCALES')") l_seg
...
subroutine parse_segment_str(ni_global, nj_global, segment_str, l, m, n, action_str, reentrant)
  ...
  if (word1(1:2)=='I=') then
    l_max = ni_global
    mn_max = nj_global
    if (.not. (word2(1:2)=='J=')) call LND_error(FATAL, "LND_open_boundary.F90, parse_segment_str: "//&
  ...
  elseif (word1(1:2)=='J=') then ! Note that the file_parser uniformaly expands "=" to " = "
    l_max = nj_global
    mn_max = ni_global
    if (.not. (word2(1:2)=='I=')) call LND_error(FATAL, "LND_open_boundary.F90, parse_segment_str: "//&
                     "Second word of string '"//trim(segment_str)//"' must start with 'I='.")
  else
    call LND_error(FATAL, "LND_open_boundary.F90, parse_segment_str"//&
  ...
  integer function interpret_int_expr(string, imax)
    ...
    if (slen==0) call LND_error(FATAL, "LND_open_boundary.F90, parse_segment_str"//&
    ...
    return
    911 call LND_error(FATAL, "LND_open_boundary.F90, parse_segment_str"//&
...
subroutine parse_segment_data_str(segment_str, idx, var, value, filename, fieldname)
  ...
  return
986 call LND_error(FATAL,'End of record while parsing segment data specification! '//trim(segment_str))
987 call LND_error(FATAL,'Error while parsing segment data specification! '//trim(segment_str))
...
!    endif
   ...
   return
 986 call LND_error(FATAL,'End of record while parsing segment data specification! '//trim(segment_str))
 987 call LND_error(FATAL,'Error while parsing segment parameter specification! '//trim(segment_str))
...
subroutine open_boundary_init(G, GV, US, param_file, OBC, restart_CSp)
  ...
  type(param_file_type),   intent(in) :: param_file !< Parameter file handle
  type(ocean_OBC_type),    pointer    :: OBC !< Open boundary control structure
  type(LND_restart_CS),    pointer    :: restart_CSp !< Restart structure, data intent(inout)
    ...
    do m = 1,segment%num_fields
      if (segment%field(m)%fid > 0) then
        ...
        if (.not.associated(segment%field(m)%buffer_dst)) then
          if (siz(3) /= segment%field(m)%nk_src) call LND_error(FATAL,'nk_src inconsistency')
...
subroutine update_OBC_ramp(Time, OBC, activate)
  ...
  call LND_error(NOTE, "LND_open_boundary: update_OBC_ramp set OBC"// &
                       " ramp to "//trim(msg))
...
subroutine register_OBC(name, param_file, Reg)
  ...
  if (Reg%nobc>=MAX_FIELDS_) then
    write(mesg,'("Increase MAX_FIELDS_ in LND_memory.h to at least ",I3," to allow for &
        &all the open boundaries being registered via register_OBC.")') Reg%nobc+1
    call LND_error(FATAL,"LND register_tracer: "//mesg)
  endif
  Reg%nobc = Reg%nobc + 1
  nobc     = Reg%nobc
  ...
  if (Reg%locked) call LND_error(FATAL, &
      "LND register_tracer was called for variable "//trim(Reg%OB(nobc)%name)//&
...
! call log_version(param_file, mdl,s version, "")
  ...
  if (init_calls > 1) then
    ...
    if (is_root_pe()) call LND_error(WARNING,"LND_open_boundary"//mesg)
  endif
...
function register_file_OBC(param_file, CS, OBC_Reg)
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "register_file_OBC called with an "// &
...
!     &" times with different registry pointers.")') init_calls
!   if (is_root_pe()) call LND_error(WARNING,"LND_tracer"//mesg)
! endif
...
! Local variables
  ...
  if (segment%tr_Reg%locked) call LND_error(FATAL, &
      "LND register_tracer was called for variable "//trim(segment%tr_Reg%Tr(ntseg)%name)//&
      " with a locked tracer registry.")
...
! Local variables
  ...
  do n=1, OBC%number_of_segments
    ...
    if (associated(segment%tr_Reg)) &
         call LND_error(FATAL,"register_temp_salt_segments: tracer array was previously allocated")
...
subroutine mask_outside_OBCs(G, US, param_file, OBC)
  ...
  do j=G%jsd,G%jed ; do i=G%isd,G%ied
    if (color(i,j) /= color2(i,j)) then
      fatal_error = .True.
      write(mesg,'("LND_open_boundary: problem with OBC segments specification at ",I5,",",I5," during\n", &
          "the masking of the outside grid points.")') i, j
      call LND_error(WARNING,"LND register_tracer: "//mesg, all_print=.true.)
    endif
    if (color(i,j) == cout) G%bathyT(i,j) = min_depth
  enddo ; enddo
  if (fatal_error) call LND_error(FATAL, &
      "LND_open_boundary: inconsistent OBC segments.")
...
subroutine open_boundary_register_restarts(HI, GV, OBC, Reg, param_file, restart_CSp, &
  ...
  if (.not. associated(OBC)) &
       call LND_error(FATAL, "open_boundary_register_restarts: Called with "//&
  ...
  if (associated(OBC%rx_normal) .or. associated(OBC%ry_normal) .or. &
      associated(OBC%rx_oblique) .or. associated(OBC%ry_oblique) .or. associated(OBC%cff_normal)) &
       call LND_error(FATAL, "open_boundary_register_restarts: Restart "//&
...
!        call LND_error(FATAL, "open_boundary_regiser_restarts: Inconsistent value for ntr")
      write(mesg,'("Inconsistent values for ntr ", I8," and ",I8,".")') OBC%ntr, Reg%ntr
      call LND_error(WARNING, 'open_boundary_register_restarts: '//mesg)
    endif
  endif
...
subroutine rotate_OBC_init(OBC_in, G, GV, US, param_file, tv, restart_CSp, OBC)
  ...
  type(LND_restart_CS), pointer, intent(in) :: restart_CSp  !< Restart CS
  type(ocean_OBC_type), pointer, intent(inout) :: OBC   !< Rotated OBC
