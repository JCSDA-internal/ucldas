...
subroutine create_dyn_horgrid(G, HI, bathymetry_at_vel)
  ...
  if (associated(G)) then
    call LND_error(WARNING, "create_dyn_horgrid called with an associated horgrid_type.")
  else
    allocate(G)
  endif
...
subroutine destroy_dyn_horgrid(G)
  ...
  if (.not.associated(G)) then
    call LND_error(FATAL, "destroy_dyn_horgrid called with an unassociated horgrid_type.")
  endif
