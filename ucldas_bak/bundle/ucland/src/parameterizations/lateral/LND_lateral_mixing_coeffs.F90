...
use LND_diag_mediator,     only : register_diag_field, safe_alloc_ptr, post_data
use LND_diag_mediator,     only : diag_ctrl, time_type, query_averaging_enabled
use LND_domains,           only : create_group_pass, do_group_pass
use LND_domains,           only : group_pass_type, pass_var, pass_vector
use LND_file_parser,       only : get_param, log_version, param_file_type
use LND_interface_heights, only : find_eta
use LND_isopycnal_slopes,  only : calc_isoneutral_slopes
...
use LND_wave_speed,        only : wave_speed, wave_speed_CS, wave_speed_init
use LND_open_boundary,     only : ocean_OBC_type, OBC_NONE
...
subroutine calc_depth_function(G, CS)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "calc_depth_function:"// &
         "Module must be initialized before it is used.")
  if (.not. CS%calculate_depth_fns) return
  if (.not. associated(CS%Depth_fn_u)) call LND_error(FATAL, &
    "calc_depth_function: %Depth_fn_u is not associated with Depth_scaled_KhTh.")
  if (.not. associated(CS%Depth_fn_v)) call LND_error(FATAL, &
...
subroutine calc_resoln_function(h, tv, G, GV, US, CS)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "calc_resoln_function:"// &
         "Module must be initialized before it is used.")
  if (CS%calculate_cg1) then
    if (.not. associated(CS%cg1)) call LND_error(FATAL, &
      "calc_resoln_function: %cg1 is not associated with Resoln_scaled_Kh.")
    if (CS%khth_use_ebt_struct) then
      if (.not. associated(CS%ebt_struct)) call LND_error(FATAL, &
  ...
  if (CS%calculate_rd_dx) then
    if (.not. associated(CS%Rd_dx_h)) call LND_error(FATAL, &
...
subroutine calc_slope_functions(h, tv, dt, G, GV, US, CS, OBC)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "LND_lateral_mixing_coeffs.F90, calc_slope_functions:"//&
         "Module must be initialized before it is used.")
...
subroutine calc_Visbeck_coeffs(h, slope_x, slope_y, N2_u, N2_v, G, GV, US, CS, OBC)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "calc_slope_function:"// &
         "Module must be initialized before it is used.")
  if (.not. CS%calculate_Eady_growth_rate) return
  if (.not. associated(CS%SN_u)) call LND_error(FATAL, "calc_slope_function:"// &
         "%SN_u is not associated with use_variable_mixing.")
  if (.not. associated(CS%SN_v)) call LND_error(FATAL, "calc_slope_function:"// &
...
# include "version_variable.h"
  character(len=40)  :: mdl = "LND_lateral_mixing_coeffs" ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "VarMix_init called with an associated "// &
                             "control structure.")
    return
  endif
  ...
  if (CS%Resoln_scaled_Kh .or. CS%Resoln_scaled_KhTh .or. CS%Resoln_scaled_KhTr) then
    ...
    if (CS%interpolate_Res_fn) then
      if (CS%Res_coef_visc /= CS%Res_coef_khth) call LND_error(FATAL, &
           "LND_lateral_mixing_coeffs.F90, VarMix_init:"//&
           "When INTERPOLATE_RES_FN=True, VISC_RES_FN_POWER must equal KH_RES_SCALE_COEF.")
      if (CS%Res_fn_power_visc /= CS%Res_fn_power_khth) call LND_error(FATAL, &
           "LND_lateral_mixing_coeffs.F90, VarMix_init:"//&
  ...
  if (CS%Use_QG_Leith_GM) then
    ...
    if (.not. CS%use_stored_slopes) call LND_error(FATAL, &
           "LND_lateral_mixing_coeffs.F90, VarMix_init:"//&
...
!> \namespace lnd_lateral_mixing_coeffs
!!
...
!! \f]
!!
!! The resolution function can be applied independently to thickness diffusion (module lnd_thickness_diffuse),
!! tracer diffusion (lnd_tracer_hordiff) lateral viscosity (lnd_hor_visc).
!!
...
!! scheme.  The factors are combined in lnd_thickness_diffuse::thickness_diffuse() but calculated in this module.
!!
!! \f[
...
!! | ------                | --------------- |
!! | -                     | <code>USE_VARIABLE_MIXING</code> |
!! | \f$ \alpha_s \f$      | <code>KHTH_SLOPE_CFF</code> (for lnd_thickness_diffuse module)|
!! | \f$ \alpha_s \f$      | <code>KHTR_SLOPE_CFF</code> (for lnd_tracer_hordiff module)|
...
!! calculated using subroutines in lnd_wave_speed.
!!
!! | Symbol                | Module parameter |
!! | ------                | --------------- |
!! | -                     | <code>KHTH_USE_EBT_STRUCT</code> |

end module LND_lateral_mixing_coeffs
