...
subroutine solve_linear_system( A, R, X, N, answers_2018 )
  ...
  logical :: old_answers  ! If true, use expressions that give the original (2008 through 2018) UCLAND answers
  integer :: i, j, k
  ...
  do i = 1,N-1
    ...
    if ( .NOT. found_pivot ) then
      write(0,*) ' A=',A
      call LND_error( FATAL, 'The linear system is singular !' )
    endif
...
subroutine solve_tridiagonal_system( Al, Ad, Au, R, X, N, answers_2018 )
  ...
  if (old_answers) then
    ! This version gives the same answers as the original (2008 through 2018) UCLAND code
