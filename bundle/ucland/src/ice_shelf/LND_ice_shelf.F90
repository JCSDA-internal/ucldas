...
!!  ice shelf dynamics, all using the LND framework and coding style.
module LND_ice_shelf

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_constants, only : hlf
use LND_cpu_clock, only : cpu_clock_id, cpu_clock_begin, cpu_clock_end
use LND_cpu_clock, only : CLOCK_COMPONENT, CLOCK_ROUTINE
use LND_diag_mediator, only : post_data, register_diag_field, safe_alloc_ptr
use LND_diag_mediator, only : diag_mediator_init, set_diag_mediator_grid, diag_ctrl, time_type
use LND_diag_mediator, only : enable_averages, enable_averaging, disable_averaging
use LND_domains, only : LND_domains_init, clone_LND_domain
use LND_domains, only : pass_var, pass_vector, TO_ALL, CGRID_NE, BGRID_NE, CORNER
use LND_dyn_horgrid, only : dyn_horgrid_type, create_dyn_horgrid, destroy_dyn_horgrid
use LND_dyn_horgrid, only : rescale_dyn_horgrid_bathymetry
...
use LND_fixed_initialization, only : LND_initialize_topography
use LND_fixed_initialization, only : LND_initialize_rotation
use user_initialization, only : user_initialize_topography
use LND_io, only : field_exists, file_exists, LND_read_data, write_version_number
use LND_io, only : slasher, fieldtype
use LND_io, only : write_field, close_file, SINGLE_FILE, MULTIPLE
use LND_restart, only : register_restart_field, query_initialized, save_restart
use LND_restart, only : restart_init, restore_state, LND_restart_CS
use LND_time_manager, only : time_type, time_type_to_real, real_to_time, operator(>), operator(-)
use LND_transcribe_grid, only : copy_dyngrid_to_LND_grid, copy_LND_grid_to_dyngrid
use LND_unit_scaling, only : unit_scale_type, unit_scaling_init, fix_restart_unit_scaling
use LND_variables, only : surface
use LND_forcing_type, only : forcing, allocate_forcing_type, LND_forcing_chksum
use LND_forcing_type, only : mech_forcing, allocate_mech_forcing, LND_mech_forcing_chksum
use LND_forcing_type, only : copy_common_forcing_fields
use LND_get_input, only : directories, Get_LND_input
use LND_EOS, only : calculate_density, calculate_density_derivs, calculate_TFreeze, EOS_domain
use LND_EOS, only : EOS_type, EOS_init
use LND_ice_shelf_dynamics, only : ice_shelf_dyn_CS, update_ice_shelf
use LND_ice_shelf_dynamics, only : register_ice_shelf_dyn_restarts, initialize_ice_shelf_dyn
use LND_ice_shelf_dynamics, only : ice_shelf_min_thickness_calve
use LND_ice_shelf_dynamics, only : ice_time_step_CFL, ice_shelf_dyn_end
use LND_ice_shelf_initialize, only : initialize_ice_thickness
!MJH use LND_ice_shelf_initialize, only : initialize_ice_shelf_boundary
use LND_ice_shelf_state, only : ice_shelf_state, ice_shelf_state_end, ice_shelf_state_init
use user_shelf_init, only : USER_initialize_shelf_mass, USER_update_shelf_mass
use user_shelf_init, only : user_ice_shelf_CS
use LND_coms, only : reproducing_sum
use LND_spatial_means, only : global_area_integral
use LND_checksums, only : hchksum, qchksum, chksum, uchksum, vchksum, uvchksum
...
type, public :: ice_shelf_CS ; private
  ! Parameters
  type(LND_restart_CS), pointer :: restart_CSp => NULL() !< A pointer to the restart control
  ...
  real :: time_step    !< this is the shortest timestep that the ice shelf sees, and
                       ...
                       !! is initialized - so need to reorganize LND driver.
...
subroutine shelf_calc_flux(sfc_state, fluxes, Time, time_step, CS, forces)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "shelf_calc_flux: "// &
        ...
        if (CS%threeeq) then
          ...
          if (CS%find_salt_root) then
            ...
            elseif ((S_a == 0.0) .and. (S_b < 0.0)) then ! It should be the case that S_b < 0.
              ...
              call LND_error(FATAL, "Impossible conditions found in 3-equation skin salinity calculation.")
            endif

            ! Safety check
            if (Sbdry(i,j) < 0.) then
              write(mesg,*) 'sfc_state%sss(i,j) = ',sfc_state%sss(i,j), 'S_a, S_b, S_c', S_a, S_b, S_c
              call LND_error(WARNING, mesg, .true.)
              write(mesg,*) 'I,J,Sbdry1,Sbdry2',i,j,Sbdry1,Sbdry2
              call LND_error(WARNING, mesg, .true.)
              call LND_error(FATAL, "shelf_calc_flux: Negative salinity (Sbdry).")
            endif
          else
            ...
            if (CS%find_salt_root) then
              ...
              if (dS_it < 0.0) then ! Sbdry is now the upper bound.
                if (Sb_max_set .and. (Sbdry(i,j) > Sb_max)) &
                  call LND_error(FATAL,"shelf_calc_flux: Irregular iteration for Sbdry (max).")
                Sb_max = Sbdry(i,j) ; dS_max = dS_it ; Sb_max_set = .true.
              else ! Sbdry is now the lower bound.
                if (Sb_min_set .and. (Sbdry(i,j) < Sb_min)) &
                   call LND_error(FATAL, "shelf_calc_flux: Irregular iteration for Sbdry (min).")
...
!      haline_driving(i,j) = sfc_state%sss(i,j) - Sbdry(i,j)
  ...
  do j=js,je ; do i=is,ie
    ...
    if ((sfc_state%ocean_mass(i,j) > CS%col_mass_melt_threshold) .and. &
      ...
      if ((abs(fluxes%iceshelf_melt(i,j))>0.0) .and. (fluxes%ustar_shelf(i,j) == 0.0)) then
        ...
        call LND_error(FATAL, "shelf_calc_flux: "//trim(mesg))
      endif
  ...
  if (CS%debug) call LND_forcing_chksum("Before add shelf flux", fluxes, G, CS%US, haloshift=0)
  ...
  if (CS%debug) call LND_forcing_chksum("End of shelf calc flux", fluxes, G, CS%US, haloshift=0)
...
subroutine add_shelf_forces(G, US, CS, forces, do_shelf_area)
  ...
  if ((CS%grid%isc /= G%isc) .or. (CS%grid%iec /= G%iec) .or. &
    ...
    call LND_error(FATAL,"add_shelf_forces: Incompatible ocean and ice shelf grids.")
...
subroutine add_shelf_pressure(G, US, CS, fluxes)
  ...
  if ((CS%grid%isc /= G%isc) .or. (CS%grid%iec /= G%iec) .or. &
      (CS%grid%jsc /= G%jsc) .or. (CS%grid%jec /= G%jec)) &
    call LND_error(FATAL,"add_shelf_pressure: Incompatible ocean and ice shelf grids.")
...
subroutine add_shelf_flux(G, US, CS, sfc_state, fluxes)
  ...
  if ((CS%grid%isc /= G%isc) .or. (CS%grid%iec /= G%iec) .or. &
      (CS%grid%jsc /= G%jsc) .or. (CS%grid%jec /= G%jec)) &
    call LND_error(FATAL,"add_shelf_flux: Incompatible ocean and ice shelf grids.")
  ...
  if (CS%debug) then
    call LND_forcing_chksum("Before adding shelf fluxes", fluxes, G, CS%US, haloshift=0)
  endif
  ...
  if (CS%debug) then
    ...
    call LND_forcing_chksum("After adding shelf fluxes", fluxes, G, CS%US, haloshift=0)
  endif
  ...
  if (CS%constant_sea_level) then
    ...
    if (CS%debug) then
      ...
      call LND_mesg(mesg)
      call LND_forcing_chksum("After constant sea level", fluxes, G, CS%US, haloshift=0)
    endif
...
# include "version_variable.h"
  ...
  character(len=40)  :: mdl = "LND_ice_shelf"  ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(FATAL, "LND_ice_shelf.F90, initialize_ice_shelf: "// &
  ...
  ! LND's grid and infrastructure.
  call Get_LND_Input(dirs=dirs)
  ...
  wd_halos(:)=0
  call LND_domains_init(CS%grid%domain, param_file, min_halo=wd_halos, symmetric=GRID_SYM_)
  ...
  call LND_grid_init(CS%grid, param_file, CS%US)

  call create_dyn_horgrid(dG, CS%grid%HI)
  call clone_LND_domain(CS%grid%Domain, dG%Domain)
  ...
  if (CS%cutoff_depth < 0.) &
    call LND_error(WARNING,"Initialize_ice_shelf: MELTING_CUTOFF_DEPTH must be >= 0.")
  ...
  if (read_TIDEAMP) then
    ...
    call LND_read_data(TideAmp_file, 'tideamp', CS%utide, G%domain, timelevel=1, scale=US%m_s_to_L_T)
  else
  ...
  !! new parameters that need to be in LND_input
  ...
  if (.not. CS%solo_ice_sheet) then
    call LND_mesg("LND_ice_shelf.F90, initialize_ice_shelf: allocating fluxes.")
    ...
    call LND_mesg("LND_ice_shelf.F90, initialize_ice_shelf: allocating fluxes in solo mode.")
    if (present(fluxes)) &
  ...
  elseif (.not.new_sim) then
    ...
    call LND_mesg("LND_ice_shelf.F90, initialize_ice_shelf: Restoring ice shelf from file.")
  ...
  do j=jsd,jed ; do i=isd,ied
    if (ISS%area_shelf_h(i,j) > G%areaT(i,j)) then
      call LND_error(WARNING,"Initialize_ice_shelf: area_shelf_h exceeds G%areaT.")
  ...
  if (save_IC) call get_param(param_file, mdl, "SHELF_IC_OUTPUT_FILE", IC_file,&
                 ...
                 "initial conditions.", default="LND_Shelf_IC")
...
subroutine initialize_shelf_mass(G, param_file, CS, ISS, new_sim)
  ...
  character(len=40)  :: mdl = "LND_ice_shelf"
  is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec
  ...
  select case ( trim(config) )
    ...
    case default ;  call LND_error(FATAL,"initialize_ice_shelf: "// &
...
subroutine solo_step_ice_shelf(CS, time_interval, nsteps, Time, min_time_step_in)
  ...
  call LND_mesg("solo_step_ice_shelf: "//mesg, 5)
  ...
  do while (remaining_time > 0.0)
    ...
    if ((time_step < min_time_step) .and. (time_step < remaining_time))  then
      call LND_error(FATAL, "LND_ice_shelf:solo_step_ice_shelf: abnormally small timestep "//mesg)
    else
      call LND_mesg("solo_step_ice_shelf: "//mesg, 5)
    endif
