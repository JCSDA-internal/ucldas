...
!! in the LND_tracer_flow_control module.
module LND_tracer_diabatic
...
subroutine tracer_vertdiff(h_old, ea, eb, dt, tr, G, GV, &
  ...
  if (nz == 1) then
    call LND_error(WARNING, "LND_tracer_diabatic.F90, tracer_vertdiff called "//&
...
!> This routine is modeled after applyBoundaryFluxesInOut in LND_diabatic_aux.F90
...
!$OMP end critical
  ...
  if (numberOfGroundings>0) then
    do i = 1, min(numberOfGroundings, maxGroundings)
      ...
      call LND_error(WARNING, "LND_tracer_diabatic.F90, applyTracerBoundaryFluxesInOut(): "//&
    ...
    if (numberOfGroundings - maxGroundings > 0) then
      ...
      call LND_error(WARNING, "LND_tracer_vertical.F90, applyTracerBoundaryFluxesInOut(): "//&
                              trim(mesg) // " groundings remaining")
    endif
  endif

end subroutine applyTracerBoundaryFluxesInOut
end module LND_tracer_diabatic
