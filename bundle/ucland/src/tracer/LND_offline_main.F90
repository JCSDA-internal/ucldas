!> The routines here implement the offline tracer algorithm used in UCLAND. These are called from step_offline
!! Some routines called here can be found in the LND_offline_aux module.
module LND_offline_main

! This file is part of UCLAND. See LICENSE.md for the license.

use LND_ALE,                  only : ALE_CS, ALE_main_offline, ALE_offline_inputs
use LND_checksums,            only : hchksum, uvchksum
...
use LND_diabatic_aux,         only : diabatic_aux_CS, set_pen_shortwave
use LND_diabatic_driver,      only : diabatic_CS, extract_diabatic_member
use LND_diabatic_aux,         only : tridiagTS
use LND_diag_mediator,        only : diag_ctrl, post_data, register_diag_field
...
use LND_io,                   only : LND_read_data, LND_read_vector, CENTER
use LND_offline_aux,          only : update_offline_from_arrays, update_offline_from_files
use LND_offline_aux,          only : next_modulo_time, offline_add_diurnal_sw
use LND_offline_aux,          only : update_h_horizontal_flux, update_h_vertical_flux, limit_mass_flux_3d
use LND_offline_aux,          only : distribute_residual_uh_barotropic, distribute_residual_vh_barotropic
use LND_offline_aux,          only : distribute_residual_uh_upwards, distribute_residual_vh_upwards
use LND_opacity,              only : opacity_CS, optics_type
use LND_open_boundary,        only : ocean_OBC_type
use LND_time_manager,         only : time_type, real_to_time
use LND_tracer_advect,        only : tracer_advect_CS, advect_tracer
use LND_tracer_diabatic,      only : applyTracerBoundaryFluxesInOut
use LND_tracer_flow_control,  only : tracer_flow_control_CS, call_tracer_column_fns, call_tracer_stocks
use LND_tracer_registry,      only : tracer_registry_type, LND_tracer_chksum, LND_tracer_chkinv
...
type, public :: offline_transport_CS ; private
  ...
  ! Pointers to relevant fields from the main LND control structure
  ...
  !> Zonal transport that may need to be stored between calls to step_LND
  real, allocatable, dimension(:,:,:) :: uhtr
  !> Meridional transport that may need to be stored between calls to step_LND
...
subroutine offline_advection_ale(fluxes, Time_start, time_interval, CS, id_clock_ale, h_pre, uhtr, vhtr, converged)
  ...
  do iter=1,CS%num_off_iter
    ...
    if (CS%debug) then
      ...
      call LND_tracer_chkinv(debug_msg, G, h_pre, CS%tracer_reg%Tr, CS%tracer_reg%ntr)
    endif
    ...
    if (MODULO(iter,CS%off_ale_mod)==0) then
      ...
      if (CS%debug) then
        ...
        call LND_tracer_chkinv(debug_msg, G, h_new, CS%tracer_reg%Tr, CS%tracer_reg%ntr)
      endif
  ...
  if (CS%debug) then
    ...
    call LND_tracer_chkinv("After offline_advection_ale", G, h_pre, CS%tracer_reg%Tr, CS%tracer_reg%ntr)
  endif
...
subroutine offline_redistribute_residual(CS, h_pre, uhtr, vhtr, converged)
  type(offline_transport_CS), pointer       :: CS    !< control structure from initialize_LND
  real, dimension(SZI_(CS%G),SZJ_(CS%G),SZK_(CS%G)), &
                              intent(inout) :: h_pre !< layer thicknesses before advection
  real, dimension(SZIB_(CS%G),SZJ_(CS%G),SZK_(CS%G)), &
  ...
  if (CS%debug) then
    call LND_tracer_chkinv("Before redistribute ", G, h_pre, CS%tracer_reg%Tr, CS%tracer_reg%ntr)
  endif
  ...
  if (CS%redistribute_upwards .or. CS%redistribute_barotropic) then
    do iter = 1, CS%num_off_iter
      ...
      if (CS%redistribute_upwards) then
        ...
        if (CS%debug) then
          call LND_tracer_chksum("Before upwards redistribute ", CS%tracer_Reg%Tr, CS%tracer_Reg%ntr, G)
        ...
        if (CS%debug) then
          call LND_tracer_chksum("After upwards redistribute ", CS%tracer_Reg%Tr, CS%tracer_Reg%ntr, G)
        endif
      ...
      if (CS%redistribute_barotropic) then
        ...
        if (CS%debug) then
          call LND_tracer_chksum("Before barotropic redistribute ", CS%tracer_Reg%Tr, CS%tracer_Reg%ntr, G)
          call uvchksum("[uv]tr before upwards redistribute", uhtr, vhtr, G%HI)
        endif
        ...
        if (CS%debug) then
          call LND_tracer_chksum("After barotropic redistribute ", CS%tracer_Reg%Tr, CS%tracer_Reg%ntr, G)
        endif
  ...
  if (CS%debug) then
    ...
    call LND_tracer_chkinv("after redistribute ", G, h_new, CS%tracer_Reg%Tr, CS%tracer_Reg%ntr)
  endif
...
subroutine offline_diabatic_ale(fluxes, Time_start, Time_end, CS, h_pre, eatr, ebtr)
  ...
  call LND_mesg("Applying tracer source, sinks, and vertical mixing")
  ...
  if (CS%debug) then
    ...
    call LND_tracer_chkinv("Before offline_diabatic_ale", CS%G, h_pre, CS%tracer_reg%Tr, CS%tracer_reg%ntr)
  endif
  ...
  if (CS%debug) then
    ...
    call LND_tracer_chkinv("After offline_diabatic_ale", CS%G, h_pre, CS%tracer_reg%Tr, CS%tracer_reg%ntr)
  endif
...
subroutine offline_fw_fluxes_into_ocean(G, GV, CS, fluxes, h, in_flux_optional)
  ...
  if ( present(in_flux_optional) ) &
    call LND_error(WARNING, "Positive freshwater fluxes with non-zero tracer concentration not supported yet")
  ...
  if (CS%debug) then
    ...
    call LND_tracer_chkinv("Before fluxes into ocean", G, h, CS%tracer_reg%Tr, CS%tracer_reg%ntr)
  endif
  ...
  if (CS%debug) then
    ...
    call LND_tracer_chkinv("After fluxes into ocean", G, h, CS%tracer_reg%Tr, CS%tracer_reg%ntr)
  endif
...
subroutine offline_fw_fluxes_out_ocean(G, GV, CS, fluxes, h, out_flux_optional)
  ...
  if ( present(out_flux_optional) ) &
    call LND_error(WARNING, "Negative freshwater fluxes with non-zero tracer concentration not supported yet")

  if (CS%debug) then
    call hchksum(h,"h before fluxes out of ocean",G%HI)
    call LND_tracer_chkinv("Before fluxes out of ocean", G, h, CS%tracer_reg%Tr, CS%tracer_reg%ntr)
  endif
  do m = 1, CS%tracer_reg%ntr
    ! Layer thicknesses should only be updated after the last tracer is finished
    update_h = ( m == CS%tracer_reg%ntr )
...
subroutine update_offline_fields(CS, h, fluxes, do_ale)
  ...
  call callTree_enter("update_offline_fields, LND_offline_main.F90")
...
! run time parameters from LND_input
subroutine offline_transport_init(param_file, CS, diabatic_CSp, G, GV, US)
  ...
  call callTree_enter("offline_transport_init, LND_offline_control.F90")

  if (associated(CS)) then
    call LND_error(WARNING, "offline_transport_init called with an associated "// &
      "control structure.")
    return
  endif
  allocate(CS)
  ...
  ! Parse LND_input for offline control
...
subroutine read_all_input(CS)
  ...
  if (CS%read_all_ts_uvh) then
    if (allocated(CS%uhtr_all)) call LND_error(FATAL, "uhtr_all is already allocated")
    if (allocated(CS%vhtr_all)) call LND_error(FATAL, "vhtr_all is already allocated")
    if (allocated(CS%hend_all)) call LND_error(FATAL, "hend_all is already allocated")
    if (allocated(CS%temp_all)) call LND_error(FATAL, "temp_all is already allocated")
    if (allocated(CS%salt_all)) call LND_error(FATAL, "salt_all is already allocated")
    ...
    call LND_mesg("Reading in uhtr, vhtr, h_start, h_end, temp, salt")
    do t = 1,ntime
      call LND_read_vector(CS%snap_file, 'uhtr_sum', 'vhtr_sum', CS%uhtr_all(:,:,1:CS%nk_input,t), &
                       CS%vhtr_all(:,:,1:CS%nk_input,t), CS%G%Domain, timelevel=t)
      call LND_read_data(CS%snap_file,'h_end', CS%hend_all(:,:,1:CS%nk_input,t), CS%G%Domain, &
        timelevel=t, position=CENTER)
      call LND_read_data(CS%mean_file,'temp', CS%temp_all(:,:,1:CS%nk_input,t), CS%G%Domain, &
        timelevel=t, position=CENTER)
      call LND_read_data(CS%mean_file,'salt', CS%salt_all(:,:,1:CS%nk_input,t), CS%G%Domain, &
        timelevel=t, position=CENTER)
    enddo
  endif
...
!> \namespace lnd_offline_main
!! \section offline_overview Offline Tracer Transport in UCLAND
...
!!  portions of the UCLAND code base that handle the 3d advection and diffusion of passive tracers.
!!
...
!!  practices for investigators seeking to use UCLAND for offline tracer modeling.
!!
!!  \section offline_technical Implementation of offline routine in UCLAND
!!
!!  The subroutine step_tracers that coordinates this can be found in LND.F90 and is only called
...
!!  Other routines related to offline tracer modeling can be found in tracers/LND_offline_control.F90
!!
