...
!> Control structure for LND_marine_ice
...
subroutine iceberg_forces(G, forces, use_ice_shelf, sfc_state, time_step, CS)
  ...
  type(marine_ice_CS),   pointer       :: CS      !< Pointer to the control structure for LND_marine_ice
...
!> Initialize control structure for LND_marine_ice
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_marine_ice"  ! This module's name.

  if (associated(CS)) then
    call LND_error(WARNING, "marine_ice_init called with an associated control structure.")
    return
