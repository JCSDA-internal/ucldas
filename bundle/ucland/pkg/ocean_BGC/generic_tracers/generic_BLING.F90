...
!   It is designed so that both GFDL Ocean models, GOLD and LND, can use it.
!
...
! TOPAZ and COBALT:
!
! Gas flux formulation updated to UCLAND
  ...
  type generic_BLING_type
     ...
     logical  ::            &
          ...
          force_update_fluxes,&                ! If OCMIP2 tracers fluxes should be updated every coupling timesteps
                                               ...
                                               !    as is the case with UCLAND  THERMO_SPANS_COUPLING option
          prevent_neg_o2,   &
          tracer_debug
...
!#######################################################################
  ...
  subroutine generic_BLING_init(tracer_list, force_update_fluxes)
    ...
    ! such as when UCLAND THERMO_SPANS_COUPLING=True , yet we want the fluxes to be updated every timestep
...
!#######################################################################
  ...
  subroutine generic_BLING_register_diag(diag_list)
    ...
    !Niki: The register_diag_field interface needs to be extended to take the UCLAND axes_grp as argument 
    !      instead of this integer array axes_grp%handle
    !      Currently the actual UCLAND diag axes is chosen to be T or Tl based on the size of the axes argument, 2 or 3.
...
!         enddo; enddo ; !} i, j
    ...
    do j=jsc,jec ; do i=isc,iec
       ...
       !      identical with bling code in LND bling%Rho_0 must be replaced with rho(i,j,1,tau)
...
!
!       co2_alpha(i,j) = co2_alpha(i,j)* sc_no_term * bling%Rho_0 !nnz: LND has rho(i,j,1,tau)
!       co2_csurf(i,j) = co2_csurf(i,j)* sc_no_term * bling%Rho_0 !nnz: LND has rho(i,j,1,tau)
!
! in 'ocmip2_new' atmos_ocean_fluxes.F90 coupler formulation, the schmidt number is carried in explicitly
!
       co2_alpha(i,j) = co2_alpha(i,j) * bling%Rho_0 !nnz: LND has rho(i,j,1,tau)
       co2_csurf(i,j) = co2_csurf(i,j) * bling%Rho_0 !nnz: LND has rho(i,j,1,tau)
...
!       o2_csurf(i,j) = o2_field(i,j,1) * bling%Rho_0 !nnz: LND has rho(i,j,1,tau)
       o2_csurf(i,j) = o2_field(i,j,1,tau) * bling%Rho_0 !nnz: LND has rho(i,j,1,tau)
