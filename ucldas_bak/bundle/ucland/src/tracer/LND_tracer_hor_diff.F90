...
use LND_diag_mediator,         only : post_data, diag_ctrl
use LND_diag_mediator,         only : register_diag_field, safe_alloc_ptr, time_type
use LND_domains,               only : sum_across_PEs, max_across_PEs
use LND_domains,               only : create_group_pass, do_group_pass, group_pass_type
use LND_domains,               only : pass_vector
use LND_debugging,             only : hchksum, uvchksum
use LND_diabatic_driver,              only : diabatic_CS
use LND_EOS,                   only : calculate_density, EOS_type, EOS_domain
...
use LND_neutral_diffusion,     only : neutral_diffusion_init, neutral_diffusion_end
use LND_neutral_diffusion,     only : neutral_diffusion_CS
use LND_neutral_diffusion,     only : neutral_diffusion_calc_coeffs, neutral_diffusion
use LND_lateral_boundary_diffusion, only : lateral_boundary_diffusion_CS, lateral_boundary_diffusion_init
use LND_lateral_boundary_diffusion, only : lateral_boundary_diffusion
use LND_tracer_registry,       only : tracer_registry_type, tracer_type, LND_tracer_chksum
...
subroutine tracer_hordiff(h, dt, MEKE, VarMix, G, GV, US, CS, Reg, tv, do_online_flag, read_khdt_x, read_khdt_y)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "LND_tracer_hor_diff: "// &
       "register_tracer must be called before tracer_hordiff.")
  if (LOC(Reg)==0) call LND_error(FATAL, "LND_tracer_hor_diff: "// &
       "register_tracer must be called before tracer_hordiff.")
  if ((Reg%ntr==0) .or. ((CS%KhTr <= 0.0) .and. .not.associated(VarMix)) ) return

  if (CS%show_call_tree) call callTree_enter("tracer_hordiff(), LND_tracer_hor_diff.F90")
  ...
  if (CS%Diffuse_ML_interior .and. CS%first_call) then ; if (is_root_pe()) then
    do m=1,ntr ; if (associated(Reg%Tr(m)%df_x) .or. associated(Reg%Tr(m)%df_y)) &
      call LND_error(WARNING, "tracer_hordiff: Tracer "//trim(Reg%Tr(m)%name)// &
  ...
  if (CS%debug) call LND_tracer_chksum("Before tracer diffusion ", Reg%Tr, ntr, G)
  ...
  if (CS%Diffuse_ML_interior) then
    ...
    if (CS%debug) call LND_tracer_chksum("Before epipycnal diff ", Reg%Tr, ntr, G)
  ...
  if (CS%debug) call LND_tracer_chksum("After tracer diffusion ", Reg%Tr, ntr, G)
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_tracer_hor_diff" ! This module's name.
  character(len=256) :: mesg    ! Message for error messages.

  if (associated(CS)) then
    call LND_error(WARNING, "tracer_hor_diff_init called with associated control structure.")
    return
  endif
  allocate(CS)
  ...
  if (CS%use_neutral_diffusion .and. CS%Diffuse_ML_interior) call LND_error(FATAL, "LND_tracer_hor_diff: "// &
...
end module LND_tracer_hor_diff
