...
use LND_cpu_clock, only : cpu_clock_id, cpu_clock_begin, cpu_clock_end, CLOCK_ROUTINE
use LND_diag_mediator, only : post_data, query_averaging_enabled, register_diag_field
use LND_diag_mediator, only : safe_alloc_ptr, diag_ctrl, enable_averaging
use LND_domains, only : min_across_PEs, clone_LND_domain, pass_vector
use LND_domains, only : To_All, Scalar_Pair, AGRID, CORNER, LND_domain_type
...
use LND_io, only : vardesc, var_desc, LND_read_data, slasher
use LND_open_boundary, only : ocean_OBC_type, OBC_SIMPLE, OBC_NONE, open_boundary_query
use LND_open_boundary, only : OBC_DIRECTION_E, OBC_DIRECTION_W
use LND_open_boundary, only : OBC_DIRECTION_N, OBC_DIRECTION_S, OBC_segment_type
use LND_restart, only : register_restart_field, register_restart_pair
use LND_restart, only : query_initialized, LND_restart_CS
use LND_tidal_forcing, only : tidal_forcing_sensitivity, tidal_forcing_CS
use LND_time_manager, only : time_type, real_to_time, operator(+), operator(-)
use LND_unit_scaling, only : unit_scale_type
use LND_variables, only : BT_cont_type, alloc_bt_cont_type
use LND_verticalGrid, only : verticalGrid_type
use LND_variables, only : accel_diag_ptrs
...
type, public :: barotropic_CS ; private
  ...
  real    :: const_dyn_psurf !< The constant that scales the dynamic surface
                             ...
                             !! The default is 0.9.
  logical :: tides           !< If true, apply tidal lndentum forcing.
  ...
  type(LND_domain_type), pointer :: BT_Domain => NULL()  !< Barotropic LND domain
...
subroutine btstep(U_in, V_in, eta_in, dt, bc_accel_u, bc_accel_v, forces, pbce, &
  ...
  real, dimension(SZIB_(G),SZJ_(G),SZK_(G)), intent(in)  :: visc_rem_u    !< Both the fraction of the lndentum
  ...
  real, dimension(SZIW_(CS),SZJW_(CS)) :: &
    ...
    gtot_N, &     ! GFS and baroclinic  contributions) in the barotropic lndentum
  ...
  if (.not.associated(CS)) call LND_error(FATAL, &
      "btstep: Module LND_barotropic must be initialized before it is used.")
  ...
  if (present(OBC)) then ; if (associated(OBC)) then
    ...
    if (apply_OBC_flather .and. .not.GV%Boussinesq) call LND_error(FATAL, &
...
! endif
  ...
  ! share with the main part of the UCLAND code.
  if (find_etaav) then
...
! barotropic lndentum equations.  This has to be done quite early to start
...
! pressure gradient and horizontal viscous terms in the layer lndentum
...
!$OMP end parallel
  ...
  if (nstep+nfilter==0 ) call LND_error(FATAL, &
    ...
    if (GV%Boussinesq) then
      do j=js,je ; do i=is,ie
        if (eta(i,j) < -GV%Z_to_H*G%bathyT(i,j)) &
          call LND_error(WARNING, "btstep: eta has dropped below bathyT.")
      enddo ; enddo
    else
      do j=js,je ; do i=is,ie
        if (eta(i,j) < 0.0) &
          call LND_error(WARNING, "btstep: negative eta in a non-Boussinesq barotropic solver.")
      enddo ; enddo
    endif
...
subroutine set_dtbt(G, GV, US, CS, eta, pbce, BT_cont, gtot_est, SSH_add)
  ...
  if (.not.associated(CS)) call LND_error(FATAL, &
      "set_dtbt: Module LND_barotropic must be initialized before it is used.")
  if (.not.CS%split) return
  is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec ; nz = G%ke
  MS%isdw = G%isd ; MS%iedw = G%ied ; MS%jsdw = G%jsd ; MS%jedw = G%jed

  if (.not.(present(pbce) .or. present(gtot_est))) call LND_error(FATAL, &
...
subroutine set_up_BT_OBC(OBC, eta, BT_OBC, BT_Domain, G, GV, US, MS, halo, use_BT_cont, &
  ...
  type(BT_OBC_type),                     intent(inout) :: BT_OBC !< A structure with the private barotropic arrays
                                                                 !! related to the open boundary conditions,
                                                                 !! set by set_up_BT_OBC.
  type(LND_domain_type),                 intent(inout) :: BT_Domain !< LND_domain_type associated with wide arrays
  type(ocean_grid_type),                 intent(inout) :: G      !< The ocean's grid structure.
  type(verticalGrid_type),               intent(in)    :: GV     !< The ocean's vertical grid structure.
  type(unit_scale_type),                 intent(in)    :: US     !< A dimensional unit scaling type
  integer,                               intent(in)    :: halo   !< The extra halo size to use here.
  ...
  if ((isdw < isd) .or. (jsdw < jsd)) then
    call LND_error(FATAL, "set_up_BT_OBC: Open boundary conditions are not "//&
...
! second order accurate estimate h = 2*(h+ * h-)/(h+ + h-).
  if (.not.associated(CS)) call LND_error(FATAL, &
      "btcalc: Module LND_barotropic must be initialized before it is used.")
  if (.not.CS%split) return
  ...
  if (test_dflt) then
    ...
    if (.not.((present(h_u) .and. present(h_v)) .or. &
              (CS%hvel_scheme == HARMONIC) .or. (CS%hvel_scheme == HYBRID) .or.&
              (CS%hvel_scheme == ARITHMETIC))) call LND_error(FATAL, &
...
!$OMP                          private(hatvtot,Ihatvtot,e_v,D_shallow_v,h_arith,h_harm,wt_arith)
  ...
  if (apply_OBCs) then ; do n=1,OBC%number_of_segments ! Test for segment type?
    ...
    elseif (OBC%segment(n)%direction == OBC_DIRECTION_W) then
      ...
      call LND_error(fatal, "btcalc encountered and OBC segment of indeterminate direction.")
    endif
  enddo ; endif
...
subroutine set_local_BT_cont_types(BT_cont, BTCL_u, BTCL_v, G, US, MS, BT_Domain, halo, dt_baroclinic)
  ...
  type(unit_scale_type),                                 intent(in)    :: US         !< A dimensional unit scaling type
  type(LND_domain_type),                                 intent(inout) :: BT_Domain  !< The domain to use for updating
...
subroutine bt_mass_source(h, eta, set_cor, G, GV, CS)
  ...
  if (.not.associated(CS)) call LND_error(FATAL, "bt_mass_source: "// &
        "Module LND_barotropic must be initialized before it is used.")
  if (.not.CS%split) return
...
#include "version_variable.h"
  ! Local variables
  character(len=40)  :: mdl = "LND_barotropic"  ! This module's name.
  ...
  if (CS%module_is_initialized) then
    call LND_error(WARNING, "barotropic_init called with a control structure "// &
  ...
  call get_param(param_file, mdl, "BTHALO", bt_halo_sz, &
                 ...
                 layoutParam=.true.)
#ifdef STATIC_MEMORY_
  if ((bt_halo_sz > 0) .and. (bt_halo_sz /= BTHALO_)) call LND_error(FATAL, &
  ...
  select case (hvel_str)
    ...
    case default
      call LND_mesg('barotropic_init: BT_THICK_SCHEME ="'//trim(hvel_str)//'"', 0)
      call LND_error(FATAL, "barotropic_init: Unrecognized setting "// &
            "#define BT_THICK_SCHEME "//trim(hvel_str)//" found in input file.")
  end select
  if ((CS%hvel_scheme == FROM_BT_CONT) .and. .not.use_BT_cont_type) &
    call LND_error(FATAL, "barotropic_init: BT_THICK_SCHEME FROM_BT_CONT "//&
  ...
  ! Initialize a version of the LND domain that is specific to the barotropic solver.
  call clone_LND_domain(G%Domain, CS%BT_Domain, min_halo=wd_halos, symmetric=.true.)
#ifdef STATIC_MEMORY_
  if (wd_halos(1) /= WHALOI_+NIHALO_) call LND_error(FATAL, "barotropic_init: "//&
          "Barotropic x-halo sizes are incorrectly resized with STATIC_MEMORY_.")
  if (wd_halos(2) /= WHALOJ_+NJHALO_) call LND_error(FATAL, "barotropic_init: "//&
          "Barotropic y-halo sizes are incorrectly resized with STATIC_MEMORY_.")
#else
  if (bt_halo_sz > 0) then
    if (wd_halos(1) > bt_halo_sz) &
      call LND_mesg("barotropic_init: barotropic x-halo size increased.", 3)
    if (wd_halos(2) > bt_halo_sz) &
      call LND_mesg("barotropic_init: barotropic y-halo size increased.", 3)
  endif
#endif
  ...
  if (CS%linear_wave_drag) then
    ...
    if (len_trim(wave_drag_file) > 0) then
      ...
      call LND_read_data(wave_drag_file, wave_drag_var, lin_drag_h, G%Domain, scale=US%m_to_Z*US%T_to_s)
...
!> This subroutine is used to register any fields from LND_barotropic.F90
...
subroutine register_barotropic_restarts(HI, GV, param_file, CS, restart_CS)
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "register_barotropic_restarts called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
...
!!  lndentum equation is second order accurate.  The Coriolis force
...
!!  These terms are exactly removed from the baroclinic lndentum
