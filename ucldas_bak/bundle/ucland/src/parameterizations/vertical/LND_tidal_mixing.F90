...
use LND_diag_mediator,      only : safe_alloc_ptr, post_data
...
use LND_io,                 only : slasher, LND_read_data, field_size
...
use LND_variables,          only : thermo_var_ptrs, p3d
...
# include "version_variable.h"
  character(len=40)  :: mdl = "LND_tidal_mixing"     !< This module's name.

  if (associated(CS)) then
    call LND_error(WARNING, "tidal_mixing_init called when control structure "// &
  ...
  if (CS%int_tide_dissipation) then
    ...
    if (CS%use_CVMix_tidal) then
      ...
      select case (CVMix_tidal_scheme_str)
        ...
        case default
        call LND_error(FATAL, "tidal_mixing_init: Unrecognized setting "// &
  ...
  elseif (CS%use_CVMix_tidal) then
        call LND_error(FATAL, "tidal_mixing_init: Cannot set INT_TIDE_DISSIPATION to False "// &
  ...
  if (CS%lee_wave_dissipation) then
    if (CS%use_CVMix_tidal) then
        call LND_error(FATAL, "tidal_mixing_init: Lee wave driven dissipation scheme cannot "// &
  ...
  if ((CS%Int_tide_dissipation .and. (CS%int_tide_profile == POLZIN_09)) .or. &
    ...
    if (CS%use_CVMix_tidal) then
        call LND_error(FATAL, "tidal_mixing_init: Polzin scheme cannot "// &
            "be used when CVMix tidal mixing scheme is active.")
    endif
  ...
  if ( (CS%Int_tide_dissipation .or. CS%Lee_wave_dissipation) .and. &
    ...
    if (read_tideamp) then
      if (CS%use_CVMix_tidal) then
          call LND_error(FATAL, "tidal_mixing_init: Tidal amplitude files are "// &
    ...
    call LND_read_data(filename, 'h2', CS%h2, G%domain, timelevel=1, scale=US%m_to_Z**2)
  ...
  if (CS%Lee_wave_dissipation) then
    ...
    call LND_read_data(filename, 'TKE_input', CS%TKE_Niku, G%domain, timelevel=1, &  ! ??? timelevel -aja
  ...
  if (CS%use_CVMix_tidal) then
    ...
    if ( .not. ( &
        ...
        call LND_error(FATAL, "tidal_mixing_init: Tidal energy file type ("//&
...
!    enddo ; enddo
  ...
  do i=is,ie
    ...
    if (CS%Lowmode_itidal_dissipation) then
      ...
      call LND_error(FATAL,trim(mesg)//": this block not supported yet. (aa)")
...
subroutine setup_tidal_diagnostics(G,CS)
  ...
  if (CS%id_Simmons_coeff > 0) then
    if (CS%CVMix_tidal_scheme .ne. SIMMONS) then
      call LND_error(FATAL, "setup_tidal_diagnostics: Simmons_coeff diagnostics is available "//&
  ...
  if (CS%id_Schmittner_coeff > 0) then
    if (CS%CVMix_tidal_scheme .ne. SCHMITTNER) then
      call LND_error(FATAL, "setup_tidal_diagnostics: Schmittner_coeff diagnostics is available "//&
  ...
  if (CS%id_tidal_qe_md > 0) then
    if (CS%CVMix_tidal_scheme .ne. SCHMITTNER) then
      call LND_error(FATAL, "setup_tidal_diagnostics: tidal_qe_md diagnostics is available "//&
                            "only when CVMix_tidal_scheme is Schmittner.")
    endif
...
! TODO: move this subroutine to LND_internal_tide_input module (?)
...
subroutine read_tidal_energy(G, US, tidal_energy_type, tidal_energy_file, CS)
  ...
  case ('JAYN') ! Jayne 2009
    ...
    call LND_read_data(tidal_energy_file,'wave_dissipation',tidal_energy_flux_2d, G%domain)
  ...
  case default
    call LND_error(FATAL, "read_tidal_energy: Unknown tidal energy file type.")
  end select
...
subroutine read_tidal_constituents(G, US, tidal_energy_file, CS)
  ...
  call LND_read_data(tidal_energy_file, 'M2', tc_m2, G%domain)
  call LND_read_data(tidal_energy_file, 'S2', tc_s2, G%domain)
  call LND_read_data(tidal_energy_file, 'K1', tc_k1, G%domain)
  call LND_read_data(tidal_energy_file, 'O1', tc_o1, G%domain)
  ! Note the hard-coded assumption that z_t and z_w in the file are in centimeters.
  call LND_read_data(tidal_energy_file, 'z_t', z_t, scale=100.0*US%m_to_Z)
  call LND_read_data(tidal_energy_file, 'z_w', z_w, scale=100.0*US%m_to_Z)
  ...
  if (any(CS%tidal_qe_3d_in<0.0)) then
    call LND_error(FATAL, "read_tidal_constituents: Negative tidal_qe_3d_in terms.")
  endif
