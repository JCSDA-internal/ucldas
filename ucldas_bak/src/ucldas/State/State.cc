...
#include "ucldas/State/StateFortran.h"
...
namespace ucldas {
  ...
  {
    ucldas_state_create_f90(keyFlds_, geom_->toFortran(), vars_);
      ...
      geom_(new Geometry(geom))
  {
    ...
    ucldas_state_create_f90(keyFlds_, geom_->toFortran(), vars);
    ucldas_state_read_file_f90(toFortran(), &file, &dtp);
  ...
  {
    ucldas_state_create_f90(keyFlds_, geom_->toFortran(), vars_);
    ucldas_state_change_resol_f90(toFortran(), other.keyFlds_);
  ...
  {
    ucldas_state_create_f90(keyFlds_, geom_->toFortran(), vars_);
    ucldas_state_copy_f90(toFortran(), other.toFortran());
    Log::trace() << "State::State copied." << std::endl;
  }
  // -----------------------------------------------------------------------------
  State::~State() {
    ucldas_state_delete_f90(toFortran());
  ...
  State & State::operator=(const State & rhs) {
    time_ = rhs.time_;
    ucldas_state_copy_f90(toFortran(), rhs.toFortran());
    return *this;
  }
  // -----------------------------------------------------------------------------
  /// Rotations
  ...
  void State::rotate2north(const oops::Variables & u,
    ...
    Log::trace() << "State::State rotate from logical to geographical North."
                 << std::endl;
    ucldas_state_rotate2north_f90(toFortran(), u, v);
  }
  // -----------------------------------------------------------------------------
  void State::rotate2grid(const oops::Variables & u,
    ...
    << std::endl;
    ucldas_state_rotate2grid_f90(toFortran(), u, v);
  }
  // -----------------------------------------------------------------------------
  ...
  State & State::operator+=(const Increment & dx) {
    ...
    ucldas_state_add_incr_f90(toFortran(), dx_hr.toFortran());
    return *this;
  }
  // -----------------------------------------------------------------------------
  /// I/O and diagnostics
  ...
  void State::read(const eckit::Configuration & files) {
    ...
    util::DateTime * dtp = &time_;
    ucldas_state_read_file_f90(toFortran(), &files, &dtp);
    Log::trace() << "State::State read done." << std::endl;
  }
  // -----------------------------------------------------------------------------
  void State::write(const eckit::Configuration & files) const {
    const util::DateTime * dtp = &time_;
    ucldas_state_write_file_f90(toFortran(), &files, &dtp);
  }
  // -----------------------------------------------------------------------------
  void State::print(std::ostream & os) const {
    os << std::endl << "  Valid time: " << validTime();
    int n0, nf;
    ucldas_state_sizes_f90(toFortran(), n0, n0, n0, nf);
    std::vector<double> zstat(3*nf);
    ucldas_state_gpnorm_f90(toFortran(), nf, zstat[0]);
  ...
  size_t State::serialSize() const {
    // Field
    size_t nn;
    ucldas_state_serial_size_f90(toFortran(), geom_->toFortran(), nn);
  ...
  void State::serialize(std::vector<double> & vect) const {
    ...
    ucldas_state_serialize_f90(toFortran(), geom_->toFortran(), nn,
                             vect_field.data());
  ...
  void State::deserialize(const std::vector<double> & vect, size_t & index) {
    ...
    ucldas_state_deserialize_f90(toFortran(), geom_->toFortran(), vect.size(),
                               vect.data(), index);
  ...
  // -----------------------------------------------------------------------------
  void State::zero() {
    ucldas_state_zero_f90(toFortran());
  }
  // -----------------------------------------------------------------------------
  void State::accumul(const double & zz, const State & xx) {
    ucldas_state_axpy_f90(toFortran(), zz, xx.toFortran());
  }
  // -----------------------------------------------------------------------------
  double State::norm() const {
    double zz = 0.0;
    ucldas_state_rms_f90(toFortran(), zz);
    return zz;
  }
  // -----------------------------------------------------------------------------
