...
!*  Runge-Kutta time stepping scheme for the lndentum and a forward-   *
!*  backward coupling between the lndentum and continuity equations.   *
...
!*  scheme that can be found in LND_dynamics_unsplit_RK2.F90.          *
!*                                                                     *
!*    The subroutine step_LND_dyn_unsplit actually does the time       *
...
!*  modules.                                                           *
!*                                                                     *
!*  Macros written all in capital letters are defined in LND_memory.h. *
!*                                                                     *
...
use LND_domains, only : LND_domains_init, pass_var, pass_vector
use LND_domains, only : pass_var_start, pass_var_complete
use LND_domains, only : pass_vector_start, pass_vector_complete
...
use LND_io, only : LND_io_init
use LND_restart, only : register_restart_field, query_initialized, save_restart
use LND_restart, only : restart_init, LND_restart_CS
use LND_time_manager, only : time_type, real_to_time, operator(+)
...
use LND_continuity, only : continuity, continuity_init, continuity_CS, continuity_stencil
...
use LND_open_boundary, only : radiation_open_bdry_conds
...
use LND_vert_friction, only : vertvisc, vertvisc_coef, vertvisc_init, vertvisc_CS
...
!> LND_dynamics_unsplit module control structure
type, public :: LND_dyn_unsplit_CS ; private
...
end type LND_dyn_unsplit_CS

public step_LND_dyn_unsplit, register_restarts_dyn_unsplit
public initialize_dyn_unsplit, end_dyn_unsplit

!>@{ CPU time clock IDs
integer :: id_clock_Cor, id_clock_pres, id_clock_vertvisc
integer :: id_clock_continuity, id_clock_horvisc, id_clock_lnd_update
...
!> Step the UCLAND dynamics using an unsplit mixed 2nd order (for continuity) and
!! 3rd order (for the inviscid lndentum equations) order scheme
subroutine step_LND_dyn_unsplit(u, v, h, tv, visc, Time_local, dt, forces, &
  ...
  type(LND_dyn_unsplit_CS), pointer      :: CS     !< The control structure set up by
...
! all of the fields except h.  h is stepped separately.
  ...
  if (CS%debug) then
    call LND_state_chksum("Start First Predictor ", u, v, h, uh, vh, G, GV, US)
  endif
...
! up = u + dt_pred * (PFu + CAu)
  ...
  if (CS%debug) then
    call LND_state_chksum("Predictor 1", up, vp, h_av, uh, vh, G, GV, US)
    call LND_accel_chksum("Predictor 1 accel", CS%CAu, CS%CAv, CS%PFu, CS%PFv, &
...
! upp = u + dt/2 * ( PFu + CAu )
  ...
  if (CS%debug) then
    call LND_state_chksum("Predictor 2", upp, vpp, h_av, uh, vh, G, GV, US)
    call LND_accel_chksum("Predictor 2 accel", CS%CAu, CS%CAv, CS%PFu, CS%PFv, &
                          CS%diffu, CS%diffv, G, GV, US)
  endif
...
! u <- u + dt d/dz visc d/dz u
  ...
  if (CS%debug) then
    call LND_state_chksum("Corrector", u, v, h, uh, vh, G, GV, US)
...
end subroutine step_LND_dyn_unsplit
...
subroutine register_restarts_dyn_unsplit(HI, GV, param_file, CS, restart_CS)
  ...
  type(LND_dyn_unsplit_CS),  pointer    :: CS         !< The control structure set up by
                                                      !! initialize_dyn_unsplit.
  type(LND_restart_CS),      pointer    :: restart_CS !< A pointer to the restart control structure.

  ! Local arguments
  character(len=40)  :: mdl = "LND_dynamics_unsplit" ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "register_restarts_dyn_unsplit called with an associated "// &
                             "control structure.")
    return
  endif
  allocate(CS)
...
subroutine initialize_dyn_unsplit(u, v, h, Time, G, GV, US, param_file, diag, CS, &
  ...
  type(LND_dyn_unsplit_CS),       pointer       :: CS         !< The control structure set up
  ...
  type(accel_diag_ptrs),  target, intent(inout) :: Accel_diag !< A set of pointers to the various
                                     !! accelerations in the lndentum equations, which can be used
  ...
  type(cont_diag_ptrs),   target, intent(inout) :: Cont_diag  !< A structure with pointers to
                                                              ...
                                                              !! equations.
  type(ocean_internal_state),     intent(inout) :: MIS        !< The "UCLAND Internal State"
  ...
  type(ocean_OBC_type),           pointer       :: OBC        !< If open boundary conditions are
                                                       ...
                                                       !! that was set up in LND_initialization.
...
# include "version_variable.h"
  ...
  if (CS%module_is_initialized) then
    call LND_error(WARNING, "initialize_dyn_unsplit called with a control "// &
...
subroutine end_dyn_unsplit(CS)
  type(LND_dyn_unsplit_CS), pointer :: CS !< unsplit dynamics control structure that
...
end module LND_dynamics_unsplit
