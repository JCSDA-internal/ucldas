!> A set of dummy interfaces for compiling the UCLAND DA driver code.
module ocean_da_core_mod
  ! LND modules
  use LND_domains, only : LND_domain_type, domain2D
  use LND_time_manager, only : time_type, set_time, get_date
  ! ODA_tools modules
...
contains
  ...
  !> Initializes the UCLAND DA driver code.
  subroutine ocean_da_core_init(Domain, global_grid, Profiles, model_time)
    type(domain2D), pointer, intent(in) :: Domain !< A LND domain type
