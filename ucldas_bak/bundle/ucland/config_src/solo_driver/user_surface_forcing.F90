...
use LND_file_parser, only : get_param, param_file_type, log_version
...
subroutine USER_wind_forcing(sfc_state, forces, day, G, US, CS)
  ...
  call LND_error(FATAL, "User_wind_surface_forcing: " // &
...
!  fluxes are in [R Z T-1 ~> kg m-2 s-1] and positive for water moving into the ocean.
  ...
  !   When modifying the code, comment out this error message.  It is here
  ! so that the original (unmodified) version is not accidentally used.
  call LND_error(FATAL, "User_buoyancy_surface_forcing: " // &
    "User forcing routine called without modification." )
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "USER_surface_forcing_init called with an associated "// &
                             "control structure.")
    return
  endif
  allocate(CS)
