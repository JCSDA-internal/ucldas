...
use LND_open_boundary,  only : OBC_segment_type, register_OBC
use LND_open_boundary,  only : OBC_DIRECTION_N, OBC_DIRECTION_E
use LND_open_boundary,  only : OBC_DIRECTION_S, OBC_DIRECTION_W
use LND_open_boundary,  only : OBC_registry_type
...
function register_Kelvin_OBC(param_file, CS, OBC_Reg)
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "register_Kelvin_OBC called with an "// &
...
subroutine Kelvin_initialize_topography(D, G, param_file, max_depth, US)
  ...
  call LND_mesg("  Kelvin_initialization.F90, Kelvin_initialize_topography: setting topography", 5)
...
subroutine Kelvin_set_OBC_data(OBC, CS, G, GV, US, h, Time)
  ...
  if (.not.associated(OBC)) call LND_error(FATAL, 'Kelvin_initialization.F90: '// &
