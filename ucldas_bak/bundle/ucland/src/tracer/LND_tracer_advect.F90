...
use LND_domains,         only : create_group_pass, do_group_pass, group_pass_type, pass_var
...
subroutine advect_tracer(h_end, uhtr, vhtr, OBC, dt, G, GV, US, CS, Reg, &
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "LND_tracer_advect: "// &
       "tracer_advect_init must be called before advect_tracer.")
  if (.not. associated(Reg)) call LND_error(FATAL, "LND_tracer_advect: "// &
...
# include "version_variable.h"
  character(len=40)  :: mdl = "LND_tracer_advect" ! This module's name.
  character(len=256) :: mesg    ! Message for error messages.

  if (associated(CS)) then
    call LND_error(WARNING, "tracer_advect_init called with associated control structure.")
    return
  endif
  allocate(CS)
  ...
  select case (trim(mesg))
    ...
    case ("PPM")
      ...
      CS%useHuynh = .false.
    case default
      call LND_error(FATAL, "LND_tracer_advect, tracer_advect_init: "//&
