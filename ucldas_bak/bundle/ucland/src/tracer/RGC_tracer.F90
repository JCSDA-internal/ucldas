...
use LND_ALE_sponge, only : set_up_ALE_sponge_field, ALE_sponge_CS, get_ALE_sponge_nz_data
...
#include "version_variable.h"
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "RGC_register_tracer called with an "// &
  ...
  if (.not.restart) then
    if (len_trim(CS%tracer_IC_file) >= 1) then
      ...
      if (.not.file_exists(CS%tracer_IC_file, G%Domain)) &
        call LND_error(FATAL, "RGC_initialize_tracer: Unable to open "// &
                        CS%tracer_IC_file)
      do m=1,NTR
...
! to set_up_sponge_field can simply be omitted.
    ...
    elseif (associated(layer_CSp)) then !layer mode
      ...
      call LND_error(FATAL, "RGC_initialize_tracer: "// &
