...
use LND_ocean_model_nuopc,     only: ocean_public_type, ocean_state_type
use LND_surface_forcing_nuopc, only: ice_ocean_boundary_type
...
subroutine lnd_import(ocean_public, ocean_grid, importState, ice_ocean_boundary, rc)
  ...
  character(len=*)  , parameter   :: subname = '(lnd_import)'
  ...
  call state_getimport(importState, 'mean_zonal_lndent_flx', isc, iec, jsc, jec, taux, rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
       line=__LINE__, &
       file=__FILE__)) &
       return  ! bail out
  call state_getimport(importState, 'mean_merid_lndent_flx', isc, iec, jsc, jec, tauy, rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
       line=__LINE__, &
       file=__FILE__)) &
       return  ! bail out
...
subroutine lnd_export(ocean_public, ocean_grid, ocean_state, exportState, clock, rc)
  type(ocean_public_type) , intent(in)    :: ocean_public !< Ocean surface state
  type(ocean_grid_type)   , intent(in)    :: ocean_grid   !< Ocean model grid
  ...
  character(len=*)  , parameter   :: subname = '(lnd_export)'
...
subroutine State_GetFldPtr_1d(State, fldname, fldptr, rc)
  ...
  integer :: lrc
  character(len=*),parameter :: subname='(LND_cap:State_GetFldPtr)'
...
subroutine State_GetImport(state, fldname, isc, iec, jsc, jec, output, do_sum, rc)
  ...
  character(len=*)  , parameter :: subname='(LND_cap_methods:state_getimport)'
  ! ----------------------------------------------
...
subroutine State_SetExport(state, fldname, isc, iec, jsc, jec, input, ocean_grid, rc)
  ...
  real(ESMF_KIND_R8), pointer   :: dataPtr1d(:)
  real(ESMF_KIND_R8), pointer   :: dataPtr2d(:,:)
  character(len=*)  , parameter :: subname='(LND_cap_methods:state_setexport)'
  ! ----------------------------------------------
