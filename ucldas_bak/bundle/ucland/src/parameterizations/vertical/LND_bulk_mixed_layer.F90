...
use LND_diag_mediator, only : time_type, diag_ctrl, diag_update_remap_grids
...
use LND_forcing_type,  only : extractFluxes1d, forcing
use LND_grid,          only : ocean_grid_type
use LND_opacity,       only : absorbRemainingSW, optics_type, extract_optics_slice
...
!> The control structure with parameters for the LND_bulk_mixed_layer module
type, public :: bulkmixedlayer_CS ; private
  ...
  logical :: convect_lnd_bug !< If true, use code with a bug that causes a loss of lndentum
...
subroutine bulkmixedlayer(h_3d, u_3d, v_3d, tv, fluxes, dt, ea, eb, G, GV, US, CS, &
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "LND_mixed_layer: "//&
         "Module must be initialized before it is used.")
  if (GV%nkml < 1) return

  if (.not. associated(tv%eqn_of_state)) call LND_error(FATAL, &
      "LND_mixed_layer: Temperature, salinity and an equation of state "//&
      "must now be used.")
  if (.NOT. associated(fluxes%ustar)) call LND_error(FATAL, &
      "LND_mixed_layer: No surface TKE fluxes (ustar) defined in mixedlayer!")
...
! treated in conjuction.
    ...
    elseif (CS%nkbl == 2) then
      ...
      call LND_error(FATAL, "LND_mixed_layer: CS%nkbl must be 1 or 2 for now.")
    endif
...
! 3-d variables.
    if (CS%Resolve_Ekman .and. (CS%nkml>1)) then
      ...
      do i=is,ie
        ...
        ! LND_vert_friction.F90, this line will have to be modified accordingly.
...
!  real :: dT_2dz, dS_2dz         ! Half the vertical gradients of T and S, in degC H-1, and ppt H-1.
  ...
  if (CS%nkbl /= 2) call LND_error(FATAL, "LND_mixed_layer"// &
...
subroutine mixedlayer_detrain_1(h, T, S, R0, Rcv, RcvTgt, dt, dt_diag, d_ea, d_eb, &
  ...
  is = G%isc ; ie = G%iec ; nz = GV%ke
  nkmb = CS%nkml+CS%nkbl
  if (CS%nkbl /= 1) call LND_error(FATAL,"LND_mixed_layer: "// &
...
!> This subroutine initializes the LND bulk mixed layer module.
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_mixed_layer"  ! This module's name.
...
! Set default, read and log parameters
  ...
  if (CS%ustar_min<=0.0) call LND_error(FATAL, "BML_USTAR_MIN must be positive.")
  ...
  call get_param(param_file, mdl, "USE_CALVING_HEAT_CONTENT", CS%use_calving_heat_content, &
                 ...
                 default=.false.)
  call get_param(param_file, mdl, "BULKML_CONV_LNDENTUM_BUG", CS%convect_lnd_bug, &
                 "If true, use code with a bug that causes a loss of lndentum conservation "//&
...
end module LND_bulk_mixed_layer
