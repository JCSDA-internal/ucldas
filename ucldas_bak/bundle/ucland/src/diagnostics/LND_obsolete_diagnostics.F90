...
#include "version_variable.h"
  ! Local variables
  character(len=40)  :: mdl = "LND_obsolete_diagnostics"  !< This module's name.
  ...
  if (foundEntry .and. is_root_pe()) &
    call LND_error(errType, 'LND_obsolete_diagnostics: '//&
...
logical function found_in_diagtable(diag, varName, newVarName)
  ...
  if (handle>0 .and. is_root_pe()) then
    if (present(newVarName)) then
      call LND_error(WARNING, 'LND_obsolete_params: '//                        &
...
end module LND_obsolete_diagnostics
