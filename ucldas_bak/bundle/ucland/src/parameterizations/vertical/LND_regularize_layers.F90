...
!> This control structure holds parameters used by the LND_regularize_layers module
...
subroutine regularize_layers(h, tv, dt, ea, eb, G, GV, US, CS)
  ...
  if (.not. associated(CS)) call LND_error(FATAL, "LND_regularize_layers: "//&
...
subroutine regularize_surface(h, tv, dt, ea, eb, G, GV, US, CS)
  ...
  if (.not.associated(tv%eqn_of_state)) call LND_error(FATAL, &
    "LND_regularize_layers: This module now requires the use of temperature and "//&
...
!  call cpu_clock_end(id_clock_EOS)
          ...
          if ((e_filt(i,k2) > e_2d(i,k1+1)) .and. (h_deficit > 0.0) .and. &
            ...
            if (h_add < h_2d(i,k1)) then
              ...
              if (h_add > 0.0) then
                ...
                if (h_add < 0.0) &
                  call LND_error(FATAL, "h_add is negative.  Some logic is wrong.")
    ...
    do i=is,ie ; if (do_i(i)) then
      ...
      ! small compared to the others, so the code here is less prone to roundoff than elsewhere in UCLAND.
      k1 = 1 ; k2 = 1
      int_top = 0.0
      do k=1,nkmb+1
      ...
      enddo
      if (k2 < nkmb) &
        call LND_error(FATAL, "Regularize_surface: Did not assign fluid to layer nkmb.")
    ...
    if (debug) then
      ...
      do k=1,nz ; do i=is,ie ; if (do_i(i)) then
        ...
        if (h(i,j,k) < 0.0) &
          call LND_error(FATAL,"regularize_surface: Negative thicknesses.")
        ...
        if (abs(h(i,j,k) - h_predicted) > MAX(1e-9*abs(h_predicted),GV%Angstrom_H)) &
          call LND_error(FATAL, "regularize_surface: d_ea mismatch.")
      endif ; enddo ; enddo
      do i=is,ie ; if (do_i(i)) then
        fatal_error = .false.
        if (abs(h_tot1(i) - h_tot2(i)) > 1e-12*h_tot1(i)) then
          ...
          call LND_error(WARNING, "regularize_surface: Mass non-conservation."//&
                          trim(mesg), .true.)
          fatal_error = .true.
        endif
        if (abs(Th_tot1(i) - Th_tot2(i)) > 1e-12*(Th_tot1(i)+10.0*h_tot1(i))) then
          ...
          call LND_error(WARNING, "regularize_surface: Heat non-conservation."//&
                          trim(mesg), .true.)
          fatal_error = .true.
        endif
        if (abs(Sh_tot1(i) - Sh_tot2(i)) > 1e-12*(Sh_tot1(i)+10.0*h_tot1(i))) then
          ...
          call LND_error(WARNING, "regularize_surface: Salinity non-conservation."//&
                          trim(mesg), .true.)
          fatal_error = .true.
        endif
        if (fatal_error) then
          write(mesg,'("Error at lat/lon ",2(ES11.4))') G%geoLatT(i,j), G%geoLonT(i,j)
          call LND_error(FATAL, "regularize_surface: Terminating with fatal error.  "//&
                          trim(mesg))
        endif
      endif ; enddo
    endif
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_regularize_layers"  ! This module's name.
  ...
  if (associated(CS)) then
    call LND_error(WARNING, "regularize_layers_init called with an associated"// &
...
end module LND_regularize_layers
