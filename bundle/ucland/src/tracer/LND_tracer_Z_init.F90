...
use LND_io, only : LND_read_data
use LND_EOS, only : EOS_type, calculate_density, calculate_density_derivs, EOS_domain
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_tracer_Z_init" ! This module's name.
  ...
  call LND_read_data(filename, tr_name, tr_in(:,:,:), G%Domain)
    ...
    do j=js,je ; do i=is,ie
      ...
      elseif (abs(tr_in(i,j,1) - missing_val) <= 1e-6*abs(missing_val)) then
        ...
        if (zero_surface) then
          call LND_error(WARNING, "tracer_Z_init: Missing value of "// &
                trim(tr_name)//" found in an ocean point at "//trim(loc_msg)// &
                " in "//trim(filename) )
          tr_in(i,j,1) = 0.0
        else
          call LND_error(FATAL, "tracer_Z_init: Missing value of "// &
                trim(tr_name)//" found in an ocean point at "//trim(loc_msg)// &
                " in "//trim(filename) )
        endif
      endif
...
subroutine read_Z_edges(filename, tr_name, z_edges, nz_out, has_edges, &
  ...
  mdl = "LND_tracer_Z_init read_Z_edges: "
  tr_msg = trim(tr_name)//" in "//trim(filename)

  status = NF90_OPEN(filename, NF90_NOWRITE, ncid)
  if (status /= NF90_NOERR) then
    call LND_error(WARNING,mdl//" Difficulties opening "//trim(filename)//&
  ...
  if (status /= NF90_NOERR) then
    call LND_error(WARNING,mdl//" Difficulties finding variable "//&
  ...
  if (status /= NF90_NOERR) then
    call LND_ERROR(WARNING,mdl//" cannot inquire about "//trim(tr_msg))
  elseif ((ndim < 3) .or. (ndim > 4)) then
    call LND_ERROR(WARNING,mdl//" "//trim(tr_msg)//&
  ...
  if (status /= NF90_NOERR) then
    call LND_ERROR(WARNING,mdl//" cannot inquire about dimension(3) of "//&
                    trim(tr_msg))
  endif
  ...
  if (status /= NF90_NOERR) then
    call LND_mesg(mdl//" "//trim(dim_msg)//&
    ...
    if (status /= NF90_NOERR) then
      call LND_error(WARNING,mdl//" Difficulties finding edge variable "//&
  ...
  if (has_edges) then
    ...
    if (status /= NF90_NOERR) then
      call LND_error(WARNING,mdl//" Difficulties reading variable "//&
          trim(dim_msg)//" - "//trim(NF90_STRERROR(status)))
      nz_out = -1 ; status = NF90_CLOSE(ncid) ; return
    endif
  else
  ...
  if (.not.monotonic) &
    call LND_error(WARNING,mdl//" "//trim(dim_msg)//" is not monotonic.")
...
!    LND_diag_to_Z.F90, and are nearly identical to `find_overlap` in
