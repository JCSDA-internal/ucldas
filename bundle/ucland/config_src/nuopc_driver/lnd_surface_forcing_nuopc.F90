...
module LND_surface_forcing_nuopc
...
use LND_diag_mediator,    only : safe_alloc_ptr, time_type
use LND_domains,          only : pass_vector, pass_var, fill_symmetric_edges
use LND_domains,          only : global_field_sum, BITWISE_EXACT_SUM
...
!> Contains pointers to the forcing fields which may be used to drive LND.
...
type, public :: surface_forcing_CS ; private
  ...
  type(LND_restart_CS), pointer :: restart_CSp => NULL()
...
!! that exactly conform to the use for LND-based coupled models.
...
subroutine convert_IOB_to_fluxes(IOB, fluxes, index_bounds, Time, valid_time, G, US, CS, &
  ...
  if (CS%liquid_runoff_from_data .and. .not. associated(IOB%lrunoff)) then
    call LND_error(FATAL, "liquid runoff is being added via data_override but "// &
...
subroutine convert_IOB_to_forces(IOB, forces, index_bounds, Time, G, US, CS)
  ...
  ! surface lndentum stress related fields as function of staggering
  if (wind_stagger == BGRID_NE) then
    if (G%symmetric) &
...
#include "version_variable.h"
  character(len=40)  :: mdl = "LND_surface_forcing_nuopc"  ! This module's name.
...
end module LND_surface_forcing_nuopc
